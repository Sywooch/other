...
if($_SERVER['REQUEST_METHOD'] == 'POST' && strlen($_POST['submit']) > 0 && empty($_POST['hidden_protection']) && $arParams['UNIQUE_FORM_ID'] == $_REQUEST['UNIQUE_FORM_ID'])
{
	if(check_bitrix_sessid())
	{
		...
		if(is_array($_FILES['UPLOAD_FILES']['name']) && $arParams['SHOW_FILES'])
		{
			...
			foreach($_FILES['UPLOAD_FILES']['name'] as $k => $v)
			{
				...
				{
					$arrFile = array(
						'name'     => $_FILES['UPLOAD_FILES']['name'][$k],
					...
					if(is_dir($arParams['UPLOAD_FOLDER']))
					{
						if(@is_uploaded_file($arrFile['tmp_name']))
						{
							...
							else
							{
								...
								{
									if(is_file($destination) && $arParams['SEND_ATTACHMENT'])
									{
		...
		if(empty($arParams['REQUIRED_FIELDS']) || !in_array('NONE', $arParams['REQUIRED_FIELDS']))
		{
			...
			foreach($arParams['REQUIRED_FIELDS'] as $FIELD)
			{
				...
				if((empty($arParams['REQUIRED_FIELDS']) || in_array($FIELD, $arParams['REQUIRED_FIELDS'])) && strlen($_POST[strtolower($FIELD)]) == 0)
		...
		if(!empty($arParams['CUSTOM_FIELDS']))
		{
			...
			{
				if(!empty($_POST['CUSTOM_FIELDS'][$k]))
				{
					$field_val[$k] = htmlspecialcharsEx(trim(implode(':~:', $_POST['CUSTOM_FIELDS'][$k])));
				}

				$forArResult['CUSTOM_FIELD_' . $k]    = strlen($field_val[$k]) > 0 ? $field_val[$k] : '';
		...
		//Validate e-mail
		if(!empty($_POST['author_email']) && !check_email($_POST['author_email']))
		...
		if(empty($arResult))
		{
			$arFields = Array(
				...
				'AUTHOR_FIO'             => htmlspecialcharsEx(strip_tags($_POST['author_fio'])),
				'AUTHOR_NAME'            => htmlspecialcharsEx(strip_tags($_POST['author_name'])),
				'AUTHOR_LAST_NAME'       => htmlspecialcharsEx(strip_tags($_POST['author_last_name'])),
				'AUTHOR_SECOND_NAME'     => htmlspecialcharsEx(strip_tags($_POST['author_second_name'])),
				'AUTHOR_EMAIL'           => htmlspecialcharsEx(strip_tags($_POST['author_email'])),
				'AUTHOR_PERSONAL_MOBILE' => htmlspecialcharsEx(strip_tags($_POST['author_personal_mobile'])),
				'AUTHOR_WWW'             => htmlspecialcharsEx(strip_tags($_POST['author_www'])),
				'AUTHOR_WORK_COMPANY'    => htmlspecialcharsEx(strip_tags($_POST['author_work_company'])),
				'AUTHOR_POSITION'        => htmlspecialcharsEx(strip_tags($_POST['author_position'])),
				'AUTHOR_PROFESSION'      => htmlspecialcharsEx(strip_tags($_POST['author_profession'])),
				'AUTHOR_STATE'           => htmlspecialcharsEx(strip_tags($_POST['author_state'])),
				'AUTHOR_CITY'            => htmlspecialcharsEx(strip_tags($_POST['author_city'])),
				'AUTHOR_STREET'          => htmlspecialcharsEx(strip_tags($_POST['author_street'])),
				'AUTHOR_ADRESS'          => htmlspecialcharsEx(strip_tags($_POST['author_adress'])),
				'AUTHOR_PERSONAL_PHONE'  => htmlspecialcharsEx(strip_tags($_POST['author_personal_phone'])),
				'AUTHOR_WORK_PHONE'      => htmlspecialcharsEx(strip_tags($_POST['author_work_phone'])),
				'AUTHOR_FAX'             => htmlspecialcharsEx(strip_tags($_POST['author_fax'])),
				'AUTHOR_MAILBOX'         => htmlspecialcharsEx(strip_tags($_POST['author_mailbox'])),
				'AUTHOR_WORK_MAILBOX'    => htmlspecialcharsEx(strip_tags($_POST['author_work_mailbox'])),
				'AUTHOR_SKYPE'           => htmlspecialcharsEx(strip_tags($_POST['author_skype'])),
				'AUTHOR_ICQ'             => htmlspecialcharsEx(strip_tags($_POST['author_icq'])),
				'AUTHOR_WORK_WWW'        => htmlspecialcharsEx(strip_tags($_POST['author_work_www'])),
				'AUTHOR_MESSAGE_THEME'   => !empty($_POST['author_message_theme']) ? htmlspecialcharsEx(strip_tags($_POST['author_message_theme'])) : GetMessage('NO_MESSAGE_THEME'),
				'AUTHOR_MESSAGE'         => htmlspecialcharsEx(strip_tags($_POST['author_message'])),
				'AUTHOR_NOTES'           => htmlspecialcharsEx(strip_tags($_POST['author_notes'])),
                ...
                'DEFAULT_EMAIL_FROM'     => $arParams['REPLACE_FIELD_FROM'] ? htmlspecialcharsEx(strip_tags(trim($_POST['author_email']))) : $arParams['EMAIL_FROM'],
				'BRANCH_NAME'            => '',
				'MSG_PRIORITY'           => '',
				'FILES'                  => $file_list,
			);

			if($arParams['BRANCH_ACTIVE'] && intval($_POST['BRANCH']) >= 0)
			{

				$arEmails = explode('###', $arParams['BRANCH_FIELDS'][intval($_POST['BRANCH'])]);
				...
				if($arParams['MSG_PRIORITY'] && !empty($_POST['MSG_PRIORITY']))
					$arFields['MSG_PRIORITY'] = htmlspecialcharsEx($_POST['MSG_PRIORITY']);
			}
			...
			if(!empty($arParams['ADMIN_EVENT_MESSAGE_ID']))
			{
				...
				{
					if(IntVal($v) > 0)
						if(!CApiFeedback::Send($arParams['EVENT_NAME'], SITE_ID, array_merge($arFields, $arCustomFields), 'N', IntVal($v), false , $semi_rand))
							$arResult['ERROR_MESSAGE'][] = GetMessage('SEND_MESSAGE_ERROR');
				}
			}
			else if(!CApiFeedback::Send($arParams['EVENT_NAME'], SITE_ID, array_merge($arFields, $arCustomFields), 'N', false, $semi_rand))
				$arResult['ERROR_MESSAGE'][] = GetMessage('SEND_MESSAGE_ERROR');


			//For USER files if SEND_ATTACHMENT == Y we send only links to see files
			if(!empty($arParams['USER_EVENT_MESSAGE_ID']))
			{
				$arFields['EMAIL_TO'] = $arFields['AUTHOR_EMAIL'];
				$arFields['DEFAULT_EMAIL_FROM'] = $arParams['EMAIL_FROM'];
				//$arFields['FILES']    = $user_file_list;

				foreach($arParams['USER_EVENT_MESSAGE_ID'] as $v)
				{
					if(IntVal($v) > 0)
						if(!CApiFeedback::Send($arParams['EVENT_NAME'], SITE_ID, array_merge($arFields, $arCustomFields), 'N', IntVal($v), true, $semi_rand))
							$arResult['ERROR_MESSAGE'][] = GetMessage('SEND_MESSAGE_ERROR');
				}
			}
		...
		$arResult['AUTHOR_FIO']             = htmlspecialcharsEx($_POST['author_fio']);
		$arResult['AUTHOR_NAME']            = htmlspecialcharsEx($_POST['author_name']);
		$arResult['AUTHOR_LAST_NAME']       = htmlspecialcharsEx($_POST['author_last_name']);
		$arResult['AUTHOR_SECOND_NAME']     = htmlspecialcharsEx($_POST['author_second_name']);
		$arResult['AUTHOR_EMAIL']           = htmlspecialcharsEx($_POST['author_email']);
		$arResult['AUTHOR_PERSONAL_MOBILE'] = htmlspecialcharsEx($_POST['author_personal_mobile']);
		$arResult['AUTHOR_WWW']             = htmlspecialcharsEx($_POST['author_www']);
		$arResult['AUTHOR_WORK_COMPANY']    = htmlspecialcharsEx($_POST['author_work_company']);
		$arResult['AUTHOR_POSITION']        = htmlspecialcharsEx($_POST['author_position']);
		$arResult['AUTHOR_PROFESSION']      = htmlspecialcharsEx($_POST['author_profession']);
		$arResult['AUTHOR_STATE']           = htmlspecialcharsEx($_POST['author_state']);
		$arResult['AUTHOR_CITY']            = htmlspecialcharsEx($_POST['author_city']);
		$arResult['AUTHOR_STREET']          = htmlspecialcharsEx($_POST['author_street']);
		$arResult['AUTHOR_ADRESS']          = htmlspecialcharsEx($_POST['author_adress']);
		$arResult['AUTHOR_PERSONAL_PHONE']  = htmlspecialcharsEx($_POST['author_personal_phone']);
		$arResult['AUTHOR_WORK_PHONE']      = htmlspecialcharsEx($_POST['author_work_phone']);
		$arResult['AUTHOR_FAX']             = htmlspecialcharsEx($_POST['author_fax']);
		$arResult['AUTHOR_MAILBOX']         = htmlspecialcharsEx($_POST['author_mailbox']);
		$arResult['AUTHOR_WORK_MAILBOX']    = htmlspecialcharsEx($_POST['author_work_mailbox']);
		$arResult['AUTHOR_SKYPE']           = htmlspecialcharsEx($_POST['author_skype']);
		$arResult['AUTHOR_ICQ']             = htmlspecialcharsEx($_POST['author_icq']);
		$arResult['AUTHOR_WORK_WWW']        = htmlspecialcharsEx($_POST['author_work_www']);
		$arResult['AUTHOR_MESSAGE_THEME']   = htmlspecialcharsEx($_POST['author_message_theme']);
		$arResult['AUTHOR_MESSAGE']         = htmlspecialcharsEx($_POST['author_message']);
		$arResult['AUTHOR_NOTES']           = htmlspecialcharsEx($_POST['author_notes']);
