var SupportPage = Backbone.View.extend({
        ...
        $.post(
            ...
            function (data) {                
                if (! data.error) {
                    ...
                    content = renderTemplate('support-chat-message',{date: self.getFullJSDate(), message : message});
    ...
    messagesResize: function() {        
        ...
        var delHeight = 310;        
        if ($(".ot_support_view_topic").hasClass('fixed')) {
            delHeight = 140;                
        } 
        if (messagesBlock.height() + 310 >= windowHeight) {            
