...
var UsersProfilePage = Backbone.View.extend({
    ...
    showOrdersListTab: function(ev){
        ...
        $.post(
            'index.php?cmd=users&do=getUserOrders',
    ...
    showAccountInfoTab: function(ev){
        ...
        $.post('index.php?cmd=users&do=getAccountInfo', {'userId' : User.id}, function (data) {
            if (data.error) {
                showError(data);
            } else {
                if (data.html) {
    ...
    saveNewPassword: function (ev) {
        ...
        $.post('index.php?cmd=users&do=savePassword&id=' + this.userId,
    ...
    updateAccount: function (ev) {
        ...
        var form = self.$(ev.target).parents('form:first');
        form.find('button').attr('disabled', 'disabled');
        $.post('index.php?cmd=users&do=updateAccount&id=' + self.userId,
            form.serialize(),
            function (data) {
