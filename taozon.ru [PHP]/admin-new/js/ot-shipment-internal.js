...
var InternalsPage = Backbone.View.extend({
    ...
    "events": {
        ...
        "click .ot_show_choose_region_modal" : "choseRegion"
    },
    ...
    save: function (callback) {
        ...
        $.post(
            'index.php?cmd=shipment&do=saveInternal',
            form.serialize(),
            function (data) {
                button.button('reset');
                if (! data.error) {
...
$(function(){
    ...
    $("#regions").treeview({
        ...
        onLoad: function(){
            $('.region-select-link').on('click', function(){
                $('[name="ExternalDeliveryRegionId"]').val($(this).attr('regid'));
                $('[name="ExternalDeliveryRegionName"]').val($(this).attr('regname'));
                $('#delivery-choosed span').html($(this).attr('regid')+' '+$(this).attr('regname'));
                $('.choose_region').modal('hide');
                return false;
            });
        }
    });
