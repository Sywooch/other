var DiscountGroupPage = Backbone.View.extend({
    ...
    confirmUserDelete: function(e) {
		...
		content = renderTemplate('discount-dialog-select', {categories: allCategories, noDelete: noDelete, msg: _.template(trans.get('delete_warning'), {item: itemName}), item: itemName});
                ...
                addUserTextEl.typeahead ({
                    source: function (query, process) {
                        if (! addUserTextEl.hasClass('searching')) {
                            ...
                            return $.get('index.php?cmd=discount&do=getUsersForDiscount&username='+query, {}, function (data) {
...
function isValidForm() {
    ...
    var minPrice = parseFloat($('[data-check = DiscountIdentificationParametr]').val());   
    
	if (discountName == '') {
        showError(trans.get('Discount_name_must_not_be_empty'));
        return false;
    }
    
    if (discount < 0) {
        showError(trans.get('Discount_must_be_greater_than_zero'));
        return false;
    }
    ...
    if (isNaN(discount)) {
        showError(trans.get('Discount_value_must_not_be_empty'));
        return false;
    }
    if (isNaN(minPrice)) {
        showError(trans.get('Discount_start_price_must_not_be_empty'));
        return false;
    }      
    return true;
}