...
class ShipmentProvider {
    ...
    {
        $result = $this->otapilib->GetCurrencyInstanceList();
        return $result;
    }
    ...
    {
        $parent = $request->getValue('root');
        if($parent == 'source') {
            $root_regions = $this->otapilib->GetRootAreaList();
            $json_regions = array();
            foreach($root_regions as $reg){
                $zip = $reg['Zip'] ? '('.$reg['Zip'].')' : '';
                $json_regions[] = array('text'=>$reg['Id'].' '.$reg['Name'].' '.$zip.' <a href="#" class="region-select-link" regid="'.$reg['Id'].'" regname="'.$reg['Name'].'">'.LangAdmin::get('choose').'</a>', 'id'=>$reg['Id'], 'expanded'=>false, 'hasChildren' => true);
            }
        } else {
            $root_regions = $this->otapilib->GetSubAreaList($parent, 1);
            $json_regions = array();
            foreach ($root_regions as $reg){
                $zip = $reg['Zip'] ? '('.$reg['Zip'].')' : '';
                $json_regions[] = array('text'=>$reg['Id'].' '.$reg['Name'].' '.$zip.' <a href="#" class="region-select-link" regid="'.$reg['Id'].'" regname="'.$reg['Name'].'">'.LangAdmin::get('choose').'</a>', 'id'=>$reg['Id'], 'expanded'=>false, 'hasChildren' => true);
    ...
    public function generateSaveShipmentXml ($request) 
    {
        ...
        if ($request->getValue('ExternalDeliveryRegionId')){
            $settings->addChild('ExternalDeliveryRegionId', $request->getValue('ExternalDeliveryRegionId'));
            $settings->addChild('ExternalDeliveryRegionName', $request->getValue('ExternalDeliveryRegionName'));
        }
        ...
        $arrayShowcaseSettings = array(
            ...
            'IsHasntShowcaseItemSellAllowed',
    ...
    private function _generateDeliveryXml($params) 
    {
        ...
        if (! empty($params['description'])) $xmlParams->addChild('Description', htmlspecialchars($params['description']));
        if (! empty($formula)) $xmlParams->addChild('Formula', htmlspecialchars($formula));
        ...
        if (
                ...
                (! empty($params['current_integration_type'])) &&
