...
class Blog extends GeneralUtil
{
    ...
    public function getProductAction($request)
    {
        ...
        try {            
            ...
            $fulliteminfo = $this->contentsProvider->BatchGetItemFullInfo('', $id, 'DeliveryCosts,Promotions,RootPath');
            
            if ($fulliteminfo) {
                $price = $fulliteminfo['Item']['ConvertedPrice'];
                if ($confId) {
                    if (array_key_exists('Promotions', $fulliteminfo['Item']) && array_key_exists('ConfiguredItems', $fulliteminfo['Item']['Promotions'])) {
                        foreach ($fulliteminfo['Item']['Promotions']['ConfiguredItems'] as $key => $value) {
                            if ($value['Id'] == $confId) {
                                $price = $value['Price']['ConvertedPrice'];
                            }
                        }    
                    } else {
                        if (array_key_exists('item_with_config', $fulliteminfo['Item']) && array_key_exists($confId, $fulliteminfo['Item']['item_with_config'])) {
                            $price = $fulliteminfo['Item']['item_with_config'][$confId]['convertedprice'];
                        }
                    }
                } 
                
                $html = '<table align="center" border="0" cellpadding="0" cellspacing="0" style="width:100%; " width="624"><tr>
                <td><p align="center"><a href="index.php?p=item&id=' . $fulliteminfo['Item']['id'] . '">' . $fulliteminfo['Item']['title'] . '</a></p></td>
                </tr><tr>
                <td><p align="center"><a href="index.php?p=item&id=' . $fulliteminfo['Item']['id'] . '"><img src="' . $fulliteminfo['Item']['mainpictureurl'] . '" width="310px" height="310px"></a></p></td>
                </tr><tr>
    ...
    private function prepareItemId($request, &$confId)
    {
        ...
        $validator->addRule(new NotEmptyNumber(), 'itemId', LangAdmin::get('Item_id_is_invalid'));
        if (! $validator->validate()) {
            $this->respondAjaxError($validator->getErrors());
        }
        return $return;
