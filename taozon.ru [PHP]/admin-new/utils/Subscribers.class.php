...
class Subscribers extends GeneralUtil
{
    ...
    public function __construct()
    {
        ...
        $this->oldSubscripersRepo = new OldSubscribersRepository($this->cms);
    }
    ...
    public function addAction($request)
    {
        ...
        try {
            ...
            if (count($users)) {
                throw new Exception('Пользователь ' . htmlspecialchars($login) . ' уже существует');
            }
            ...
            if ((string)$e->getErrorCode() == 'NotFound') {
                $this->respondAjaxError(new Exception('Пользователя '.htmlspecialchars($login).' не существует'));
            } else {
    ...
    private function importSubscribersFromXML($xml)
    {
        foreach ($xml->User as $user) {
            $subscriberExists = $this->dataMappper->findByOtapiId((string)$user->OtapiId);
     ...
     * @param RequestWrapper $request
     */
    public function importOldSubscribersAction($request)
    {
        $subscribers = $this->oldSubscripersRepo->findAll();
        foreach ($subscribers as $user) {
            try {
                if (!$user['email']) {
                    continue;
                }
                $subscriberExists = $this->dataMappper->findByEmail($user['email']);
                if (!count($subscriberExists)) {
                    $S = new SubscriberEntity();
                    $S->setOtapiId($user['user_id'] ? $user['user_id'] : self::PseudoUserId);
                    $S->setLogin($user['name']);
        ...
        Session::setMessage(LangAdmin::get('subscribers_imported'));
        $this->redirect($request->env('HTTP_REFERER'));
    }
}