...
class Users extends GeneralUtil
{
    ...
    public function profileAction($request)
    {
        ...
        try {
            ...
            $activeTab = Cookie::get(self::ACTIVE_TAB_COOKIE, 'about');
    ...
    public function updateAccountAction()
    {
        ...
        try {
            ...
            $userData->ClearAccountInfoCacheById($userId);
    ...
    public function savePasswordAction()
    {
        ...
        try {
            ...
            $result = $this->usersProvider->EditUser($sid, $userId, $fields);
            if ($result && $user['email']) {
                $data = array(
                    'username'  => $user['login'],
                    'password'   => $password
                );
                Notifier::generalUserNotification($user['email'], 'email_recover_password', Lang::get('your_new_login_details'), $data);
            }
        } catch (ServiceException $e) {
            $this->respondAjaxError($e->getMessage());
        }
     ...
     * @param RequestWrapper $request
     */
    public function saveUserAjaxAction($request)
    {
        $sid = Session::get('sid');
        $userId = RequestWrapper::post('id');
        ...
        try {
            ...
            if ($userId) {
                ...
                Plugins::invokeEvent('onEditUser', array('user' => $_POST));
                $this->sendAjaxResponse(array('error' => 0, 'message' => LangAdmin::get('Data_updated_successfully')));
            } else {
                $newUserId = $this->usersProvider->AddUser($sid, $fields);
                if (empty($newUserId)) {
                    throw new ServiceException(__METHOD__, '', $this->usersProvider->getError(), 1);
                ...
                $this->cms->SetSubscribe($request->getValue('Login'), $request->getValue('Email'));
                Plugins::invokeEvent('onAddUser', array('user' => $_POST, 'newUserId' => $newUserId));
                $this->sendAjaxResponse(array('error' => 0, 'message' => LangAdmin::get('Data_added_successfully')));
            }
        } catch (Exception $e) {
    ...
    public function activateUserAction($request)
    {
        ...
        $userId = $request->getValue('id');
        try {
            $result = $this->usersProvider->ActivateUser($sid, $userId);
            if (empty($result)) {
                throw new ServiceException(__METHOD__, '', $this->usersProvider->getError(), 1);
            }
        } catch (ServiceException $e) {
    ...
    public function bulkActivateUserAction($request)
    {
        $sid = Session::get('sid');
        $userIds = $request->getValue('ids');
        try {
            foreach ($userIds as $id) {
                $result = $this->usersProvider->ActivateUser($sid, $id);
                if (empty($result)) {
                    throw new ServiceException(__METHOD__, '', $this->usersProvider->getError(), 1);
                }
            }
    ...
    private function _generateUserFields()
    {
        ...
        if (! is_null(RequestWrapper::post('Country'))) $xmlParams->addChild('Country', htmlspecialchars(RequestWrapper::post('Country')));
