...
class Roles extends GeneralUtil
{
        ...
        try {
            ...
            foreach ($users as $user) {
                if (isset($user['Roles'][0]['Name'])) {
                    $usedRoles[] = $user['Roles'][0]['Name'];
                }
            }

            $this->tpl->assign('roles', $roles);
            $this->tpl->assign('used_roles', $usedRoles);
        } catch (ServiceException $e) {
            $this->tpl->assign('roles', array());
    ...
    {
        $sid = Session::get('sid');
        $roleName = RequestWrapper::get('name') ? urldecode(RequestWrapper::get('name')): '';

        try {
            $roles = $this->rolesProvider->GetAvailableRoleList($sid);
            $templateRoles = $this->rolesProvider->GetTemplateRoleList($sid);
            $availableRoles = RightsManager::getAvailableRoles();
            foreach ($templateRoles as $key => $role) {
                if (! in_array($role['name'], $availableRoles)) {
                    unset($templateRoles[$key]);
                }
            }
            if ($roleName) {
        ...
        if ($roleName) {
            $this->tpl->assign('actionTitle', LangAdmin::get('editing') . ' - ' . $roleName);
        } else {
    ...
    static public function printRoleTree($item, $level = 2, $roleRights = array())
    {
        if (!isset($item['Item'])) {
            return;
        }
        ...
        foreach ($item['Item'] as $child) {
            $id = $child['Id'];
            $name = "Rights[]";
            $style = 'style="margin-left:' . ($level - 1) * 10 . 'px"';
            $checked = ($child['IsTurnOn'] == 'true') ? "checked='checked'" : "";


            echo '<li>';
            echo '<label class="checkbox" ' . $style . '>';
            echo '<input disabled="disabled" data-right="' . $child['Name'] . '" data-level="'. $level . '" class="not_from_template right-'. $id .'" type="checkbox" name="'.
                $name.'" value="'.(string)$id.'" '. $checked."> ";

            echo LangAdmin::get('Feature_' . $child['Name']);

            echo '</label>';
            if (isset($child['Item'])) {
                self::printRoleTree($child, $level+1, $roleRights);
            }
            echo '</li>';
        }
    }
