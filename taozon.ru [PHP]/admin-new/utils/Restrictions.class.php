...
class Restrictions extends GeneralUtil
{
    ...
    {   
        try {
            ...
            if (! empty($blackList)) {
                $this->restrictionsProvider->initGetItemInfoList(implode(";", array_slice($blackList, $positions['from'], $positions['to'])));
    ...
    {     
        ...
        try {
            ...
            $positions = $this->assignPagination($request, $blackList->GetContent() ? count($blackList->GetContent()) : 0);
            $blackList = array_slice($blackList->GetContent()->toArray(), $positions['from'], $positions['to']);
            $this->tpl->assign('blackList', $blackList);                       
        } catch (ServiceException $e) {
            ErrorHandler::registerError($e);
        }
    ...
    {             
        try {            
            $blackList = $this->getRestriction($request->getValue('type'));
            ...
            $this->restrictionsProvider->initEditBlackListContents($xml);
            $this->restrictionsProvider->doRequests(); 
		} catch (Exception $e) {
