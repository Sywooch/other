...
class ContentMenu extends GenerateBlock
{
    ...
    protected function setVars()
    {
        if ($this->cms->Check()) {
            try {
                $thisPage = $this->cms->GetPageByAlias($this->request->getValue('p'));
                ...
                foreach ($left_menu_ids as $pid) {
                    ...
                    $pageCurrent = $isContentPage ? $this->cms->GetPageByID($pid) : array('alias'=>$pid,'title'=>Lang::get($pid));
    ...
    public function getChildrenPages($id)
    {
        if ($this->cms->Check()) {
            try {
                ...
                $q = mysql_query('SELECT * FROM `site_pages_parents` WHERE `parent_id`="' . $id . '" ORDER BY `page_id`');
                $children = array();
                if(!$q){
                ...
                while($row = mysql_fetch_assoc($q)){
                    $children[] = $this->cms->GetPageByID($row['page_id']);
                }
