...
class Profile extends GenerateBlock
{
    ...
    private function validateFieldsForDelivery($fields){
        ...
        if(!$fields['Address'])
            return array(false, Lang::get('not_entered_required_data'));
        if (!$fields['PostalCode'] && General::getConfigValue('is_postal_code_required')) {
            return array(false, Lang::get('not_entered_required_data'));
        }
    ...
    public function saveDeliveryProfile($fields){
        ...
        try {
            $error = $this->validateFieldsForDelivery($fields);
    ...
    public function addNewDeliveryProfile($params) {
        global $otapilib;
        $error = $this->validateFieldsForDelivery($_POST);
        if($error) {
            header('Location: index.php?p=profile&new_profile&error=' . $error[1]);
            die;
        }
        ...
        try {
            ...
            header('Location: index.php?p=profile&mode=delivery&success');
        } catch(ServiceException $e) {
            Session::setError($e->getMessage(), $e->getErrorCode());
            header('Location: index.php?p=profile&new_profile&error=' . $e->getMessage());
        } catch(Exception $e){
            Session::setError($e->getMessage(), $e->getErrorCode());
            header('Location: index.php?p=profile&new_profile&error=' . $e->getMessage());
        }
    ...
    protected function setVars()
    {
        ...
        if (isset($_GET['mode']) && $_GET['mode'] == 'delivery') {
            $commonSettings = $otapilib->GetCommonInstanceOptionsInfo();

            if (! isset($commonSettings['userprofile']) or ! $commonSettings['userprofile']) {
                $commonSettings['userprofile'] = self::DEFAULT_USER_PROFILE_COUNT;
            }
            $this->tpl->assign('commonSettings', $commonSettings);
            $this->tpl->assign('error', isset($_GET['error']) ? $_GET['error'] : false);
        ...
        if(@$_POST['change_email']) {
            ...
            if(!$success){
                ...
                if ($delivery) { 
                    header('Location: index.php?p=profile&mode=delivery&success_update');
                } else {
