...
class Support extends GenerateBlock
{
    ...
    private function chatAdmin(){
		...
		$id = str_replace('Ticket-', '', $_GET['id']);
		try {
        	$chat['TicketInfo'] = $this->SupportRepository->getTicketDetails($_GET['userid'], $id);
        	$chat['TicketMessageList'] = $this->SupportRepository->getTicketMessageList($_GET['userid'], $id);
			$this->SupportRepository->markRead($id, 'In',$setDirection);
		} catch (DBException $e) {
           	Session::setError($e->getMessage(), 'DBError');                
        }
	...
	private function CloseTicket(){ 
		$id = str_replace('Ticket-', '', $_GET['id']);
		try {
        	$this->SupportRepository->closeticket($_GET['userid'], $id);
		} catch (DBException $e) {
           	Session::setError($e->getMessage(), 'DBError');                
        }
        header('Location: /admin/index.php?cmd=support');
    ...
    private function addToChat($fields){
        ...
        $id = str_replace('Ticket-', '', $_GET['id']);
        try {
        	$add = $this->SupportRepository->createTicketMessage($userid, $id, $fields['Text'], $this->isAdmin);
		...
		if ($this->isAdmin) {				
			$sid = $_SESSION['sid'];
			$userdata = $otapilib->GetUserInfoForOperator($sid, $_GET['userid']);	
			$userdata['ticket_id'] = $id;
			$userdata['txt_message'] = $fields['Text'];						
    ...
    {
		if(!Session::getUserData() && !@$_GET['admin']){            
