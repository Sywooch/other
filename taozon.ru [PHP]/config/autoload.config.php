...
function  opentao__autoload($class_name)
{
    ...
    {
        $appendAutoloadResult = Plugins::invokeEvent('onAppendCustomAutoload', array('className' => $class_name));
        if(!$appendAutoloadResult) die('Невозможно загрузить класс '.$class_name);
    }
}
