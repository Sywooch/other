        ...
        <div class="col700">
            ...
            <? if ((string)$webui->Settings->SelectedTranslateType == 'AutoTranslation') { ?>
                <div align="right" class="flr">
                    ...
                    <a class="linktext" href='index.php?cmd=category&do=export&sid=<?=$GLOBALS['ssid']?>'><?=LangAdmin::get('exports')?></a>
...
function update_search_filter(category_id, type, filterid)
{
    if (!update_filter_activate) {
        ...
        $.post('index.php?cmd=category&do=updateSearchFilter&sid=<?=$GLOBALS['ssid']?>',
            {'category_id': category_id, 'text': text, 'type': type, 'filterid': filterid}, function(data) {
                if (data == '1') {
