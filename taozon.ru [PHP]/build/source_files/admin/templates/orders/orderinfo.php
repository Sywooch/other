...
<script>
    $(function(){
        $('.cache').each(function(){
            ...
            var span = $(this);
            $.post('/admin/write_cache_image.php', {'src': src}, function(data){
            });
        })
    });
</script>
	...
	<table>
        <thead>
            <tr>
                ...
                <th width="5%"></th>
                <th width="5%"><input type="checkbox" name="all_products" value="1" style="width:17px"/></th>
            </tr>
        </thead>
                    ...
                    <? if(isset($status_list[$item['id']])) { ?>
                        <select class="__combolist" id="status<?=$item['id']?>" name="status<?=$item['id']?>" style="width:180px">
                            ...
                            <? foreach($status_list[$item['id']] as $status) { ?>
                                <option value="<?=$status['id']?>" title="<?=@$status_desc[@$status['id']]?>"><?=$status['name']?></option>
                            <? } ?>
                        </select>
                        <br/>
