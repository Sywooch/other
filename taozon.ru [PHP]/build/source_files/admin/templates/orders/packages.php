...
<? if(isset($action)) { ?>
    <? if($action == 'createpackage') {?>
        <strong><?=LangAdmin::get('method_of_delivery')?>:</strong> 
            ...
            <? } ?>
        <br/><br/>
        <strong><?=LangAdmin::get('delivery_address')?>, телефон, <?=LangAdmin::get('name_of_recipient')?>:</strong> <br/><br/>
                ...
                <tbody>
                    ...
                    <td width="20%" style="text-align: right;">
                        <? if((int)$package['candelete']) { ?> 
                            <button class="deletepackage" value="<?=$package['id']?>" title="<?=LangAdmin::get('remove')?>"><?=LangAdmin::get('remove')?></button><br/><br/>
                        <? } ?>
                        <? if((int)$package['canupdate']) { ?> 
                            <button class="updatepackage" value="<?=$package['id']?>" title="<?=LangAdmin::get('edit')?>"><?=LangAdmin::get('edit')?></button>
                        <? } ?>
						<br/><br/>
						<button class="printpackage" value="<?=$package['id']?>" title="<?=LangAdmin::get('printpackage')?>"><?=LangAdmin::get('print')?></button>
					</td>
                    </tr>
                    <? } ?>
                <tbody>
