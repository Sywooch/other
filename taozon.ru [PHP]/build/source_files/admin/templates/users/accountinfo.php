...
<? //var_dump($user); ?>
    ...
    <table>
        ...
        <tbody>
            <tr id="user<?=$user['id']?>">
                <td><a href="index.php?sid=<?=$GLOBALS['ssid'];?>&cmd=users&do=userinfo&id=<?=$user['id']?>"><?=$user['login']?></a></td>
                <td><?=(defined('CFG_PREFIX_REPLACE_USR')?str_replace('USR', CFG_PREFIX_REPLACE_USR, (string)$user_account['num']):(string)$user_account['num'])?></td> 
    ...
    <? } ?> <br/><br/>
    <form action="index.php?cmd=users&do=accountaction&sid=<?=$GLOBALS['ssid']?>&id=<?=$user['id']?>" method="POST">
    <table>
        <tr>
