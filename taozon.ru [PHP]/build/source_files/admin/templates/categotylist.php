...
if (is_array($cats)) {
    ...
    foreach ($cats as $cat) {
		...
		if ($cat['deletestatus']=="ParentOfHiddenDeleted") { $stle="#FCF"; }
?>
    <div id="cat<?=$cat['id']?>" <? if ($cat['ishidden']=='true') { ?>class="hidden2"<? if (!@$_COOKIE['HiddenState']) {?> style="display:none"<?} } ?>>
      <div class="folder">
        ...
        <? } ?>
                ...
                <a class="linktext <?=@$stle?>"><b><?=$cat['name']?></b><?if ($cat['approxweight']){?> (прим. вес: <?=(float)$cat['approxweight']?> кг.)<?}?> <? if ($cat['externalid']){?> [<?=$cat['externalid']?>]<?}?></a>
            </span>
        ...
        <input type="hidden" class="edfld" id="edit2sp_<?=$cat['id']?>" value="<?=@$cat['seo']['seo_title']?>">
          ...
          <div style="float:right; cursor:pointer;" class="windialog"> 
            ...
            <a class="buttons" target="_blank" href="../index.php?p=subcategory<?=($cat['isvirtual'] == 'true' ? '&virt' : '')?>&amp;cid=<?=$cat['id']?>"><img src="<?=TPL_DIR;?>i/preview.png" alt="<?=LangAdmin::get('review')?>" title="<?=LangAdmin::get('review')?>" align="middle"/></a>
