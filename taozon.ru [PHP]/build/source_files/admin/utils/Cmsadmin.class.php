...
//        $all_docs = $cms->GetPagesByLang($current_lang);
        $allPages = $cms->GetPagesByLang($current_lang);
        $all_docs = array();
        foreach($allPages as $page){
...
//        }
    ...
    public function menusaveAction(){
        if(@$_POST['TopMenu']){
            ...
            if(mysql_result($q, 0)){
                mysql_query('UPDATE `site_blocks` SET `properties`="'.  mysql_escape_string($_POST['TopMenu']).'" WHERE `type`="'.$type.'"');
            }
            else{
                mysql_query('INSERT INTO `site_blocks` SET `properties`="'.  mysql_escape_string($_POST['TopMenu']).'", `type`="'.$type.'"');
            }
        }
        ...
        if(@$_POST['LeftMenu']){
            ...
            $q = mysql_query('SELECT COUNT(*) FROM `site_blocks` WHERE `type`="'.$type.'"');
            if(mysql_result($q, 0)){
                mysql_query('UPDATE `site_blocks` SET `properties`="'.  mysql_escape_string($_POST['LeftMenu']).'" WHERE `type`="'.$type.'"');
            }
            else{
                mysql_query('INSERT INTO `site_blocks` SET `properties`="'.  mysql_escape_string($_POST['LeftMenu']).'", `type`="'.$type.'"');
            }
        }
