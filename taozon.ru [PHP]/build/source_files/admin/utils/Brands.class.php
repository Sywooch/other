...
class Brands extends GeneralUtil {
    ...
    function addAction(){
        if (Login::auth())
        {
			...
			//=======================
            header('Location: index.php?cms=brands');
        } else {
            include(TPL_DIR.'login.php');
        } 
    }
    ...
    function saveAction(){
        if (Login::auth())
        {
            ...
            $cms->callCMSMethod('setCategorySEO', array('data' => $to_edit));			
			//=======================
    ...
    function editAction(){
        if (Login::auth())
        {
			...
			$cms = new SafedCMS();
			$brandData['seo'] = $cms->callCMSMethod('getCategorySEO', array('cid' => @$_GET['id']));
