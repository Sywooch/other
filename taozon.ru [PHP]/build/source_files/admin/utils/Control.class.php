...
class Control {
    ...
    function defaultAction() {
        if (Login::auth()) {
            ...
            list($data, $instanceOptionsInfo, $siteConfig) = $this->_getMainSettings();
    ...
    function setroundsettingsAction () {
        ...
        $sid = $_SESSION['sid'];
        $round = $_GET['round'];
        $xml = "<EditablePriceFormationSettings PriceRoundingFactor='{$round}' />";
    ...
    function clearCacheAction(){
		...
		//Удаляем кэш с ДБ
		$DBCacheDel = new DBCache();
		$DBCacheDel->ClearDBCache();		
    ...
    private function _getMainSettings(){
        ...
        return array($result, $instanceOptionsInfo, @$resConf);
    }
