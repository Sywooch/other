...
class Category extends GeneralUtil
{
    ...
    function defaultAction()
    {
        if (Login::auth()) {
			...
			if (is_array($cats))
            foreach ($cats as $k => $c) {
                $c['alias'] = $cms->callCMSMethod('getCategoryAlias', array('cid' => $c['Id']));
                $c['seo'] = $cms->callCMSMethod('getCategorySEO', array('cid' => $c['Id']));
            }
            ...
            $example_url = 'http://demo.opentao.net/index.php?p=search&search=&brand=29527';
            $sxml = $this->parseSearchUrl($example_url);
    ...
    {
        setcookie("HiddenState", @$_GET['stat'], time() + 3600 * 24 * 30);
    }
    ...
    function saveAction()
    {
        if (Login::auth()) {
            ...
            $alias = @$_GET['alias'];
            $cms = new SafedCMS();
            $cms->callCMSMethod('setCategoryAlias', array('cid' => $cid, 'alias' => $alias));
            $cms->callCMSMethod('setCategorySEO', array('data' => $_GET));
    ...
    function order2Action()
    {
        if (Login::auth()) {
            ...
            $data = $otapilib->EditOrderOfCategory($pos + 1, $cid, $sid);
            if ($otapilib->error_message == 'SessionExpired') {
                print 'RELOGIN';
                die;
            }
    ...
    public function saveSeoTextAction(){
        ...
        mysql_query('DELETE FROM `site_categories_seo_texts` WHERE `category_id`="'.$id.'"');
        mysql_query('INSERT INTO `site_categories_seo_texts` SET `category_id`="'.$id.'", `text`="'.
        ...
        if (Login::auth()) {
            ...
            switch ($type) {
                case 'ItemPropertyName': {
                    ...
                    $otapilib->EditTranslateByKey($sid, $text, $key, $category_id, $lang);					
                } break;
