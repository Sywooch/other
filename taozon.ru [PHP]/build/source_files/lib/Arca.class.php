...
class Arca
{
     ...
     * @var string
     */
    private $server = 'http://paymentsystemconnector.payments.demo.services.opentao.net/arca.callback';
    ...
    public function notifyOTAPIPaymentSystem($paymentInfo){
        ...
        $this->log->logDebug('Notify: '.$this->server.'?'.http_build_query($paymentInfo));
    ...
    private function callArcaRpc($method, $arr) {
        $this->log->logInfo('Call Arca RPC: '.$method, $arr);
        ...
        $this->log->logInfo('Call result', $decoded);
