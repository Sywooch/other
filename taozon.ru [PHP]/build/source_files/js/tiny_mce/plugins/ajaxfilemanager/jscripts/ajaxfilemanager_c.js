...
{ var fpath = (num.indexOf(urls.view) >=0?num: encodeURIComponent(files[num].path));}else
{ var fpath = encodeURIComponent(files[num].path);}
...
( { url:appendQueryString(getUrl('upload', false, false), 'folder=' + encodeURIComponent(currentFolder.path), ['folder']), secureuri:false, fileElementId:elementId, dataType: 'json', success: function (data, status)
{ if(typeof(data.error) != 'undefined')
{ if(data.error != '')
