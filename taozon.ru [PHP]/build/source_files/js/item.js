...
function recountTotalPrice() {
    ...
    $.post('index.php?p=getprice',iteminfo,function(data){
        $('#total').html('');
...
function getPromotions(){
    ...
    $.ajax({url: 'index.php?p=getpromotions&itemid='+iteminfo.id,
        ...
        success: function(data){
            if (!data) return;
            if (data[0] == 'NotAvailable') setTimeout(getPromotions, 3000);
            else {
...
function init(iteminfoAjax, langsAjax){
    ...
    $('.switch').click(function(){
        ...
        img.onload = function(){
            $('#main-image').attr('src', switchClass.attr('href')+'_310x310.jpg');
            $('#main-image').closest('a').attr('href', switchClass.attr('href'));
            $('#main-image').closest('a').colorbox();
        }
