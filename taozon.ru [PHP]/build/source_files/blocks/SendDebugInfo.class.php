...
class SendDebugInfo extends GenerateBlock
{
    ...
    public function setVars(){
        try{
            ...
            $lang = @$_SESSION['active_lang'] == 'ru' ? 'ru' : 'en';
            $c = new Curl('http://support.opentao.net/'.$lang.'/test_site_speed/', 60, true, 10);

            $debugFile = CFG_APP_ROOT . '/cache/DebugInfo/' . md5($_SESSION['sid']) . '.dat';
            if(!file_exists($debugFile))
                throw new NotFoundException('debug_file_not_found');

            $debugInfo = unserialize(file_get_contents($debugFile));
            ...
            header('Location: http://support.opentao.net/'.$lang.'/test_site_speed/result/'.$json->test_id);
        }
