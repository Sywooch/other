...
class ExportOrderNew
{
    ...
    **/
    public static function onBulkExportOrdersItems(array $ordersItemsIds = array())
    {
        ...
        OTBase::import('system.admin-new.lib.otapi_providers.OrdersProvider');
        ...
        $limit = $anotherParams['itemsCount'] ? $anotherParams['itemsCount'] : 1000;
        $limit = $limit > 0 ? $limit : 1000;
        $xml = $ordersProvider->generateItemsSearchParams($filter, array());
        $items = $ordersProvider->SearchOrderLines($sid, $xml, 0);
        $items = isset($items['Content']) ? $items['Content'] : $items;
