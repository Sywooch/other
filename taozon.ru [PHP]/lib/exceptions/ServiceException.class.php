...
class ServiceException extends Exception
{
    ...
    public function __construct($method, $params, $message, $code, $serviceUrl = null, $subCode = null)
    {
        ...
        $paramsString = htmlspecialchars(json_encode($params));
        ErrorLog::WriteErrorLog($method, $paramsString, $message, $code);
        $this->errorMessage = $exception = (string)$message;
        if (OTBase::isTest() && Session::get('sid') && $serviceUrl) {            
            $params = array_map(create_function('$a', 'return preg_replace("#<\?xml[^>]*>|\n#si", "", $a);'), $params);
