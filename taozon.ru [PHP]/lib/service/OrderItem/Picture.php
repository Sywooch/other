...
class Picture
{
    ...
    {
        if (strpos($this->file, 'http://' . HOST_NAME) !== false) {
            return preg_replace('#http://' . preg_quote(HOST_NAME) . '#si', CFG_APP_ROOT, $this->file);
        }
        return $this->file;
    }
    ...
    {
        return str_replace(CFG_APP_ROOT, 'http://' . HOST_NAME, is_null($file) ? $this->file : $file);
    }
