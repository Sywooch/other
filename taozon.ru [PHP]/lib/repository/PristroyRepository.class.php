...
class PristroyRepository extends Repository
{
    ...
    public function getList($offset = 0, $limit = false, $status = self::STATUS_APPROVED, $prepareData = true)
    {
        ...
        ' ORDER BY `created_at` DESC LIMIT ' . (int)$offset . ', ' . (int)$limit;
    ...
    private function getItemConfig($config_id, $item)
    {
        ...
        if (! empty($item['item_with_config'][$config_id])) {
            ...
            foreach ($config as $key => $val) {
                ...
                $result[$key]['values'] = array_filter($configs[$key]['values'], create_function('$a', 'return $a["id"] == '.$val.';'));
    ...
    private function prepareData(array $data, $singleItem = false)
    {
        if (! $singleItem) {
            foreach ($data as &$product) {
                ...
                $currency = ! empty($product['currency']) ? $product['currency'] : $product['fullinfo']['Item']['currencysign'];
                $product['display_price'] = $product['price'] . ' ' . $currency;
            }
        } else {
            ...
            $currency = ! empty($data['currency']) ? $data['currency'] : $data['fullinfo']['Item']['currencysign'];
            $data['display_price'] = $data['price'] . ' ' . $currency;
        }
