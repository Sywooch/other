...
class SupportRepositoryNew extends SupportRepository
{
    ...
    public function getOrderTicketsNew($from, $perpage, $arFilter, $isOrders)
    {
        ...
        $whereSQL = $this->getFilter($arFilter, $isOrders);
        $userTicketsQ = "
            SELECT DISTINCT `s1`.`orderid` FROM `site_support` as `s1`
            $whereSQL `s1`.`parent` = 0
    ...
    public function getTotalCountNotAnswered($isOrders)
    {
        $this->cms->checkTable('site_support');
        if ($isOrders) {
            $whereSQL = "`s1`.`orderid` <> '' ";
    ...
    public function getTotalCountNew($isOrders)
    {
        $this->cms->checkTable('site_support');
        if ($isOrders) {
            $whereSQL = "`s1`.`orderid` <> '' ";
    ...
    public static function getMessagesCountStatic($new = false, $isOrder = false)
    {
        $cms = new CMS();
        ...
        $whereString = 'WHERE ' . implode(' AND ', $where) . ' AND `parent` = 0';
        $sql = "
                SELECT COUNT(*)
                FROM `site_support`
                $whereString
