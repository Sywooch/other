...
class PaymentProxy
{
    ...
    public function __construct($ps_name, $cms) {
        ...
        $server = defined('CFG_PAYMENT_SERVER') ? CFG_PAYMENT_SERVER :
            (OTBase::isTest() ?
                'http://paymentsystemconnector.payments.dev.services.opentao.net/' :
                'http://paymentsystemconnector.payments.demo.services.opentao.net/');
    ...
    public function success(){
        $page = $this->cms->GetPageByAlias($this->_ps_name.'_success');
        if($page){
            header('Location: http://'.$_SERVER['HTTP_HOST'].'/index.php?p='.
                $this->_ps_name.'_success');
        }
        else{
            header('Location: http://'.$_SERVER['HTTP_HOST'].'/index.php?p=payment_success');
        }
    }

    public  function fail(){
        $page = $this->cms->GetPageByAlias($this->_ps_name.'_fail');
        if($page){
            header('Location: http://'.$_SERVER['HTTP_HOST'].'/index.php?p='.
                $this->_ps_name.'_fail');
        }
        else{
            header('Location: http://'.$_SERVER['HTTP_HOST'].'/index.php?p=payment_fail');
        }
    }
}
