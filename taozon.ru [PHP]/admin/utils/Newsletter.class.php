...
class Newsletter  extends GeneralUtil{
    ...
    public function defaultAction()
    {
        ...
        //var_dump($cms->getSiteConfig()); die;
        $this->tpl->assign('siteConfig', $cms->getSiteConfig());
	...
	public function importAction(){
        ...
        public function sendTestLetterAction(){
            @define('NO_DEBUG', 1);
            
            $params = $_POST;
            $cfg = $this->cms->getSiteConfig();
            $params['site_config'] = $cfg[1];
            
            if (!trim($params['PublicationTitle'])) {
                echo LangAdmin::get('enter_pub_title');
                return;
            } elseif (!trim($params['PublicationText'])) {
                echo LangAdmin::get('enter_pub_text');
                return;
            }
            
            try {
                Notifier::sendAdminTestLetter($params);
            }
            catch(Exception $e){
                $this->throwAjaxError($e);
            }
            echo LangAdmin::get('sended');
