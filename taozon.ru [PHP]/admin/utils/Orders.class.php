...
class Orders extends GeneralUtil {
    ...
    function savenewpackageAction()
    {
        if (Login::auth())
        {
            ...
            Notifier::sendEmailCreatePackage($user, $id, $r['id']);
            ...
            */
            ...
            if (! $error) {
                if ((isset($_POST['PriceInternal'])) && (isset($_POST['CurrentPriceInternal'])) && ((float)$_POST['PriceInternal'] != (float)$_POST['CurrentPriceInternal'])) {
                    $user_info  = $otapilib->GetUserInfoForOperator($sid, $order_info['salesorderinfo']['custid']);
                    Notifier::notifyUserOnChangePackagePrice($user_info, $order_info, $pid, $_POST['PriceInternal'], $_POST['CurrentPriceInternal']);
                }
                ...
                if (isset($_POST['status']) && ($_POST['status'] != @$_POST['old_status'])) {
                    ...
                    Notifier::notifyUserOnChangePackageStatus(
                        $userInfo, $order_info, $pid, $oldStatusId, $newStatusId, $oldStatus, $newStatus
                    );
                }
            }

            if ($order_info['salesorderinfo']['statuscode'] == self::STATUS_WAITING_SURCHARGE) {
                Notifier::notifyUserOnOrderNeedSurcharge($order_info);
            }
    ...
    private function ClearAccountInfoCacheByOrderInfo($otapilib, $sid, $orderInfo)
    {
        ...
        try {
            $userId = $orderInfo['SalesOrderInfo']['CustId'];
