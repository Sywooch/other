...
class Update {
    ...
    public function defaultAction() {
        ...
        ob_start();
        $c = curl_init('http://support.opentao.net/ru/test_site_speed/update_silent_check/'.
            $versions->Version[0]->Number);
        if(! ini_get('safe_mode') && ! ini_get('open_basedir') && ! defined('CFG_NO_CURLOPT_FOLLOWLOCATION')) {
            curl_setopt($c, CURLOPT_FOLLOWLOCATION, true);
        }
        curl_setopt($c, CURLOPT_REFERER, 'http://' . $_SERVER['HTTP_HOST'] . '/admin/');
        curl_exec($c);
    ...
    public function RecoverAction($request) {
        ...
        if(@$request->getValue('file')) {
            chdir(dirname(dirname(dirname(__FILE__))));
            require_once 'admin/lib/pclzip/pclzip.lib.php';
            $path = 'backup/'.$request->getValue('file');
            $archive = new PclZip($path);
            $list = $archive->extract(PCLZIP_CB_PRE_EXTRACT, 'ExtractCallBack');

            if(!$list){
                print 'Распаковка не удалась. Служебная информация:<br />'."ERROR : ".$archive->errorInfo(true);
            } else {
    ...
    {
        $url = 'http://tools.opentao.net/update_rep/info/info.php';
        $ch = curl_init();
