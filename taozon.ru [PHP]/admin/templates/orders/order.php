...
<div class="main">
    ...
    ?>
        ...
        <? if(CMS::IsFeatureEnabled('CDEKIntegration')){ ?>
            <div style="display: block; border-radius: 5px; background-color: #ccc; width: 40%; padding: 5px">
                <?=LangAdmin::get('cdek_test_available_message')?>
            </div>
        <? } ?>
...
.button()
.click(function () {
    window.open('http://<?=$_SERVER['SERVER_NAME']?>/admin/index.php?cmd=orders&do=printdeclaration&id=<?=$order_info['salesorderinfo']['id']?>', '_blank');
});
