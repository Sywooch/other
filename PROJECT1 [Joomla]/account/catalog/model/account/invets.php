<?php
class ModelAccountInvets extends Model {	
	...
	$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE balance > '0' ");
	...
	$last_quer = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_transaction WHERE customer_id =  '".(int)$customer['customer_id']."' AND date_added <='".$last_date."' ORDER BY date_added ASC");
	...
	$last_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_transaction WHERE customer_id =  '".(int)$customer['customer_id']."' AND date_added >'".$last_date."' ORDER BY date_added ASC");
	...
	$quer = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_transaction WHERE customer_id =  '".(int)$customer['customer_id']."' AND date_added >'".$last_date."' ORDER BY date_added ASC");
	...
	$this->db->query("INSERT INTO " . DB_PREFIX . "customer_transaction SET customer_id = '" . (int)$customer['customer_id'] . "', description = 'Начисление прибыли', amount = '" . (float)$add . "', date_added = NOW()");
		
	$this->db->query("UPDATE " . DB_PREFIX . "customer SET balance=balance+'".(float)$add."' WHERE customer_id = '" . (int)$customer['customer_id'] . "'");
	
	if($customer['partner_id']) {
		
		///тут нужно зачислить средства на счет привлекшего

	$this->db->query("INSERT INTO " . DB_PREFIX . "customer_transaction SET customer_id = '" . (int)$customer['partner_id'] . "', description = 'Прибыль от рефералов', amount = '" . (float)$ref . "', date_added = NOW()");
		
	$this->db->query("UPDATE " . DB_PREFIX . "customer SET balance=balance+'".(float)$ref."' WHERE customer_id = '" . (int)$customer['partner_id'] . "'");
	$this->db->query("UPDATE " . DB_PREFIX . "customer SET ref_balance=ref_balance+'".(float)$ref."' WHERE customer_id = '" . (int)$customer['partner_id'] . "'");
	}
