...
############################################
function get_naclad(){
	$sekects=red("SELECT * FROM ad_requests WHERE id ='".$_GET['nums']."' limit 1");
	 ...
	 if(is_array($sse) && count($sse)>0){
		 ...
		 foreach ($sse as $key => $sssxx) {
			 		  $text.='<tr>
								...
								<td align="center">
								<select class="" name="tupe_obrash['.$sssxx['NakNom'].']">
										<option value="0">Выбрать</option>
										<option value="1">Брак</option>
										<option value="2">Возврат</option>
										<option value="3">Обмен</option>
								</select>
								</td>
								<td align="center"><input name="vozvrat['.$sssxx['NakNom'].']" style="width:50px" type="text" value="0"></td>
							  </tr>';
		 }
		  $text.='</table>';
		  return $text;
...
function get_naclad2(){
	$sekects=red("SELECT * FROM ad_requests WHERE id ='".$_GET['nums']."' limit 1");
$sarray[0]='-';$sarray[1]='Брак';$sarray[2]='Возврат';$sarray[3]='Обмен';
	$crossp=red("SELECT * FROM as_bad_quest WHERE requet_id ='".$_GET['nums']."'");
...
<head>
	...
	.users p,.agent p{font-size:12px;display: inline-block;width: 450px;}
	</style>
...
<?
$error_titl="";$tupe_obrash="";$comment_obrash="";
if(isset($_POST) && isset($_POST['names_title']) && isset($_POST['tupe_obrash']) && isset($_POST['comment_obrash']) && isset($_POST['numms'])){
	//print_r($_POST);
	if(isset($_POST['names_title']) && strip_tags($_POST['names_title'])!=""){
		$names_titl=strip_tags($_POST['names_title']);
	}else{
		$error_titl="error";
	}
	if(isset($_POST['tupe_obrash'])){
		$tupe_obrash['tupe']=$_POST['tupe_obrash'];
		$tupe_obrash['col']=$_POST['vozvrat'];
	}
	...
	if($comment_obrash!="error" && $tupe_obrash!="error" && $error_titl!="error"){
		$my_file=array();
		...
		query("INSERT INTO as_bad_quest VALUES ('' ,'".mktime()."', '".$_POST['numms']."', '".$names_titl."', '".serialize($tupe_obrash)."', '".$comment_obrash."', '".serialize($my_file)."', '', 'NEW')");
		$sss="COMPLATE";
	}
...
if($sss=="COMPLATE"){
	...
	$crop=red("SELECT * FROM as_bad_quest WHERE requet_id ='".$_GET['nums']."'");
...
if(isset($_POST) && isset($_POST['comment_obrash']) && isset($_POST['numms']) && !isset($_POST['tupe_obrash'])){
	...
	if($comment_obrash!="error"){
		...
		//print_r($my_file);
		$my_sar=array();
		$provs=red("SELECT *  FROM as_bad_quest WHERE requet_id ='".$_POST['numms']."'");
		...
		//print_r($my_sar);
		query("UPDATE as_bad_quest SET comment_tups='".serialize($my_sar)."',status ='OPERATOR' WHERE requet_id ='".$_POST['numms']."'");
	...
	<h3>Обращение по заказу № <?=$crop[0]['requet_id']?></h3>
	...
	<table class="sdkljhb" >
		...
		?>
		<form class="dss" method="post" action="" enctype="multipart/form-data"">
		<input type="hidden" name="numms" value="<?=$_GET['nums']?>">
		<tr>
			<td colspan="2"><textarea placeholder="Написать коментарий" style="width: 496px;"  class="comment_obrash" name="comment_obrash" style="<?=(($comment_obrash=="error")?"border:1px solid red":"")?>"></textarea></td>
		</tr>
		<tr>
			<td style="width:200px;"><b>Прикрепить файлы</b> <a href="javascript:void(0)" title="добавить файл" class="add_filess_tupe">+</a></td>
...
if($sss=="COMPLATE"){
	echo "<br><br><br><br><br><br><br><center><b>Ваша обращение отправлена. С вами свяжется наш менеджер</b></center>";
...
<form class="dss" method="post" action="" enctype="multipart/form-data"">
<input type="hidden" name="numms" value="<?=$_GET['nums']?>">
	<h3>Оформление обращения</h3>
	<?=get_naclad();?>
	<table class="sdkljhb" width="500px">
		<tr>
			<td width="200px"><b>Заголовок обрашения</b></td>
			<td><input type="text" name="names_title" class="names_title" value="<?=$names_titl?>" style="<?=(($error_titl=="error")?"border:1px solid red":"")?>"></td>
		</tr>
		<tr>
			<td colspan="2"><b>Описание проблемы</b></td>
		</tr>
		<tr>
			<td colspan="2"><textarea class="comment_obrash" name="comment_obrash" style="<?=(($comment_obrash=="error")?"border:1px solid red":"")?>"><?=(($comment_obrash!="error")?$comment_obrash:"")?></textarea></td>
		</tr>
		<tr>
			<td><b>Прикрепить файлы</b> <a href="javascript:void(0)" title="добавить файл" class="add_filess_tupe">+</a></td>
			<td class="propends"><input type="file" id="file_name" name="file_name[]" value=""></td>
		</tr>
		<tr>
			<td colspan="2"><br><br><center><div class="button_left" rel=""> <div class="button_right"><div class="button_middle" id="dfsgh">Отправить претензию<input class="dont" type="submit" value=" Отправить претензию" style="position: absolute; top: -99999px; left: -99999px;"></input></div></div></div></center></td>
		</tr>
	</table>
	</form>
<?}}?>
