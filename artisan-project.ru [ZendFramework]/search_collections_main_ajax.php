...
//echo $start_number."+".$start_number2."+";
		...
		//foreach($tmp as $value) {
			...
			*/
			...
			if(($log2==1)&&($log3==1)&&($log4==1)&&($log5==1)&&($log6==1)&&($log7==1)&&($log8==1)){
				...
				if(($i_global%4==0)&&($i_global!=0)||($i_global==4)){ 
					
				$search_collections_m[$i_global-1]=$search_collections_m[$i_global-1].'
                </div><div class="row row_catalog_front">';
						
                }
				if($i_global>=32){ $_SESSION['start_collection']=$i_global2;  break; }; 
				$i_global++;
				$local_cnt++;
		...
		$search_collections='<div class="row row_catalog_front">';
		foreach($search_collections_m as $val){	
			$search_collections=$search_collections.$val;	
			
		}
		$search_collections=$search_collections.'</div>';
		if($local_cnt<32){ $search_collections=$search_collections.'<style type="text/css">.moreBtns{ display:none; }</style>'; }
		
		
		$search_collections=$search_collections."-!-".$_SESSION['start_collection'];
