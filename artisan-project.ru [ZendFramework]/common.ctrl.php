...
class CommonController extends Site_Common_Controller
{
	public function run()
	{
		...
		//////////////////////
		/*$new_arr=array('549439','549688','549687','549686');
		$cond            = array('where' => array('goods_to_order' . '.id' => array('IN', $new_arr, '(?l)')));
		$goods           = $this->model('db')->getList(
                    'goods_to_order',
                    array(), $cond
                );
                $counts          = 1;
                ...
                foreach ($goods as $good) {
                   ...
                   // $art_key = array_search($good['id'], (!empty($_POST['ids']) ? $_POST['ids'] : $this->app->session->ids));
                    $count   = 1;
                    //if (isset($_POST['count'][$art_key]) and !empty($_POST['count'][$art_key])) {
                    //    $count = $_POST['count'][$art_key];
                    //}
                    $count = str_replace(',', '.', $count);
					$this->page->settings['euro_rub']=1;
                    if ($count < $good['colprice2']) $good['price'] = number_format($good['price1'] * $this->page->settings['euro_rub'], 2, '.', '');
                    else $good['price'] = number_format($good['price2'] * $this->page->settings['euro_rub'], 2, '.', '');

                    if ($good['max_sk'] == 100) $category_amount_of_discount = 0 / 100;
                    else $category_amount_of_discount = $good['max_sk'] / 100;

                    if ($category_amount_of_discount) {
                        $good['price'] = number_format($good['price'] - ($good['price'] * $category_amount_of_discount), 2, '.', '');
                    }

                    $good['total'] = $good['price'] * $count;
                    $good['count'] = $count;
                   // isset($_POST['count_pack'][$art_key]) ? $good['count_pack'] = $_POST['count_pack'][$art_key] : $good['count_pack'] = 0;
                   // isset($_POST['count_unit'][$art_key]) ? $good['count_unit'] = $_POST['count_unit'][$art_key] : $good['count_unit'] = 0;
                    //$goods_tmp[$art_key] = $good;
                    //$counts[]            = $goods_tmp[$art_key]['count'];
                }
		...
		*/
		...
		//echo "=".$this->page->dealer['category_amount_of_discount']."=";
		...
		//print_r($_SESSION);
		//echo"</pre>";
		//echo 'Время #1 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #2 : '.(microtime(true) - $start).' сек.<br>';
		...
		if ($this->app->session->did) {
			...
			if (time() - $ch_pass_time > $s['pwd_exp_dealer']*30*24*60*60) {
				if (strpos($_SERVER['REQUEST_URI'],'profile/change_pass')===false){
					
					$this->page->change_pass_error = array('Срок действия пароля истек, введите новый пароль.');
					
					
					header('Location: '.zf::$root_url.'profile/change_pass/?chp=1');
       ...
       // $this->page->main_slider = $this->model('banners')->getList('banners2', array('id', 'title', 'descr', 'image', 'url'), array('order' => array('id' => 'asc')));
		...
		//##echo 'Время #3 : '.(microtime(true) - $start).' сек.<br>';
		...
		//лента документов в личном кабинете
		if( strpos($this->app->request->uri,'requests')!=false || strpos($this->app->request->uri,'shipment')!=false || strpos($this->app->request->uri,'complaints')!=false || strpos($this->app->request->uri,'profile')!=false ){
		
		
			$docs_lk=$this->model('documents')->getList('documents', array('id', 'cdate', 'title', 'file'), array('order' => array('lk_num' => 'asc'), 'where' => array('lk' => 'yes'), 'limit' => '3'));
			
			
			for($i=0;$i<count($docs_lk);$i++){
				$title=$docs_lk[$i]['title'];
				$title=strip_tags($title);
				$title=trim($title);
				$title_full=$title;
				//echo "==".$desc."==<br>";
				$title = $this->getPrewText($title,6,25);
				$docs_lk[$i]['title']=$title;
				$docs_lk[$i]['title_full']=$title_full;
				//echo $docs_lk[$i]['title_full']."<br>";
					
			}	
			
			$this->page->docs_lk = $docs_lk;
			
			
			
			
			$news_lk=$this->model('news')->getList('news', array('id', 'cdate', 'title'), array('order' => array('cdate' => 'desc'), 'where' => array('lk' => 'yes'), 'limit' => '2'));
			
			
			for($i=0;$i<count($news_lk);$i++){
				$title=$news_lk[$i]['title'];
				$title=strip_tags($title);
				$title=trim($title);
				//echo "==".$desc."==<br>";
				$title_full=$title;
				$title = $this->getPrewText($title,4,25);
				
				$news_lk[$i]['title']=$title;
				$news_lk[$i]['title_full']=$title_full;
			}	
			
			$this->page->news_lk = $news_lk;
		...
		//##echo 'Время #4 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #5 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #6 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #7 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #8 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #9 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #10 : '.(microtime(true) - $start).' сек.<br>';
		...
		if((strpos($this->app->request->uri,'cat/id'))!=false ){
			$collections2based_sizes_1=$this->model('db')->getList('based_sizes2collections', array('based_size_id'), array('order' => array('collection_id' => 'ASC'),'where' => array('collection_id' => $this->app->request->vars['id'])));
			...
			foreach ($collections2based_sizes_1 as $value1) {
				...
				//$value1['based_size_id']
				$tmp=$this->model('db')->getList('based_sizes', array('title'), array('order' => array('title' => 'ASC'),'where' => array('id' => $value1['based_size_id'])));
				
				$based_sizes_detail[]=$tmp[0]['title'];
				
			
			}
			$this->page->based_sizes_detail=$based_sizes_detail;
		}
		...
		//##echo 'Время #11 : '.(microtime(true) - $start).' сек.<br>';
		...
		$this->page->fabrics1 = $this->model('db')->getList(
                       'factories',
                        array('id','url','title','country_id','end'), array('where' => array('hidden' => 'no','country_id' => $tmp_country[0]['id']), 'order' => array('country_id' => 'ASC','title' => 'ASC'))
                    );
		//##echo "fabrics1= ".count($this->page->fabrics1)."<br>";			
		}else{
		$this->page->fabrics1 = $this->model('db')->getList(
                       'factories',
                        array('id','url','title','country_id','end'), array('where' => array('hidden' => 'no'), 'order' => array('country_id' => 'ASC','title' => 'ASC'))
                    );
		...
		//##echo 'Время #12 : '.(microtime(true) - $start).' сек.<br>';
		...
		foreach($this->page->collections_sale as $v){
			//идентификатор коллекции - цена коллекции
			$collection_price[$v['id']]=$this->collection_price($v['id']);
			
			$tmp=$this->model('db')->getList(
                       'factories',
                        array('id','title','country_id','url'), array('where' => array('id' => $v['factory_id']), 'order' => array('title' => 'ASC'))
                    );
			//идентификатор коллекции - наименование фабрики, ссылка на страницу фабрики
			$collection_param[$v['id']]['factory_title']=$tmp[0]['title'];
			$collection_param[$v['id']]['factory_url']=$tmp[0]['url'];
			
			$country_id=$tmp[0]['country_id'];
			
			$tmp=$this->model('db')->getList(
                       'countries',
                        array('id','url','import_title'), array('where' => array('id' => $country_id), 'order' => array('import_title' => 'ASC'))
                    );
			$collection_param[$v['id']]['country_title']=$tmp[0]['import_title'];
			$collection_param[$v['id']]['country_url']=$tmp[0]['url'];
		...
		foreach($this->page->collections_new as $v){
			//идентификатор коллекции - цена коллекции
			$collection_price[$v['id']]=$this->collection_price($v['id']);
		...
		foreach($this->page->collections_action as $v){
			//идентификатор коллекции - цена коллекции
			$collection_price[$v['id']]=$this->collection_price($v['id']);
		...
		$this->page->collection_param=$collection_param;
		
		
		//главная страница или страница каталога
		}
		$this->page->collection_price=$collection_price;
					
		//##echo 'Время #13 : '.(microtime(true) - $start).' сек.<br>';		
		...
		//##echo 'Время #14 : '.(microtime(true) - $start).' сек.<br>';
		...
		*/
		...
		//##echo 'Время #15 : '.(microtime(true) - $start).' сек.<br>';	
		...
		$collections_list=$this->model('db')->getList(
                       'collections',
                        array('id','title','factory_id','novice','sale','action','image'), array('where' => array('hidden' => 'no'), 'order' => array('title' => 'ASC'),'limit' => '1')
                    );
		...
		*/
		...
		//##echo 'Время #16 : '.(microtime(true) - $start).' сек.<br>';
		...
		//$list_collections_prices=$this->list_collections_prices();
		...
		//##echo 'Время #17 : '.(microtime(true) - $start).' сек.<br>';
		...
		if(strpos($this->app->request->uri,'cat/id')!=false){
			...
			$tmp=$this->model('db')->getList(
                      'collections',
                       array('title','action','novice','sale'), array('where' => array('id' => $this->app->request->vars['id']))
                   );
			$this->page->collection_name=$tmp[0]['title'];
			$this->page->current_factory=$this->current_factory($this->app->request->vars['id']);
			$this->page->current_price=$this->collection_price($this->app->request->vars['id']);
			
			$collection_sale=$tmp[0]['action'];
			$collection_novice=$tmp[0]['novice'];
			$collection_action=$tmp[0]['sale'];
			
			$this->page->collection_sale=$collection_sale;
			$this->page->collection_novice=$collection_novice;
			$this->page->collection_action=$collection_action;
			
			
			$list_collections_images2=$this->model('db')->getList(
                       'images',
                        array('id','image'), array('where' => array('hidden' => 'no','model' => 'collections','pid' => $this->app->request->vars['id']), 'order' => array('id' => 'ASC'))
                    );
			foreach($list_collections_images2 as $value2) {
				//echo $value["id"]."=";
				$detail_images_list[]=$value2['image'];
			...
			//# $list_collections_goods_purposes[идентфикатор товара]=назначение товара
			//вытащить все товары коллекции
			...
			foreach($goods_tmp as $value2) {	
				...
				foreach($list_goods_purposes_1 as $value3) {
					//$list_collections_goods_purposes[$value["id"]][$value2["id"]]="";
					//[идентификатор коллекции][идентификатор товара]=идентификатор назначения товара
					$list_collections_goods_purposes[$value2["id"]][]=$value3["purpose_id"];
			...
			$current_factory_id=$this->current_factory_id($this->app->request->vars['id']);
			$list_factories2_collections=$this->model('db')->getList(
                       'collections',
                        array('id','title'), array('where' => array('hidden' => 'no','factory_id' => $current_factory_id), 'order' => array('title' => 'ASC'))
                    );
			...
			foreach($list_factories2_collections as $value2) {
				//список коллекций оппределённой фабрики
				echo $value2['title']." - ".$value2['id']."<br>";
				
				$list_fabric_collections['title'][]=$value2['title'];
				$list_fabric_collections['id'][]=$value2['id'];
			}
			*/
			
			$this->page->list_fabric_collections=$list_factories2_collections;
		...
		$this->page->fabrics3 = $this->model('db')->getList(
                       'factories',
                        array('id','url','title','country_id','end'), array('where' => array('hidden' => 'yes'), 'order' => array('title' => 'ASC'))
                   );
		...
		$fabrics1 = $this->model('db')->getList(
                       'factories',
                        array('id','url','title','country_id','end'), array('where' => array('hidden' => 'no'), 'order' => array('title' => 'ASC','country_id' => 'ASC'))
                    );
		*/			
		/*	
		$fabrics1 = $this->model('db')->getList(
			'factories',
			$this->model('db')->getFieldsNames('factories', 'search'),
			array('where' => array('factories.hidden' => 'no'),'order' => array('title' => 'ASC'))
		);
		*/	
		$fabrics1 = $this->model('db')->getFactories(0); 			
		
		
		
		
		$data=file_get_contents($_SERVER['DOCUMENT_ROOT']."/dilers/".$_SESSION['did'].".td");
		$data_array=unserialize($data);
		
				
		$new_factory=array();
		foreach ($fabrics1 as $k=> $v) {
			if(!in_array($v['id'],$data_array)){
				$new_factory[]=$v;
			}
		}
		...
		*/	
		...
		$list_collections2  = $this->model('db')->getCollections($_GET['fabrics'], (boolean)$hid2);
		...
		foreach($list_collections2 as $vals){
			...
			echo '<option value="'.$vals['id'].'">'.iconv("utf-8","windows-1251",$vals['title']).'</option>';
		...
		if(isset($_POST['arts']) && $_POST['arts']!=""){
				...
				$db='goods';
				$t_fields=array('id','title','art','photo','collection_id','unit_id','weight','packCntByUnit','packCntByArea','packCntByCount', 'sale', 'action');
				//$hid='no';
				if($_POST['rtupe']=="reservation"){
					
					$db='goods_to_order';
					$t_fields=array('id','title','art','photo','collection_id','unit_id','weight','packCntByUnit','packCntByArea','packCntByCount','price1');
				}
				$goodsss=$this->model('db')->getList(
                       $db,
                       $t_fields, array('order' => array('id' => 'DESC'),'where' => array('art' => array('LIKE', '%'.$_POST['arts'].'%', "?") )) 
                    );//'hidden' => 'no',
			...
			if(isset($_POST['namma']) && $_POST['namma']!=""){
				...
				if($_POST['rtupe']=="reservation"){
					...
					$goodsss=$this->model('db')->getList(
                       $db,
                        $t_fields, array('order' => array('id' => 'DESC'),'where' => array('title' => array('LIKE', '%'.$_POST['namma'].'%', "?")))
                    );	
				//'hidden' => 'no',
			}else{
			$db='goods';
			...
			if($_GET['rtupe']=="reservation"){
				$db='goods_to_order';
				$t_fields=array('id','title','art','photo','collection_id','unit_id','weight','packCntByUnit','packCntByArea','packCntByCount','price1','price2','max_sk');
			}
			...
			$goodsss=$this->model('db')->getList(
                       $db,
                       $t_fields, array('order' => array('art' => 'ASC'),'where' => array('collection_id' => $_GET['colllection']))
                    );
					//'hidden' => 'no',
...
//print_r($goodsss);	
		...
		$tmp = zf::$db->query("SELECT * FROM ad_dealers WHERE id ='".$_SESSION['did']."' ");
		$category_id=$tmp[0]['category_id'];	
		$tmp = zf::$db->query("SELECT * FROM ad_dealers_categories WHERE id ='".$category_id."' ");
		$amount_of_discount=$tmp[0]['amount_of_discount'];	
		...
		foreach($goodsss as $k => $value1) {
			...
			if($_GET['rtupe']=="reservation"){
				...
				$prices_1[$value1["id"]]=$value1["price1"];
				$prices_2[$value1["id"]]=$value1["price1"];
		...
		$propers2='';
		//print_r($goodsss);
		if(isset($_GET['zakaz']) && $_GET['zakaz']==1){$propers='<span class="underTheOrderMarker">под заказ</span>'; $propers2="OrderMarker"; }else{
			$propers='<span class="nalMarker"></span>'; 	
		}
		...
		if($_GET['rtupe']=="reservation"){
			...
			$tmp1=$this->model('db')->getList(
                       'prices2goods',
                        array(), array('where' => array('good_id' => $val["id"]))
                    );
			if(count($tmp1)==0){ continue; };
		}
		
		
			
			
		//пропустить товары, чьи наименования содержат "постер" и пр.
		if((mb_stripos($val['title'],'постер', 0, 'UTF-8') !== false) || (mb_stripos($val['title'],'каталог', 0, 'UTF-8') !== false) || (mb_stripos($val['title'],'фолдер', 0, 'UTF-8') !== false) || (mb_stripos($val['title'],'панель', 0, 'UTF-8') !== false)){
			continue;
		...
		//под заказ	
			//echo "--".$val['max_sk']."--";
			$tmp = zf::$db->query("SELECT * FROM ad_settings WHERE name ='euro_rub' ");
			$euro_rub=$tmp[0]['value'];
			//echo "==".$euro_rub."==";
			
			
			
			$count=1;
			if ($count < $val['colprice2']) $price1 = number_format($prices_1[$val['id']] * $euro_rub, 2, '.', '');
                    else $price1 = number_format($prices_2[$val['id']] * $euro_rub, 2, '.', '');

			//echo "--".$price1."--";
						
			$amount_of_discount=0;
			if($val['max_sk']==100){
				$amount_of_discount=$amount_of_discount/100;
			}else{
				$amount_of_discount=$val['max_sk']/100;
			}
			
			//echo "++".$amount_of_discount."++";
			
			if ($amount_of_discount) {
				$price1=$prices_1[$val['id']]-($prices_1[$val['id']]*$amount_of_discount);
			}
			...
			if ($count < $val['colprice2']) $price2 = number_format($prices_2[$val['id']] * $euro_rub, 2, '.', '');
                   else $price2 = number_format($prices_1[$val['id']] * $euro_rub, 2, '.', '');

			
			//$price2=$prices_2[$val['id']]*$euro_rub;		
			...
			if ($amount_of_discount) {
				...
				$price2=$prices_2[$val['id']]-($prices_2[$val['id']]*$amount_of_discount);
				//$price2=$prices_2[$val['id']]*$euro_rub*$amount_of_discount;
			}
			
			
			$price_html='<p class="formPrice">Цена <span class="price">'.round($price1,2).'</span> руб<!--, 
			Цена2 <span class="price2">'.$price2.'</span> руб <span class="result">-->, стоимость <span class="lastPrice">0</span> руб</span></p>
		...
		//в наличии	
			$price_html='<p class="formPrice">Цена <span class="price">'.round($prices_1[$val['id']],1).'</span> руб <span class="result">, стоимость <span class="lastPrice">0</span> руб</span></p>';
			
			$tmp_img='<img style="max-height:100px;max-width: 350px;" src="'.$val['photo'].'" alt="'.$val['title'].'">';
		}
                                                ...
                                                <div class="inputs">
                                                    <div class="calculate">
                                                        <div class="meters block">
                                                            <input class="fileStyle" name="count[]" autocomplete="off"  data-type="" data="'.(($val['unit_id']==1)?'м2':'шт').'" step="0.01" type="text" disabled><span class="type">'.(($val['unit_id']==1)?'м2':'шт').'</span>

                                                        </div>
                                                        <div class="wraps block">
                                                            <input class="fileStyle" name="count_pack[]" autocomplete="off" data="'.$val['packCntByUnit'].'" type="number" disabled><span class="type">уп</span>

                                                        </div>
                                                        <div class="sum block"> 
                                                            <input class="fileStyle" name="count_unit[]" autocomplete="off" data="'.$val['packCntByCount'].'" type="number" disabled><span class="type">пл</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="remain">
		...
		//number_format(round($prices_1[$val['id']],1), 1, '.', ' ' )
		...
		//##echo 'Время #18 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #19 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #20 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #21 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #22 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #23 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #24 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #25 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #26 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #27 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #28 : '.(microtime(true) - $start).' сек.<br>';
		...
		//##echo 'Время #29 : '.(microtime(true) - $start).' сек.<br>';
		...
		foreach($tmp as $value) {
			...
			$price=$this->collection_price($value["id"]);//цена коллекции
			...
			if(($log1==1)&&($log2==1)&&($log3==1)&&($log4==1)&&($log5==1)&&($log6==1)&&($log7==1)&&($log8==1)){
				...
				$tmp=$this->model('db')->getList(
                       'factories',
                        array('id','title','country_id','url'), array('where' => array('id' => $value['factory_id']), 'order' => array('title' => 'ASC'))
                    );
				//идентификатор коллекции - наименование фабрики, ссылка на страницу фабрики
				$collection_param[$value['id']]['factory_title']=$tmp[0]['title'];
				$collection_param[$value['id']]['factory_url']=$tmp[0]['url'];
				...
				$collection_param[$value['id']]['country_title']=$tmp[0]['import_title'];
				$collection_param[$value['id']]['country_url']=$tmp[0]['url'];
			
				
				$value['price']=$this->collection_price($value['id']);
		...
		$this->page->collection_param=$collection_param;
		$this->page->collection_next_number=$count_global;
	...
	//получение цены коллекции по идентификатору коллекции
	public function collection_price($collection_id){
		$goods_tmp=$this->model('db')->getList(
                       'goods',
                        array('id'), array('order' => array('id' => 'DESC'),'where' => array('hidden' => 'no','collection_id' => $collection_id))
                    );
		unset($tmp);
	...
	//получение сведений о фабрике по идентификатору коллекции
	public function current_factory($collection_id){
		$tmp=$this->model('db')->getList(
                       'collections',
                        array('factory_id'), array('where' => array('id' => $collection_id))
                    );	
		$factory_id=$tmp[0]['factory_id'];
		
		$tmp=$this->model('db')->getList(
                       'factories',
                        array('title','url','country_id'), array('where' => array('id' => $factory_id))
                    );	
		$current_factory['url']=$tmp[0]['url'];
		$current_factory['name']=$tmp[0]['title'];
		$country_id=$tmp[0]['country_id'];
		$tmp=$this->model('db')->getList(
                       'countries',
                        array('flag'), array('where' => array('id' => $country_id))
                    );
		$current_factory['flag']=$tmp[0]['flag'];			
		return $current_factory;
	}
	
	public function current_factory_id($collection_id){
		$tmp=$this->model('db')->getList(
                       'collections',
                        array('factory_id'), array('where' => array('id' => $collection_id))
                    );	
	...
	/*
	//список базовых размеров коллекции
	public function current_based_sizes($collection_id){
	
		return $current_based_sizes;
	}
	*/
	...
	public function declension_words($num,$arWords){
    if ($num < 21){
        if ($num == 1)
            $w = $arWords[0];
        elseif ($num > 1 && $num < 5)
