...
class form
{
	...
	public function get_images_input($inp, $attrStr)
	{
		if (isset($inp['link'])) {
			...
			$ret .= "<input class=\"fileStyle show\" type=\"file\" name=\"{$inp['name']}[]\" multiple {$attrStr}/>";
	...
	public function get_file_input($inp, $attrStr)
	{
		if (!is_array($inp['value']) && $inp['value'] && strrpos($inp['value'], '.tmp') != strlen($inp['value']) - 4) {
			...
			$url = (strpos($this->ctrl->app->request->uri, 'modify') !== false) ?
				(str_replace('modify', 'delete_file', $this->ctrl->app->request->uri)) :
				$this->ctrl->app->request->uri.'delete_file/field/'.$inp['name'];
			$ret .= '&nbsp;<a href="'.zf::$root_url.$url.'" title="без подтверждения">удалить</a>';
		} else {
			$ret = '';
		}
