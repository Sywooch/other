    ...
    */
    ...
    if ($curpage > 1) {
    	if ($txt_first != '') {
			...
			if(strpos($_SERVER['REQUEST_URI'],"requests/pretenzii")!==false){
    			$retval .= '<a class="first" page="1" href="/processing/requests/pretenzii/'.$append.'" '.$css_class.'>'.$txt_first.'</a>';
			}else{
    	...
    	if ($txt_prev != '') {
			...
			if(strpos($_SERVER['REQUEST_URI'],"requests/pretenzii")!==false){
    			$retval .= '<a class="prev" page="'.($curpage - 1).'" href="/processing/requests/pretenzii/'.$append.'" '.$css_class.'>'.$txt_prev.'</a>';
			}else{
    ...
    if ($links[0] != 1) {
		...
		if(strpos($_SERVER['REQUEST_URI'],"requests/pretenzii")!==false){
    		$retval .= '<a page="1" href="/processing/requests/pretenzii/'.$append.'" '.$css_class.'>1</a>';
		}else{
    ...
    for($i = 0; $i < sizeof($links); $i++) {
        if ($links[$i] != $curpage or $link_current) {
			...
			if(strpos($_SERVER['REQUEST_URI'],"requests/pretenzii")!==false){
    			$retval .= '<a href="/processing/requests/pretenzii/'.$append.'" page="'.$links[$i].'" '.$css_class.'>'.$links[$i].'</a>';
			}else{
    ...
    if ($links[sizeof($links) - 1] != $pagecount) {
		...
		if(strpos($_SERVER['REQUEST_URI'],"requests/pretenzii")!==false){
				...
				$retval .= '<a href="/processing/requests/pretenzii/'.$baseurl.$page_append.$pagecount.$url_append.'" page="'.$pagecount.'" '.$css_class.'>'.$pagecount.'</a>';
		}else{
    ...
    if ($curpage != $pagecount) {
		...
		if(strpos($_SERVER['REQUEST_URI'],"requests/pretenzii")!==false){
    		$retval .= '<a class="next" page="'.($curpage + 1).'" href="/processing/requests/pretenzii/'.$baseurl.$page_append.($curpage + 1).$url_append.'" '.$css_class.'>'.$txt_next.'</a>';
		}else{	
        ...
        if ($txt_last != '') {
			...
			if(strpos($_SERVER['REQUEST_URI'],"requests/pretenzii")!==false){
				$retval .= '<a class="last" page="'.($pagecount).'" href="/processing/requests/pretenzii/'.$baseurl.$page_append.$pagecount.$url_append.'" '.$css_class.'>'.$txt_last.'</a>';
			}else{
