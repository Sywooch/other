...
class AdvancedModel extends Model
{
    ...
    public function initValues($fields, $misc = null)
    {
		...
		//echo "1=".strripos('employees', 'employees')."=";
		$cPattern = $_SERVER['REQUEST_URI']; //шаблон регулярного выражения
		mb_regex_encoding('UTF-8');   //кодировка строки
  		$vRegs = "employees/modify/id";   //массив с подстроками
		...
		$cPattern2 = $_SERVER['REQUEST_URI']; //шаблон регулярного выражения
		mb_regex_encoding('UTF-8');   //кодировка строки
  		$vRegs2 = "employees/modify_employees_contact/id";   //массив с подстроками
  		mb_eregi($cPattern2, $pValue, $vRegs2);
		
		$cPattern3 = $_SERVER['REQUEST_URI']; //шаблон регулярного выражения
		mb_regex_encoding('UTF-8');   //кодировка строки
  		$vRegs3 = "db/modify_merchandising_element/id";   //массив с подстроками
  		mb_eregi($cPattern3, $pValue, $vRegs3);
		...
		$cPattern5 = $_SERVER['REQUEST_URI']; //шаблон регулярного выражения
		mb_regex_encoding('UTF-8');   //кодировка строки
  		$vRegs5 = "documents/modify/id";   //массив с подстроками
  		mb_eregi($cPattern5, $pValue, $vRegs5);
		
		
		
		if( ($_SERVER['REQUEST_URI']=="employees/add/") || (count($vRegs)>0) ){
			
			$ad=zf::$db->query("SELECT * FROM ad_employees_otdels ORDER BY id");
			
			
			foreach ($ad as $key => $el) {
				//$this->values["employees_otdels"][$key+1]=$el["title"]; 
				$this->values["employees_otdels"][$el["id"]]=$el["title"]; 
				
			}
		}
		
		
		if( ($_SERVER['REQUEST_URI']=="employees/add_employees_contact/") || (count($vRegs2)>0) ){
		
			$ad=zf::$db->query("SELECT * FROM ad_contacts ORDER BY id");
			
			foreach ($ad as $key => $el) {
				//$this->values["employees_otdels"][$key+1]=$el["title"]; 
				$this->values["contacts"][$el["id"]]=$el["title"]; 
				
			}
		}


		if( ($_SERVER['REQUEST_URI']=="db/add_merchandising_element/") || (count($vRegs3)>0) ){
		
			 
			$ad=zf::$db->query("SELECT * FROM ad_collections WHERE hidden='no' ORDER BY title");
			
			foreach ($ad as $key => $el) {
				//$this->values["employees_otdels"][$key+1]=$el["title"]; 
				$this->values["collection"][$el["id"]]=$el["title"]; 
				
			}
			
			$ad=zf::$db->query("SELECT * FROM ad_merchandising_types ORDER BY title");
			
			foreach ($ad as $key => $el) {
				//$this->values["employees_otdels"][$key+1]=$el["title"]; 
				$this->values["type"][$el["id"]]=$el["title"]; 
				
			}
			
			$ad=zf::$db->query("SELECT * FROM ad_factories WHERE hidden='no' ORDER BY title");
			
			foreach ($ad as $key => $el) {
				//$this->values["employees_otdels"][$key+1]=$el["title"]; 
				$this->values["factory"][$el["id"]]=$el["title"]; 
				
			}
			
			$ad=zf::$db->query("SELECT * FROM ad_documents_folders ORDER BY title");
			
			foreach ($ad as $key => $el) {
				//$this->values["employees_otdels"][$key+1]=$el["title"]; 
				$this->values["documents_folders"][$el["id"]]=$el["title"]; 
