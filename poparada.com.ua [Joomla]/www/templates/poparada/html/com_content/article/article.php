...
<!-- ARTICLE BODY -->
	<?php if (strpos($this->item->text, '{intro}') !== false) : ?>
		<?php 
			$introtext = preg_split('/<p>{intro}<\/p>/', $this->item->text, -1, PREG_SPLIT_NO_EMPTY);
			$text = strip_tags($introtext[0], '<img>');
			$textArr = preg_split('/[\s]*(<[^>]*>)/i', $text, -1, PREG_SPLIT_NO_EMPTY);
			preg_match_all('/((?>src).*.(?>"))/i', $text, $imgArr);
		 	
			$this->item->text = end($introtext);
		 ?>
				<div class="container-fluid">
						...
						<?php for ($i=0; $i < count($textArr); $i++):?>
							...
							<?php if($i==0) : ?>
							<?php $url = str_replace('src=','',$imgArr[0][$i]);?> 
							<div class="row"> 
								<div class="col-sm-6">
									<div class="narrow-img" style="background-image: url(<?php echo($url) ?>)">
										<p class="lead"><?php echo($textArr[$i]); ?></p>
									</div>
									<!-- <img <?php echo($src) ?> alt="" class="img-responsive"> -->
								</div>
								<div class="col-sm-6">
