...
';
        ...
        */
        if ($item->category->parent == 3) $params->set('page_title', $item->title . ' - бескаркасная мебель POPARADA');
        elseif ($item->catid == 19) $params->set('page_title', $item->title . ' - Попарада');
        else $params->set('page_title', $item->title . ' - мягкая мебель интернет-магазин Попарада');
        ...
        if ($menu) {
            if (isset($menu->query['view']) && ($menu->query['view'] != 'item' || $menu->query['id'] != $item->id)) {
                if (!isset($menu->query['task']) || $menu->query['task'] != 'category' || $menu->query['id'] != $item->catid) $pathway->addItem($item->category->name, $item->category->link);
        ...
        if ($item->catid == 1) $document->setDescription($item->title . '. БЕСКАРКАСНАЯ МЯГКАЯ МЕБЕЛЬ от производителя ★ Акции и скидки ✈ Доставка Киев, Украина ☎ 044 232 86 40');
        elseif ($item->catid == 19) $document->setDescription($item->title . ' ★ БЕСКАРКАСНАЯ МЯГКАЯ МЕБЕЛЬ от производителя. Низкие цены ✈ Доставка Киев, Украина!');
        else $document->setDescription($item->title . ' ★ МЯГКАЯ МЕБЕЛЬ от производителя. Бескаркасная, офисная, детская. Низкие цены ✈ Доставка Киев, Украина ☎ 093 701-40-40');
