...
//	var $defaultShippingMethod = null; // set in constructor
	...
	function login_validate() {
		...
		if ($user->id) {
			$json['redirect'] = $redirect_url."--9";
		}

		if ((!$cart_helper->hasProducts())) {
			$json['redirect'] = $redirect_url."--10";
		}
	...
	function register_validate() {
		...
		if ($user->id) {
			$json['redirect'] = $redirect_url."--12";
		}

		// Validate cart has products and has stock.
		if (!K2StoreHelperCart::hasProducts()) {
			$json['redirect'] = $redirect_url."--13";
		}
	...
	function guest_shipping_validate() {
		...
		// Validate if customer is logged in.
		if (JFactory::getUser()->id) {
			$json['redirect'] = $redirect_url."--18";
		}

		// Validate cart has products and has stock.
		if ((!$cart_helper->hasProducts())) {
			$json['redirect'] = $redirect_url."--19";
		}
	...
	function billing_address_validate() {
		...
		if (!$user->id) {
			$json['redirect'] = $redirect_url."--1";
		}

		// Validate cart has products and has stock.
		if (!K2StoreHelperCart::hasProducts()) {
			$json['redirect'] = $redirect_url."--2";
		}
...
function shipping_address_validate() {
	...
	if ($showShipping == false) {
		$json['redirect'] = $redirect_url."--4";
	}

	// Validate cart has products and has stock.
	if (!K2StoreHelperCart::hasProducts()) {

		$json['redirect'] = $redirect_url."--5";
	}
	// TODO Validate minimum quantity requirments.
	...
	function shipping_payment_method_validate() {
		...
		if (!$json) {
			...
			if (empty($billing_address)) {
				$json['redirect'] = $redirect_url."--6";
			}

			//cart has products?
			if(!$cart_helper->hasProducts()) {
				$json['redirect'] = $redirect_url."--7";
			}
