...
<?
//проверить есть ли у товара скидка в свойствах
?>    
...
class="bx_price tmp1" 
<?
//если имеется скидка, то старую цену перечёркиваем
 if($arItem['DISPLAY_PROPERTIES']['SALE']['VALUE']!=""){ echo 'style="text-decoration:line-through;"'; } ?>>
...
<?
	...
	?></div>
    <?
	//если скидка задана, то выводим новую цену
    if($arItem['DISPLAY_PROPERTIES']['SALE']['VALUE']!=""){
		$new_price=$result_price - (($result_price * $arItem['DISPLAY_PROPERTIES']['SALE']['VALUE'])/100);
		echo number_format($new_price, 2, '.', ' ')." руб.<br>";//цена с учётом скидки
		echo $arItem['DISPLAY_PROPERTIES']['SALE']['VALUE']." %";
	}
