...
$row_time_options=substr($row['time'],0,5);
	...
	echo'<option value="'.$tmp_count.'">'.$row_time_options.'</option>';
...
//проверка: соответствует ли данному времени какой-либо выпуск новостей.
...
//если будет найдена пара radio - time, 
//соответствующая паре $radio_check - $time_check

$log_check=0;//примет значение 1, если пара будет найдена
...
echo'
                    ...
                    <li align="center" style="height:50px !important; cursor:pointer !important;">
						...
						if($log_check==1){
							...
							echo'onclick="show_div_new( '.$tmp_count2.' );"'; 
						}				
			echo'			align="center" 
