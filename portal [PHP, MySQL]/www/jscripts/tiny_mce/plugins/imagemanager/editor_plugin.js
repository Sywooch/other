					...
					mcImageManager.browse(tinymce.extend(s, {
						oninsert : function(o) {
							...
							ed.execCommand('mceInsertContent', false, mcImageManagerPlugin.replace(
								ed.getParam('imagemanager_insert_template', '<audio2 src="{$url}" width="{$custom.width}" height="{$custom.height}" />'),
								o.focusedFile,
								{
				...
				if (cmd == 'mceInsertImage') {
					mcImageManager.browse(tinyMCE.extend({
						...
						oninsert : function(o) {
							...
							ed.execCommand('mceInsertContent', false, mcImageManagerPlugin.replace(
								tinyMCE.getParam('imagemanager_insert_template', '<a href="{$url}" rel="lightbox"><audio src="{$custom.thumbnail_url}" width="{$custom.twidth}" height="{$custom.theight}" /></a>'),
								o.focusedFile,
								{
									urlencode : function(v) {
										return escape(v);
