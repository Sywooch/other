...
<?php
//выставляет все поля столбца edit в значение 0.
//это сделано на тот случай, если пользователь закроет браузер или перезагрузит
// страницу при открытой в 3-м фрейме новости, не нажав при этом кнопку 
//"Готово".

$mysqli22 = new mysqli(DB_SERVER, DB_USER, DB_PASS, DB_BASE); 
$mysqli22->set_charset("utf8");
if (mysqli_connect_errno()) { 
    printf("Подключение невозможно: %s\n", mysqli_connect_error()); 
    exit(); 
} 

$mysqli22->query("UPDATE news SET edit='0'"); 
...
$count_news=0; //количество новостей, соответствующих определённой дате
...
echo'<div id="delete_news_block'.$row['id'].'" name="delete_news_block'.$row['id'].'"
 ...
 if($count_news==1){ echo'margin-top:0px;'; }else{ echo'margin-top:10px;'; };

echo' 
 border-bottom:2px black solid; display:block;" >';
...
$t_list=mysqli_query($dbh,'SHOW TABLES');
 ...
 while($row2 = mysqli_fetch_array($t_list)) {
...
$query3=mysqli_query($dbh,"SELECT * FROM ".$buffer2." WHERE id=".$row['id']."");

 if (!$result = mysqli_fetch_array($query3)){
        // записи нет
...
style="width:100%; height:310px; position:absolute; background-color:transparent; z-index:99999999999; 
display:none;"></div>';
...
if(isset($_GET['id'])){
 if(($row['id']==$id_yellow)){
  echo'<div style="width:100%; height:310px; border-bottom:2px black solid; cursor:pointer; background-color:yellow;">';
  $count_scroll2=$count_scroll;
  }else{
  echo'<div style="width:100%; height:310px; border-bottom:2px black solid; cursor:pointer; ">';
 }
}else{
 echo'<div style="width:100%; height:310px; border-bottom:2px black solid; cursor:pointer; ">';
}
...
<div align="left" style="width:100%; height:40px; padding-top:5px; padding-bottom:5px;">
<div align="left" style=" float:left; height:40px; width:80%; font-size:16pt !important; font-weigh:bold !important;
 overflow:hidden !important; ">
 <div style="width:10px; height:40px; float:left; background-color:transparent;"></div>
 <span style="color:black; font-size:12pt !important; margin-left:0px; "  id="head'.$id.'" name="head'.$id.'">
...
<div style="width:90%; height:22px; background-color:transparent; overflow:hidden;">
...
<span style="font-size:14pt;">Для данной даты не создано новостей</span>
</div>
';
...
$count_news=0;//количество новостей, соответствующих дате
...
font-size:16pt !important; font-weigh:bold !important; overflow:hidden !important; ">
<div style="width:10px; height:40px; float:left; background-color:transparent;"></div>
...
echo'<span style="margin-left:10px; color:black;" id="tags'.$id.'" name="tags'.$id.'" >'.$row['tags'].'</span>';
