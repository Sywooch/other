(function ($) {
    $.fn.tmMultimediaGallery = function (o) {
        return this.each(function () {
            var object = $(this), data = object.data('tmMultimediaGallery'), getObject = {
                startIndex: 0,
                showOnInit: true,
                autoPlayState: false,
                autoPlayTime: 4,
                ...
                hideControlsOnClick: true,
                paginationDisplay: false,
                dragEnable: false,
                animationSpeed: '0.7',
                mobile: false,
                resizableContainer: false,
                galleryOpenClass: 'galleryOpen',
                container: '.galleryContainer',
                imageHolder: '.imageHolder',
                pagination: '.inner',
                outerPagination: false,
                description: '.galleryDescription',
                descriptionActiveClass: '.active',
                descriptionShowClass: '.show',
                ...
                frontClass: '.frontImg',
                animationClass: 'flip',
                animationClassSub: 'scale',
                noAnimationClass: '.animationDisable',
                ...
                constructor: function (params) {
                    if (object.length) {
                        ...
                        var image = $("img", getObject.imageHolder), video = $("video", getObject.imageHolder), imageSRCLink = $("ul li a", getObject.pagination.eq(0)), description = $("li", getObject.description), imageDeltaX, imageDeltaY, prevImg = -1, currImg = -1, allImg = imageSRCLink.length, loadComplete = true, autoPlayTimer, dragDirection = 1, doc = $(document), win = $(window), body = $('body'), galleryActive = false, showControls = false, mouseDrag = false, mouseDown = false, msie9 = Boolean(navigator.userAgent.match(/MSIE ([9]+)\./));
                    ...
                    function init() {
                        ...
                        if (getObject.paginationDisplay) {
                            $(getObject.pagination).each(function () {
                                $(this).find("ul li a").on('mousedown', function () {
                                    mouseDown = true;
                                }).on('mouseup', function () {
                                    clearTimeout(id);
                        ...
                        object.on('showGallery', function (e, d) {
                            reset();
                            ...
                            object.on('goTo', goTo);
                            if (!d) {
                                d = getObject.startIndex.toString();
                            }
                            ...
                            videoStop();
                            getObject.imageHolder.removeClass('videoInside').off('click').children('div').removeClass('videoShow')
                    ...
                    function init_mouse_move() {
                        var currTop = 0;
                        if (!getObject.mobile) {
                            getObject.imageHolder.on('mousemove', function (e) {
                                if (!galleryActive) {
                                    return false;
                                }
                            ...
                            if ($.fn.swipe) {
                                getObject.imageHolder.find('.frontImg, .backImg').swipe({
                    ...
                    function goTo(e, d, q) {
                        if (d) {
                            d = parseInt(d);
                            if (getObject.pagination && getObject.pagination.find('a') && getObject.pagination.find('a').eq(d)) {
                    ...
                    function descriptionChange() {
                        description.stop(true).removeClass(getObject.descriptionActiveClass).eq(currImg).addClass(getObject.descriptionActiveClass).end().delay(500).queue(descriptionResize);
                    }
                    ...
                    function videoInit() {
                        ...
                        getObject.imageHolder.removeClass('videoInside').off('click').find('video').remove().end().children('div').removeClass('videoShow');
                        video = null;
                        ...
                        if ($("ul li", getObject.pagination).eq(currImg).hasClass('video')) {
                            ...
                            getObject.imageHolder.addClass('videoInside')
                            image.after('<video preload poster="' + image.attr('src') + '" style="display: none;"><source src="' + videoArray[0] + '" type="video/mp4" /><source src="' + videoArray[1] + '" type="video/webm" /><source src="' + videoArray[2] + '" type="video/ogg" />Video tag not supported. Download the video <a href="' + videoArray[0] + '.webm">here</a>.</video>');
                    ...
                    function resizeElement(obj) {
                        if (obj.length) {
                            var imageWidth = obj.width(), imageHeight = obj.height(), screenWidth = windowW(), screenHeight = windowH(), imageCoef = imageHeight / imageWidth, holderCoef = screenHeight / screenWidth, imageDeltaX = 0, imageDeltaY = 0;
                            if (getObject.alignMode === 'fill') {
                                ...
                                var alignIMG = obj.attr('data-image-align') ? obj.attr('data-image-align') : getObject.alignIMG, imageScroll = obj.attr('data-image-scroll');
                    ...
                    function resizeImageHandler() {
                        ...
                        if (getObject.resizableContainer) {
                            getObject.container.css({'width': win.width(), 'height': win.height()});
                        }
                    ...
                    function changeImageHandler(d) {
                        if (allImg > 1) {
                            ...
                            $(getObject.pagination).each(function () {
                                $(this).find("ul li").eq(currImg).addClass("active").end().eq(prevImg).removeClass("active");
                            });
                            loadComplete = false;
                            image = $("img", getObject.imageHolder);
                            var imgSRC = imageSRCLink.eq(currImg).attr("href"), alignIMG = imageSRCLink.eq(currImg).attr("data-image-align"), imageScroll = imageSRCLink.eq(currImg).attr("data-image-scroll");
                            alignIMG = alignIMG ? 'data-image-align="' + alignIMG + '"' : "";
                            imageScroll = imageScroll ? 'data-image-scroll="' + imageScroll + '"' : "";
                            $(getObject.backClass, getObject.imageHolder).css({'display': 'block'}).empty().append('<img src="' + imgSRC + '" alt="" ' + alignIMG + ' ' + imageScroll + '>').find('img').on("load", loadHandler);
                            descriptionChange();
                            descriptionToggle(true);
                        }
                    ...
                    function loadImageHandler(quickChange) {
                        ...
                        if (quickChange === 'quick') {
                            ...
                            $(getObject.frontClass, getObject.imageHolder).empty().css({
                                'zIndex': 1,
                                'display': 'none'
                            });
                        }
                        setTimeout(function () {
                            ...
                            if (msie9) {
                                getObject.imageHolder.find(getObject.frontClass).stop(true).animate({'opacity': 0}, time).end().find(getObject.backClass).css('opacity', 0).stop(true, true).animate({'opacity': 1}, time).end()
                            }
                            getObject.imageHolder.find(getObject.backClass + ">img").off("load", loadImageHandler);
                            getObject.imageHolder.stop(true, true).delay(getObject.animationSpeed * time).queue(onAnimateComplete);
