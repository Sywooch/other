...
?>
	...
	<div class="item_current_price tmp2" id="<? echo $arItemIDs['PRICE']; ?>"
    <?
	//если имеется скидка, то старую цену перечёркиваем
 	if($arResult['PROPERTIES']['SALE']['VALUE']!=""){ echo 'style="text-decoration:line-through;"'; } ?>>
    <? echo $minPrice['PRINT_DISCOUNT_VALUE']; $result_price=$minPrice['DISCOUNT_VALUE']; ?></div>
    <?
	if($arResult['PROPERTIES']['SALE']['VALUE']!=""){
		$new_price=$result_price - (($result_price * $arResult['PROPERTIES']['SALE']['VALUE'])/100);
