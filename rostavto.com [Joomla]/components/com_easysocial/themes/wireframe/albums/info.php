...
<div data-album-info class="es-media-info es-album-info">
	...
	<small>
		<?php if( $options[ 'view' ] == 'all' ){ ?>
			<span class="es-album-author mr-5"><i class="ies-user"></i> <?php echo JText::sprintf( 'Created by %1s' , $this->html( 'html.user' , $album->user_id , true ) ); ?></span>
		<?php } ?>

		<?php if ($album->hasDate()) { ?>
			<span data-album-date class="es-album-date"><i class="ies-calendar"></i> <?php echo $this->html( 'string.date' , $album->getAssignedDate() , "COM_EASYSOCIAL_ALBUMS_DATE_FORMAT"); ?></span>
			<?php
