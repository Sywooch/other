	...
	<?php if( $streams ){ ?>
		...
		<?php if( $isguest && isset( $nextlimit ) ) { ?>
			<?php if( Foundry::user()->id != 0 ) { ?>
				<li class="pagination" data-stream-pagination-guest data-nextlimit="<?php echo $nextlimit; ?>" >
					<?php if( $nextlimit ){ ?>
			...
			<?php } ?>
		<?php } else { ?>
			<li class="pagination" data-stream-pagination data-startdate="<?php echo $nextdate; ?>" data-enddate="<?php echo $enddate; ?>" >
				<?php if( $nextdate ){ ?>
					<a class="btn btn-es-primary btn-stream-updates" href="javascript:void(0);">
						<i class="ies-refresh"></i> <?php echo JText::_( 'COM_EASYSOCIAL_STREAM_LOAD_PREVIOUS_STREAM_ITEMS' ); ?>
					</a>
