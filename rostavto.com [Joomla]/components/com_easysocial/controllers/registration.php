...
class EasySocialControllerRegistration extends EasySocialController
{
	...
	public function saveStep()
	{
		...
		if( $completed )
		{
			...
			if( $profile->getParams()->get( 'email.moderators') )
			{
	...
	public function moduleRegister()
	{
		...
		// Get the module params to ensure that the profile_id is not tampered
		$params 	= Foundry::registry( $module->params );
		...
		if( !$profileId )
		{
			...
			return $view->call( __FUNCTION__ , $redirect );
		}
		...
		$model 	= Foundry::model( 'Registration' );
		$user	= $model->moduleCreateUser( $profileId , $data );
	...
	public function miniRegister()
	{
		...
		{
			$register->load( $registration->values );
		}
