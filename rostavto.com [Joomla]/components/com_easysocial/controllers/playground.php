	...
	public function explorer() {
		...
		switch ($hook) {
			...
			case 'getFolders':
				...
				// // Sample code to use
				// $explorer 	= Foundry::explorer( 1 , SOCIAL_TYPE_GROUP );
			...
			case 'addFolder':
				...
				if (empty($name)) {
					$exception = Foundry::exception('Invalid name provided');
			...
			case 'removeFolder':
				...
				if ($error) {
					$exception = Foundry::exception('Unable to remove folder.');
					$ajax->reject($exception);
				} else {
					$exception = Foundry::exception('Remove successful!', SOCIAL_MSG_SUCCESS);
			...
			case 'removeFile':
				...
				if ($error) {
					$exception = Foundry::exception('Unable to remove file.');
					$ajax->reject($exception);
				} else {
					$exception = Foundry::exception('Remove successful!', SOCIAL_MSG_SUCCESS);
					$ajax->resolve($exception);
			...
			default:
				$exception = Foundry::exception('Invalid hook provided.');
				$ajax->reject($exception->toArray());
				break;
		}
