	...
	function showFieldForm($field,$content,$default)
	{
		...
		switch($field->type)
		{
			...
			case 'select':
				...
				if ($field->required == 1)
					$return .= "<select style='background-color:transparent;' id='$name' name='$name' mosReq='1' mosLabel=\"$strtitle\" class='adsmanager_required' $disabled>\n";
				else
...
$query_events_3->select(array('id', 'options'));
$query_events_3->from('#__community_fields');
$query_events_3->where('id LIKE \'17\'');
...
echo'<option value="1">1</option>123 ';
...
for ($i = 0; $i < (count($MAS_option3)-1); $i++) 
  { 
    echo "<option value=\"".$i."\">".$MAS_option3[$i]."</option>"; 
...
$query_events_h->where('ad_model LIKE \''.$tmp_model_1.'\'');
...
foreach($resultsh as $element_h) {
   $tmp_service_count++;
						...
						if($tmp_service_count==0){
						
						$return .= "<option value=\"".$MAS_option[$i]."\">".$MAS_option[$i]."</option>\n";
					...
					//	$return .= "<option  style=\"background-color:red;\" value=\"".htmlspecialchars($v->fieldvalue)."\" >".$ftitle."</option>\n";
						...
						$return .= "<option  style=\"background-color:transparent;\" value=\"".htmlspecialchars($v->fieldvalue)."\" >".$ftitle."</option>\n";
