...
class CUserHelper
{
    ...
    static function parseTaggedUserNotification($message, $actor, $object = null, $info = array()){
        ...
        if( isset($matches[1]) && count($matches[1]) > 0 ){
            ...
            }else if($info['type'] == 'album-comment') {
            ...
            }else if($info['type'] == 'video-comment'){
            ...
            }else if($info['type'] == 'image-comment'){
            ...
            }else if($info['type'] == 'post-comment' && $object != null){
                //default notification, and object is activity
                //set parameter to be replaced in the template
                $url = CRoute::emailLink('index.php?option=com_community&view=profile&userid=' . $actor->id . '&actid=' . $object->id.'#activity-stream-container');
                $params = new CParameter();
                $params->set('url', $url);
                $params->set('content', $message);
