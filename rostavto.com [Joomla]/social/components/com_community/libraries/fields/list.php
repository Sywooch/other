...
class CFieldsList extends CProfileField
{
	...
	public function getFieldHTML( $field , $required )
	{
		...
		$html	= '<select  style="background-color:transparent; height:300px;"  id="field'.$field->id.'" name="field' . $field->id . '[]" type="select-multiple" multiple="multiple" class="jomNameTips tipRight select'.$class.'" title="' . CStringHelper::escape( JText::_( $field->tips ) ) . '">';
		...
		foreach( $field->options as $option )
		{
			...
			$option2=str_replace(" ","",$option);
			$option2=strtolower($option2);
			$option2=mb_strtolower($option2);
			$html	.= '<option id="'.$option2.'" value="' . $option . '"' . $selected . '>' . JText::_( $option ) . '</option>';
		}
