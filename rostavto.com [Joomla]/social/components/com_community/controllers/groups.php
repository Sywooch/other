...
//				$params->set( 'group_url' , 'index.php?option=com_community&view=groups&task=viewgroup&groupid=' . $group->id );
    ...
    public function ajaxJoinGroup($groupId, $fastJoin = 'no') {
        ...
        if ($fastJoin == 'yes') {
            ...
            if ($groupModel->isMember($my->id, $groupId)) {
                $objResponse->addScriptCall("joms.jQuery('.group-top').html('<div class=\"info\">" . JText::_('COM_COMMUNITY_GROUPS_ALREADY_MEMBER', true) . "</div>');");
            } else {
                $url = CRoute::getExternalURL('index.php?option=com_community&view=groups&task=viewgroup&groupid=' . $groupId, false);
                ...
                if ($member->approved) {
                    $objResponse->addScriptCall("joms.jQuery('.group-top').html('<div class=\"info\">" . JText::_('COM_COMMUNITY_GROUPS_JOIN_SUCCESS', true) . "</div>');");
                } else {
                    $objResponse->addScriptCall("joms.jQuery('.group-top').html('<div class=\"info\">" . JText::_('COM_COMMUNITY_GROUPS_APPROVAL_NEED', true) . "</div>');");
                }
