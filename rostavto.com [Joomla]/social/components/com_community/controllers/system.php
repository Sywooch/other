...
//		$objResponse->addScriptCall('joms.friends.updateFriendList',$html,JText::_('COM_COMMUNITY_INVITE_NO_FRIENDS'));
    ...
    public function ajaxSubmitInvitation($callback, $cid, $values) {
        ...
        // we shouldn't invite users through emal from groups and event
    ...
    public function ajaxUpdateWall($wallId, $message, $editableFunc, $photoId = 0) {
        ...
        if (!$isEditable) {
            $objResponse->addScriptCall('cWindowAddContent', JText::_('COM_COMMUNITY_NOT_ALLOWED_TO_EDIT'));
            return $objResponse->sendResponse();
        }
    ...
    public function ajaxRemoveWallPreview($wallId) {
        ...
        $response->addScriptCall('joms.jQuery("#wall-message-' . $wallId . ' .joms-fetch-wrapper").remove', '');
    ...
    public function ajaxRemoveCommentPreview($itemId) {
        ...
        $response->addScriptCall('joms.jQuery("div[data-commentid=' . $itemId . '] .joms-fetch-wrapper").remove', '');
