...
abstract class modRseventsProUpcoming {
	...
	public static function getEvents($params) {
		...
		$today		= $now->formatLikeDate('Y-m-d H:i:s');
		
		
		$now->addSeconds(86400);
		$today2		= $now->formatLikeDate('Y-m-d H:i:s');
		...
		$active_today	= '(('.$db->qn('e.start').' <= '.$db->q($today).' AND '.$db->qn('e.end').' >= '.$db->q($today).') OR ('.$db->qn('e.start').' >= '.$db->q($today).' AND '.$db->qn('e.start').' <= '.$db->q($today2).'))';
