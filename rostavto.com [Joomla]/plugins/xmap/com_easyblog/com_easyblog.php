...
class xmap_com_easyblog {
	...
	function getBlogEntries($type, $typeId, $limit)
	{
		...
		if( $config->get( 'integrations_easysocial_privacy' ) && $easysocial->exists() && $type != 'teamblog' )
		{
...
// 			echo '<br><br>'.$query;
    ...
    function getEasyBlog( &$xmap, &$parent,&$params )
    {
		...
		if ($params['include_latest_post'])
		{
            ...
            $node->name = JText::_( 'COM_EASYBLOG_XMAP_LATEST_POST' );
            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=latest");
            $node->modified = time();
            $node->newsItem = 1;
        	...
        	if(count($rows) > 0)
        	{
        	    ...
        	    foreach($rows as $row)
        	    {
		            ...
		            $node->name = $row->title;
		            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=entry&id=" . $row->id);
		            $node->newsItem = 1;
		            $date   =& JFactory::getDate($row->modified);
		            $node->modified = $date->toUnix();
		...
		if ($params['include_cetegories'])
		{
            ...
            $node->name = JText::_( 'COM_EASYBLOG_XMAP_CATEGORIES' );
            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=categories");
            $node->modified = time();
            $node->expandible = true;
            $xmap->printNode($node);
			...
			if(count($rows) > 0)
			{
			    ...
			    foreach($rows as $row)
			    {
		            ...
		            $node->changefreq = $params['category_changefreq'];
		            $node->name = $row->title;
		            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=categories&layout=listings&id=" . $row->id);
		            $node->modified = time();
		            $node->expandible = true;
		            $xmap->printNode($node);
		            ...
		            if($params['include_category_posts'])
		            {
		            	...
		            	if(count($items) > 0)
		            	{
                            ...
                            foreach($items as $item)
                            {
					            ...
					            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=entry&id=" . $item->id);
		...
		if ($params['include_tag_clouds'])
		{
            ...
            $node->name = JText::_( 'COM_EASYBLOG_XMAP_TAGS' );
            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=tags");
            $node->modified = time();
            $node->expandible = true;
            $xmap->printNode($node);
			...
			if(count($rows) > 0)
			{
			    ...
			    foreach($rows as $row)
			    {
		            ...
		            $node->changefreq = $params['tag_changefreq'];
		            $node->name = $row->title;
		            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=tags&layout=tag&id=" . $row->id);
		            $node->modified = time();
		            $node->expandible = true;
		            $xmap->printNode($node);
		...
		if ($params['include_bloggers'])
		{
            ...
            $node->name = JText::_( 'COM_EASYBLOG_XMAP_BLOGGERS' );
            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=blogger");
            $node->modified = time();
            $node->expandible = true;
            $xmap->printNode($node);
			...
			if(count($rows) > 0)
			{
			    ...
			    foreach($rows as $row)
			    {
		            ...
		            $node->changefreq = $params['blogger_changefreq'];
		            $node->name = $row->title;
		            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=blogger&layout=listings&id=" . $row->blogger_id);
		            $node->modified = time();
		            $node->expandible = true;
		            $xmap->printNode($node);
		...
		if ($params['include_teamblog'])
		{
            ...
            $node->name = JText::_( 'COM_EASYBLOG_XMAP_TEAMBLOGS' );
            ...
            if(count($rows) > 0)
            {
			    ...
			    foreach($rows as $row)
			    {
		            ...
		            $node->changefreq = $params['teamblog_changefreq'];
		            $node->name = $row->title;
		            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=teamblog&layout=listings&id=" . $row->id);
		            $node->modified = time();
		            $node->expandible = true;
		            $xmap->printNode($node);
		            ...
		            if($params['include_teamblog_posts'])
		            {
		            	...
		            	if(count($items) > 0)
		            	{
                           	...
                           	foreach($items as $item)
                            {
					            ...
					            $node->changefreq = $params['entry_changefreq'];
					            $node->name = $item->title;
					            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=entry&id=" . $item->id . '&&team=' . $row->id);
					            $date   =& JFactory::getDate($item->modified);
					            $node->modified = $date->toUnix();
					            $node->expandible = false;
					            $xmap->printNode($node);
		...
		if ($params['include_archives'])
		{
            ...
            $node->name = JText::_( 'COM_EASYBLOG_XMAP_ARCHIVES' );
            $node->link = JURI::base() . EasyBlogRouter::getRoutedURL("index.php?option=com_easyblog&view=archive&archiveyear=" . $year . '&archivemonth=' . $month);
            $node->modified = time();
            $node->expandible = true;
            $xmap->printNode($node);
