...
class KunenaViewCommon extends KunenaView {
	...
	function displayWhosonline($tpl = null) {
		...
		$who='';
		//$who = ''.($onlineusers['user']+$onlineusers['user']).' ';
		$all_u=$onlineusers['user']+$onlineusers['guest'];
		
		$database2	= & JFactory::getDBO();
		$database2->setQuery("SELECT * FROM #__statistic");
		$list2 = $database2->loadObjectList();
 
		foreach($list2 as $user) {
		if(($user->count) < $all_u){
			//$who="yes ";
			$database2->setQuery("UPDATE #__statistic SET count='".$all_u."',time='".date( " d-m-y H:i" )."' ");
			$database2->query();
		...
		$who .= '<span >'.$onlineusers['user'].' </span>';
		...
		$who .= '<span > '. $onlineusers['guest'].' </span>';
		if($onlineusers['guest']==1) {
			$who .= JText::_('COM_KUNENA_WHO_ONLINE_GUEST').')&nbsp;';
		} else {
			$who .= JText::_('COM_KUNENA_WHO_ONLINE_GUESTS').')&nbsp;<br>';
		}
		//$who .= JText::_('COM_KUNENA_WHO_ONLINE_NOW');
		$who .= 'Рекорд одновременного пребывания ';
		...
		$who .=$user->count." это было ".$user->time;
