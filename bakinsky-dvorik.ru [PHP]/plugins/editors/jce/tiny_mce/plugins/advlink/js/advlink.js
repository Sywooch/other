var AdvLinkDialog = {
	...
	init : function() {
		...
		if (action == "update") {
			var href = ed.convertURL(ed.dom.getAttrib(el, 'href'));
			
			// Setup form data
			dom.value('href', decodeURI(href));
	...
	insertAndClose : function(){
		...
		// decode in case it includes encoded characters
		var href = decodeURI(dom.value('href'));
		...
		tinymce.extend(args, {
			...
			'class' 	: dom.value('classes') == '' ? dom.getSelect('classlist') : dom.value('classes'),
			rel 		: dom.value('rel'),
			rev 		: dom.value('rev'),
	...
	initTree : function(){		
		this.tree = new Tree('link-options', {
			...
			onNodeLoad : function(node){
				...
				this.xhr('getLinks', {'json' : query}, function(o){
					if(o){
						if(!o.error){
							var ul = $E('ul', node);
							if(ul){
