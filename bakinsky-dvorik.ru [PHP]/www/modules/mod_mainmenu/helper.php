...
class JMenuTree extends JTree
{
	...
	function _getLevelXML($depth)
	{
		...
		while ($this->_current->hasChildren())
		{
			...
			$this->_buffer .= '</ul><i class="part"> </i>';
		}
...
/*				
$uri_tmp1 = &JFactory::getURI();
$url_tmp1 = $uri_tmp1->toString(array('path', 'query', 'fragment'));
echo $url_tmp1."  ";
*/
							if ( (JSite::getMenu()->getActive()->id == $item->id)||($item->name == JSite::getMenu()->getActive()->name) ){
	$data = '<span>'.$tmp->url.$image.$tmp->name.'</span>';
} else {
	$data = '<a href="'.$tmp->url.'" name="'.$tmp->url.'_">'.$image.$tmp->name.'</a>';
}		
				...
				case 1:
					// _blank
					$data = '<a rel="nofollow"  href="'.$tmp->url.'" target="_blank">'.$image.$tmp->name.'</a>';
					break;
				case 2:
					// window.open
					...
					$data = '<a rel="nofollow"  href="'.$link.'" onclick="window.open(this.href,\'targetWindow\',\''.$attribs.'\');return false;">'.$image.$tmp->name.'</a>';
					break;
			}
		} else {
			$data = '<a rel="nofollow" >'.$image.$tmp->name.'</a>';
		}
