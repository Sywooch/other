...
function com_uninstall()
{
	...
	if ($Ok) {
	  ...
	  if ($yvComment->TableExists($TableName)) {
	    $deleted = false;	
	  	
	    $nRecords = $yvComment->DLookup_db($db, 'COUNT(*)',$TableName);
	    if ($nRecords == 0) {
			  ...
			  if ($Ok) {
				  ...
				  $mainframe->enqueueMessage($yvComment->printDbErrors($db), 'error');
			  }
	    }
			if (!$deleted) {
				$mainframe->enqueueMessage(
