...
var Plugin = new Class({
    ...
    initialize : function(plugin, options) {
        ...
        // add IE browser flags
        if (window.ie) {
        	$$('body').addClass('ie');
        	
        	// IE7
            if (!!window.XMLHttpRequest && !document.querySelector) {
            	$$('body').addClass('ie7');
            }
            // IE6
...
var IFrame = new Class({
    ...
    initialize : function(options) {
        ...
        temp.innerHTML = '<iframe src="javascript:&quot;&quot;;" name="'+ this.options.name +'" id="'+ this.options.name +'" style="display:none;"></iframe>';
        var iframe = temp.firstChild;
        $E('form').adopt(temp.firstChild);
        
        // Create action
        if(this.options.action) {
            this.action = new Element('input').setProperties({
                'type' 	: 'hidden',
        ...
        $E('form').setProperty('target', this.options.name).addEvent('submit', function() {
            this.fireEvent('onStart');
        }.bind(this));

        // Add load event
        $(iframe).addEvent('load', function() {
            var el 	= iframe.contentWindow.document || iframe.contentDocument || window.frames[iframe.id].document;
    ...
    remove : function() {
        ...
        if($(this.options.name + '_action')) {
            $(this.options.name + '_action').remove();
        }
    },
