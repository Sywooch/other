		...
		foreach($products as $v){
			if($v->virtuemart_product_id == $this->product->virtuemart_product_id)
				...
				if ($v->virtuemart_product_id >0){
					echo '<h2 class="item'.$i.'">Недавно просмотренные товары</h2>'; 		
					echo '<li>';
					...
					echo '( Категория: '.JHTML::link ( JRoute::_('index.php?option=com_virtuemart&view=category&virtuemart_category_id='.$v->virtuemart_category_id), $v->category_name).');';
					echo '</li>';
					$i++;
				}
