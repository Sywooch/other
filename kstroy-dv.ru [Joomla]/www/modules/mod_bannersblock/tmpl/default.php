...
?>
		...
		for ($i=1; $i<=$j; $i++)
			...
			<a href="<?php echo JRoute::_( $params->get( 'bannerlink'.$i.'')); ?>" target="_self" title="<?php echo $params->get( 'bannertitle'.$i.'' ) ?>">
			 <?php if ($params->get('mod_'.$i.'_image')!=null){ ?><img class="banner_img" src="<?php if($params->get('mod_'.$i.'_image')!=null){echo $params->get('mod_'.$i.'_image');}?>" /><?php } ?>
			<span><?php echo $params->get('mod_image_'.$i.'_para'); ?>
