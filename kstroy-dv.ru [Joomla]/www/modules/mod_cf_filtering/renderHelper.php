			...
			if(count($options_ar)>0){
				foreach($disp_typeArray as $disp_type){
					...
					if($disp_type==1){
						foreach($options_ar as $op){
							...
							if($dispCounter && isset($opt->counter) && $opt->active){
								$label=$opt->label.'&nbsp;<span class="cf_flt_counter"  style="tmp4">('.$opt->counter.')</span>';
							}
							...
							if($option_anchor){
								...
								if(($disp_type==2 || $disp_type==3) && empty($opt->isparent)){
									...
									if($disp_input){
										...
										if($dispCounter && isset($opt->counter) && $opt->active){
											$innerHTML.='<span class="cf_flt_counter" style="tmp5">('.$opt->counter.')</span>';
										}
									}else{
								...
								//links (image,simple links), buttons, or clears
										...
										if(!empty($img) && $opt->type!='clear'){
											...
											if(!$opt->active){
												if($display_empty_opt==1){
													...
													if($dispCounter && isset($opt->counter) && $opt->active){
														$img_option_anchor.='<span class="cf_flt_counter" style="tmp6">('.$opt->counter.')</span>';
													}
												...
												if($dispCounter && isset($opt->counter) && $opt->active){
													$imgWrapper.='<span class="cf_flt_counter" style="tmp1">('.$opt->counter.')</span>';
												}
												$imgWrapper.='</span>';
												$linkAttributes['data-module-id']=$this->module->id;
												$linkAttributes['id']=$element_id.'_a';
											...
											if($dispCounter && isset($opt->counter) && $opt->active){
												$innerHTML.='<span class="cf_flt_counter" style="tmp2">('.$opt->counter.')</span>';
											}
										}
									}
									...
									//simple links
										...
										$innerHTML.=$option_anchor;
										if($dispCounter && isset($opt->counter) && $opt->active){
											$innerHTML.='<span class="cf_flt_counter"  style="tmp3">('.$opt->counter.')</span>';
										}
									}
					...
					//-- Inputs--//
						...
						if(!$sliderExist && $this->results_trigger!='btn' && $value_found){//generate the clear link
