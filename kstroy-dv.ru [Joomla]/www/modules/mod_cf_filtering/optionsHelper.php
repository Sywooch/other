...
class ModCfilteringOptions{
	...
	function getActiveOptions($field,$customfilter,$joinFieldData=false){
		...
		foreach($selected_flt as $key=>$ar_value){if($key=='keyword')continue;
			$wherecf=array();
			...
			if(count($ar_value)>0){
				if($key!=$field){
					...
					//keyword
						...
						$product_ids_from_search=$this->input->get('productIdsBySearch',array(),'array');						
						if(is_array($product_ids_from_search) && !empty($product_ids_from_search))$where[]="{$table_db_flds[$field]}.virtuemart_product_id IN(".implode(',', $product_ids_from_search).")";
						//empty set of products
						else return;
					}
					//other filters than customfilters but not product_price or keyword
					else if($key!='price'){
						$sel_field=$key;
		...
		if(!empty($where_productIds)){ //echo $field_key,' :';print_r($where_productIds);
