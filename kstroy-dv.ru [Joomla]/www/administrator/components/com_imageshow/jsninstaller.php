...
abstract class JSNInstallerScript
{
	...
	public function preflight($mode, $parent)
	{
		...
		if ($canInstall && $canInstallExtension && $canInstallSiteLanguage && $canInstallAdminLanguage)
		{
			...
			if (!is_null($extInfo))
			{
				...
				{
					$fileISData = $this->parentInstaller->getPath('source') . DS . 'admin' . DS . 'classes' . DS . 'jsn_is_data.php';
				...
				$fileUpgradeHelper = $this->parentInstaller->getPath('source') . DS . 'admin' . DS . 'subinstall' . DS . 'upgrade.helper.php';
	...
	protected function removeThumbFolder()
	{
		...
		$path = JPATH_SITE . DS . 'images' . DS . 'jsn_is_thumbs';
