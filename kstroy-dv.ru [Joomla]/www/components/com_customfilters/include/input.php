...
Class CfInput{
	...
	private function buildInputs($module=''){
		...
		if(!empty($module))$moduleparams=cftools::getModuleparams($module);
		else $moduleparams='';
		$selected_flt=array();
		$rangeVars=array();
		...
		if($reset_filters_on_new_search){
			...
			$previous_keyword=$app->getUserState('com_customfilters.keyword','');
			$app->setUserState('com_customfilters.keyword',$current_keyword);
		 ...
		 */
		$disp_cf=true;
		if(!empty($moduleparams))$disp_cf=$moduleparams->get('custom_flt_published');
		//$disp_cf=ModCfilteringOptions::getDisplayControl($flt_sfx='custom_flt',$selected_flt, $moduleparams);

		if($disp_cf && $reset_all_filters==false){
