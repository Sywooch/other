...
$ch=$_POST['ch'];//количество пар в заходе
...
if($count==0){
	...
	$rs=$mysqli->query("INSERT INTO active_reglament_parameters (zahod_rotation) VALUES ('".$ch."')");
	if ($rs===false) {
		printf("Ошибка #2: %s\n", $mysqli->error);
	}
	...
	$rs=$mysqli->query("UPDATE active_reglament_parameters SET zahod_rotation='".$ch."' ");
	if ($rs===false) {
		printf("Ошибка #2: %s\n", $mysqli->error);
	}
