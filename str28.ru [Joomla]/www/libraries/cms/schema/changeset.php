...
JLoader::register('JSchemaChangeelement', RPATH_LIBRARIES . '/cms/schema/changeelement.php');


/**
 * Contains a set of JSchemaChange objects for a particular instance of retina.
...
{
	/**
	 * Array of JSchemaChangeelement objects
	 *
	 * @var    string
	 */
	protected $changeelements = array();
	...
	*/
	...
	/**
	 *
	 * Constructor: builds array of $changeelements by processing the .sql files in a folder.
	 * The folder for the retina core updates is admin/components/com_admin/sql/updates/<database>.
	 *
	...
	public function __construct($db, $folder = null)
	{
		...
		foreach ($updateQueries as $obj) {
			$this->changeelements[] = JSchemaChangeelement::getInstance($db, $obj->file, $obj->updateQuery);
		}
	}
	...
	{
		$errors = array();
		foreach ($this->changeelements as $element) {
			if ($element->check() === -2) { // error found
				$errors[] = $element;
			}
		}
		return $errors;
	...
	*
	* @return  array  associative array of changeelements grouped by unchecked, ok, error, and skipped
	*
	* @since   2.5
	*/
	...
	public function getSchema() {
		...
		$result = new SplFileInfo($element->file);
	...
	private function getUpdateFiles() {
		...
		if (!$this->folder) {
			$this->folder = RPATH_admin.'/components/com_admin/sql/updates/'.$sqlFolder;
		}
