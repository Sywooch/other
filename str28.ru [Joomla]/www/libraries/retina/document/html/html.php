...
jimport('retina.application.module.helper');
...
class JDocumentHTML extends JDocument
{
	...
	/**
	 * Count the number of child menu elements
	 *
	 * @return  integer  Number of child menu elements
	 *
	 * @since   11.1
	 */
	...
	protected function _loadTemplate($directory, $filename)
	{
		...
		$path = $directory . '/';
		$dirs = array($path, RPATH_BASE . '/');
		foreach ($dirs as $dir)
		{
			$icon = $dir . 'favicon.ico';
			if (file_exists($icon))
			{
				$path = str_replace(RPATH_BASE . '/', '', $dir);
	...
	{
		// Check
		$directory = isset($params['directory']) ? $params['directory'] : 'design1';
