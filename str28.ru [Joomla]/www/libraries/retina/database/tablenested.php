 ...
 * @package     retina.Platform
 * @subpackage  Database
 * @link        http://docs.retina.org/JTableNested
 * @since       11.1
 */
class JTableNested extends JTable
{
	 ...
	 *
	 * @link    http://docs.retina.org/JTableNested/getPath
	 * @since   11.1
	 */
	public function getPath($pk = null, $diagnostic = false)
	{
		...
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_GET_PATH_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	 ...
	 * @return  mixed    Boolean false on failure or array of node objects on success.
	 *
	 * @link    http://docs.retina.org/JTableNested/getTree
	 * @since   11.1
	 */
	public function getTree($pk = null, $diagnostic = false)
	{
		...
		if ($this->_db->getErrorNum())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_GET_TREE_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	 ...
	 *
	 * @link    http://docs.retina.org/JTableNested/isLeaf
	 * @since   11.1
	 */
	 ...
	 *
	 * @link    http://docs.retina.org/JTableNested/setLocation
	 * @since   11.1
	 */
	public function setLocation($referenceId, $position = 'after')
	{
		...
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_INVALID_LOCATION', get_class($this)));
			$this->setError($e);
			return false;
		}
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTableNested/moveByReference
	 * @since   11.1
	 */
	...
	public function moveByReference($referenceId, $position = 'after', $pk = null)
	{
		...
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_INVALID_NODE_RECURSION', get_class($this)));
			$this->setError($e);
			return false;
		}
		...
		$this->_runQuery($query, 'RLIB_DATABASE_ERROR_MOVE_FAILED');
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTableNested/delete
	 * @since   11.1
	 */
	public function delete($pk = null, $children = true)
	{
		...
		if ($this->_trackAssets)
		{
			...
			{
				// Delete the node in retina_097115115101116115 table.
		...
		if ($children)
		{
			...
			$query->from($this->_tbl);
			$query->where('lft BETWEEN ' . (int) $node->lft . ' AND ' . (int) $node->rgt);
			$this->_runQuery($query, 'RLIB_DATABASE_ERROR_DELETE_FAILED');
	...
	public function check()
	{
		...
		if ($this->parent_id > 0)
		{
			...
			else
			{
				if ($this->_db->getErrorNum())
				{
					$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_CHECK_FAILED', get_class($this), $this->_db->getErrorMsg()));
					$this->setError($e);
				}
				else
				{
					$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_INVALID_PARENT_ID', get_class($this)));
					$this->setError($e);
				}
			}
		}
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTableNested/store
	 * @since   11.1
	 */
	public function store($updateNulls = false)
	{
		...
		if (empty($this->$k))
		{
			...
			if ($this->_location_id >= 0)
			{
				...
				$query->set('lft = lft + 2');
				$query->where($repositionData->left_where);
				$this->_runQuery($query, 'RLIB_DATABASE_ERROR_STORE_FAILED');
			...
			else
			{
				...
				$e = new JException(RText::_('RLIB_DATABASE_ERROR_INVALID_PARENT_ID'));
				$this->setError($e);
				return false;
			}
		}
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTableNested/publish
	 * @since   11.1
	 */
	public function publish($pks = null, $state = 1, $userId = 0)
	{
		...
		if (empty($pks))
		{
			...
			{
				$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_NO_ROWS_SELECTED', get_class($this)));
				$this->setError($e);
				return false;
			}
		}
		...
		foreach ($pks as $pk)
		{
			...
			if ($checkoutSupport)
			{
				...
				if ($this->_db->loadResult())
				{
					$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_CHILD_ROWS_CHECKED_OUT', get_class($this)));
					$this->setError($e);
					return false;
				}
			}
			...
			if ($node->parent_id)
			{
				...
				if (!empty($rows))
				{
					$e = new JException(RText::_('RLIB_DATABASE_ERROR_ANCESTOR_NODES_LOWER_STATE'));
					$this->setError($e);
					return false;
				}
			}
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTableNested/orderUp
	 * @since   11.1
	 */
	public function orderUp($pk)
	{
		...
		if ($this->_db->getErrorNum())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_ORDERUP_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			$this->_unlock();
			return false;
		}
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTableNested/orderDown
	 * @since   11.1
	 */
	public function orderDown($pk)
	{
		...
		if ($this->_db->getErrorNum())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_ORDERDOWN_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			$this->_unlock();
			return false;
		}
	...
	/**
	 * Gets the ID of the root element in the tree
	 *
	...
	public function getRootId()
	{
		...
		if ($this->_db->getErrorNum())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_GETROOTID_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
		...
		else
		{
			...
			elseif (property_exists($this, 'alias'))
			{
				...
				{
					$e = new JException(RText::_('RLIB_DATABASE_ERROR_ROOT_NODE_NOT_FOUND'));
					$this->setError($e);
					return false;
				}
			}
	 ...
	 *
	 * @link    http://docs.retina.org/JTableNested/rebuild
	 * @since   11.1
	 */
	public function rebuild($parentId = null, $leftId = 0, $level = 0, $path = '')
	{
		...
		foreach ($children as $node)
		{
			...
			// Add this element's alias to the path (but avoid a leading /)
		...
		if (!$this->_db->query())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_REBUILD_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTableNested/rebuildPath
	 * @since   11.1
	 */
	public function rebuildPath($pk = null)
	{
		...
		if (!$this->_db->query())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_REBUILDPATH_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	...
	protected function _getNode($id, $key = null)
	{
		...
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_GETNODE_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	...
	protected function _runQuery($query, $errorMessage)
	{
		...
		if (!$this->_db->query())
		{
			$e = new JException(RText::sprintf('$errorMessage', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			$this->_unlock();
			return false;
		}
