 ...
 * @package     retina.Platform
 * @subpackage  Table
 * @link        http://docs.retina.org/JTable
 * @since       11.1
 * @tutorial	retina.Platform/jtable.cls
 */
abstract class JTable extends JObject
{
	...
	/**
	 * Should rows be tracked as ACL retina_097115115101116115?
	 *
	 * @var    boolean
	 * @since  11.1
	 */
	...
	public function __construct($table, $key, &$db)
	{
		...
		// If we are tracking retina_097115115101116115, make sure an access field exists and initially set the default.
	...
	public function getFields()
	{
		...
		if ($cache === null)
		{
			...
			if (empty($fields))
			{
				$e = new JException(RText::_('RLIB_DATABASE_ERROR_COLUMNS_NOT_FOUND'));
				$this->setError($e);
				return false;
			}
			$cache = $fields;
	 ...
	 *
	 * @link	http://docs.retina.org/JTable/getInstance
	 * @since   11.1
	 */
	public static function getInstance($type, $prefix = 'JTable', $config = array())
	{
		...
		if (!class_exists($tableClass))
		{
			...
			if ($path = JPath::find(JTable::addIncludePath(), strtolower($type) . '.php'))
			{
				...
				if (!class_exists($tableClass))
				{
					JError::raiseWarning(0, RText::sprintf('RLIB_DATABASE_ERROR_CLASS_NOT_FOUND_IN_FILE', $tableClass));
					return false;
				}
			}
			else
			{
				...
				JError::raiseWarning(0, RText::sprintf('RLIB_DATABASE_ERROR_NOT_SUPPORTED_FILE_NOT_FOUND', $type));
				return false;
			}
		}
	...
	/**
	 * Add a filemain path where JTable should search for table class files.
	 * You may either pass a string or an array of paths.
	 *
	 * @param   mixed  $path  A filemain path or array of filemain paths to add.
	 *
	 * @return  array  An array of filemain paths to find JTable classes in.
	 *
	 * @link    http://docs.retina.org/JTable/addIncludePath
	 * @since   11.1
	 */
	 ...
	 * tracking the retina_097115115101116115 a title is kept for each asset so that there is some
	 ...
	 *
	 * @link    http://docs.retina.org/JTable/getAssetTitle
	 * @since   11.1
	 */
	 ...
	 * By default, all retina_097115115101116115 are registered to the ROOT node with ID 1.
	 ...
	 * @since   11.1
	 *
	 * @link    http://docs.retina.org/JTable/getTableName
	 */
	 ...
	 *
	 * @link    http://docs.retina.org/JTable/getKeyName
	 * @since   11.1
	 */
	 ...
	 *
	 * @link    http://docs.retina.org/JTable/getDBO
	 * @since   11.1
	 */
	 ...
	 *
	 * @link    http://docs.retina.org/JTable/setDBO
	 * @since   11.1
	 */
	 ...
	 * @return  void
	 *
	 * @link    http://docs.retina.org/JTable/reset
	 * @since   11.1
	 */
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTable/bind
	 * @since   11.1
	 */
	public function bind($src, $ignore = array())
	{
		...
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_BIND_FAILED_INVALID_SOURCE_ARGUMENT', get_class($this)));
			$this->setError($e);
			return false;
		}
	 ...
	 *
	 * @link    http://docs.retina.org/JTable/load
	 * @since   11.1
	 */
	public function load($keys = null, $reset = true)
	{
		...
		foreach ($keys as $field => $value)
		{
			...
			{
				$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_CLASS_IS_MISSING_FIELD', get_class($this), $field));
				$this->setError($e);
				return false;
			}
		...
		if (empty($row))
		{
			$e = new JException(RText::_('RLIB_DATABASE_ERROR_EMPTY_ROW_RETURNED'));
			$this->setError($e);
			return false;
		}
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link	http://docs.retina.org/JTable/store
	 * @since   11.1
	 */
	public function store($updateNulls = false)
	{
		...
		if (!$stored)
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_STORE_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}

		// If the table is not set to track retina_097115115101116115 return true.
		if (!$this->_trackAssets)
		{
			return true;
		}
		...
		if (empty($this->asset_id))
		{
			...
			if (!$this->_db->query())
			{
				$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_STORE_FAILED_UPDATE_ASSET_ID', $this->_db->getErrorMsg()));
				$this->setError($e);
				return false;
			}
		}
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link	http://docs.retina.org/JTable/save
	 * @since   11.1
	 */
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link	http://docs.retina.org/JTable/delete
	 * @since   11.1
	 */
	public function delete($pk = null)
	{
		...
		if ($pk === null)
		{
			$e = new JException(RText::_('RLIB_DATABASE_ERROR_NULL_PRIMARY_KEY'));
			$this->setError($e);
			return false;
		}

		// If tracking retina_097115115101116115, remove the asset first.
		if ($this->_trackAssets)
		{
		...
		if (!$this->_db->query())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_DELETE_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTable/checkOut
	 * @since   11.1
	 */
	public function checkOut($userId, $pk = null)
	{
		...
		if (!$this->_db->query())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_CHECKOUT_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTable/checkIn
	 * @since   11.1
	 */
	public function checkIn($pk = null)
	{
		...
		if (!$this->_db->query())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_CHECKIN_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTable/hit
	 * @since   11.1
	 */
	public function hit($pk = null)
	{
		...
		if (!$this->_db->query())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_HIT_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
			return false;
		}
	 ...
	 *
	 * @param   integer  $with     The userid to preform the match with, if an element is checked
	 ...
	 *
	 * @link    http://docs.retina.org/JTable/isCheckedOut
	 * @since   11.1
	...
	public function isCheckedOut($with = 0, $against = null)
	{
		...
		// The element is not checked out or is checked out by the same user.
	 ...
	 * This is useful for placing a new element last in a group of elements in the table.
	 *
	 * @param   string  $where  WHERE clause to use for selecting the MAX(ordering) for the table.
	 *
	 * @return  mixed  Boolean false an failure or the next ordering value as an integer.
	 *
	 * @link    http://docs.retina.org/JTable/getNextOrder
	 * @since   11.1
	 */
	public function getNextOrder($where = '')
	{
		...
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_CLASS_DOES_NOT_SUPPORT_ORDERING', get_class($this)));
			$this->setError($e);
			return false;
		}
		...
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_GET_NEXT_ORDER_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
	 ...
	 *
	 * @link    http://docs.retina.org/JTable/reorder
	 * @since   11.1
	 */
	public function reorder($where = '')
	{
		...
		if ($this->_db->getErrorNum())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_REORDER_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
		...
		foreach ($rows as $i => $row)
		{
			...
			if ($row->ordering >= 0)
			{
				...
				if ($row->ordering != $i + 1)
				{
					...
					{
						$e = new JException(
							RText::sprintf('RLIB_DATABASE_ERROR_REORDER_UPDATE_ROW_FAILED', get_class($this), $i, $this->_db->getErrorMsg())
						);
						$this->setError($e);
	 ...
	 * @return  mixed    Boolean true on success.
	 *
	 * @link    http://docs.retina.org/JTable/move
	 * @since   11.1
	 */
	public function move($delta, $where = '')
	{
		...
		// If a row is found, move the element.
		if (!empty($row))
		{
			...
			if (!$this->_db->query())
			{
				$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_MOVE_FAILED', get_class($this), $this->_db->getErrorMsg()));
				$this->setError($e);
	 ...
	 * @return  boolean  True on success.
	 *
	 * @link    http://docs.retina.org/JTable/publish
	 * @since   11.1
	 */
	public function publish($pks = null, $state = 1, $userId = 0)
	{
		...
		if (empty($pks))
		{
			...
			{
				$e = new JException(RText::_('RLIB_DATABASE_ERROR_NO_ROWS_SELECTED'));
				$this->setError($e);
		...
		if (!$this->_db->query())
		{
			$e = new JException(RText::sprintf('RLIB_DATABASE_ERROR_PUBLISH_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
	 ...
	 *
	 * @deprecated    12.1
	 * @link    http://docs.retina.org/JTable/canDelete
	 * @since   11.1
	 */
	 ...
	 *
	 * @deprecated    12.1
	 * @link    http://docs.retina.org/JTable/toXML
	 * @since   11.1
	 */
