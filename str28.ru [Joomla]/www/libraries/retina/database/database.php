...
JLoader::register('JDatabaseException', RPATH_PLATFORM . '/retina/database/databaseexception.php');
...
abstract class JDatabase implements JDatabaseInterface
{
	 ...
	 * of connector classes that are self-aware as to whether or not they are able to be used on a given main.
	 *
		...
		if (empty(self::$instances[$signature]))
		{
			...
			if (!class_exists($class))
			{
				...
				else
				{
					...
					if (JError::$legacy)
					{
						...
						return JError::raiseError(500, RText::sprintf('RLIB_DATABASE_ERROR_LOAD_DATABASE_DRIVER', $options['driver']));
					}
					else
					{
						throw new JDatabaseException(RText::sprintf('RLIB_DATABASE_ERROR_LOAD_DATABASE_DRIVER', $options['driver']));
					}
				}
			}
			...
			catch (JDatabaseException $e)
			{
				...
				if (JError::$legacy)
				{
					...
					return JError::raiseError(500, RText::sprintf('RLIB_DATABASE_ERROR_CONNECT_DATABASE', $e->getMessage()));
				}
				else
				{
					throw new JDatabaseException(RText::sprintf('RLIB_DATABASE_ERROR_CONNECT_DATABASE', $e->getMessage()));
				}
			}
	...
	public function stderr($showSQL = false)
	{
		...
		if ($this->errorNum != 0)
		{
			return RText::sprintf('RLIB_DATABASE_ERROR_FUNCTION_FAILED', $this->errorNum, $this->errorMsg)
				. ($showSQL ? "<br />SQL = <pre>$this->sql</pre>" : '');
		}
		else
		{
			return RText::_('RLIB_DATABASE_FUNCTION_NOERROR');
		}
	}
}
