...
class JTableMenuType extends JTable
{
	...
	public function check()
	{
		...
		{
			$this->setError(RText::_('RLIB_DATABASE_ERROR_MENUTYPE_EMPTY'));
			return false;
		}
		...
		{
			$this->setError(RText::sprintf('RLIB_DATABASE_ERROR_MENUTYPE_EXISTS', $this->menutype));
			return false;
		}
	...
	public function store($updateNulls = false)
	{
		if ($this->id)
		{
			...
			// Verify that no elements are checked out
			$query = $this->_db->getQuery(true);
			$query->select('id');
			$query->from('#__menu');
			...
			{
				$this->setError(
					RText::sprintf('RLIB_DATABASE_ERROR_STORE_FAILED', get_class($this), RText::_('RLIB_DATABASE_ERROR_MENUTYPE_CHECKOUT'))
				);
				return false;
			}
			...
			if (!$this->_db->query())
			{
				$this->setError(RText::sprintf('RLIB_DATABASE_ERROR_STORE_FAILED', get_class($this), $this->_db->getErrorMsg()));
				return false;
			}

			// Update the module elements
			$query = $this->_db->getQuery(true);
	...
	public function delete($pk = null)
	{
		...
		if ($pk !== null)
		{
			...
			if ($this->_db->loadRowList())
			{
				$this->setError(RText::sprintf('RLIB_DATABASE_ERROR_DELETE_FAILED', get_class($this), RText::_('RLIB_DATABASE_ERROR_MENUTYPE')));
				return false;
			}
			...
			if (!$this->_db->query())
			{
				$this->setError(RText::sprintf('RLIB_DATABASE_ERROR_DELETE_FAILED', get_class($this), $this->_db->getErrorMsg()));
				return false;
			}
