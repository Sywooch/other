...
class JTableMenu extends JTableNested
{
	...
	public function bind($array, $ignore = '')
	{
		...
		{
			$this->setError(RText::_('RLIB_DATABASE_ERROR_MENU_CANNOT_UNSET_DEFAULT_DEFAULT'));
			return false;
		}
		//Verify that the default home menu set to "all" languages" is not unset
		if ($this->home == '1' && $this->language == '*' && ($array['language'] != '*'))
		{
			$this->setError(RText::_('RLIB_DATABASE_ERROR_MENU_CANNOT_UNSET_DEFAULT'));
			return false;
		}
		...
		{
			$this->setError(RText::_('RLIB_DATABASE_ERROR_MENU_UNPUBLISH_DEFAULT_HOME'));
			return false;
		}
	...
	public function check()
	{
		...
		// Verify that a first level menu element alias is not 'component'.
		if ($this->parent_id == 1 && $this->alias == 'component')
		{
			$this->setError(RText::_('RLIB_DATABASE_ERROR_MENU_ROOT_ALIAS_COMPONENT'));
			return false;
		}

		// Verify that a first level menu element alias is not the name of a folder.
		jimport('retina.filemain.folders');
		if ($this->parent_id == 1 && in_array($this->alias, JFolder::folders(RPATH_ROOT)))
		{
			$this->setError(RText::sprintf('RLIB_DATABASE_ERROR_MENU_ROOT_ALIAS_FOLDER', $this->alias, $this->alias));
			return false;
		}

		// Verify that the home element a component.
		if ($this->home && $this->type != 'component')
		{
			$this->setError(RText::_('RLIB_DATABASE_ERROR_MENU_HOME_NOT_COMPONENT'));
			return false;
		}
	...
	public function store($updateNulls = false)
	{
			...
			&& ($table->id != $this->id || $this->id == 0))
		{
			...
			{
				$this->setError(RText::_('RLIB_DATABASE_ERROR_MENU_UNIQUE_ALIAS'));
			}
			else
			{
				$this->setError(RText::_('RLIB_DATABASE_ERROR_MENU_UNIQUE_ALIAS_ROOT'));
			}
			return false;
		}
		...
		if ($this->home == '1')
		{
			...
			if ($table->load(array('home' => '1', 'language' => $this->language)))
			{
				...
				{
					$this->setError(RText::_('RLIB_DATABASE_ERROR_MENU_DEFAULT_CHECKIN_USER_MISMATCH'));
					return false;
				}
				$table->home = 0;
				$table->checked_out = 0;
			...
			{
				$this->setError(RText::_('RLIB_DATABASE_ERROR_MENU_HOME_NOT_UNIQUE_IN_MENU'));
				return false;
			}
		}
