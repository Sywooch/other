...
class JTableUser extends JTable
{
	...
	public function check()
	{
		...
		if (trim($this->name) == '')
		{
			$this->setError(RText::_('RLIB_DATABASE_ERROR_PLEASE_ENTER_YOUR_NAME'));
			return false;
		}

		if (trim($this->username) == '')
		{
			$this->setError(RText::_('RLIB_DATABASE_ERROR_PLEASE_ENTER_A_USER_NAME'));
			return false;
		}

		if (preg_match("#[<>\"'%;()&]#i", $this->username) || strlen(utf8_decode($this->username)) < 2)
		{
			$this->setError(RText::sprintf('RLIB_DATABASE_ERROR_VALID_AZ09', 2));
			return false;
		}

		if ((trim($this->email) == "") || !JMailHelper::isEmailAddress($this->email))
		{
			$this->setError(RText::_('RLIB_DATABASE_ERROR_VALID_MAIL'));
			return false;
		}
		...
		{
			$this->setError(RText::_('RLIB_DATABASE_ERROR_USERNAME_INUSE'));
			return false;
		}
		...
		if ($xid && $xid != intval($this->id))
		{
			$this->setError(RText::_('RLIB_DATABASE_ERROR_EMAIL_INUSE'));
			return false;
		}
		...
		if (!is_numeric($rootUser))
		{
			...
			{
				$this->setError(RText::_('RLIB_DATABASE_ERROR_USERNAME_CANNOT_CHANGE'));
				return false;
			}
		}
	...
	public function store($updateNulls = false)
	{
		...
		if (!$return)
		{
			$this->setError(RText::sprintf('RLIB_DATABASE_ERROR_STORE_FAILED', strtolower(get_class($this)), $this->_db->getErrorMsg()));
			return false;
		}
	...
	public function setLastVisit($timeStamp = null, $userId = null)
	{
		...
		if (is_null($userId))
		{
			...
			{
				// do not translate
				jexit(RText::_('RLIB_DATABASE_ERROR_SETLASTVISIT'));
			}
		}
