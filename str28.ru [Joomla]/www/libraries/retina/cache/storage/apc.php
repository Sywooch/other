...
class JCacheStorageApc extends JCacheStorage
{
	...
	public function getAll()
	{
		...
		foreach ($keys as $key)
		{
			...
			if ($namearr !== false && $namearr[0] == $secret && $namearr[1] == 'cache')
			{
				...
				if (!isset($data[$group]))
				{
					$element = new JCacheStorageHelper($group);
				}
				else
				{
					$element = $data[$group];
				...
				$element->updateSize($key['mem_size'] / 1024);
	...
	/**
	 * Force garbage collect expired cache data as elements are removed only on fetch!
	 *
	 * @return  boolean  True on success, false otherwise.
	 *
	 * @since   11.1
	...
	/**
	 * Lock cached element - override parent as this is more efficient
	 *
	 * @param   string   $id        The cache data id
	 * @param   string   $group     The cache data group
	...
	/**
	 * Unlock cached element - override parent for cacheid compatibility with lock
	 *
	 * @param   string  $id     The cache data id
	 * @param   string  $group  The cache data group
	 *
