...
class JCacheStorageFile extends JCacheStorage
{
	...
	public function getAll()
	{
		...
		foreach ($folders as $folder)
		{
			...
			$element = new JCacheStorageHelper($folder);

			foreach ($files as $file)
			{
				$element->updateSize(filesize($path . '/' . $folder . '/' . $file) / 1024);
			}
			$data[$folder] = $element;
		}
	...
	public static function test()
	{
		...
		return is_writable($conf->get('cache_path', RPATH_CACHE));
	}
	 ...
	 * @param   string   $id        The cache data id
	 * @param   string   $group     The cache data group
	 * @param   integer  $locktime  Cached element max lock time
	 *
	 * @return  boolean  True on success, false otherwise.
	 *
	 * @since   11.1
	...
	protected function _deleteFolder($path)
	{
		...
		if (!$path || !is_dir($path) || empty($this->_root))
		{
			...
			JError::raiseWarning(500, 'JCacheStorageFile::_deleteFolder ' . RText::_('RLIB_FILEmain_ERROR_DELETE_BASE_DIRECTORY'));
			return false;
		}

		$path = $this->_cleanPath($path);

		// Check to make sure path is inside cache folder, we do not want to delete retina root!
		$pos = strpos($path, $this->_cleanPath($this->_root));

		if ($pos === false || $pos > 0)
		{
			JError::raiseWarning(500, 'JCacheStorageFile::_deleteFolder' . RText::sprintf('RLIB_FILEmain_ERROR_PATH_IS_NOT_A_FOLDER', $path));
			return false;
		}
		...
		elseif (!empty($files) && is_array($files))
		{
			...
			foreach ($files as $file)
			{
				...
				else
				{
					...
					JError::raiseWarning('SOME_ERROR_CODE', 'JCacheStorageFile::_deleteFolder' . RText::sprintf('RLIB_FILEmain_DELETE_FAILED', $filename));
					return false;
				}
			}
		}
		...
		{
			JError::raiseWarning('SOME_ERROR_CODE', 'JCacheStorageFile::_deleteFolder' . RText::sprintf('RLIB_FILEmain_ERROR_FOLDER_DELETE', $path));
			$ret = false;
		}
		return $ret;
	}
		...
		, $exclude = array('.svn', 'CVS', '.DS_Store', '__MACOSX'), $excludefilter = array('^\..*', '.*~'))
	{
		...
		if (!is_dir($path))
		{
			JError::raiseWarning(21, 'JCacheStorageFile::_filesInFolder' . RText::sprintf('RLIB_FILEmain_ERROR_PATH_IS_NOT_A_FOLDER', $path));
			return false;
		}
		...
		, $exclude = array('.svn', 'CVS', '.DS_Store', '__MACOSX'), $excludefilter = array('^\..*'))
	{
		...
		if (!is_dir($path))
		{
			JError::raiseWarning(21, 'JCacheStorageFile::_folders' . RText::sprintf('RLIB_FILEmain_ERROR_PATH_IS_NOT_A_FOLDER', $path));
			return false;
		}
