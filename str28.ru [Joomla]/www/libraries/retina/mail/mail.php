...
/**
 * Email Class.  Provides a common interface to send email from the Retina Platform
 *
 * @package     retina.Platform
 * @subpackage  Mail
 * @since       11.1
...
class JMail extends PHPMailer
{
	...
	public function __construct()
	{
		...
		$this->SetLanguage('retina', RPATH_PLATFORM . '/phpmailer/language/');
	}
	 ...
	 * values, use an id string that is not 'retina'.
	 *
	 ...
	 * @since   11.1
	 */
	public static function getInstance($id = 'retina')
	{
	...
	public function Send()
	{
		...
		{
			return JError::raiseNotice(500, RText::_('RLIB_MAIL_FUNCTION_DISABLED'));
		}
		...
		if ($result == false)
		{
			...
			$result = JError::raiseNotice(500, RText::_($this->ErrorInfo));
		}
	...
	public function setSender($from)
	{
		...
		else
		{
			...
			JError::raiseWarning(0, RText::sprintf('RLIB_MAIL_INVALID_EMAIL_SENDER', $from));
		}
	...
	/**
	 * Sends mail to admin for approval of a user submission
	 *
	 * @param   string  $adminName   Name of admin
	 * @param   string  $adminEmail  Email address of admin
	 * @param   string  $email       [NOT USED TODO: Deprecate?]
	 * @param   string  $type        Type of element to approve
	 * @param   string  $title       Title of element to approve
	 * @param   string  $author      Author of element to approve
	...
	{
		$subject = RText::sprintf('RLIB_MAIL_USER_SUBMITTED', $type);

		$message = sprintf(RText::_('RLIB_MAIL_MSG_ADMIN'), $adminName, $type, $title, $author, $url, $url, 'admin', $type);
		$message .= RText::_('RLIB_MAIL_MSG') . "\n";
