...
jimport('retina.filemain.archive');
jimport('retina.filemain.path');
jimport('retina.base.adapter');

/**
 * retina base installer class
 *
 * @package     retina.Platform
 * @subpackage  Installer
 * @since       11.1
...
class JInstaller extends JAdapter
{
	...
	public function abort($msg = null, $type = null)
	{
		...
		if ($debug)
		{
			JError::raiseError(500, RText::_('RLIB_INSTALLER_ABORT_DEBUG') . $msg);
		}
	...
	public function install($path = null)
	{
		...
		{
			$this->abort(RText::_('RLIB_INSTALLER_ABORT_NOINSTALLPATH'));
			return false;
		}

		if (!$this->setupInstall())
		{
			$this->abort(RText::_('RLIB_INSTALLER_ABORT_DETECTMANIFEST'));
	...
	public function discover_install($eid = null)
	{
		if ($eid)
		{
			...
			{
				$this->abort(RText::_('RLIB_INSTALLER_ABORT_LOAD_DETAILS'));
			...
			{
				$this->abort(RText::_('RLIB_INSTALLER_ABORT_ALREADYINSTALLED'));
			...
			if (is_object($this->_adapters[$this->extension->type]))
			{
				...
				{
					$this->abort(RText::_('RLIB_INSTALLER_ABORT_METHODNOTSUPPORTED'));
		...
		{
			$this->abort(RText::_('RLIB_INSTALLER_ABORT_EXTENSIONNOTVALID'));
	...
	public function discover()
	{
		...
		{
			// Retina 1.5 installation adapter legacy support
	...
	public function update($path = null)
	{
		...
		{
			$this->abort(RText::_('RLIB_INSTALLER_ABORT_NOUPDATEPATH'));
		}

		if (!$this->setupInstall())
		{
			return $this->abort(RText::_('RLIB_INSTALLER_ABORT_DETECTMANIFEST'));
		}
	...
	public function refreshManifestCache($eid)
	{
		if ($eid)
		{
			...
			if ($this->extension->state == -1)
			{
				$this->abort(RText::_('RLIB_INSTALLER_ABORT_REFRESH_MANIFEST_CACHE'));
				return false;
			}
			...
			if (is_object($this->_adapters[$this->extension->type]))
			{
				...
				{
					$this->abort(RText::sprintf('RLIB_INSTALLER_ABORT_METHODNOTSUPPORTED_TYPE', $this->extension->type));
		...
		{
			$this->abort(RText::_('RLIB_INSTALLER_ABORT_REFRESH_MANIFEST_CACHE_VALID'));
	...
	public function parseQueries($element)
	{
		...
		foreach ($queries as $query)
		{
			...
			if (!$db->query())
			{
				JError::raiseWarning(1, RText::sprintf('RLIB_INSTALLER_ERROR_SQL_ERROR', $db->stderr(true)));
	...
	public function parseSQLFiles($element)
	{
		...
		foreach ($element->children() as $file)
		{
			...
			if ($fCharset == 'utf8' && $fDriver == $dbDriver)
			{
				...
				{
					JError::raiseWarning(1, RText::sprintf('RLIB_INSTALLER_ERROR_SQL_FILENOTFOUND', $sqlfile));
				...
				if ($buffer === false)
				{
					JError::raiseWarning(1, RText::_('RLIB_INSTALLER_ERROR_SQL_READBUFFER'));
	...
	/**
	 * Method to process the updates for an element
	 *
	...
	public function parseFiles($element, $cid = 0, $oldFiles = null, $oldMD5 = null)
	{
		...
		foreach ($element->children() as $file)
		{
			...
			if (basename($path['dest']) != $path['dest'])
			{
				...
				{
					JError::raiseWarning(1, RText::sprintf('RLIB_INSTALLER_ERROR_CREATE_DIRECTORY', $newdir));
					return false;
				}
			}
	...
	public function parseLanguages($element, $cid = 0)
	{
		...
		// 'languages' Files are copied to RPATH_BASE/language/ folder
	...
	public function parseMedia($element, $cid = 0)
	{
		...
		// Here we set the folder we are going to copy the files to.
		//	Default 'media' Files are copied to the RPATH_BASE/media folder

		$folder = ((string) $element->attributes()->destination) ? '/' . $element->attributes()->destination : null;
		$destination = JPath::clean(RPATH_ROOT . '/media' . $folder);
		...
		if (is_array($files) && count($files) > 0)
		{
			...
			foreach ($files as $file)
			{
				...
				if (!file_exists($filesource))
				{
					 ...
					 * and return false.
					 */
					JError::raiseWarning(1, RText::sprintf('RLIB_INSTALLER_ERROR_NO_FILE', $filesource));
				...
				elseif (($exists = file_exists($filedest)) && !$overwrite)
				{
					...
					JError::raiseWarning(1, RText::sprintf('RLIB_INSTALLER_ERROR_FILE_EXISTS', $filedest));
				...
				else
				{
					...
					if ($filetype == 'folder')
					{
						...
						{
							JError::raiseWarning(1, RText::sprintf('RLIB_INSTALLER_ERROR_FAIL_COPY_FOLDER', $filesource, $filedest));
							return false;
						}
					...
					else
					{
						...
						{
							JError::raiseWarning(1, RText::sprintf('RLIB_INSTALLER_ERROR_FAIL_COPY_FILE', $filesource, $filedest));
	...
	public function findManifest()
	{
		...
		if (!empty($xmlfiles))
		{
			...
			{
				// Is it a valid retina installation manifest file?
			...
			JError::raiseWarning(1, RText::_('RLIB_INSTALLER_ERROR_NOTFINDretinaXMLSETUPFILE'));
		...
		else
		{
			...
			JError::raiseWarning(1, RText::_('RLIB_INSTALLER_ERROR_NOTFINDXMLSETUPFILE'));
			return false;
		}
	}

	/**
	 * Is the XML file a valid retina installation manifest file.
	 *
	 ...
	 * @param   string   $folder   The folder of the extension (plugins; e.g. main)
	 * @param   integer  $client   The client application (admin or site)
	 *
