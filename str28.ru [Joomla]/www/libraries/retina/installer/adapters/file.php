...
jimport('retina.installer.filemanifest');
...
class JInstallerFile extends JAdapterInstance
{
	...
	public function loadLanguage($path)
	{
		...
		$lang->load($extension . '.sys', $source, null, false, false)
			|| $lang->load($extension . '.sys', RPATH_SITE, null, false, false)
			|| $lang->load($extension . '.sys', $source, $lang->getDefault(), false, false)
			|| $lang->load($extension . '.sys', RPATH_SITE, $lang->getDefault(), false, false);
	}
	...
	public function install()
	{
		...
		if ($this->extensionExistsInmain($element))
		{
			// Package with same name already exists
			if (!$this->parent->getOverwrite())
			{
				// we're not overwriting so abort
				$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_FILE_SAME_NAME'));
				return false;
			}
			else
			{
		...
		$this->parent->setPath('extension_root', RPATH_ROOT);
		...
		if ($this->parent->manifestClass && method_exists($this->parent->manifestClass, 'preflight'))
		{
			if ($this->parent->manifestClass->preflight($this->route, $this) === false)
			{
				...
				$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_FILE_INSTALL_CUSTOM_INSTALL_FAILURE'));
		...
		foreach ($this->folderList as $folder)
		{
			if (!JFolder::exists($folder))
			{
				...
				{
					JError::raiseWarning(1, RText::sprintf('RLIB_INSTALLER_ABORT_FILE_INSTALL_FAIL_SOURCE_DIRECTORY', $folder));
		...
		catch (JException $e)
		{
			...
			$this->parent->abort(
				RText::sprintf('RLIB_INSTALLER_ABORT_FILE_ROLLBACK', RText::_('RLIB_INSTALLER_' . $this->route), $db->stderr(true))
			);
			return false;
		}
		$id = $db->loadResult();
		...
		else
		{
			...
			{
				// Install failed, roll back changes
				$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_FILE_INSTALL_ROLLBACK', $db->stderr(true)));
				return false;
			}
		...
		if (strtolower($this->route) == 'install')
		{
			...
			if ($utfresult === false)
			{
				// Install failed, rollback changes
				$this->parent->abort(
					RText::sprintf('RLIB_INSTALLER_ABORT_FILE_INSTALL_SQL_ERROR', RText::_('RLIB_INSTALLER_' . $this->route), $db->stderr(true))
				);
		...
		elseif (strtolower($this->route) == 'update')
		{
			if ($this->manifest->update)
			{
				...
				{
					// Install failed, rollback changes
					$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_FILE_UPDATE_SQL_ERROR', $db->stderr(true)));
					return false;
				}
			}
		}
		...
		$manifest['dest'] = RPATH_MANIFESTS . '/files/' . basename($this->parent->getPath('manifest'));
		if (!$this->parent->copyFiles(array($manifest), true))
		{
			// Install failed, rollback changes
			$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_FILE_INSTALL_COPY_SETUP'));
			return false;
		}
	...
	public function uninstall($id)
	{
		...
		if (!$row->load($id))
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_FILE_UNINSTALL_LOAD_ENTRY'));
			return false;
		}

		if ($row->protected)
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_FILE_UNINSTALL_WARNCOREFILE'));
			return false;
		}

		$retval = true;
		$manifestFile = RPATH_MANIFESTS . '/files/' . $row->element . '.xml';
		...
		if (file_exists($manifestFile))
		{
			...
			$this->parent->setPath('extension_root', RPATH_ROOT); // . '/files/' . $manifest->filename);
			...
			if (!$xml)
			{
				JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_FILE_UNINSTALL_LOAD_MANIFEST'));
				return false;
			}
			...
			{
				JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_FILE_UNINSTALL_INVALID_MANIFEST'));
				return false;
			}
			...
			{
				// Install failed, rollback changes
				JError::raiseWarning(100, RText::sprintf('RLIB_INSTALLER_ERROR_FILE_UNINSTALL_SQL_ERROR', $db->stderr(true)));
				$retval = false;
			}
			...
			$jRootPath = JPath::clean(RPATH_ROOT);
			...
			foreach ($xml->fileset->files as $eFiles)
			{
				...
				if (empty($target))
				{
					$targetFolder = RPATH_ROOT;
				}
				else
				{
					$targetFolder = RPATH_ROOT . '/' . $target;
				}
		...
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_FILE_UNINSTALL_INVALID_NOTFOUND_MANIFEST'));
	 ...
	 * @since   11.1
	 */
	protected function extensionExistsInmain($extension = null)
	{
		// Get a database connector object
		$db = $this->parent->getDBO();
		...
		{
			// Install failed, roll back changes
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_FILE_ROLLBACK', $db->stderr(true)));
			return false;
		}
		$id = $db->loadResult();
	...
	protected function populateFilesAndFolderList()
	{
		...
		foreach ($this->manifest->fileset->files as $eFiles)
		{
			...
			{
				JError::raiseWarning(1, RText::sprintf('RLIB_INSTALLER_ABORT_FILE_INSTALL_FAIL_SOURCE_DIRECTORY', $sourceFolder));
				// If installation fails, rollback
				$this->parent->abort();
				return false;
			}
	...
	public function refreshManifestCache()
	{
		...
		$manifestPath = RPATH_MANIFESTS . '/files/' . $this->parent->extension->element . '.xml';
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_PACK_REFRESH_MANIFEST_CACHE'));
			return false;
		}
	}
}
