...
jimport('retina.installer.packagemanifest');
...
class JInstallerPackage extends JAdapterInstance
{
	...
	public function install()
	{
		...
		if (!empty($group))
		{
			$this->parent->setPath('extension_root', RPATH_MANIFESTS . '/packages/' . implode(DS, explode('/', $group)));
		}
		else
		{
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_PACK_INSTALL_NO_PACK', RText::_('RLIB_INSTALLER_' . strtoupper($this->route))));
			return false;
		}
		...
		if ($this->parent->manifestClass && method_exists($this->parent->manifestClass, 'preflight'))
		{
			if ($this->parent->manifestClass->preflight($this->route, $this) === false)
			{
				...
				$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_PACKAGE_INSTALL_CUSTOM_INSTALL_FAILURE'));
		...
		if (count($this->manifest->files->children()))
		{
			...
			foreach ($this->manifest->files->children() as $child)
			{
				...
				if (!$tmpInstaller->install($package['dir']))
				{
					$this->parent->abort(
						RText::sprintf(
							'RLIB_INSTALLER_ABORT_PACK_INSTALL_ERROR_EXTENSION', RText::_('RLIB_INSTALLER_' . strtoupper($this->route)),
							basename($file)
						)
					);
					return false;
		...
		{
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_PACK_INSTALL_NO_FILES', RText::_('RLIB_INSTALLER_' . strtoupper($this->route))));
			return false;
		}
		...
		if (!$row->store())
		{
			...
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_PACK_INSTALL_ROLLBACK', $row->getError()));
			return false;
		}
		...
		$manifest['dest'] = RPATH_MANIFESTS . '/packages/' . basename($this->parent->getPath('manifest'));

		if (!$this->parent->copyFiles(array($manifest), true))
		{
			// Install failed, rollback changes
			$this->parent->abort(
				RText::sprintf('RLIB_INSTALLER_ABORT_PACK_INSTALL_COPY_SETUP', RText::_('RLIB_INSTALLER_ABORT_PACK_INSTALL_NO_FILES'))
			);
			return false;
		}
		...
		if ($this->get('manifest_script'))
		{
			...
			if (!file_exists($path['dest']) || $this->parent->getOverwrite())
			{
				...
				{
					// Install failed, rollback changes
					$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_PACKAGE_INSTALL_MANIFEST'));
	...
	public function uninstall($id)
	{
		...
		if ($row->protected)
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_PACK_UNINSTALL_WARNCOREPACK'));
			return false;
		}

		$manifestFile = RPATH_MANIFESTS . '/packages/' . $row->get('element') . '.xml';
		$manifest = new JPackageManifest($manifestFile);

		// Set the package root path
		$this->parent->setPath('extension_root', RPATH_MANIFESTS . '/packages/' . $manifest->packagename);
		...
		{
			// TODO: Fail?
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_PACK_UNINSTALL_MISSINGMANIFEST'));
			return false;
		...
		if (!$xml)
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_PACK_UNINSTALL_LOAD_MANIFEST'));
			return false;
		}
		...
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_PACK_UNINSTALL_INVALID_MANIFEST'));
			return false;
		}
		...
		foreach ($manifest->filelist as $extension)
		{
			...
			if ($id)
			{
				...
				{
					$error = true;
					JError::raiseWarning(100, RText::sprintf('RLIB_INSTALLER_ERROR_PACK_UNINSTALL_NOT_PROPER', basename($extension->filename)));
				}
			}
			else
			{
				JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_PACK_UNINSTALL_UNKNOWN_EXTENSION'));
			}
		}
		...
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_PACK_UNINSTALL_MANIFEST_NOT_REMOVED'));
		}
	 ...
	 * @param   integer  $client  The application id (0: retina CMS site; 1: retina CMS admin).
	...
	protected function _getExtensionID($type, $id, $client, $group)
	{
		...
		switch ($type)
		{
			...
			case 'module':
			case 'template':
				// Languages, modules and design1 have a client but not a folder
		...
		// Note: For design1, libraries and packages their unique name is their key.
	...
	public function refreshManifestCache()
	{
		...
		$manifestPath = RPATH_MANIFESTS . '/packages/' . $this->parent->extension->element . '.xml';
