...
class JInstallerComponent extends JAdapterInstance
{
	...
	/**
	 *
	 * The list of current files fo the Retina CMS admin that are installed and is read
	...
	public function loadLanguage($path = null)
	{
		...
		if (!$source)
		{
			$this->parent
				->setPath(
				'source',
				($this->parent->extension->client_id ? RPATH_admin : RPATH_SITE) .
		...
		$source = $path ? $path : ($this->parent->extension->client_id ? RPATH_admin : RPATH_SITE) . '/components/' . $extension;
		...
		$lang->load($extension . '.sys', $source, null, false, false) || $lang->load($extension . '.sys', RPATH_admin, null, false, false)
	...
	public function install()
	{
		...
		$this->parent->set('message', RText::_((string) $this->manifest->description));

		// Set the installation target paths
		$this->parent->setPath('extension_site', JPath::clean(RPATH_SITE . '/components/' . $this->get('element')));
		$this->parent->setPath('extension_admin', JPath::clean(RPATH_admin . '/components/' . $this->get('element')));

		// copy this as its used as a common base
		$this->parent->setPath('extension_root', $this->parent->getPath('extension_admin'));
		...
		{
			JError::raiseWarning(1, RText::_('RLIB_INSTALLER_ERROR_COMP_INSTALL_ADMIN_ELEMENT'));
			return false;
		}

		// Filemain Processing Section
		...
		if (file_exists($this->parent->getPath('extension_site')) || file_exists($this->parent->getPath('extension_admin')))
		{
			...
			elseif (!$this->parent->getOverwrite())
			{
				...
				if (file_exists($this->parent->getPath('extension_site')))
				{
					...
					JError::raiseWarning(1, RText::sprintf('RLIB_INSTALLER_ERROR_COMP_INSTALL_DIR_SITE', $this->parent->getPath('extension_site')));
				}
				else
				{
					...
					JError::raiseWarning(
						1,
						RText::sprintf('RLIB_INSTALLER_ERROR_COMP_INSTALL_DIR_ADMIN', $this->parent->getPath('extension_admin'))
					);
				}
				return false;
			}
		...
		if ($this->parent->manifestClass && method_exists($this->parent->manifestClass, 'preflight'))
		{
			if ($this->parent->manifestClass->preflight('install', $this) === false)
			{
				...
				$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_COMP_INSTALL_CUSTOM_INSTALL_FAILURE'));
				return false;
			}
		}
		...
		if (!file_exists($this->parent->getPath('extension_site')))
		{
			if (!$created = JFolder::create($this->parent->getPath('extension_site')))
			{
				JError::raiseWarning(
					1,
					RText::sprintf('RLIB_INSTALLER_ERROR_COMP_INSTALL_FAILED_TO_CREATE_DIRECTORY_SITE', $this->parent->getPath('extension_site'))
				);
				return false;
			}
		}
		...
		if (!file_exists($this->parent->getPath('extension_admin')))
		{
			if (!$created = JFolder::create($this->parent->getPath('extension_admin')))
			{
				JError::raiseWarning(
					1,
					RText::sprintf(
						'RLIB_INSTALLER_ERROR_COMP_INSTALL_FAILED_TO_CREATE_DIRECTORY_ADMIN',
						$this->parent->getPath('extension_admin')
					)
				);
		...
		if ($created)
		{
			$this->parent->pushStep(array('type' => 'folder', 'path' => $this->parent->getPath('extension_admin')));
		}
		...
		if ($installFile)
		{
			...
			if (!file_exists($this->parent->getPath('extension_admin') . '/' . $installFile) || $this->parent->getOverwrite())
			{
				$path['src'] = $this->parent->getPath('source') . '/' . $installFile;
				$path['dest'] = $this->parent->getPath('extension_admin') . '/' . $installFile;

				if (!$this->parent->copyFiles(array($path)))
				{
					// Install failed, rollback changes
					$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_COMP_INSTALL_PHP_INSTALL'));
		...
		{
			// Make sure it hasn't already been copied (this would be an error in the XML install file)
			if (!file_exists($this->parent->getPath('extension_admin') . '/' . $uninstallFile) || $this->parent->getOverwrite())
			{
				$path['src'] = $this->parent->getPath('source') . '/' . $uninstallFile;
				$path['dest'] = $this->parent->getPath('extension_admin') . '/' . $uninstallFile;

				if (!$this->parent->copyFiles(array($path)))
				{
					// Install failed, rollback changes
					$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_COMP_INSTALL_PHP_UNINSTALL'));
					return false;
				}
			}
		}
		...
		if ($this->get('manifest_script'))
		{
			...
			$path['dest'] = $this->parent->getPath('extension_admin') . '/' . $this->get('manifest_script');
			...
			if (!file_exists($path['dest']) || $this->parent->getOverwrite())
			{
				...
				{
					// Install failed, rollback changes
					$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_COMP_INSTALL_MANIFEST'));
		 ...
		 * If retina 1.5 compatible, with discreet sql files - execute appropriate
		 * file for utf-8 support or non-utf-8 support
		 */
		// Try for retina 1.5 type queries
		...
		if (isset($this->manifest->install->sql))
		{
			...
			{
				// Install failed, rollback changes
				$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_COMP_INSTALL_SQL_ERROR', $db->stderr(true)));
		...
		{
			if (is_file($this->parent->getPath('extension_admin') . '/' . $this->get('install_script')) || $this->parent->getOverwrite())
			{
				$notdef = false;
				$ranwell = false;
				ob_start();
				...
				require_once $this->parent->getPath('extension_admin') . '/' . $this->get('install_script');
		...
		if (!$row->store())
		{
			...
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_COMP_INSTALL_ROLLBACK', $db->stderr(true)));
			return false;
		}
		...
		{
			// Install failed, rollback changes
			$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_COMP_INSTALL_COPY_SETUP'));
			return false;
		}
		...
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ABORT_COMP_BUILDADMINMENUS_FAILED'));
		...
		// Register the component container just under root in the retina_097115115101116115 table.
	...
	public function update()
	{
		...
		if ($description)
		{
			$this->parent->set('message', RText::_($description));
		}
		else
		{
		...
		$this->parent->setPath('extension_site', JPath::clean(RPATH_SITE . '/components/' . $this->get('element')));
		$this->parent->setPath('extension_admin', JPath::clean(RPATH_admin . '/components/' . $this->get('element')));
		$this->parent->setPath('extension_root', $this->parent->getPath('extension_admin')); // copy this as its used as a common base
		...
		// Look in the admin first
		$tmpInstaller = new JInstaller;
		$tmpInstaller->setPath('source', $this->parent->getPath('extension_admin'));
		...
		if (!$this->manifest->administration)
		{
			JError::raiseWarning(1, RText::_('RLIB_INSTALLER_ABORT_COMP_UPDATE_ADMIN_ELEMENT'));
			return false;
		}
		...
		if (!file_exists($this->parent->getPath('extension_site')))
		{
			if (!$created = JFolder::create($this->parent->getPath('extension_site')))
			{
				JError::raiseWarning(
					1,
					RText::sprintf('RLIB_INSTALLER_ERROR_COMP_UPDATE_FAILED_TO_CREATE_DIRECTORY_SITE', $this->parent->getPath('extension_site'))
				);
		...
		if (!file_exists($this->parent->getPath('extension_admin')))
		{
			if (!$created = JFolder::create($this->parent->getPath('extension_admin')))
			{
				JError::raiseWarning(
					1,
					RText::sprintf(
						'RLIB_INSTALLER_ERROR_COMP_UPDATE_FAILED_TO_CREATE_DIRECTORY_ADMIN',
						$this->parent->getPath('extension_admin')
					)
				);
				// Install failed, rollback any changes
		...
		if ($installFile)
		{
			...
			if (!file_exists($this->parent->getPath('extension_admin') . '/' . $installFile) || $this->parent->getOverwrite())
			{
				...
				{
					// Install failed, rollback changes
					$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_COMP_UPDATE_PHP_INSTALL'));
					return false;
				}
			}
		...
		if ($uninstallFile)
		{
			...
			if (!file_exists($this->parent->getPath('extension_admin') . '/' . $uninstallFile) || $this->parent->getOverwrite())
			{
				...
				{
					// Install failed, rollback changes
					$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_COMP_UPDATE_PHP_UNINSTALL'));
		...
		if ($this->get('manifest_script'))
		{
			...
			if (!file_exists($path['dest']) || $this->parent->getOverwrite())
			{
				...
				{
					// Install failed, rollback changes
					$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_COMP_UPDATE_MANIFEST'));
		...
		if ($this->manifest->update)
		{
			...
			{
				// Install failed, rollback changes
				$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_COMP_UPDATE_SQL_ERROR', $db->stderr(true)));
		...
		{
			// Install failed, roll back changes
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_COMP_UPDATE_ROLLBACK', $db->stderr(true)));
		...
		{
			// Install failed, rollback changes
			$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_COMP_UPDATE_COPY_SETUP'));
	...
	public function uninstall($id)
	{
		...
		if (!$row->load((int) $id))
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_COMP_UNINSTALL_ERRORUNKOWNEXTENSION'));
			return false;
		}
		...
		if ($row->protected)
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_COMP_UNINSTALL_WARNCORECOMPONENT'));
			return false;
		}

		// Get the admin and site paths for the component
		$this->parent->setPath('extension_admin', JPath::clean(RPATH_admin . '/components/' . $row->element));
		$this->parent->setPath('extension_site', JPath::clean(RPATH_SITE . '/components/' . $row->element));
		$this->parent->setPath('extension_root', $this->parent->getPath('extension_admin')); // copy this as its used as a common base
		...
		$this->parent->setPath('source', $this->parent->getPath('extension_admin'));
		...
		if (!$this->manifest)
		{
			...
			JFolder::delete($this->parent->getPath('extension_admin'));
			...
			// Raise a warning
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_COMP_UNINSTALL_ERRORREMOVEMANUALLY'));
		...
		$this->loadLanguage(RPATH_admin . '/components/' . $element);
		...
		if ($uninstallFile)
		{
			...
			if (is_file($this->parent->getPath('extension_admin') . '/' . $uninstallFile))
			{
				ob_start();
				ob_implicit_flush(false);

				require_once $this->parent->getPath('extension_admin') . '/' . $uninstallFile;

				if (function_exists('com_uninstall'))
				{
					if (com_uninstall() === false)
					{
						JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_COMP_UNINSTALL_CUSTOM'));
						$retval = false;
					}
				}
		 ...
		 * If retina CMS 1.5 compatible, with discrete sql files - execute appropriate
		...
		if (isset($this->manifest->uninstall->sql))
		{
			...
			{
				// Install failed, rollback changes
				JError::raiseWarning(100, RText::sprintf('RLIB_INSTALLER_ERROR_COMP_UNINSTALL_SQL_ERROR', $db->stderr(true)));
				$retval = false;
			}
		}
		...
		// Remove the component container in the retina_097115115101116115 table.
		$asset = JTable::getInstance('Asset');
		...
		if ($db->getErrorNum())
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_COMP_UNINSTALL_FAILED_DELETE_CATEGORIES'));
		...
		if (trim($row->element))
		{
			...
			if (is_dir($this->parent->getPath('extension_site')))
			{
				...
				{
					JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_COMP_UNINSTALL_FAILED_REMOVE_DIRECTORY_SITE'));
					$retval = false;
				}
			}

			// Delete the component admin directory
			if (is_dir($this->parent->getPath('extension_admin')))
			{
				if (!JFolder::delete($this->parent->getPath('extension_admin')))
				{
					JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_COMP_UNINSTALL_FAILED_REMOVE_DIRECTORY_ADMIN'));
					$retval = false;
				}
			}
		...
		else
		{
			...
			JError::raiseWarning(100, 'RLIB_INSTALLER_ERROR_COMP_UNINSTALL_NO_OPTION');
			return false;
		}
	}
	...
	protected function _buildAdminMenus()
	{
		...
		if ($componentrow)
		{
			...
			// Remove existing menu elements if overwrite has been enabled
			if ($option)
			{
		...
		if ($menuElement)
		{
			...
			/*
			 * Since we have created a menu element, we add it to the installation step stack
			 * so that if we have to rollback the changes we can undo it.
			 */
			$this->parent->pushStep(array('type' => 'menu', 'id' => $component_id));
		}
		// No menu element was specified, Let's make a generic menu element
		else
		{
			$data = array();
			$data['menutype'] = 'main';
	...
	protected function _removeAdminMenus(&$row)
	{
		...
		// Get the ids of the menu elements
		$query = $db->getQuery(true);
		$query->select('id');
		$query->from('#__menu');
		...
		{
			JError::raiseWarning('', RText::_('RLIB_INSTALLER_ERROR_COMP_REMOVING_ADMIN_MENUS_FAILED'));
		...
		elseif (!empty($ids))
		{
			// Iterate the elements to delete each one.
	 ...
	 * - Roll back the component menu element
	 *
	...
	{
		$results = array();
		$site_components = JFolder::folders(RPATH_SITE . '/components');
		$admin_components = JFolder::folders(RPATH_admin . '/components');

		foreach ($site_components as $component)
		{
			if (file_exists(RPATH_SITE . '/components/' . $component . '/' . str_replace('com_', '', $component) . '.xml'))
			{
				$manifest_details = JApplicationHelper::parseXMLInstallFile(
					RPATH_SITE . '/components/' . $component . '/' . str_replace('com_', '', $component) . '.xml'
				);
		...
		{
			if (file_exists(RPATH_admin . '/components/' . $component . '/' . str_replace('com_', '', $component) . '.xml'))
			{
				$manifest_details = JApplicationHelper::parseXMLInstallFile(
					RPATH_admin . '/components/' . $component . '/' . str_replace('com_', '', $component) . '.xml'
				);
				$extension = JTable::getInstance('extension');
				$extension->set('type', 'component');
	...
	public function discover_install()
	{
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_COMP_DISCOVER_STORE_DETAILS'));
			return false;
		}
		...
		if ($description)
		{
			$this->parent->set('message', RText::_((string) $description));
		}
		else
		{
			$this->parent->set('message', '');
		...
		if ($this->get('install_script'))
		{
			...
			if (is_file($this->parent->getPath('extension_admin') . '/' . $this->get('install_script')))
			{
				ob_start();
				ob_implicit_flush(false);
	...
	public function refreshManifestCache()
	{
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_COMP_REFRESH_MANIFEST_CACHE'));
			return false;
		}
	}
}
