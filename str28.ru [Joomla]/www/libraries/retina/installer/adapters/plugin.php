...
class JInstallerPlugin extends JAdapterInstance
{
	...
	public function loadLanguage($path = null)
	{
		...
		if (!$source)
		{
			$this->parent->setPath('source', RPATH_PLUGINS . '/' . $this->parent->extension->folder . '/' . $this->parent->extension->element);
		}
		...
		if ($element)
		{
			...
			if ($name)
			{
				...
				$source = $path ? $path : RPATH_PLUGINS . "/$group/$name";
	...
	public function install()
	{
		...
		{
			$this->parent->setPath('extension_root', RPATH_PLUGINS . '/' . $group . '/' . $element);
		}
		else
		{
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_PLG_INSTALL_NO_FILE', RText::_('RLIB_INSTALLER_' . $this->route)));
			return false;
		}
		...
		catch (JException $e)
		{
			...
			$this->parent
				->abort(RText::sprintf('RLIB_INSTALLER_ABORT_PLG_INSTALL_ROLLBACK', RText::_('RLIB_INSTALLER_' . $this->route), $db->stderr(true)));
			return false;
		}
		$id = $db->loadResult();
		...
		if (file_exists($this->parent->getPath('extension_root')) && (!$this->parent->getOverwrite() || $this->parent->getUpgrade()))
		{
			...
			elseif (!$this->parent->getOverwrite())
			{
				...
				$this->parent
					->abort(
					RText::sprintf(
						'RLIB_INSTALLER_ABORT_PLG_INSTALL_DIRECTORY', RText::_('RLIB_INSTALLER_' . $this->route),
		...
		if ($this->parent->manifestClass && method_exists($this->parent->manifestClass, 'preflight'))
		{
			if ($this->parent->manifestClass->preflight($this->route, $this) === false)
			{
				...
				$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_PLG_INSTALL_CUSTOM_INSTALL_FAILURE'));
				return false;
			}
		}
		...
		if (!file_exists($this->parent->getPath('extension_root')))
		{
			if (!$created = JFolder::create($this->parent->getPath('extension_root')))
			{
				$this->parent
					->abort(
					RText::sprintf(
						'RLIB_INSTALLER_ABORT_PLG_INSTALL_CREATE_DIRECTORY', RText::_('RLIB_INSTALLER_' . $this->route),
						$this->parent->getPath('extension_root')
					)
				);
				return false;
		...
		if ($this->get('manifest_script'))
		{
			...
			if (!file_exists($path['dest']))
			{
				if (!$this->parent->copyFiles(array($path)))
				{
					// Install failed, rollback changes
					$this->parent
						->abort(RText::sprintf('RLIB_INSTALLER_ABORT_PLG_INSTALL_MANIFEST', RText::_('RLIB_INSTALLER_' . $this->route)));
					return false;
				}
			}
		}
		...
		if ($id)
		{
			if (!$this->parent->getOverwrite())
			{
				...
				$this->parent
					->abort(
					RText::sprintf(
						'RLIB_INSTALLER_ABORT_PLG_INSTALL_ALLREADY_EXISTS', RText::_('RLIB_INSTALLER_' . $this->route),
						$this->get('name')
					)
				);
				return false;
		...
		else
		{
			...
			if (!$row->store())
			{
				...
				$this->parent
					->abort(
					RText::sprintf('RLIB_INSTALLER_ABORT_PLG_INSTALL_ROLLBACK', RText::_('RLIB_INSTALLER_' . $this->route), $db->stderr(true))
				);
				return false;
			}

			// Since we have created a plugin element, we add it to the installation step stack
		...
		if (strtolower($this->route) == 'install')
		{
			...
			if ($utfresult === false)
			{
				...
				$this->parent
					->abort(
					RText::sprintf('RLIB_INSTALLER_ABORT_PLG_INSTALL_SQL_ERROR', RText::_('RLIB_INSTALLER_' . $this->route), $db->stderr(true))
				);
				return false;
			}
		...
		elseif (strtolower($this->route) == 'update')
		{
			if ($this->manifest->update)
			{
				...
				{
					// Install failed, rollback changes
					$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_PLG_UPDATE_SQL_ERROR', $db->stderr(true)));
					return false;
				}
			}
		}
		...
		{
			// Install failed, rollback changes
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_PLG_INSTALL_COPY_SETUP', RText::_('RLIB_INSTALLER_' . $this->route)));
			return false;
		}
	...
	public function uninstall($id)
	{
		...
		if (!$row->load((int) $id))
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_PLG_UNINSTALL_ERRORUNKOWNEXTENSION'));
			return false;
		}
		...
		if ($row->protected)
		{
			JError::raiseWarning(100, RText::sprintf('RLIB_INSTALLER_ERROR_PLG_UNINSTALL_WARNCOREPLUGIN', $row->name));
			return false;
		}
		...
		if (trim($row->folder) == '')
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_PLG_UNINSTALL_FOLDER_FIELD_EMPTY'));
			return false;
		}

		// Set the plugin root path
		if (is_dir(RPATH_PLUGINS . '/' . $row->folder . '/' . $row->element))
		{
			// Use 1.6 plugins
			$this->parent->setPath('extension_root', RPATH_PLUGINS . '/' . $row->folder . '/' . $row->element);
		}
		else
		{
			// Use Legacy 1.5 plugins
			$this->parent->setPath('extension_root', RPATH_PLUGINS . '/' . $row->folder);
		}
		...
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_PLG_UNINSTALL_INVALID_NOTFOUND_MANIFEST'));
			return false;
		}
		...
		if (!$xml)
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_PLG_UNINSTALL_LOAD_MANIFEST'));
			return false;
		}
		...
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_PLG_UNINSTALL_INVALID_MANIFEST'));
			return false;
		}

		// Attempt to load the language file; might have uninstall strings
		$this->parent->setPath('source', RPATH_PLUGINS . '/' . $row->folder . '/' . $row->element);
		$this->loadLanguage(RPATH_PLUGINS . '/' . $row->folder . '/' . $row->element);
		...
		{
			// Install failed, rollback changes
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_PLG_UNINSTALL_SQL_ERROR', $db->stderr(true)));
			return false;
		}
	...
	{
		$results = array();
		$folder_list = JFolder::folders(RPATH_SITE . '/plugins');

		foreach ($folder_list as $folder)
		{
			$file_list = JFolder::files(RPATH_SITE . '/plugins/' . $folder, '\.xml$');
			foreach ($file_list as $file)
			{
				$manifest_details = JApplicationHelper::parseXMLInstallFile(RPATH_SITE . '/plugins/' . $folder . '/' . $file);
			...
			$folder_list = JFolder::folders(RPATH_SITE . '/plugins/' . $folder);
			foreach ($folder_list as $plugin_folder)
			{
				$file_list = JFolder::files(RPATH_SITE . '/plugins/' . $folder . '/' . $plugin_folder, '\.xml$');
				foreach ($file_list as $file)
				{
					$manifest_details = JApplicationHelper::parseXMLInstallFile(
						RPATH_SITE . '/plugins/' . $folder . '/' . $plugin_folder . '/' . $file
					);
					$file = JFile::stripExt($file);
	...
	public function discover_install()
	{
		...
		// Similar to modules and design1, rather easy
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_PLG_DISCOVER_STORE_DETAILS'));
			return false;
		}
	}
	...
	public function refreshManifestCache()
	{
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_PLG_REFRESH_MANIFEST_CACHE'));
			return false;
		}
	}
}
