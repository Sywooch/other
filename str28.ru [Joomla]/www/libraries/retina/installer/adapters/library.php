...
jimport('retina.installer.librarymanifest');
...
class JInstallerLibrary extends JAdapterInstance
{
	...
	public function loadLanguage($path = null)
	{
		...
		if (!$source)
		{
			$this->parent->setPath('source', RPATH_PLATFORM . '/' . $this->parent->extension->element);
		}
		...
		$source = $path ? $path : RPATH_PLATFORM . "/$name";
	...
	public function install()
	{
		...
		if ($result)
		{
			...
			else
			{
				...
				$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_LIB_INSTALL_ALREADY_INSTALLED'));
				return false;
			}
		}
		...
		if (!$group)
		{
			$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_LIB_INSTALL_NOFILE'));
			return false;
		}
		else
		{
			$this->parent->setPath('extension_root', RPATH_PLATFORM . '/' . implode(DS, explode('/', $group)));
		}
		...
		if (!file_exists($this->parent->getPath('extension_root')))
		{
			...
			{
				$this->parent->abort(
					RText::sprintf('RLIB_INSTALLER_ABORT_LIB_INSTALL_FAILED_TO_CREATE_DIRECTORY', $this->parent->getPath('extension_root'))
				);
				return false;
			}
		}
		...
		{
			// Install failed, roll back changes
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_LIB_INSTALL_ROLLBACK', $db->stderr(true)));
			return false;
		}
		...
		$manifest['dest'] = RPATH_MANIFESTS . '/libraries/' . basename($this->parent->getPath('manifest'));
		if (!$this->parent->copyFiles(array($manifest), true))
		{
			// Install failed, rollback changes
			$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_LIB_INSTALL_COPY_SETUP'));
			return false;
		}
	...
	public function uninstall($id)
	{
		...
		{
			JError::raiseWarning(100, RText::_('ERRORUNKOWNEXTENSION'));
			return false;
		}
		...
		if ($row->protected)
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_LIB_UNINSTALL_WARNCORELIBRARY'));
			return false;
		}

		$manifestFile = RPATH_MANIFESTS . '/libraries/' . $row->element . '.xml';
		...
		if (file_exists($manifestFile))
		{
			...
			$this->parent->setPath('extension_root', RPATH_PLATFORM . '/' . $manifest->libraryname);
			...
			if (!$xml)
			{
				JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_LIB_UNINSTALL_LOAD_MANIFEST'));
				return false;
			}
			...
			{
				JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_LIB_UNINSTALL_INVALID_MANIFEST'));
				return false;
			}
		...
		else
		{
			...
			unset($row);
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_LIB_UNINSTALL_INVALID_NOTFOUND_MANIFEST'));
			return false;
		}
	...
	{
		$results = array();
		$file_list = JFolder::files(RPATH_MANIFESTS . '/libraries', '\.xml$');
		foreach ($file_list as $file)
		{
			$manifest_details = JApplicationHelper::parseXMLInstallFile(RPATH_MANIFESTS . '/libraries/' . $file);
	...
	public function discover_install()
	{
		 ...
		 * 1) The manifest file (stored in /RPATH_MANIFESTS/libraries)
		 * 2) The actual files (stored in /RPATH_PLATFORM/libraryname)
		...
		$manifestPath = RPATH_MANIFESTS . '/libraries/' . $this->parent->extension->element . '.xml';
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_LIB_DISCOVER_STORE_DETAILS'));
			return false;
		}
	}
	...
	public function refreshManifestCache()
	{
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_LIB_REFRESH_MANIFEST_CACHE'));
			return false;
		}
	}
}
