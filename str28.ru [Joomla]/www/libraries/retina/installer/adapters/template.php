...
jimport('retina.installer.extension');
...
class JInstallerTemplate extends JAdapterInstance
{
	...
	public function loadLanguage($path = null)
	{
		...
		if (!$source)
		{
			$this->parent
				->setPath(
				'source',
				($this->parent->extension->client_id ? RPATH_admin : RPATH_SITE) . '/design1/' . $this->parent->extension->element
			);
		}
		...
		// Load admin language if not set.
		if (!$client)
		{
			$client = 'admin';
		}
		...
		$source = $path ? $path : ($this->parent->extension->client_id ? RPATH_admin : RPATH_SITE) . '/design1/' . $name;
	...
	public function install()
	{
		...
		if ($cname = (string) $xml->attributes()->client)
		{
			...
			if ($client === false)
			{
				$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_TPL_INSTALL_UNKNOWN_CLIENT', $cname));
				return false;
			}
		...
		$this->parent->setPath('extension_root', $basePath . '/design1/' . $element);
		...
		if (file_exists($this->parent->getPath('extension_root')) && !$this->parent->getOverwrite())
		{
			JError::raiseWarning(
				100,
				RText::sprintf('RLIB_INSTALLER_ABORT_TPL_INSTALL_ANOTHER_TEMPLATE_USING_DIRECTORY', $this->parent->getPath('extension_root'))
			);
			return false;
		}
		...
		if (!file_exists($this->parent->getPath('extension_root')))
		{
			...
			{
				$this->parent
					->abort(RText::sprintf('RLIB_INSTALLER_ABORT_TPL_INSTALL_FAILED_CREATE_DIRECTORY', $this->parent->getPath('extension_root')));
		...
		$this->parent->set('message', RText::_((string) $xml->description));

		// Lastly, we will copy the manifest file to its appropriate place.
		if (!$this->parent->copyManifest(-1))
		{
			// Install failed, rollback changes
			$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_TPL_INSTALL_COPY_SETUP'));
		...
		else
		{
			...
			// There is no folder for design1
			$row->folder = '';
			$row->enabled = 1;
			$row->protected = 0;
			$row->access = 1;
		...
		if (!$row->store())
		{
			...
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_TPL_INSTALL_ROLLBACK', $db->stderr(true)));
		...
		if ($this->route == 'install')
		{
			...
			$query->set('title=' . $db->Quote(RText::sprintf('RLIB_INSTALLER_DEFAULT_STYLE', RText::_($this->get('name')))));
			$lang->setDebug($debug);
	...
	public function uninstall($id)
	{
		...
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_TPL_UNINSTALL_ERRORUNKOWNEXTENSION'));
			return false;
		}
		...
		if ($row->protected)
		{
			JError::raiseWarning(100, RText::sprintf('RLIB_INSTALLER_ERROR_TPL_UNINSTALL_WARNCORETEMPLATE', $row->name));
			return false;
		}
		...
		// For a template the id will be the template name which represents the subfolder of the design1 folder that the template resides in.
		if (!$name)
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_TPL_UNINSTALL_TEMPLATE_ID_EMPTY'));
		...
		if ($db->loadResult() != 0)
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_TPL_UNINSTALL_TEMPLATE_DEFAULT'));
		...
		if (!$client)
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_TPL_UNINSTALL_INVALID_CLIENT'));
			return false;
		}

		$this->parent->setPath('extension_root', $client->path . '/design1/' . strtolower($name));
		...
		if (!($manifest instanceof JXMLElement))
		{
			...
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_TPL_UNINSTALL_INVALID_NOTFOUND_MANIFEST'));
		...
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_TPL_UNINSTALL_TEMPLATE_DIRECTORY'));
			$retval = false;
		}
	...
	/**
	 * Discover existing but uninstalled design1
	 *
	...
	{
		$results = array();
		$site_list = JFolder::folders(RPATH_SITE . '/design1');
		$admin_list = JFolder::folders(RPATH_admin . '/design1');
		...
		foreach ($site_list as $template)
		{
			if ($template == 'main')
			{
				...
				// Ignore special main template
			}
			$manifest_details = JApplicationHelper::parseXMLInstallFile(RPATH_SITE . "/design1/$template/templateDetails.xml");
		...
		foreach ($admin_list as $template)
		{
			...
			$manifest_details = JApplicationHelper::parseXMLInstallFile(RPATH_admin . "/design1/$template/templateDetails.xml");
			$extension = JTable::getInstance('extension');
			$extension->set('type', 'template');
	...
	{
		// design1 are one of the easiest
		// If its not in the extensions table we just add it
		$client = JApplicationHelper::getClientInfo($this->parent->extension->client_id);
		$manifestPath = $client->path . '/design1/' . $this->parent->extension->element . '/templateDetails.xml';
		...
		if ($this->parent->extension->store())
		{
			...
			$query->set('home=0');
			$query->set('title=' . $db->Quote(RText::sprintf('RLIB_INSTALLER_DEFAULT_STYLE', $this->parent->extension->name)));
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_TPL_DISCOVER_STORE_DETAILS'));
	...
	public function refreshManifestCache()
	{
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_TPL_REFRESH_MANIFEST_CACHE'));
			return false;
		}
	}
}
