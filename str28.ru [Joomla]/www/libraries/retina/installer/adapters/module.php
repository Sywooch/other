...
class JInstallerModule extends JAdapterInstance
{
	...
	public function loadLanguage($path = null)
	{
		...
		if (!$source)
		{
			$this->parent
				->setPath(
				'source',
				($this->parent->extension->client_id ? RPATH_admin : RPATH_SITE) . '/modules/' . $this->parent->extension->element
			);
		}
		...
		if ($this->manifest->files)
		{
			...
			if ($extension)
			{
				...
				$source = $path ? $path : ($this->parent->extension->client_id ? RPATH_admin : RPATH_SITE) . '/modules/' . $extension;
				...
				$lang->load($extension . '.sys', $source, null, false, false)
					|| $lang->load($extension . '.sys', constant('RPATH_' . strtoupper($client)), null, false, false)
					|| $lang->load($extension . '.sys', $source, $lang->getDefault(), false, false)
					|| $lang->load($extension . '.sys', constant('RPATH_' . strtoupper($client)), $lang->getDefault(), false, false);
			}
		}
	}
	...
	public function install()
	{
		...
		if ($cname = (string) $this->manifest->attributes()->client)
		{
			...
			{
				$this->parent
					->abort(RText::sprintf('RLIB_INSTALLER_ABORT_MOD_UNKNOWN_CLIENT', RText::_('RLIB_INSTALLER_' . $this->route), $client->name));
				return false;
			}
		...
		{
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_MOD_INSTALL_NOFILE', RText::_('RLIB_INSTALLER_' . $this->route)));
		...
		catch (JException $e)
		{
			...
			$this->parent
				->abort(RText::sprintf('RLIB_INSTALLER_ABORT_MOD_ROLLBACK', RText::_('RLIB_INSTALLER_' . $this->route), $db->stderr(true)));
		...
		if (file_exists($this->parent->getPath('extension_root')) && (!$this->parent->getOverwrite() || $this->parent->getUpgrade()))
		{
			...
			elseif (!$this->parent->getOverwrite())
			{
				...
				$this->parent
					->abort(
					RText::sprintf(
						'RLIB_INSTALLER_ABORT_MOD_INSTALL_DIRECTORY', RText::_('RLIB_INSTALLER_' . $this->route),
		...
		if ($this->parent->manifestClass && method_exists($this->parent->manifestClass, 'preflight'))
		{
			if ($this->parent->manifestClass->preflight($this->route, $this) === false)
			{
				...
				$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_MOD_INSTALL_CUSTOM_INSTALL_FAILURE'));
		...
		if (!file_exists($this->parent->getPath('extension_root')))
		{
			if (!$created = JFolder::create($this->parent->getPath('extension_root')))
			{
				$this->parent
					->abort(
					RText::sprintf(
						'RLIB_INSTALLER_ABORT_MOD_INSTALL_CREATE_DIRECTORY', RText::_('RLIB_INSTALLER_' . $this->route),
						$this->parent->getPath('extension_root')
					)
				);
		...
		if ($this->get('manifest_script'))
		{
			...
			if (!file_exists($path['dest']) || $this->parent->getOverwrite())
			{
				...
				{
					// Install failed, rollback changes
					$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_MOD_INSTALL_MANIFEST'));
		...
		else
		{
			...
			// Since we have created a module element, we add it to the installation step stack
			...
			$name = preg_replace('#[\*?]#', '', RText::_($this->get('name')));
		...
		// If retina 1.5 compatible, with discrete sql files, execute appropriate
		...
		if (strtolower($this->route) == 'install')
		{
			...
			if ($utfresult === false)
			{
				...
				$this->parent
					->abort(
					RText::sprintf('RLIB_INSTALLER_ABORT_MOD_INSTALL_SQL_ERROR', RText::_('RLIB_INSTALLER_' . $this->route), $db->stderr(true))
				);
		...
		elseif (strtolower($this->route) == 'update')
		{
			if ($this->manifest->update)
			{
				...
				{
					// Install failed, rollback changes
					$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT_MOD_UPDATE_SQL_ERROR', $db->stderr(true)));
					return false;
				}
			}
		}
		...
		{
			// Install failed, rollback changes
			$this->parent->abort(RText::_('RLIB_INSTALLER_ABORT_MOD_INSTALL_COPY_SETUP'));
	 ...
	 *
	 * This is really a shell for the install main
	 *
	 * @return  boolean  True on success.
	 *
	 * @since   11.1
	...
	{
		$results = array();
		$site_list = JFolder::folders(RPATH_SITE . '/modules');
		$admin_list = JFolder::folders(RPATH_admin . '/modules');
		$site_info = JApplicationHelper::getClientInfo('site', true);
		$admin_info = JApplicationHelper::getClientInfo('admin', true);

		foreach ($site_list as $module)
		{
			$manifest_details = JApplicationHelper::parseXMLInstallFile(RPATH_SITE . "/modules/$module/$module.xml");
		...
		{
			$manifest_details = JApplicationHelper::parseXMLInstallFile(RPATH_admin . "/modules/$module/$module.xml");
			$extension = JTable::getInstance('extension');
			$extension->set('type', 'module');
	...
	{
		// Modules are like design1, and are one of the easiest
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_MOD_DISCOVER_STORE_DETAILS'));
	...
	public function refreshManifestCache()
	{
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_MOD_REFRESH_MANIFEST_CACHE'));
	...
	public function uninstall($id)
	{
		...
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_MOD_UNINSTALL_ERRORUNKOWNEXTENSION'));
			return false;
		}
		...
		if ($row->protected)
		{
			JError::raiseWarning(100, RText::sprintf('RLIB_INSTALLER_ERROR_MOD_UNINSTALL_WARNCOREMODULE', $row->name));
			return false;
		}
		...
		if ($client === false)
		{
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ERROR_MOD_UNINSTALL_UNKNOWN_CLIENT', $row->client_id));
			return false;
		}
		...
		$this->loadLanguage(($row->client_id ? RPATH_admin : RPATH_SITE) . '/modules/' . $element);
		...
		if (!($this->manifest instanceof JXMLElement))
		{
			...
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_MOD_UNINSTALL_INVALID_NOTFOUND_MANIFEST'));
		...
		{
			// Install failed, rollback changes
			JError::raiseWarning(100, RText::sprintf('RLIB_INSTALLER_ERROR_MOD_UNINSTALL_SQL_ERROR', $db->stderr(true)));
			$retval = false;
		}
		...
		if (count($modules))
		{
			...
			// Wipe out any elements assigned to menus
			...
			{
				JError::raiseWarning(100, RText::sprintf('RLIB_INSTALLER_ERROR_MOD_UNINSTALL_EXCEPTION', $db->stderr(true)));
				$retval = false;
			}
