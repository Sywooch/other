...
class JInstallerLanguage extends JAdapterInstance
{
	...
	public function install()
	{
		...
		if (!$source)
		{
			$this->parent
				->setPath(
				'source',
				($this->parent->extension->client_id ? RPATH_admin : RPATH_SITE) . '/language/' . $this->parent->extension->element
			);
		}
		...
		{
			JError::raiseWarning(42, RText::_('RLIB_INSTALLER_ERROR_DEPRECATED_FORMAT'));
			$element = $this->manifest->site->files;
			if (!$this->_install('site', RPATH_SITE, 0, $element))
			{
				return false;
			}

			$element = $this->manifest->administration->files;
			if (!$this->_install('admin', RPATH_admin, 1, $element))
			{
				return false;
			}
		...
		elseif ($cname = (string) $this->manifest->attributes()->client)
		{
			...
			if ($client === null)
			{
				$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT', RText::sprintf('RLIB_INSTALLER_ERROR_UNKNOWN_CLIENT_TYPE', $cname)));
				return false;
			}
	...
	protected function _install($cname, $basePath, $clientId, &$element)
	{
		...
		if (!$tag)
		{
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT', RText::_('RLIB_INSTALLER_ERROR_NO_LANGUAGE_TAG')));
			return false;
		}
		...
		if (!$this->_core)
		{
			...
			{
				$this->parent
					->abort(RText::sprintf('RLIB_INSTALLER_ABORT', RText::sprintf('RLIB_INSTALLER_ERROR_NO_CORE_LANGUAGE', $this->get('tag'))));
				return false;
			}
		}
		...
		if (!file_exists($this->parent->getPath('extension_site')))
		{
			if (!$created = JFolder::create($this->parent->getPath('extension_site')))
			{
				$this->parent
					->abort(
					RText::sprintf(
						'RLIB_INSTALLER_ABORT',
						RText::sprintf('RLIB_INSTALLER_ERROR_CREATE_FOLDER_FAILED', $this->parent->getPath('extension_site'))
					)
				);
				return false;
			}
			...
			elseif (!$this->parent->getOverwrite())
			{
				...
				if (file_exists($this->parent->getPath('extension_site')))
				{
					...
					JError::raiseWarning(
						...
						RText::sprintf(
							'RLIB_INSTALLER_ABORT',
							RText::sprintf('RLIB_INSTALLER_ERROR_FOLDER_IN_USE', $this->parent->getPath('extension_site'))
						)
					);
				}
				else
				{
					...
					JError::raiseWarning(
						...
						RText::sprintf(
							'RLIB_INSTALLER_ABORT',
							RText::sprintf('RLIB_INSTALLER_ERROR_FOLDER_IN_USE', $this->parent->getPath('extension_admin'))
						)
					);
				}
				return false;
		...
		if (!$row->store())
		{
			...
			$this->parent->abort(RText::sprintf('RLIB_INSTALLER_ABORT', $row->getError()));
			return false;
		}
	...
	public function uninstall($eid)
	{
		...
		if (empty($element))
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_LANG_UNINSTALL_ELEMENT_EMPTY'));
			return false;
		}
		...
		if ($protected == 1)
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_LANG_UNINSTALL_PROTECTED'));
			return false;
		}
		...
		{
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_LANG_UNINSTALL_DEFAULT'));
			return false;
		}
		...
		if (!JFolder::exists($path))
		{
			...
			$extension->delete();
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_LANG_UNINSTALL_PATH_EMPTY'));
			return false;
		}
		...
		if (!JFolder::delete($path))
		{
			...
			JError::raiseWarning(100, RText::_('RLIB_INSTALLER_ERROR_LANG_UNINSTALL_DIRECTORY'));
			return false;
		}
		...
		if (!empty($count))
		{
			JError::raiseNotice(500, RText::plural('RLIB_INSTALLER_NOTICE_LANG_RESET_USERS', $count));
		}
	...
	{
		$results = array();
		$site_languages = JFolder::folders(RPATH_SITE . '/language');
		$admin_languages = JFolder::folders(RPATH_admin . '/language');
		foreach ($site_languages as $language)
		{
			if (file_exists(RPATH_SITE . '/language/' . $language . '/' . $language . '.xml'))
			{
				$manifest_details = JApplicationHelper::parseXMLInstallFile(RPATH_SITE . '/language/' . $language . '/' . $language . '.xml');
				$extension = JTable::getInstance('extension');
		...
		{
			if (file_exists(RPATH_admin . '/language/' . $language . '/' . $language . '.xml'))
			{
				$manifest_details = JApplicationHelper::parseXMLInstallFile(RPATH_admin . '/language/' . $language . '/' . $language . '.xml');
				$extension = JTable::getInstance('extension');
				$extension->set('type', 'language');
	...
	public function discover_install()
	{
		...
		{
			JError::raiseWarning(101, RText::_('RLIB_INSTALLER_ERROR_LANG_DISCOVER_STORE_DETAILS'));
			return false;
		}
