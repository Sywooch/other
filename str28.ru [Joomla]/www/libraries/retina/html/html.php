...
JHtml::addIncludePath(RPATH_PLATFORM . '/retina/html/html');

jimport('retina.environment.uri');
jimport('retina.environment.browser');
...
abstract class JHtml
{
	...
	public static function _($key)
	{
		...
		if (!class_exists($className))
		{
			...
			if ($path = JPath::find(JHtml::$includePaths, strtolower($file) . '.php'))
			{
				...
				if (!class_exists($className))
				{
					JError::raiseError(500, RText::sprintf('RLIB_HTML_ERROR_NOTFOUNDINFILE', $className, $func));
					return false;
				}
			}
			else
			{
				JError::raiseError(500, RText::sprintf('RLIB_HTML_ERROR_NOTSUPPORTED_NOFILE', $prefix, $file));
				return false;
			}
		}
		...
		{
			JError::raiseError(500, RText::sprintf('RLIB_HTML_ERROR_NOTSUPPORTED', $className, $func));
			return false;
		}
	}
	...
	protected static function call($function, $args)
	{
		...
		{
			JError::raiseError(500, RText::_('RLIB_HTML_ERROR_FUNCTION_NOT_SUPPORTED'));
			return false;
		}
	}
	...
	protected static function includeRelativeFiles($folder, $file, $relative, $detect_browser, $detect_debug)
	{
		...
		else
		{
			...
			if ($relative)
			{
				...
				foreach ($potential as $strip)
				{
					...
					foreach ($files as $file)
					{
						...
						if (file_exists(RPATH_THEMES . "/$template/$folder/$file"))
						{
							$includes[] = JURI::base(true) . "/design1/$template/$folder/$file";
							break;
						}
						else
						{
							...
							if (strpos($file, '/'))
							{
								...
								if (strpos($file, '/'))
								{
									...
									if (file_exists(RPATH_ROOT . "/media/$extension/$element/$folder/$file"))
									{
									...
									elseif (file_exists(RPATH_ROOT . "/media/$extension/$folder/$element/$file"))
									{
										$includes[] = JURI::root(true) . "/media/$extension/$folder/$element/$file";
										break;
									}
									// Try to deal with main files in the template folder
									elseif (file_exists(RPATH_THEMES . "/$template/$folder/main/$element/$file"))
									{
										$includes[] = JURI::root(true) . "/design1/$template/$folder/main/$element/$file";
										break;
									}
									// Try to deal with main files in the media folder
									elseif (file_exists(RPATH_ROOT . "/media/main/$folder/$element/$file"))
									{
										$includes[] = JURI::root(true) . "/media/main/$folder/$element/$file";
										break;
									}
								}
								// Try to deals in the extension media folder
								elseif (file_exists(RPATH_ROOT . "/media/$extension/$folder/$file"))
								{
									$includes[] = JURI::root(true) . "/media/$extension/$folder/$file";
									break;
								}
								// Try to deal with main files in the template folder
								elseif (file_exists(RPATH_THEMES . "/$template/$folder/main/$file"))
								{
									$includes[] = JURI::root(true) . "/design1/$template/$folder/main/$file";
									break;
								}
								// Try to deal with main files in the media folder
								elseif (file_exists(RPATH_ROOT . "/media/main/$folder/$file"))
								{
									$includes[] = JURI::root(true) . "/media/main/$folder/$file";
									break;
								}
							}
							// Try to deal with main files in the media folder
			...
			else
			{
				...
				{
					// Detect debug mode
					if ($detect_debug && JFactory::getConfig()->get('debug') && file_exists(RPATH_ROOT . "/$strip-uncompressed.$ext"))
					{
						$includes[] = JURI::root(true) . "/$strip-uncompressed.$ext";
					}
					elseif (file_exists(RPATH_ROOT . "/$strip.$ext"))
					{
	...
	public static function stylesheet($file, $attribs = array(), $relative = false, $path_only = false, $detect_browser = true, $detect_debug = true)
	{
		...
		// Function stylesheet($filename, $path = 'media/main/css/', $attribs = array())
	...
	{
		// Need to adjust for the change in API from 1.5 to 1.6.
		// function script($filename, $path = 'media/main/js/', $mootools = true)
	...
	/**
	 * Add the /media/main/js/core Javascript file.
	 *
	...
	public static function calendar($value, $name, $id, $format = '%Y-%m-%d', $attribs = null)
	{
		...
		return '<input type="text" title="' . (0 !== (int) $value ? JHtml::_('date', $value) : '') . '" name="' . $name . '" id="' . $id
			...
			. ($readonly ? ''
			: JHtml::_('image', 'main/calendar.png', RText::_('RLIB_HTML_CALENDAR'), array('class' => 'calendar', 'id' => $id . '_img'), true));
	}
