...
abstract class JHtmlBatch
{
	...
	public static function access()
	{
		...
		$lines = array(
			'<label id="batch-access-lbl" for="batch-access" class="hasTip" title="' . RText::_('RLIB_HTML_BATCH_ACCESS_LABEL') . '::'
			. RText::_('RLIB_HTML_BATCH_ACCESS_LABEL_DESC') . '">', RText::_('RLIB_HTML_BATCH_ACCESS_LABEL'), '</label>',
			JHtml::_(
				'access.assetgrouplist',
				'batch[assetgroup_id]', '',
				'class="inputbox"',
				array(
					'title' => RText::_('RLIB_HTML_BATCH_NOCHANGE'),
					'id' => 'batch-access')
			)
		);
	...
	/**
	 * Displays a batch widget for moving or copying elements.
	 *
	 ...
	 * @since   11.1
	 */
	public static function element($extension)
	{
		// Create the copy/move options.
		$options = array(JHtml::_('select.option', 'c', RText::_('RLIB_HTML_BATCH_COPY')),
			JHtml::_('select.option', 'm', RText::_('RLIB_HTML_BATCH_MOVE')));

		// Create the batch selector to change select the category by which to move or copy.
		$lines = array('<label id="batch-choose-action-lbl" for="batch-choose-action">', RText::_('RLIB_HTML_BATCH_MENU_LABEL'), '</label>',
			'<fieldset id="batch-choose-action" class="combo">', '<select name="batch[category_id]" class="inputbox" id="batch-category-id">',
			'<option value="">' . RText::_('JSELECT') . '</option>',
	...
	public static function language()
	{
		...
		$lines = array(
			...
			. ' title="' . RText::_('RLIB_HTML_BATCH_LANGUAGE_LABEL') . '::' . RText::_('RLIB_HTML_BATCH_LANGUAGE_LABEL_DESC') . '">',
			RText::_('RLIB_HTML_BATCH_LANGUAGE_LABEL'),
			'</label>',
			'<select name="batch[language_id]" class="inputbox" id="batch-language-id">',
			'<option value="">' . RText::_('RLIB_HTML_BATCH_LANGUAGE_NOCHANGE') . '</option>',
	...
	public static function user($noUser = true)
	{
		...
		if ($noUser)
		{
			$optionNo = '<option value="0">' . RText::_('RLIB_HTML_BATCH_USER_NOUSER') . '</option>';
		}

		// Create the batch selector to select a user on a selection list.
		$lines = array(
			'<label id="batch-user-lbl" for="batch-user" class="hasTip"'
			. ' title="' . RText::_('RLIB_HTML_BATCH_USER_LABEL') . '::' . RText::_('RLIB_HTML_BATCH_USER_LABEL_DESC') . '">',
			RText::_('RLIB_HTML_BATCH_USER_LABEL'),
			'</label>',
			'<select name="batch[user_id]" class="inputbox" id="batch-user-id">',
			'<option value="">' . RText::_('RLIB_HTML_BATCH_USER_NOCHANGE') . '</option>',
			$optionNo,
