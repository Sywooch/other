...
abstract class JHtmlJGrid
{
		...
		$inactive_class = '', $enabled = true, $translate = true, $checkbox = 'cb')
	{
		...
		if ($enabled)
		{
			...
			$html[] = ' href="javascript:void(0);" onclick="return listelementTask(\'' . $checkbox . $i . '\',\'' . $prefix . $task . '\')"';
			$html[] = ' title="' . addslashes(htmlspecialchars($translate ? RText::_($active_title) : $active_title, ENT_COMPAT, 'UTF-8')) . '">';
			$html[] = '<span class="state ' . $active_class . '">';
			$html[] = $text ? ('<span class="text">' . ($translate ? RText::_($text):$text) . '</span>') : '';
			$html[] = '</span>';
			$html[] = '</a>';
		}
		else
		{
			$html[] = '<a class="jgrid' . ($tip ? ' hasTip' : '') . '"';
			$html[] = ' title="' . addslashes(htmlspecialchars($translate ? RText::_($inactive_title) : $inactive_title, ENT_COMPAT, 'UTF-8')) . '">';
	...
	public static function published($value, $i, $prefix = '', $enabled = true, $checkbox = 'cb', $publish_up = null, $publish_down = null)
	{
		...
		$states = array(1 => array('unpublish', 'JPUBLISHED', 'RLIB_HTML_UNPUBLISH_element', 'JPUBLISHED', false, 'publish', 'publish'),
			0 => array('publish', 'JUNPUBLISHED', 'RLIB_HTML_PUBLISH_element', 'JUNPUBLISHED', false, 'unpublish', 'unpublish'),
			2 => array('unpublish', 'JARCHIVED', 'RLIB_HTML_UNPUBLISH_element', 'JARCHIVED', false, 'archive', 'archive'),
			-2 => array('publish', 'JTRASHED', 'RLIB_HTML_PUBLISH_element', 'JTRASHED', false, 'trash', 'trash'));
		...
		if ($publish_up || $publish_down)
		{
			...
			if ($publish_up)
			{
				$tips[] = RText::sprintf('RLIB_HTML_PUBLISHED_START', $publish_up->format(JDate::$format, true));
			}
			if ($publish_down)
			{
				$tips[] = RText::sprintf('RLIB_HTML_PUBLISHED_FINISHED', $publish_down->format(JDate::$format, true));
			}
			...
			{
				// Create special titles for published elements
				if ($key == 1)
				{
					$states[$key][2] = $states[$key][3] = 'RLIB_HTML_PUBLISHED_element';
					if ($publish_up > $nullDate && $nowDate < $publish_up->toUnix())
					{
						$states[$key][2] = $states[$key][3] = 'RLIB_HTML_PUBLISHED_PENDING_element';
						$states[$key][5] = $states[$key][6] = 'pending';
					}
					if ($publish_down > $nullDate && $nowDate > $publish_down->toUnix())
					{
						$states[$key][2] = $states[$key][3] = 'RLIB_HTML_PUBLISHED_EXPIRED_element';
				...
				if ($tip)
				{
					$states[$key][1] = RText::_($states[$key][1]);
					$states[$key][2] = RText::_($states[$key][2]) . '::' . $tip;
					$states[$key][3] = RText::_($states[$key][3]) . '::' . $tip;
					$states[$key][4] = true;
				}
			}
	...
	public static function isdefault($value, $i, $prefix = '', $enabled = true, $checkbox = 'cb')
	{
		...
		$states = array(
			1 => array('unsetDefault', 'JDEFAULT', 'RLIB_HTML_UNSETDEFAULT_element', 'JDEFAULT', false, 'default', 'default'),
			0 => array('setDefault', '', 'RLIB_HTML_SETDEFAULT_element', '', false, 'notdefault', 'notdefault'),
		);
	...
	public static function publishedOptions($config = array())
	{
		...
		{
			$options[] = JHtml::_('select.option', '*', 'RALL');
		}
		return $options;
	}
	...
	public static function checkedout($i, $editorName, $time, $prefix = '', $enabled = false, $checkbox = 'cb')
	{
		...
		$date = addslashes(htmlspecialchars(JHtml::_('date', $time, RText::_('DATE_FORMAT_LC')), ENT_COMPAT, 'UTF-8'));
		$time = addslashes(htmlspecialchars(JHtml::_('date', $time, 'H:i'), ENT_COMPAT, 'UTF-8'));
		$active_title = RText::_('RLIB_HTML_CHECKIN') . '::' . $text . '<br />' . $date . '<br />' . $time;
		$inactive_title = RText::_('RLIB_HTML_CHECKED_OUT') . '::' . $text . '<br />' . $date . '<br />' . $time;

		return self::action(
			$i, 'checkin', $prefix, RText::_('RLIB_HTML_CHECKED_OUT'), $active_title, $inactive_title, true, 'checkedout',
	 ...
	 * @since   11.1
	 */
	public static function orderUp($i, $task = 'orderup', $prefix = '', $text = 'RLIB_HTML_MOVE_UP', $enabled = true, $checkbox = 'cb')
	{
		if (is_array($prefix))
		{
			$options = $prefix;
	 ...
	 * @since   11.1
	 */
	public static function orderDown($i, $task = 'orderdown', $prefix = '', $text = 'RLIB_HTML_MOVE_DOWN', $enabled = true, $checkbox = 'cb')
	{
		if (is_array($prefix))
		{
			$options = $prefix;
