...
abstract class JHtmlSelect
{
	 ...
	 * @see     JFormFieldRadio
	 */
	public static function booleanlist($name, $attribs = null, $selected = null, $yes = 'RYES', $no = 'RNO', $id = false)
	{
		$arr = array(JHtml::_('select.option', '0', RText::_($no)), JHtml::_('select.option', '1', RText::_($yes)));
	 ...
	 *                            used.
	 *                            group.elements: The property in each group to use as the array of
	 *                            elements in the group. Defaults to "elements". If set to null, group.id and
	...
	public static function groupedlist($data, $name, $options = array())
	{
		...
		$options = array_merge(
			JHtml::$formatOptions,
			array('format.depth' => 0, 'group.elements' => 'elements', 'group.label' => 'text', 'group.label.toHtml' => true, 'id' => false),
			$options
		);

		// Apply option rules
		if ($options['group.elements'] === null)
		{
		...
		foreach ($data as $dataKey => $group)
		{
			...
			elseif (is_array($group))
			{
				...
				$subList = $group[$options['group.elements']];
	...
	public static function options($arr, $optKey = 'value', $optText = 'text', $selected = null, $translate = false)
	{
		...
		foreach ($arr as $elementKey => &$element)
		{
			...
			{
				$html .= $baseIndent . '<optgroup label="' . ($options['list.translate'] ? RText::_($text) : $text) . '">' . $options['format.eol'];
				$baseIndent = str_repeat($options['format.indent'], ++$options['format.depth']);
			}
		...
		$translate = false)
	{
		...
		{
			$k = $obj->$optKey;
			$t = $translate ? RText::_($obj->$optText) : $obj->$optText;
