...
{
	/**
	 * Get a grouped list of pre-retina 1.6 access levels
	 *
	 ...
	 * @param   string  $active      The selected element
	...
	public static function images($name, $active = null, $javascript = null, $directory = null, $extensions = "bmp|gif|jpg|png")
	{
		...
		if (!$javascript)
		{
			$javascript = "onchange=\"if (document.forms.adminForm." . $name
				...
				. ".options[selectedIndex].value} else {document.imagelib.src='media/main/images/blank.png'}\"";
		}

		jimport('retina.filemain.folder');
		$imageFiles = JFolder::files(RPATH_SITE . '/' . $directory);
		$images = array(JHtml::_('select.option', '', RText::_('ROPTION_SELECT_IMAGE')));
	...
	public static function genericordering($sql, $chop = '30')
	{
		...
		if (empty($elements))
		{
			$options[] = JHtml::_('select.option', 1, RText::_('ROPTION_ORDER_FIRST'));
			return $options;
		}

		$options[] = JHtml::_('select.option', 0, '0 ' . RText::_('ROPTION_ORDER_FIRST'));
		for ($i = 0, $n = count($elements); $i < $n; $i++)
		{
			$elements[$i]->text = RText::_($elements[$i]->text);
			if (JString::strlen($elements[$i]->text) > $chop)
			{
				$text = JString::substr($elements[$i]->text, 0, $chop) . "...";
			}
			else
			{
			...
			$options[] = JHtml::_('select.option', $elements[$i]->value, $elements[$i]->value . '. ' . $text);
		}
		$options[] = JHtml::_('select.option', $elements[$i - 1]->value + 1, ($elements[$i - 1]->value + 1) . ' ' . RText::_('ROPTION_ORDER_LAST'));
	 ...
	 * @param   integer  $id        The id for an existing element in the list
	 * @param   string   $query     The query
	 * @param   integer  $neworder  1 if new and first, -1 if new and last,
	 *                              0  or null if existing element
	 *
	 ...
	 * @param   string   $selected  The selected element
	 * @param   integer  $neworder  1 if new and first, -1 if new and last, 0  or null if existing element
	...
	public static function ordering($name, $query, $attribs = null, $selected = null, $neworder = null, $chop = null)
	{
		...
		else
		{
			if ($neworder > 0)
			{
				$text = RText::_('RGLOBAL_NEWelementSLAST_DESC');
			}
			elseif ($neworder <= 0)
			{
				$text = RText::_('RGLOBAL_NEWelementSFIRST_DESC');
			}
	...
	public static function users($name, $active, $nouser = 0, $javascript = null, $order = 'name', $reg = 1)
	{
		...
		if ($nouser)
		{
			$users[] = JHtml::_('select.option', '0', RText::_('ROPTION_NO_USER'));
	...
	public static function positions($name, $active = null, $javascript = null, $none = 1, $center = 1, $left = 1, $right = 1, $id = false)
	{
		...
		if ($center)
		{
			$pos['center'] = RText::_('RGLOBAL_CENTER');
		}

		if ($left)
		{
			$pos['left'] = RText::_('RGLOBAL_LEFT');
		}

		if ($right)
		{
			$pos['right'] = RText::_('RGLOBAL_RIGHT');
		}
	...
	public static function category($name, $extension, $selected = null, $javascript = null, $order = null, $size = 1, $sel_cat = 1)
	{
		...
		if ($sel_cat)
		{
			array_unshift($categories, JHtml::_('select.option', '0', RText::_('ROPTION_SELECT_CATEGORY')));
		}
