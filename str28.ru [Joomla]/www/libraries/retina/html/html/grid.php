...
abstract class JHtmlGrid
{
	...
	public static function boolean($i, $value, $taskOn = null, $taskOff = null)
	{
		...
		// Build the title.
		$title = ($value) ? RText::_('RYES') : RText::_('RNO');
		$title .= '::' . RText::_('RGLOBAL_CLICK_TO_TOGGLE_STATE');
	...
	public static function sort($title, $order, $direction = 'asc', $selected = 0, $task = null, $new_direction = 'asc')
	{
		...
		$html = '<a href="#" onclick="retina.tableOrdering(\'' . $order . '\',\'' . $direction . '\',\'' . $task . '\');" title="'
			. RText::_('RGLOBAL_CLICK_TO_SORT_THIS_COLUMN') . '">';
		$html .= RText::_($title);

		if ($order == $selected)
		{
			$html .= JHtml::_('image', 'main/' . $images[$index], '', null, true);
		}
	 ...
	 * @param   boolean  $checkedOut  True if element is checke out
	 * @param   string   $name        The name of the form element
	 *
	 * @return  mixed    String of html with a checkbox if element is not checked out, null if checked out.
	 */
	public static function id($rowNum, $recId, $checkedOut = false, $name = 'cid')
	{
		...
		else
		{
			return '<input type="checkbox" id="cb' . $rowNum . '" name="' . $name . '[]" value="' . $recId
				. '" onclick="retina.isChecked(this.checked);" title="' . RText::sprintf('JGRID_CHECKBOX_ROW_N', ($rowNum + 1)) . '" />';
		}
	}
	 ...
	 * @param   boolean  $archived  True if the element is archived
	 *
	 * @return  string
	 *
	 * @deprecated  12.1
	...
	public static function access(&$row, $i, $archived = null)
	{
		...
		if ($archived == -1)
		{
			$href = RText::_($row->groupname);
		}
		else
		{
			$href = '
			<a href="javascript:void(0);" onclick="return listelementTask(\'cb' . $i . '\',\'' . $task_access . '\')" ' . $color_access . '>
			' . RText::_($row->groupname) . '</a>';
		}
	...
	/**
	 * Method to create a clickable icon to change the state of an element
	 *
	...
	public static function published($value, $i, $img1 = 'tick.png', $img0 = 'publish_x.png', $prefix = '')
	{
		...
		$alt = $value ? RText::_('JPUBLISHED') : RText::_('JUNPUBLISHED');
		$action = $value ? RText::_('RLIB_HTML_UNPUBLISH_element') : RText::_('RLIB_HTML_PUBLISH_element');

		$href = '
		<a href="#" onclick="return listelementTask(\'cb' . $i . '\',\'' . $prefix . $task . '\')" title="' . $action . '">'
	 ...
	 * By default the filter shows only published and unpublished elements
	 *
	 * @param   string  $filter_state  The initial filter state
	 * @param   string  $published     The RText string for published
	 * @param   string  $unpublished   The RText string for Unpublished
	 * @param   string  $archived      The RText string for Archived
	 * @param   string  $trashed       The RText string for Trashed
	 *
	 * @return  string
	 *
	 * @since   11.1
	...
	{
		$state = array('' => '- ' . RText::_('RLIB_HTML_SELECT_STATE') . ' -', 'P' => RText::_($published), 'U' => RText::_($unpublished));

		if ($archived)
		{
			$state['A'] = RText::_($archived);
		}

		if ($trashed)
		{
			$state['T'] = RText::_($trashed);
		}
		...
		return JHtml::_(
			...
			'filter_state',
			array(
				'list.attr' => 'class="inputbox" size="1" onchange="retina.submitform();"',
	...
	{
		// $image = JHtml::_('image','admin/'.$image, RText::_('RLIB_HTML_SAVE_ORDER'), NULL, true);
		$href = '<a href="javascript:saveorder(' . (count($rows) - 1) . ', \'' . $task . '\')" class="saveorder" title="'
			. RText::_('RLIB_HTML_SAVE_ORDER') . '"></a>';
	...
	protected static function _checkedOut(&$row, $overlib = 1)
	{
		...
		if ($overlib)
		{
			...
			$date = JHtml::_('date', $row->checked_out_time, RText::_('DATE_FORMAT_LC1'));
			$time = JHtml::_('date', $row->checked_out_time, 'H:i');

			$hover = '<span class="editlinktip hasTip" title="' . RText::_('RLIB_HTML_CHECKED_OUT') . '::' . $text . '<br />' . $date . '<br />'
				. $time . '">';
		}
		...
		window.addEvent(\'domready\', function(){
			...
			actions.each(function(a){
				a.addEvent(\'click\', function(){
					...
					listelementTask(args.id, args.task);
				});
			});
