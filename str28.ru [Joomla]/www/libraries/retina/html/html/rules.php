...
abstract class JHtmlRules
{
	 ...
	 * @see     JFormFieldRules
	 */
	public static function assetFormWidget($actions, $assetId = null, $parent = null, $control = 'rinputform[rules]', $idPrefix = 'rinputform_rules')
	{
		...
		$html[] = JHtml::_('tabs.panel', RText::_('RLIB_HTML_ACCESS_SUMMARY'), 'summary');
		$html[] = '			<p>' . RText::_('RLIB_HTML_ACCESS_SUMMARY_DESC') . '</p>';
		$html[] = '			<table class="aclsummary-table" summary="' . RText::_('RLIB_HTML_ACCESS_SUMMARY_DESC') . '">';
		$html[] = '			<caption>' . RText::_('RLIB_HTML_ACCESS_SUMMARY_DESC_CAPTION') . '</caption>';
		$html[] = '			<tr>';
		$html[] = '				<th class="col1 hidelabeltxt">' . RText::_('RLIB_RULES_GROUPS') . '</th>';
		foreach ($actions as $i => $action)
		{
			$html[] = '				<th class="col' . ($i + 2) . '">' . RText::_($action->title) . '</th>';
		}
		$html[] = '			</tr>';
		...
		{
			$actionTitle = RText::_($action->title);
			$actionDesc = RText::_($action->description);
			$html[] = JHtml::_('tabs.panel', $actionTitle, $action->name);
			$html[] = '			<p>' . $actionDesc . '</p>';
			$html[] = '			<table class="aclmodify-table" summary="' . strip_tags($actionDesc) . '">';
			$html[] = '			<caption>' . RText::_('RLIB_HTML_ACCESS_MODIFY_DESC_CAPTION_ACL') . ' ' . $actionTitle . ' '
				. RText::_('RLIB_HTML_ACCESS_MODIFY_DESC_CAPTION_TABLE') . '</caption>';
			$html[] = '			<tr>';
			$html[] = '				<th class="col1 hidelabeltxt">' . RText::_('RLIB_RULES_GROUP') . '</th>';
			$html[] = '				<th class="col2">' . RText::_('RLIB_RULES_INHERIT') . '</th>';
			$html[] = '				<th class="col3 hidelabeltxt">' . RText::_('JMODIFY') . '</th>';
			$html[] = '				<th class="col4">' . RText::_('JCURRENT') . '</th>';
			$html[] = '			</tr>';
			...
			foreach ($groups as $i => $group)
			{
				...
				$html[] = '					<select id="' . $idPrefix . '_' . $action->name . '_' . $group->value
					...
					. RText::sprintf('RLIB_RULES_SELECT_ALLOW_DENY_GROUP', $actionTitle, $group->text) . '">';
				$html[] = '						<option value=""' . ($selected === null ? ' selected="selected"' : '') . '>'
					. RText::_('RLIB_RULES_INHERIT') . '</option>';
				$html[] = '						<option value="1"' . ($selected === true ? ' selected="selected"' : '') . '>'
					. RText::_('RLIB_RULES_ALLOWED') . '</option>';
				$html[] = '						<option value="0"' . ($selected === false ? ' selected="selected"' : '') . '>'
					. RText::_('RLIB_RULES_DENIED') . '</option>';
				$html[] = '					</select>';
				$html[] = '				</td>';
				$html[] = '				<td class="col4">'
					. ($assetId ? ($inherited->allow($action->name, $group->identities) ? $images['allow'] : $images['deny'])
		...
		$html[] = '	<div class="div-hidden-both"></div>';
		$html[] = '	<ul class="acllegend fltlft">';
		$html[] = '		<li class="acl-allowed">' . RText::_('RLIB_RULES_ALLOWED') . '</li>';
		$html[] = '		<li class="acl-denied">' . RText::_('RLIB_RULES_DENIED') . '</li>';
		$html[] = '	</ul>';
		$html[] = '</div>';
	...
	protected static function _getParentAssetId($assetId)
	{
		...
		$query->from($db->quoteName('#__retina_097115115101116115'));
	...
	{
		$images['allow-l'] = '<label class="icon-16-allow" title="' . RText::_('RLIB_RULES_ALLOWED') . '">' . RText::_('RLIB_RULES_ALLOWED')
			. '</label>';
		$images['deny-l'] = '<label class="icon-16-deny" title="' . RText::_('RLIB_RULES_DENIED') . '">' . RText::_('RLIB_RULES_DENIED') . '</label>';
		$images['allow'] = '<a class="icon-16-allow" title="' . RText::_('RLIB_RULES_ALLOWED') . '"> </a>';
		$images['deny'] = '<a class="icon-16-deny" title="' . RText::_('RLIB_RULES_DENIED') . '"> </a>';
		$images['allow-i'] = '<a class="icon-16-allowinactive" title="' . RText::_('JRULE_ALLOWED_INHERITED') . '"> </a>';
		$images['deny-i'] = '<a class="icon-16-denyinactive" title="' . RText::_('JRULE_DENIED_INHERITED') . '"> </a>';
