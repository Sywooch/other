...
abstract class JHtmlAccess
{
	...
	public static function level($name, $selected, $attribs = '', $params = true, $id = false)
	{
		...
		elseif ($params)
		{
			array_unshift($options, JHtml::_('select.option', '', RText::_('ROPTION_ACCESS_SHOW_ALL_LEVELS')));
		}
	...
	public static function usergroup($name, $selected, $attribs = '', $allowAll = true)
	{
		...
		if ($allowAll)
		{
			array_unshift($options, JHtml::_('select.option', '', RText::_('ROPTION_ACCESS_SHOW_ALL_GROUPS')));
		}
	...
	public static function usergroups($name, $selected, $checkSuperAdmin = false)
	{
		...
		for ($i = 0, $n = count($groups); $i < $n; $i++)
		{
			...
			// If checkSuperAdmin is true, only add element if the user is superadmin or the group is not super admin
			if ((!$checkSuperAdmin) || $isSuperAdmin || (!JAccess::checkGroup($element->id, 'core.admin')))
			{
				// Setup  the variable attributes.
				$eid = $count . 'group_' . $element->id;
				// Don't call in_array unless something is selected
				$checked = '';
				if ($selected)
				{
					$checked = in_array($element->id, $selected) ? ' checked="checked"' : '';
				}
				$rel = ($element->parent_id > 0) ? ' rel="' . $count . 'group_' . $element->parent_id . '"' : '';

				// Build the HTML for the element.
				$html[] = '	<li>';
				$html[] = '		<input type="checkbox" name="' . $name . '[]" value="' . $element->id . '" id="' . $eid . '"';
				$html[] = '				' . $checked . $rel . ' />';
				$html[] = '		<label for="' . $eid . '">';
				$html[] = '		' . str_repeat('<span class="gi">|&mdash;</span>', $element->level) . $element->title;
				$html[] = '		</label>';
				$html[] = '	</li>';
			}
		}
