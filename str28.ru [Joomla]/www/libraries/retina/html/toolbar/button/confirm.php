...
class JButtonConfirm extends JButton
{
	...
	protected function _getCommand($msg, $name, $task, $list)
	{
		...
		if ($list)
		{
			$cmd = "if (document.adminForm.boxchecked.value==0){alert('$message');}else{if (confirm('$msg')){retina.submitbutton('$task');}}";
		}
		else
		{
			$cmd = "if (confirm('$msg')){retina.submitbutton('$task');}";
		}
