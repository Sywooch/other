...
class JParameter extends JRegistry
{
	...
	public function setXML(&$xml)
	{
		...
		if (is_object($xml))
		{
			...
			{
				$this->addElementPath(RPATH_ROOT . str_replace('/', DS, $dir));
			}
		}
	}
	...
	public function render($name = 'params', $group = '_default')
	{
		...
		if (count($params) < 1)
		{
			$html[] = "<p class=\"noparams\">" . RText::_('RLIB_HTML_NO_PARAMETERS_FOR_THIS_element') . "</p>";
		}
	...
	public function getParam(&$node, $control_name = 'params', $group = '_default')
	{
		...
		if ($element === false)
		{
			...
			$result[1] = RText::_('Element not defined for type') . ' = ' . $type;
			$result[5] = $result[0];
			return $result;
		}
