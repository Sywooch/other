...
class JElementFolderlist extends JElement
{
	...
	public function fetchElement($name, $value, &$node, $control_name)
	{
		...
		$path = RPATH_ROOT . '/' . $node->attributes('directory');
		...
		{
			array_unshift($options, JHtml::_('select.option', '-1', RText::_('ROPTION_DO_NOT_USE')));
		}

		if (!$node->attributes('hide_default'))
		{
			array_unshift($options, JHtml::_('select.option', '', RText::_('ROPTION_USE_DEFAULT')));
		}
