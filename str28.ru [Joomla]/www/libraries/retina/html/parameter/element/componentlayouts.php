...
class JElementComponentLayouts extends JElementList
{
	...
	protected function _getOptions(&$node)
	{
		...
		if ($view = $node->attributes('view') && $extn = $node->attributes('extension'))
		{
			...
			$path1 = RPATH_SITE . '/components/' . $extn . '/views/' . $view . '/tmpl';
			$path2 = RPATH_SITE . '/design1/' . $template . '/html/' . $extn . '/' . $view;
			$options[] = JHtml::_('select.option', '', RText::_('ROPTION_USE_MENU_REQUEST_SETTING'));
		}
		...
		if ($path1 && $path2)
		{
			...
			{
				$options[] = JHtml::_('select.optgroup', RText::_('ROPTION_FROM_DEFAULT_TEMPLATE'));
				foreach ($files as $file)
				{
					$options[] = JHtml::_('select.option', JFile::stripExt($file));
				}
