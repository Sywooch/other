...
class JArchiveZip extends JObject
{
	...
	private function _extract($archive, $destination, $options)
	{
		...
		{
			$this->set('error.message', RText::_('RLIB_FILEmain_ZIP_NOT_SUPPORTED'));
		...
		{
			$this->set('error.message', RText::_('RLIB_FILEmain_ZIP_UNABLE_TO_READ'));
		...
		{
			$this->set('error.message', RText::_('RLIB_FILEmain_ZIP_INFO_FAILED'));
		...
		for ($i = 0, $n = count($this->_metadata); $i < $n; $i++)
		{
			...
			if ($lastPathCharacter !== '/' && $lastPathCharacter !== '\\')
			{
				...
				{
					$this->set('error.message', RText::_('RLIB_FILEmain_ZIP_UNABLE_TO_CREATE_DESTINATION'));
				...
				{
					$this->set('error.message', RText::_('RLIB_FILEmain_ZIP_UNABLE_TO_WRITE_ENTRY'));
	...
	private function _extractNative($archive, $destination, $options)
	{
		if ($zip = zip_open($archive))
		{
			if (is_resource($zip))
			{
				...
				while ($file = @zip_read($zip))
				{
					...
					{
						$this->set('error.message', RText::_('RLIB_FILEmain_ZIP_UNABLE_TO_READ_ENTRY'));
		...
		{
			$this->set('error.message', RText::_('RLIB_FILEmain_ZIP_UNABLE_TO_OPEN_ARCHIVE'));
	...
	private function _readZipInfo(&$data)
	{
		...
		do
		{
			...
			{
				$this->set('error.message', RText::_('RLIB_FILEmain_ZIP_INVALID_ZIP_DATA'));

				return false;
			}

			$info = unpack('rsethod/VTime/VCRC32/VCompressed/VUncompressed/vLength', substr($data, $fhStart + 10, 20));
			...
			$info = unpack('rsethod/VTime/VCRC32/VCompressed/VUncompressed/vLength/vExtraLength', substr($data, $lfhStart + 8, 25));
