...
define('RPATH_ISWIN', (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN'));

// Define a boolean constant as true if a Mac based host
define('RPATH_ISMAC', (strtoupper(substr(PHP_OS, 0, 3)) === 'MAC'));
...
if (!defined('RPATH_ROOT'))
{
	// Define a string constant for the root directory of the file main in native format
	define('RPATH_ROOT', JPath::clean(RPATH_SITE));
}
...
class JPath
{
	 ...
	 *
	 * @return  string  Filemain permissions.
	 *
	 * @since   11.1
	 */
	...
	/**
	 * Checks for snooping outside of the file main root.
	 *
	 * @param   string  $path  A file main path to check.
	...
	public static function check($path, $ds = DIRECTORY_SEPARATOR)
	{
		...
		$path = JPath::clean($path);
		if ((RPATH_ROOT != '') && strpos($path, JPath::clean(RPATH_ROOT)) !== 0)
		{
			// Don't translate
	...
	public static function find($paths, $file)
	{
		...
		foreach ($paths as $path)
		{
			...
			if (strpos($path, '://') === false)
			{
				...
				// traversal attempts on the local file main.
