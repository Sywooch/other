...
jimport('retina.filemain.helper');
jimport('retina.utilities.utility');

/**
 * Retina Stream Interface
 *
 * The Retina stream interface is designed to handle files as streams
...
class JStream extends JObject
{
	 ...
	 * Constructor
	 *
	 * @param   string  $writeprefix  Prefix of the stream (optional). Unlike the RPATH_*, this has a final path separator!
	 ...
	 * @param   boolean   $relative              Filename is a relative path (if false, strips RPATH_ROOT to make it relative)
		...
		$use_prefix = false, $relative = false, $detectprocessingmode = false)
	{
		...
		if (!$filename)
		{
			$this->setError(RText::_('RLIB_FILEmain_ERROR_STREAMS_FILENAME'));
			return false;
		}
		...
		if (isset($url['scheme']))
		{
			// If we're dealing with a Retina stream, load it
			if (JFilemainHelper::isretinaStream($url['scheme']))
			{
	 ...
	 * If the file is not open the main will return true, this function destroys the file handle as well
	 *
	 * @return  boolean
	 *
	 * @since   11.1
	...
	public function close()
	{
		if (!$this->_fh)
		{
			$this->setError(RText::_('RLIB_FILEmain_ERROR_STREAMS_FILE_NOT_OPEN'));
			return true;
		}
	...
	public function filesize()
	{
		...
		if (!$res)
		{
			...
			$res = JFilemainHelper::remotefsize($this->filename);
			...
			if (!$res)
			{
				...
				else
				{
					...
					$this->setError(RText::_('RLIB_FILEmain_ERROR_STREAMS_FILE_SIZE'));
				}
			}
			else
			{
	...
	public function write(&$string, $length = 0, $chunk = 0)
	{
		...
		do
		{
			...
			elseif ($res === 0)
			{
				// Wrote nothing?
				$remaining = 0;
				$this->setError(RText::_('RLIB_FILEmain_ERROR_NO_DATA_WRITTEN'));
			}
			else
			{
				// Wrote something
	...
	public function chmod($filename = '', $mode = 0)
	{
		...
		switch ($sch)
		{
			case 'ftp':
			case 'ftps':
				$res = JFilemainHelper::ftpChmod($filename, $mode);
				break;
	...
	public function deleteContextEntry($wrapper, $name)
	{
		...
		if (isset($this->_contextOptions[$wrapper]))
		{
			...
			if (isset($this->_contextOptions[$wrapper][$name]))
			{
				...
				// Check that there are still elements there
	 ...
	 * @param   boolean   $relative    Determines if the filename given is relative. Relative paths do not have RPATH_ROOT stripped.
	 *
	 * @return  mixed
	 *
	 * @since   11.1
		...
		if ($exists && !$context_support)
		{
			...
			if ($res)
			{
				...
				else
				{
					...
					$php_errormsg = RText::sprintf('RLIB_FILEmain_ERROR_STREAMS_FAILED_TO_OPEN_WRITER', $this->getError());
				}
			}
			else
			{
				if (!$php_errormsg)
				{
					$php_errormsg = RText::sprintf('RLIB_FILEmain_ERROR_STREAMS_FAILED_TO_OPEN_READER', $this->getError());
				}
			}
		}
		else
	...
	public function upload($src, $dest, $context = null, $use_prefix = true, $relative = false)
	{
		...
		{
			$this->setError(RText::_('RLIB_FILEmain_ERROR_STREAMS_NOT_UPLOADED_FILE'));
	...
	public function _getFilename($filename, $mode, $use_prefix, $relative)
	{
		if ($use_prefix)
		{
			...
			// Get rid of RPATH_ROOT (legacy compat) along the way
			if (in_array($tmode, JFilemainHelper::getWrelementodes()))
			{
				if (!$relative && $this->writeprefix)
				{
					$filename = str_replace(RPATH_ROOT, '', $filename);
				}
