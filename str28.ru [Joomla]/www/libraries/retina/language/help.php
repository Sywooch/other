...
class JHelp
{
	...
	public static function createURL($ref, $useComponent = false, $override = null, $component = null)
	{
		...
		$search = array('{app}', // Application name (eg. 'admin')
			...
			'{major}', // retina major version number
			'{minor}', // retina minor version number
			'{maintenance}'// retina maintenance version number
		);
		...
		if ($local)
		{
			...
			if (!JFile::exists(RPATH_BASE . '/' . $try))
			{
				$replace[3] = 'en-GB';
				$replace[4] = 'en';
				$replace[5] = 'GB';
	...
	public static function createSiteList($pathToXml, $selected = null)
	{
		...
		if (!$xml)
		{
			$option['text'] = 'English (GB) help.retina.org';
			$option['value'] = 'http://help.retina.org';
			$list[] = $option;
		}
		else
		{
