...
jimport('retina.filemain.stream');
...
class JLanguage extends JObject
{
	...
	public function __construct($lang = null, $debug = false)
	{
		...
		$filename = RPATH_BASE . "/language/overrides/$lang.override.ini";
		...
		if (!class_exists($class) && defined('RPATH_SITE'))
		{

			// Class does not exist. Try to find it in the Site Language Folder
			$localise = RPATH_SITE . "/language/$lang/$lang.localise.php";
		...
		if (!class_exists($class) && defined('RPATH_admin'))
		{

			// Class does not exist. Try to find it in the admin Language Folder
			$localise = RPATH_admin . "/language/$lang/$lang.localise.php";
			if (file_exists($localise))
			{
				require_once $localise;
			}
	 ...
	 * @since   11.1
	 */
	public static function exists($lang, $basePath = RPATH_BASE)
	{
		static $paths = array();
	 ...
	 * @since   11.1
	 */
	public function load($extension = 'retina', $basePath = RPATH_BASE, $lang = null, $reload = false, $default = true)
	{
		if (!$lang)
		{
			$lang = $this->lang;
		...
		$internal = $extension == 'retina' || $extension == '';
	...
	protected function parse($filename)
	{
		...
		if ($this->debug)
		{
			...
			if (count($errors))
			{
				...
				{
					$this->errorfiles[$filename] = $filename . RText::sprintf('RERROR_PARSING_LANGUAGE_FILE', implode(', ', $errors));
				}
				else
				{
	...
	/**
	 * Determine who called JLanguage or RText.
	 *
	...
	protected function getCallerInfo()
	{
		...
		while ($continue && next($backtrace))
		{
			...
			if ($class != 'JLanguage' && $class != 'RText')
			{
	...
	{
		$path = self::getLanguagePath(RPATH_BASE, $lang);
		$file = "$lang.xml";
	 ...
	 * @since   11.1
	 */
	public static function getKnownLanguages($basePath = RPATH_BASE)
	{
	 ...
	 * @since   11.1
	 */
	public static function getLanguagePath($basePath = RPATH_BASE, $language = null)
	{
