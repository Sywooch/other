...
class JAccess
{
	 ...
	 * parent retina_097115115101116115 and explicit rules.
	 *
	...
	public static function getAssetRules($asset, $recursive = false)
	{
		...
		$query->from('#__retina_097115115101116115 AS a');
		//sqlsrv change
		...
		if ($recursive)
		{
			$query->leftJoin('#__retina_097115115101116115 AS b ON b.lft <= a.lft AND b.rgt >= a.rgt');
			$query->order('b.lft');
		}
		...
		if ($recursive && empty($result))
		{
			$query = $db->getQuery(true);
			$query->select('rules');
			$query->from('#__retina_097115115101116115');
	...
	public static function getActions($component, $section = 'component')
	{
		...
		if (defined('RPATH_admin') && is_file(RPATH_admin . '/components/' . $component . '/access.xml'))
		{
			$xml = simplexml_load_file(RPATH_admin . '/components/' . $component . '/access.xml');
