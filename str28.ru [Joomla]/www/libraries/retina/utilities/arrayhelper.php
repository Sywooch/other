...
class JArrayHelper
{
		...
		if (is_array($array))
		{
			foreach ($array as $key => $element)
			{
				if (is_array($element))
				{
					...
					$output[] = JArrayHelper::toString($element, $inner_glue, $outer_glue, $keepOuterKey);
				}
				else
				{
					$output[] = $key . $inner_glue . '"' . $element . '"';
				}
			}
		}
	 ...
	 *
	 * @param   mixed    $element     The source object or array
	 * @param   boolean  $recurse  True to recurse through multi-level objects
	 * @param   string   $regex    An optional regular expression to match on field names
	 *
	 * @return  array  The array mapped from the given object
	 ...
	 * @since   11.1
	 */
	protected static function _fromObject($element, $recurse, $regex)
	{
	...
	public static function getColumn(&$array, $index)
	{
		...
		if (is_array($array))
		{
			...
			for ($i = 0; $i < $n; $i++)
			{
				...
				if (is_array($element) && isset($element[$index]))
				{
					$result[] = $element[$index];
				}
				elseif (is_object($element) && isset($element->$index))
				{
					$result[] = $element->$index;
				}
