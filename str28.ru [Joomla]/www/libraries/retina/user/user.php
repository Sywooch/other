...
class JUser extends JObject
{
	...
	/**
	 * User type
	 * Used in retina 1.0 and 1.5 for access control.
	 *
	 * @var    string
	 * @deprecated    12.1
	 * @see    $_authGroups
	...
	/**
	 * Should this user receive main email
	 *
	 * @var    integer
	 * @since  11.1
	 */
	...
	public static function getInstance($identifier = 0)
	{
		...
		if (!is_numeric($identifier))
		{
			...
			{
				JError::raiseWarning('SOME_ERROR_CODE', RText::sprintf('RLIB_USER_ERROR_ID_NOT_EXISTS', $identifier));
				$retval = false;
				return $retval;
			}
		}
	...
	public function getAuthorisedCategories($component, $action)
	{
		...
		$query = $db->getQuery(true)->select('c.id AS id, a.name AS asset_name')->from('#__categories AS c')
			->innerJoin('#__retina_097115115101116115 AS a ON c.asset_id = a.id')->where('c.extension = ' . $db->quote($component))->where('c.published = 1');
		$db->setQuery($query);
	...
	public function getParameters($loadsetupfile = false, $path = null)
	{
		...
		if (!isset($parampath))
		{
			$parampath = RPATH_admin . 'components/com_users/models';
		}
	...
	/**
	 * Метод работает с ассоциативным массивом данных для объекта пользователя.  
	 *
	 * @param   array  &$array  Ассоциативный массив для привязки к объекту. 
	 *
	 * @return  boolean  True в случае успешного завершения.
	 *
	 * @since   11.1
	 */
	public function bind(&$array)
	{
		...
		if (empty($this->id))
		{
			// Проверить пароль и создать зашифрованный пароль
			if (empty($array['password']))
			{
				$array['password'] = JUserHelper::genRandomPassword();
				$array['password2'] = $array['password'];//случайный пароль.
			}
			...
			{
				$this->setError(RText::_('RLIB_USER_ERROR_PASSWORD_NOT_MATCH'));
				return false;
			}
			...
			// Проверка, что имя пользователя не больше 150 символов.
			...
			// Проверка, что пароль не больше 100 символов
		...
		{
			$this->setError(RText::_('RLIB_USER_ERROR_BIND_ARRAY'));
			return false;
		}

		// Проверка, что это целое
		$this->id = (int) $this->id;
			...
			if ($iAmSuperAdmin != true)
			{
				if ($isNew)
				{
					...
					foreach ($this->groups as $groupId)
					{
						...
						{
							throw new Exception(RText::_('RLIB_USER_ERROR_NOT_SUPERADMIN'));
						}
					}
				}
				else
	...
	public function load($id)
	{
		...
		if (!$table->load($id))
		{
			JError::raiseWarning('SOME_ERROR_CODE', RText::sprintf('RLIB_USER_ERROR_UNABLE_TO_LOAD_USER', $id));
			return false;
		}
