...
define('RERROR_ILLEGAL_OPTIONS', 1);
// Error Definition: Callback does not exist
define('RERROR_CALLBACK_NOT_CALLABLE', 2);
// Error Definition: Illegal Handler
define('RERROR_ILLEGAL_MODE', 3);
...
abstract class JError
{
	...
	public static function raise($level, $code, $msg, $info = null, $backtrace = false)
	{
		...
		jimport('retina.error.exception');
	...
	public static function setErrorHandling($level, $mode, $options = null)
	{
		...
		if (!is_callable(array('JError', $function)))
		{
			return JError::raiseError(E_ERROR, 'JError:' . RERROR_ILLEGAL_MODE, 'Error Handling mode is not known', 'Mode: ' . $mode . ' is not implemented.');
		}
		...
		foreach ($levels as $eLevel => $eTitle)
		{
			...
			if ($mode == 'callback')
			{
				if (!is_array($options))
				{
					return JError::raiseError(E_ERROR, 'JError:' . RERROR_ILLEGAL_OPTIONS, 'Options for callback not valid');
				}
				...
				if (!is_callable($options))
				{
					...
					return JError::raiseError(
						E_ERROR,
						'JError:' . RERROR_CALLBACK_NOT_CALLABLE,
	...
	public static function handleEcho(&$error, $options)
	{
		...
		// If main debug is set, then output some more information.
		if (defined('JDEBUG'))
		{
	 ...
	 * Enqueues the error message into the main queue
	 *
	 * @param   object  &$error   Exception object to handle
	 * @param   array   $options  Handler options
	 *
	...
	/**
	 * Log error handler
	 * Logs the error message to a main log file
	 *
	 * @param   object  &$error   Exception object to handle
	 * @param   array   $options  Handler options
	 *
	...
	public static function customErrorPage(&$error)
	{
		...
		if ($document)
		{
			...
			@ob_end_clean();
			$document->setTitle(RText::_('Error') . ': ' . $error->get('code'));
			$data = $document->render(false, array('template' => $template, 'directory' => RPATH_THEMES, 'debug' => $config->get('debug')));
