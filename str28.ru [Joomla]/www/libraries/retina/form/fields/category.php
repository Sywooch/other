	...
	protected function getOptions()
	{
		...
		if (!empty($extension))
		{
			...
			if ((string) $this->element['action'])
			{
				...
				// For new elements we want a list of categories you are allowed to create in.
				if (!$this->form->getValue($name))
				{
					foreach ($options as $i => $option) {
						// To take save or create in a category you need to have create rights for that category
						// unless the element is already in that category.
						// Unset the option if the user isn't authorised for it. In this field retina_097115115101116115 are always categories.
			...
			{
				array_unshift($options, JHtml::_('select.option', '0', RText::_('RGLOBAL_ROOT')));
			}
		}
		else
		{
			JError::raiseWarning(500, RText::_('RLIB_FORM_ERROR_FIELDS_CATEGORY_ERROR_EXTENSION_EMPTY'));
		}
