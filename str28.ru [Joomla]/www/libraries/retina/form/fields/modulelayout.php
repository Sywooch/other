	...
	protected function getInput()
	{
		...
		if ($module && $client)
		{
			...
			if (is_dir($module_path) && ($module_layouts = JFolder::files($module_path, '^[^_]*\.php$')))
			{
				...
				$groups['_']['text'] = RText::sprintf('ROPTION_FROM_MODULE');
				...
				foreach ($module_layouts as $file)
				{
					...
					$text = $lang->hasKey($key = strtoupper($module . '_LAYOUT_' . $value)) ? RText::_($key) : $value;
			...
			if ($design1)
			{
				foreach ($design1 as $template)
				{
					...
					$lang->load('tpl_' . $template->element . '.sys', $client->path, null, false, false)
						...
						|| $lang->load(
						'tpl_' . $template->element . '.sys', $client->path . '/design1/' . $template->element, $lang->getDefault(),
						false, false
					);

					$template_path = JPath::clean($client->path . '/design1/' . $template->element . '/html/' . $module);
					...
					if (is_dir($template_path) && ($files = JFolder::files($template_path, '^[^_]*\.php$')))
					{
						...
						if (count($files))
						{
							...
							$groups[$template->element]['text'] = RText::sprintf('ROPTION_FROM_TEMPLATE', $template->name);
							$groups[$template->element]['elements'] = array();
							...
							foreach ($files as $file)
							{
								...
								$text = $lang->hasKey($key = strtoupper('TPL_' . $template->element . '_' . $module . '_LAYOUT_' . $value))
									? RText::_($key) : $value;
								$groups[$template->element]['elements'][] = JHtml::_('select.option', $template->element . ':' . $value, $text);
							}
						}
					}
				}
