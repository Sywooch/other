...
class JFormFieldRules extends JFormField
{
	...
	protected function getInput()
	{
		...
		$html[] = '<p class="rule-desc">' . RText::_('RLIB_RULES_SETTINGS_DESC') . '</p>';
		$html[] = '<ul id="rules">';
		...
		foreach ($groups as $group)
		{
			...
			$html[] = '<span class="acl-action">' . RText::_('RLIB_RULES_ACTION') . '</span>';
			$html[] = '</th>';

			$html[] = '<th class="settings" id="settings-th' . $group->value . '">';
			$html[] = '<span class="acl-action">' . RText::_('RLIB_RULES_SELECT_SETTING') . '</span>';
			$html[] = '</th>';
			...
			if ($canCalculateSettings)
			{
				...
				$html[] = '<span class="acl-action">' . RText::_('RLIB_RULES_CALCULATED_SETTING') . '</span>';
				$html[] = '</th>';
			}
			...
			foreach ($actions as $action)
			{
				...
				$html[] = '<label class="hasTip" for="' . $this->id . '_' . $action->name . '_' . $group->value . '" title="'
					. htmlspecialchars(RText::_($action->title) . '::' . RText::_($action->description), ENT_COMPAT, 'UTF-8') . '">';
				$html[] = RText::_($action->title);
				$html[] = '</label>';
				$html[] = '</td>';
				...
				$html[] = '<select name="' . $this->name . '[' . $action->name . '][' . $group->value . ']" id="' . $this->id . '_' . $action->name
					. '_' . $group->value . '" title="'
					. RText::sprintf('RLIB_RULES_SELECT_ALLOW_DENY_GROUP', RText::_($action->title), trim($group->text)) . '">';
				...
				$html[] = '<option value=""' . ($assetRule === null ? ' selected="selected"' : '') . '>'
					. RText::_(empty($group->parent_id) && empty($component) ? 'RLIB_RULES_NOT_SET' : 'RLIB_RULES_INHERITED') . '</option>';
				$html[] = '<option value="1"' . ($assetRule === true ? ' selected="selected"' : '') . '>' . RText::_('RLIB_RULES_ALLOWED')
					. '</option>';
				$html[] = '<option value="0"' . ($assetRule === false ? ' selected="selected"' : '') . '>' . RText::_('RLIB_RULES_DENIED')
					. '</option>';
				...
				{
					$html[] = RText::_('RLIB_RULES_CONFLICT');
				}
				...
				if ($canCalculateSettings)
				{
					...
					if (JAccess::checkGroup($group->value, 'core.admin') !== true)
					{
						...
						{
							$html[] = '<span class="icon-16-unset">' . RText::_('RLIB_RULES_NOT_ALLOWED') . '</span>';
						}
						elseif ($inheritedRule === true)
						{
							$html[] = '<span class="icon-16-allowed">' . RText::_('RLIB_RULES_ALLOWED') . '</span>';
						}
						elseif ($inheritedRule === false)
						{
							if ($assetRule === false)
							{
								$html[] = '<span class="icon-16-denied">' . RText::_('RLIB_RULES_NOT_ALLOWED') . '</span>';
							}
							else
							{
								$html[] = '<span class="icon-16-denied"><span class="icon-16-locked">' . RText::_('RLIB_RULES_NOT_ALLOWED_LOCKED')
									. '</span></span>';
							}
						}
					}
					...
					{
						$html[] = '<span class="icon-16-allowed"><span class="icon-16-locked">' . RText::_('RLIB_RULES_ALLOWED_ADMIN')
							. '</span></span>';
					}
					else
					{
						...
						elseif ($inheritedRule === false)
						{
							...
							$html[] = '<span class="icon-16-denied"><span class="icon-16-locked">'
								. RText::_('RLIB_RULES_NOT_ALLOWED_ADMIN_CONFLICT') . '</span></span>';
						}
						else
						{
							$html[] = '<span class="icon-16-allowed"><span class="icon-16-locked">' . RText::_('RLIB_RULES_ALLOWED_ADMIN')
		...
		{
			$html[] = RText::_('RLIB_RULES_SETTING_NOTES');
		}
		else
		{
			$html[] = RText::_('RLIB_RULES_SETTING_NOTES_element');
		}
		$html[] = '</div></div>';
