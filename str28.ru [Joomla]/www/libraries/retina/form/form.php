...
/**
 * Form Class for the retina Platform.
 *
...
class JForm
{
	 ...
	 * the form control is set to 'retina' however, the fields would be rendered like:
	 * <input name="retina[foo]" /> and <input name="retina[bar]" />.
	 *
	 ...
	 * to false.
	 *
	 * @param   string  $file   The filemain path of an XML file.
	...
	protected function validateField($element, $group = null, $value = null, $input = null)
	{
		...
		if (!$element instanceof JXMLElement)
		{
			return new JException(RText::_('RLIB_FORM_ERROR_VALIDATE_FIELD'), -1, E_ERROR);
		}
		...
		if ($required)
		{
			...
			if (($value === '') || ($value === null))
			{
				...
				else
				{
					if ($element['label'])
					{
						$message = RText::_($element['label']);
					}
					else
					{
						$message = RText::_($element['name']);
					}
					$message = RText::sprintf('RLIB_FORM_VALIDATE_FIELD_REQUIRED', $message);
				}
		...
		if ($type = (string) $element['validate'])
		{
			...
			if ($rule === false)
			{
				return new JException(RText::sprintf('RLIB_FORM_VALIDATE_FIELD_RULE_MISSING', $type), -2, E_ERROR);
			}
		...
		if ($valid === false)
		{
			...
			if ($message)
			{
				return new JException(RText::_($message), 1, E_WARNING);
			}
			else
			{
				return new JException(RText::sprintf('RLIB_FORM_VALIDATE_FIELD_INVALID', RText::_((string) $element['label'])), 1, E_WARNING);
			}
		}
	...
	public static function getInstance($name, $data = null, $options = array(), $replace = true, $xpath = false)
	{
		...
		if (!isset($forms[$name]))
		{
			...
			if (empty($data))
			{
				throw new Exception(RText::_('RLIB_FORM_ERROR_NO_DATA'));
			}
			...
			if (substr(trim($data), 0, 1) == '<')
			{
				...
				{
					throw new Exception(RText::_('RLIB_FORM_ERROR_XML_FILE_DID_NOT_LOAD'));
