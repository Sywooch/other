...
class JUpdaterExtension extends JUpdateAdapter
{
	...
	public function findUpdate($options)
	{
		...
		if (!($fp = @fopen($url, "r")))
		{
			...
			$app->enqueueMessage(RText::sprintf('RLIB_UPDATER_ERROR_EXTENSION_OPEN_URL', $url), 'warning');
			return false;
		}
		...
		while ($data = fread($fp, 8192))
		{
			if (!xml_parse($this->xml_parser, $data, feof($fp)))
			{
				...
				$app = JFactory::getApplication();
				$app->enqueueMessage(RText::sprintf('RLIB_UPDATER_ERROR_EXTENSION_PARSE_URL', $url), 'warning');
				return false;
			}
		}
