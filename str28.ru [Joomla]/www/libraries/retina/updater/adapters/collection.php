...
jimport('retina.updater.updateadapter');
...
class JUpdaterCollection extends JUpdateAdapter
{
	...
	/**
	 * Used to control if an element has a child or not
	 *
	 * @var    boolean
	 * @since  11.1
	 */
	...
	public function _startElement($parser, $name, $attrs = array())
	{
		...
		switch ($name)
		{
			case 'CATEGORY':
				...
				{
					// This element will have children, so prepare to attach them
					$this->pop_parent = 1;
				}
				break;
			case 'EXTENSION':
	...
	public function findUpdate($options)
	{
		...
		if (!($fp = @fopen($url, "r")))
		{
			...
			$app->enqueueMessage(RText::sprintf('RLIB_UPDATER_ERROR_COLLECTION_OPEN_URL', $url), 'warning');
			return false;
		}
		...
		while ($data = fread($fp, 8192))
		{
			if (!xml_parse($this->xml_parser, $data, feof($fp)))
			{
				JLog::add("Error parsing url: " . $url, JLog::WARNING, 'updater');
				$app = JFactory::getApplication();
				$app->enqueueMessage(RText::sprintf('RLIB_UPDATER_ERROR_COLLECTION_PARSE_URL', $url), 'warning');
				return false;
			}
		}
