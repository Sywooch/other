...
class JUpdate extends JObject
{
	...
	public function _endElement($parser, $name)
	{
		...
		switch ($name)
		{
			...
			case 'UPDATE':
				...
				break;
			case 'UPDATES':
				// If the latest element is set then we transfer it to where we want to
				if (isset($this->_latest))
				{
	...
	public function _characterData($parser, $data)
	{
		...
		//$this->$tag->_data .= $data;
		// Throw the data for this element together
		$tag = strtolower($tag);
	...
	public function loadFromXML($url)
	{
		if (!($fp = @fopen($url, 'r')))
		{
			...
			JError::raiseWarning('101', RText::sprintf('RLIB_UPDATER_ERROR_EXTENSION_OPEN_URL', $url));
			return false;
		}
