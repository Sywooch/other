...
class JCategories
{
	...
	/**
	 * Name of the elements state field
	 *
	 * @var    string
	 * @since  11.1
	 */
	...
	public static function getInstance($extension, $options = array())
	{
		...
		{
			$path = RPATH_SITE . '/components/' . $component . '/helpers/category.php';
			if (is_file($path))
			{
				include_once $path;
			}
	...
	protected function _load($id)
	{
		...
		$query->where('(c.extension=' . $db->Quote($extension) . ' OR c.extension=' . $db->Quote('main') . ')');
		...
		// i for element
		if (isset($this->_options['countelements']) && $this->_options['countelements'] == 1)
		{
			if ($this->_options['published'] == 1)
			{
				$query->leftJoin(
			...
			$query->select('COUNT(i.' . $db->quoteName($this->_key) . ') AS numelements');
		}
...
class JCategoryNode extends JObject
{
	...
	/**
	 * Number of elements in this category or descendants of this category
	 *
	 * @var    integer
	 * @since  11.1
	 */
	public $numelements = null;
	 ...
	 * @since  11.1
	 */
	public $childrennumelements = null;
	...
	/**
	 * Array of  retina_097115115101116115
	 *
	 * @var    array
	 * @since  11.1
	 */
	public $retina_097115115101116115 = null;
	 ...
	 * @since 11.1
	 */
	public function getNumelements($recursive = false)
	{
		if ($recursive)
		{
			...
			{
				$count = $count + $child->getNumelements(true);
			}
