...
abstract class JModuleHelper
{
	...
	public static function renderModule($module, $attribs = array())
	{
		...
		$path = RPATH_BASE . '/modules/' . $module->module . '/' . $module->module . '.php';
		...
		if (empty($module->user) && file_exists($path))
		{
			...
			$lang->load($module->module, RPATH_BASE, null, false, false) ||
				$lang->load($module->module, dirname($path), null, false, false) ||
				$lang->load($module->module, RPATH_BASE, $lang->getDefault(), false, false) ||
		...
		include_once RPATH_THEMES . '/main/html/modules.php';
		$chromePath = RPATH_THEMES . '/' . $app->getTemplate() . '/html/modules.php';
	...
	public static function getLayoutPath($module, $layout = 'default')
	{
		...
		$tPath = RPATH_THEMES . '/' . $template . '/html/' . $module . '/' . $layout . '.php';
		$bPath = RPATH_BASE . '/modules/' . $module . '/tmpl/' . $defaultLayout . '.php';
	...
	protected static function &_load()
	{
		...
		$cacheid = md5(serialize(array($elementid, $groups, $clientId, $lang)));
		...
		if (!($clean = $cache->get($cacheid)))
		{
			...
			$query->where('(mm.menuid = ' . (int) $elementid . ' OR mm.menuid <= 0)');
			...
			if ($db->getErrorNum())
			{
				JError::raiseWarning(500, RText::sprintf('RLIB_APPLICATION_ERROR_MODULE_LOAD', $db->getErrorMsg()));
				return $clean;
			}

			// Apply negative selections and eliminate duplicates
			$negId = $elementid ? -(int) $elementid : false;
			$dupes = array();
			for ($i = 0, $n = count($modules); $i < $n; $i++)
			{
				...
				// the elementid is missing or zero and the module is in exclude mode.
				$negHit = ($negId === (int) $module->menuid) || (!$negId && (int) $module->menuid < 0);

				if (isset($dupes[$module->id]))
				{
					// If this element has been excluded, keep the duplicate flag set,
					// but remove any element from the cleaned array.
					if ($negHit)
					{
	 ...
	 * 'elementid'      Changes on elementid change, to be called from inside the module:
		...
		switch ($cacheparams->cachemode)
		{
			...
			default:
				$ret = $cache->get(
					array($cacheparams->class, $cacheparams->method),
					$cacheparams->methodparams,
					$module->id . $view_levels . JRequest::getVar('elementid', null, 'default', 'INT'),
					$wrkarounds,
					$wrkaroundoptions
				);
				break;
