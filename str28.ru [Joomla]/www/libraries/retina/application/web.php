...
jimport('retina.application.web.webclient');
jimport('retina.environment.uri');
jimport('retina.event.dispatcher');

/**
 * Base class for a Retina Web application.
 *
 * @package     retina.Platform
 * @subpackage  Application
 * @since       11.4
...
class JApplicationWeb
{
	...
	protected function render()
	{
		...
		{
			$options['directory'] = (defined('RPATH_BASE') ? RPATH_BASE : dirname(__FILE__)) . '/themes';
		}
	...
	protected function compress()
	{
		...
		foreach ($encodings as $encoding)
		{
			if (($supported[$encoding] == 'gz') || ($supported[$encoding] == 'deflate'))
			{
				...
				$this->setHeader('X-Content-Encoded-By', 'retina');
	...
	protected function fetchConfigurationData($file = '', $class = 'JConfig')
	{
		...
		if (empty($file) && defined('RPATH_BASE'))
		{
			$file = RPATH_BASE . '/configuration.php';
	...
	/**
	 * Method to load the main URI strings for the application.
	 *
	 ...
	 * @since   11.3
	 */
	protected function loadmainUris($requestUri = null)
	{
		// Set the request URI.
		// @codeCoverageIgnoreStart
		if (!empty($requestUri))
		...
		// No explicit site URI was set so use the main one.
		else
		{
			$uri = JUri::getInstance($this->get('uri.request'));
		}
