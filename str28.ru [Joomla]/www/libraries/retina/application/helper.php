...
class JApplicationHelper
{
	...
	public static function getClientInfo($id = null, $byName = false)
	{
		...
		if (self::$_clients === null)
		{
			...
			$obj->id = 1;
			$obj->name = 'admin';
			$obj->path = RPATH_admin;
			self::$_clients[1] = clone $obj;

			// Installation Client
			$obj->id = 2;
			$obj->name = 'installation';
			$obj->path = RPATH_INSTALLATION;
	...
	public static function getPath($varname, $user_option = null)
	{
		...
		switch ($varname)
		{
			...
			case 'html':
			case 'front_html':
				if (!($result = self::_checkPath('/design1/' . JApplication::getTemplate() . '/components/' . $name . '.html.php', 0)))
				{
	...
	public static function parseXMLInstallFile($path)
	{
		...
		$data['creationDate'] = ((string) $xml->creationDate) ? (string) $xml->creationDate : RText::_('Unknown');
		$data['author'] = ((string) $xml->author) ? (string) $xml->author : RText::_('Unknown');
	...
	public static function parseXMLLangMetaFile($path)
	{
		...
		$data['creationDate'] = ((string) $xml->creationDate) ? (string) $xml->creationDate : RText::_('RLIB_UNKNOWN');
		$data['author'] = ((string) $xml->author) ? (string) $xml->author : RText::_('RLIB_UNKNOWN');
	...
	/**
	 * Tries to find a file in the admin or site areas
	 *
