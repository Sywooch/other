...
abstract class JModelAdmin extends JModelForm
{
	...
	/**
	 * Method to perform batch operations on an element or a set of elements.
	 *
	 * @param   array  $commands  An array of commands to perform.
	 * @param   array  $pks       An array of element ids.
	 * @param   array  $contexts  An array of element contexts.
	 *
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		if (empty($pks))
		{
			$this->setError(RText::_('RGLOBAL_NO_element_SELECTED'));
			return false;
		}
		...
		if (!$done)
		{
			$this->setError(RText::_('RLIB_APPLICATION_ERROR_INSUFFICIENT_BATCH_INFORMATION'));
			return false;
		}
	...
	protected function batchAccess($value, $pks, $contexts)
	{
		...
		foreach ($pks as $pk)
		{
			...
			{
				$this->setError(RText::_('RLIB_APPLICATION_ERROR_BATCH_CANNOT_EDIT'));
				return false;
			}
		}
	...
	/**
	 * Batch copy elements to a new category or current.
	 *
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		if ($categoryId)
		{
			...
			if (!$categoryTable->load($categoryId))
			{
				...
				{
					$this->setError(RText::_('RLIB_APPLICATION_ERROR_BATCH_MOVE_CATEGORY_NOT_FOUND'));
					return false;
				}
			}
		}
		...
		{
			$this->setError(RText::_('RLIB_APPLICATION_ERROR_BATCH_CANNOT_CREATE'));
			return false;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$table->load($pk))
			{
				...
				{
					// Not fatal error
					$this->setError(RText::sprintf('RLIB_APPLICATION_ERROR_BATCH_MOVE_ROW_NOT_FOUND', $pk));
					continue;
				}
			}
	...
	/**
	 * Batch move elements to a new category
	 *
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				...
				else
				{
					...
					// Prune elements that you can't change.
					unset($pks[$i]);
					$error = $this->getError();
					if ($error)
					{
					...
					{
						JError::raiseWarning(403, RText::_('RLIB_APPLICATION_ERROR_DELETE_NOT_PERMITTED'));
						return false;
					}
				}
	 ...
	 * @since   11.1
	 */
	public function getelement($pk = null)
	{
		// Initialise variables.
		$pk = (!empty($pk)) ? $pk : (int) $this->getState($this->getName() . '.id');
		$table = $this->getTable();
		...
		if (property_exists($element, 'params'))
		{
			$registry = new JRegistry;
			$registry->loadString($element->params);
			$element->params = $registry->toArray();
		}
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if (!$this->canEditState($table))
				{
					// Prune elements that you can't change.
					unset($pks[$i]);
					JError::raiseWarning(403, RText::_('RLIB_APPLICATION_ERROR_EDITSTATE_NOT_PERMITTED'));
					return false;
				}
			}
		}
	 ...
	 *
	 * @return  mixed  False on failure or error, true on success, null if the $pk is empty (no elements selected).
	 *
	 * @since   11.1
	 */
	...
	public function saveorder($pks = null, $order = null)
	{
		...
		if (empty($pks))
		{
			return JError::raiseWarning(500, RText::_($this->text_prefix . '_ERROR_NO_elementS_SELECTED'));
		}
