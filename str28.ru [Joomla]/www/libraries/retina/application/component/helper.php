...
class JComponentHelper
{
	...
	public static function renderComponent($option, $params = array())
	{
		...
		$lang->load('tpl_' . $template, RPATH_BASE, null, false, false)
			|| $lang->load('tpl_' . $template, RPATH_THEMES . "/$template", null, false, false)
			|| $lang->load('tpl_' . $template, RPATH_BASE, $lang->getDefault(), false, false)
			|| $lang->load('tpl_' . $template, RPATH_THEMES . "/$template", $lang->getDefault(), false, false);

		if (empty($option))
		{
			// Throw 404 if no component
			JError::raiseError(404, RText::_('RLIB_APPLICATION_ERROR_COMPONENT_NOT_FOUND'));
			return;
		}
		...
		define('RPATH_COMPONENT', RPATH_BASE . '/components/' . $option);
		define('RPATH_COMPONENT_SITE', RPATH_SITE . '/components/' . $option);
		define('RPATH_COMPONENT_admin', RPATH_admin . '/components/' . $option);

		// Get component path
		if ($app->isAdmin() && file_exists(RPATH_COMPONENT . '/admin.' . $file . '.php'))
		{
			$path = RPATH_COMPONENT . '/admin.' . $file . '.php';
		}
		else
		{
			$path = RPATH_COMPONENT . '/' . $file . '.php';
		}
		...
		$lang->load($option, RPATH_BASE, null, false, false) || $lang->load($option, RPATH_COMPONENT, null, false, false)
			|| $lang->load($option, RPATH_BASE, $lang->getDefault(), false, false)
			|| $lang->load($option, RPATH_COMPONENT, $lang->getDefault(), false, false);
	...
	protected static function _load($option)
	{
		...
		$cache = JFactory::getCache('_main', 'callback');
		...
		{
			// Fatal error.
			JError::raiseWarning(500, RText::sprintf('RLIB_APPLICATION_ERROR_COMPONENT_NOT_LOADING', $option, $error));
			return false;
		}
