...
abstract class JModel extends JObject
{
	...
	public static function getInstance($type, $prefix = '', $config = array())
	{
		...
		if (!class_exists($modelClass))
		{
			...
			if ($path)
			{
				...
				if (!class_exists($modelClass))
				{
					JError::raiseWarning(0, RText::sprintf('RLIB_APPLICATION_ERROR_MODELCLASS_NOT_FOUND', $modelClass));
					return false;
				}
			}
			else
	...
	public function __construct($config = array())
	{
		...
		if (empty($this->option))
		{
			...
			{
				JError::raiseError(500, RText::_('RLIB_APPLICATION_ERROR_MODEL_GET_NAME'));
			}
		...
		elseif (defined('RPATH_COMPONENT_admin'))
		{
			$this->addTablePath(RPATH_COMPONENT_admin . '/tables');
		}
	...
	public function getTable($name = '', $prefix = 'Table', $options = array())
	{
		...
		JError::raiseError(0, RText::sprintf('RLIB_APPLICATION_ERROR_TABLE_NAME_NOT_SUPPORTED', $name));
	...
	protected function cleanCache($group = null, $client_id = 0)
	{
		...
		$options = array(
			...
			'cachebase' => ($client_id) ? RPATH_admin . '/cache' : $conf->get('cache_path', RPATH_SITE . '/cache'));
