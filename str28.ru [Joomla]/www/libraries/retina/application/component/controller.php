...
/**
 * Base class for a retina Controller
 *
...
class JController extends JObject
{
	...
	public static function getInstance($prefix, $config = array())
	{
		...
		$basePath = array_key_exists('base_path', $config) ? $config['base_path'] : RPATH_COMPONENT;
		...
		if (!class_exists($class))
		{
			...
			{
				throw new InvalidArgumentException(RText::sprintf('RLIB_APPLICATION_ERROR_INVALID_CONTROLLER', $type, $format));
			}
		}
		...
		{
			throw new InvalidArgumentException(RText::sprintf('RLIB_APPLICATION_ERROR_INVALID_CONTROLLER_CLASS', $class));
		}
	...
	protected function createView($name, $prefix = '', $type = '', $config = array())
	{
		...
		if (!class_exists($viewClass))
		{
			...
			if ($path)
			{
				...
				if (!class_exists($viewClass))
				{
					JError::raiseError(500, RText::sprintf('RLIB_APPLICATION_ERROR_VIEW_CLASS_NOT_FOUND', $viewClass, $path));
	...
	public function execute($task)
	{
		...
		{
			return JError::raiseError(404, RText::sprintf('RLIB_APPLICATION_ERROR_TASK_NOT_FOUND', $task));
		}
		...
		{
			return JError::raiseError(403, RText::_('RLIB_APPLICATION_ERROR_ACCESS_FORBIDDEN'));
		}
	}
	...
	public function getModel($name = '', $prefix = '', $config = array())
	{
		...
		if ($model = $this->createModel($name, $prefix, $config))
		{
			...
			if (is_object($menu))
			{
				...
				{
					$params = $menu->getParams($element->id);
	...
	public function getView($name = '', $type = '', $prefix = '', $config = array())
	{
		...
		if (empty($views[$name]))
		{
			...
			{
				$result = JError::raiseError(500, RText::sprintf('RLIB_APPLICATION_ERROR_VIEW_NOT_FOUND', $name, $type, $prefix));
