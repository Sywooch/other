...
class QRcode {
	...
	// Reed-Solomon elements

	/**
	 * Reed-Solomon elements.
	 * @protected
	 */
	protected $rselements = array();
	...
	protected function encodeMask($mask) {
		...
		$this->datacode = $this->getByteStream($this->elements);
	 ...
	 */
	 protected function eatNum() {
		...
		$this->elements = $this->appendNewInputelement($this->elements, QR_MODE_NM, $run, str_split($this->dataStr));
		return $run;
	}
	 ...
	 */
	 protected function eatAn() {
		...
		$this->elements = $this->appendNewInputelement($this->elements, QR_MODE_AN, $run, str_split($this->dataStr));
		return $run;
	}
	 ...
	 */
	 protected function eatKanji() {
		...
		$this->elements = $this->appendNewInputelement($this->elements, QR_MODE_KJ, $p, str_split($this->dataStr));
		return $run;
	}
	 ...
	 */
	 protected function eat8() {
		...
		$run = $p;
		$this->elements = $this->appendNewInputelement($this->elements, QR_MODE_8B, $run, str_split($this->dataStr));
		return $run;
	}
	 ...
	 */
	 protected function newInputelement($mode, $size, $data, $bstream=null) {
		...
		$inputelement = array();
		$inputelement['mode'] = $mode;
		$inputelement['size'] = $size;
		$inputelement['data'] = $setData;
		$inputelement['bstream'] = $bstream;
		return $inputelement;
	}

	/**
	 * encodeModeNum
	 * @param $inputelement (array)
	 * @param $version (int)
	 * @return array input element
	 */
	 protected function encodeModeNum($inputelement, $version) {
		$words = (int)($inputelement['size'] / 3);
		$inputelement['bstream'] = array();
		$val = 0x1;
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 4, $val);
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], $this->lengthIndicator(QR_MODE_NM, $version), $inputelement['size']);
		for ($i=0; $i < $words; ++$i) {
			$val  = (ord($inputelement['data'][$i*3  ]) - ord('0')) * 100;
			$val += (ord($inputelement['data'][$i*3+1]) - ord('0')) * 10;
			$val += (ord($inputelement['data'][$i*3+2]) - ord('0'));
			$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 10, $val);
		}
		if ($inputelement['size'] - $words * 3 == 1) {
			$val = ord($inputelement['data'][$words*3]) - ord('0');
			$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 4, $val);
		} elseif (($inputelement['size'] - ($words * 3)) == 2) {
			$val  = (ord($inputelement['data'][$words*3  ]) - ord('0')) * 10;
			$val += (ord($inputelement['data'][$words*3+1]) - ord('0'));
			$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 7, $val);
		}
		return $inputelement;
	}
	 ...
	 * @return array input element
	 */
	 protected function encodeModeAn($inputelement, $version) {
		$words = (int)($inputelement['size'] / 2);
		$inputelement['bstream'] = array();
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 4, 0x02);
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], $this->lengthIndicator(QR_MODE_AN, $version), $inputelement['size']);
		for ($i=0; $i < $words; ++$i) {
			$val  = (int)($this->lookAnTable(ord($inputelement['data'][$i*2])) * 45);
			$val += (int)($this->lookAnTable(ord($inputelement['data'][($i*2)+1])));
			$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 11, $val);
		}
		if ($inputelement['size'] & 1) {
			$val = $this->lookAnTable(ord($inputelement['data'][($words * 2)]));
			$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 6, $val);
		}
		return $inputelement;
	}
	 ...
	 * @return array input element
	 */
	 protected function encodeMode8($inputelement, $version) {
		$inputelement['bstream'] = array();
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 4, 0x4);
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], $this->lengthIndicator(QR_MODE_8B, $version), $inputelement['size']);
		for ($i=0; $i < $inputelement['size']; ++$i) {
			$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 8, ord($inputelement['data'][$i]));
		}
		return $inputelement;
	}
	 ...
	 * @return array input element
	 */
	 protected function encodeModeKanji($inputelement, $version) {
		$inputelement['bstream'] = array();
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 4, 0x8);
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], $this->lengthIndicator(QR_MODE_KJ, $version), (int)($inputelement['size'] / 2));
		for ($i=0; $i<$inputelement['size']; $i+=2) {
			$val = (ord($inputelement['data'][$i]) << 8) | ord($inputelement['data'][$i+1]);
			if ($val <= 0x9ffc) {
				$val -= 0x8140;
			} else {
				$val -= 0xc140;
			...
			$h = ($val >> 8) * 0xc0;
			$val = ($val & 0xff) + $h;
			$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 13, $val);
		}
		return $inputelement;
	}
	 ...
	 * @return array input element
	 */
	 protected function encodeModeStructure($inputelement) {
		$inputelement['bstream'] = array();
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 4, 0x03);
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 4, ord($inputelement['data'][1]) - 1);
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 4, ord($inputelement['data'][0]) - 1);
		$inputelement['bstream'] = $this->appendNum($inputelement['bstream'], 8, ord($inputelement['data'][2]));
		return $inputelement;
	}
	 ...
	 * @return array input element
	 */
	 protected function encodeBitStream($inputelement, $version) {
		$inputelement['bstream'] = array();
		$words = $this->maximumWords($inputelement['mode'], $version);
		if ($inputelement['size'] > $words) {
			$st1 = $this->newInputelement($inputelement['mode'], $words, $inputelement['data']);
			$st2 = $this->newInputelement($inputelement['mode'], $inputelement['size'] - $words, array_slice($inputelement['data'], $words));
			$st1 = $this->encodeBitStream($st1, $version);
			$st2 = $this->encodeBitStream($st2, $version);
			$inputelement['bstream'] = array();
			$inputelement['bstream'] = $this->appendBitstream($inputelement['bstream'], $st1['bstream']);
			$inputelement['bstream'] = $this->appendBitstream($inputelement['bstream'], $st2['bstream']);
		} else {
			switch($inputelement['mode']) {
				case QR_MODE_NM: {
					$inputelement = $this->encodeModeNum($inputelement, $version);
					break;
				}
				case QR_MODE_AN: {
					$inputelement = $this->encodeModeAn($inputelement, $version);
					break;
				}
				case QR_MODE_8B: {
					$inputelement = $this->encodeMode8($inputelement, $version);
					break;
				}
				case QR_MODE_KJ: {
					$inputelement = $this->encodeModeKanji($inputelement, $version);
					break;
				}
				case QR_MODE_ST: {
					$inputelement = $this->encodeModeStructure($inputelement);
					break;
				}
				default: {
					break;
	 ...
	 * @param $elements (arrray) input elements
	 ...
	 *
	 */
	protected function appendNewInputelement($elements, $mode, $size, $data) {
		$newelement = $this->newInputelement($mode, $size, $data);
		if (!empty($newelement)) {
			$elements[] = $newelement;
		}
		return $elements;
	 ...
	 */
	 protected function insertStructuredAppendHeader($elements, $size, $index, $parity) {
		...
		$entry = $this->newInputelement(QR_MODE_ST, 3, buf);
		array_unshift($elements, $entry);
		return $elements;
	}
	 ...
	 * @return int parity
	 */
	 protected function calcParity($elements) {
		$parity = 0;
		foreach ($elements as $element) {
			if ($element['mode'] != QR_MODE_ST) {
				for ($i=$element['size']-1; $i>=0; --$i) {
					$parity ^= $element['data'][$i];
				}
			}
		}
		return $parity;
	 ...
	 * @return int bits
	 */
	 protected function estimateBitStreamSize($elements, $version) {
		$bits = 0;
		if ($version == 0) {
			$version = 1;
		}
		foreach ($elements as $element) {
			switch($element['mode']) {
				case QR_MODE_NM: {
					$bits = $this->estimateBitsModeNum($element['size']);
					break;
				}
				case QR_MODE_AN: {
					$bits = $this->estimateBitsModeAn($element['size']);
					break;
				}
				case QR_MODE_8B: {
					$bits = $this->estimateBitsMode8($element['size']);
					break;
				}
				case QR_MODE_KJ: {
					$bits = $this->estimateBitsModeKanji($element['size']);
					break;
				}
				case QR_MODE_ST: {
			...
			$l = $this->lengthIndicator($element['mode'], $version);
			$m = 1 << $l;
			$num = (int)(($element['size'] + $m - 1) / $m);
			$bits += $num * (4 + $l);
		}
		return $bits;
	}
	 ...
	 * @return int version
	 */
	 protected function estimateVersion($elements) {
		$version = 0;
		$prev = 0;
		do {
			$prev = $version;
			$bits = $this->estimateBitStreamSize($elements, $prev);
	 ...
	 * createBitStream
	 * @param $elements (array)
	 * @return array of elements and total bits
	 */
	 protected function createBitStream($elements) {
		$total = 0;
		foreach ($elements as $key => $element) {
			$elements[$key] = $this->encodeBitStream($element, $this->version);
			$bits = count($elements[$key]['bstream']);
			$total += $bits;
		}
		return array($elements, $total);
	}
	 ...
	 * @return array elements
	 */
	 protected function convertData($elements) {
		$ver = $this->estimateVersion($elements);
		if ($ver > $this->version) {
			$this->version = $ver;
		}
		for (;;) {
			$cbs = $this->createBitStream($elements);
			$elements = $cbs[0];
			$bits = $cbs[1];
			if ($bits < 0) {
				return -1;
	 ...
	 * @return array bitstream
	 */
	 protected function mergeBitStream($elements) {
		$elements = $this->convertData($elements);
		if (!is_array($elements)) {
			return null;
		}
		$bstream = array();
		foreach ($elements as $element) {
			$bstream = $this->appendBitstream($bstream, $element['bstream']);
		}
		return $bstream;
	}
	 ...
	 */
	protected function getBitStream($elements) {
		$bstream = $this->mergeBitStream($elements);
	 ...
	 * @return array padded merged byte stream
	 */
	protected function getByteStream($elements) {
		$bstream = $this->getBitStream($elements);
	...
	* Return block number 1
	...
	/**
	 * Initialize a Reed-Solomon codec and add it to existing rselements
	 ...
	 */
	 protected function init_rs($symsize, $gfpoly, $fcr, $prim, $nroots, $pad) {
		...
		array_unshift($this->rselements, $rs);
		return $rs;
	}
