...
abstract class JLoader
{
	...
	public static function import($key, $base = null)
	{
		...
		if (!isset(self::$imported[$key]))
		{
			...
			// If we are importing a library from the retina namespace set the class to autoload.
			if (strpos($path, 'retina') === 0)
			{

				// Since we are in the retina namespace prepend the classname with J.
				$class = 'J' . $class;
			...
			/*
			 * If we are not importing a library from the retina namespace directly include the
	...
	/**
	 * Method to setup the autoloaders for the retina Platform.  Since the SPL autoloaders are
	...
	/**
	 * Autoload a retina Platform class based on name.
	 *
	 * @param   string  $class  The class to be loaded.
	 *
	 * @return  void
	...
	private static function _autoload($class)
	{
		...
		// Only attempt autoloading if we are dealing with a retina Platform class.
		if ($class[0] == 'J')
		{
			...
			$path = RPATH_PLATFORM . '/retina/' . implode('/', array_map('strtolower', $parts)) . '.php';
