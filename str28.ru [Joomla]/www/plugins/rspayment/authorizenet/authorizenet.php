...
if (!class_exists('Creditcard')) {
    require_once(RPATH_rs_admin . DS . 'helpers' . DS . 'creditcard.php');
}
if (!class_exists('rsPSPlugin'))
    require(RPATH_rs_PLUGINS . DS . 'rspsplugin.php');

class plgrspaymentAuthorizenet extends rsPSPlugin {
     ...
     */
    function plgrsDisplayListFEPayment(retinashopCart $cart, $selected = 0, &$htmlIn) {
	JHTML::_('behavior.tooltip');

	if ($this->getPluginMethods($cart->vendorId) === 0) {
	    if (empty($this->_name)) {
		$app = JFactory::getApplication();
		$app->enqueueMessage(RText::_('COM_RETINASHOP_CART_NO_' . strtoupper($this->_psType)));
		return false;
	    } else {
		return false;
	    }
	...
	JHTML::script('rscreditcard.js', 'components/com_retinashop/retina_097115115101116115/js/', false);
	...
	foreach ($this->methods as $method) {
		...
		if ($selected == $method->retinashop_paymentmethod_id) {
		...
		$creditCardList = '';
		if ($creditCards) {
		    $creditCardList = ($this->_renderCreditCardList($creditCards, $this->_cc_type, $method->retinashop_paymentmethod_id, false));
		}
		$sandbox_msg = "";
		if ($method->sandbox) {
		    $sandbox_msg .= '<br />' . RText::_('rsPAYMENT_AUTHORIZENET_SANDBOX_TEST_NUMBERS');
		}

		$cvv_images = $this->_displayCVVImages($method);
		$html .= '<br /><span class="rspayment_cardinfo">' . RText::_('rsPAYMENT_AUTHORIZENET_COMPLETE_FORM') . $sandbox_msg . '
		    <table border="0" cellspacing="0" cellpadding="2" width="100%">
		    <tr valign="top">
		        <td nowrap width="10%" align="right">
		        	<label for="creditcardtype">' . RText::_('rsPAYMENT_AUTHORIZENET_CCTYPE') . '</label>
		        </td>
		        <td>' . $creditCardList .
			'</td>
		    </tr>
		    ...
		    <tr valign="top">
		        <td nowrap width="10%" align="right">
		        	<label for="cc_type">' . RText::_('rsPAYMENT_AUTHORIZENET_CCNUM') . '</label>
		        </td>
		        <td>
		        <input type="text" class="inputbox" id="cc_number_' . $method->retinashop_paymentmethod_id . '" name="cc_number_' . $method->retinashop_paymentmethod_id . '" value="' . $this->_cc_number . '"    autocomplete="off"   onchange="ccError=razCCerror(' . $method->retinashop_paymentmethod_id . ');
	CheckCreditCardNumber(this . value, ' . $method->retinashop_paymentmethod_id . ');
	if (!ccError) {
	    this.value=\'\';}" />
		        <div id="cc_cardnumber_errormsg_' . $method->retinashop_paymentmethod_id . '"></div>
		    </td>
		    </tr>
		    <tr valign="top">
		        <td nowrap width="10%" align="right">
		        	<label for="cc_cvv">' . RText::_('rsPAYMENT_AUTHORIZENET_CVV2') . '</label>
		        </td>
		        <td>
		            <input type="text" class="inputbox" id="cc_cvv_' . $method->retinashop_paymentmethod_id . '" name="cc_cvv_' . $method->retinashop_paymentmethod_id . '" maxlength="4" size="5" value="' . $this->_cc_cvv . '" autocomplete="off" />

			<span class="hasTip" title="' . RText::_('rsPAYMENT_AUTHORIZENET_WHATISCVV') . '::' . RText::sprintf("rsPAYMENT_AUTHORIZENET_WHATISCVV_TOOLTIP", $cvv_images) . ' ">' .
			RText::_('rsPAYMENT_AUTHORIZENET_WHATISCVV') . '
			</span></td>
		    </tr>
		    <tr>
		        <td nowrap width="10%" align="right">' . RText::_('rsPAYMENT_AUTHORIZENET_EXDATE') . '</td>
		        <td> ';
		$html .= shopfunctions::listMonths('cc_expire_month_' . $method->retinashop_paymentmethod_id, $this->_cc_expire_month);
		$html .= " / ";

		$html .= shopfunctions::listYears('cc_expire_year_' . $method->retinashop_paymentmethod_id, $this->_cc_expire_year, null, null, "onchange=\"var month = document.getElementById('cc_expire_month_'.$method->retinashop_paymentmethod_id); if(!CreditCardisExpiryDate(month.value,this.value, '.$method->retinashop_paymentmethod_id.')){this.value='';month.value='';}\" ");
		$html .='<div id="cc_expiredate_errormsg_' . $method->retinashop_paymentmethod_id . '"></div>';
	...
	if (!isset($address['retinashop_country_id']))
	    $address['retinashop_country_id'] = 0;
	if (count($countries) == 0 || in_array($address['retinashop_country_id'], $countries) || count($countries) == 0) {
	    return true;
	}
	...
	$session->set('authorizenet', serialize($sessionAuthorizeNet), 'rs');
    }

    function _getAuthorizeNetIntoSession() {
	$session = JFactory::getSession();
	$authorizenetSession = $session->get('authorizenet', 0, 'rs');
     ...
     * @author Valerie Cartan Isaksen
     */
    function plgrsOnCheckoutCheckDataPayment(retinashopCart $cart) {

	if (!$this->selectedThisByMethodId($cart->retinashop_paymentmethod_id)) {
	...
	$this->_cc_type = JRequest::getVar('cc_type_' . $cart->retinashop_paymentmethod_id, '');
	$this->_cc_name = JRequest::getVar('cc_name_' . $cart->retinashop_paymentmethod_id, '');
	$this->_cc_number = str_replace(" ", "", JRequest::getVar('cc_number_' . $cart->retinashop_paymentmethod_id, ''));
	$this->_cc_cvv = JRequest::getVar('cc_cvv_' . $cart->retinashop_paymentmethod_id, '');
	$this->_cc_expire_month = JRequest::getVar('cc_expire_month_' . $cart->retinashop_paymentmethod_id, '');
	$this->_cc_expire_year = JRequest::getVar('cc_expire_year_' . $cart->retinashop_paymentmethod_id, '');
    ...
    public function plgrsOnSelectedCalculatePricePayment(retinashopCart $cart, array &$cart_prices, &$payment_name) {

	if (!($method = $this->getrsPluginMethod($cart->retinashop_paymentmethod_id))) {
	    return null; // Another method was selected, do nothing
	}
...
// 		$logo = $params->get($this->_psType . '_logos');
     ...
     */
    function plgrsOnShowOrderBEPayment($retinashop_order_id, $retinashop_payment_id) {
	if (!$this->selectedThisByMethodId($retinashop_payment_id)) {
	    return null; // Another method was selected, do nothing
	}
    ...
    /**
     * Reimplementation of rsPaymentPlugin::plgrsOnConfirmedOrderStorePaymentData()
     *
     * @author Valerie Isaken

      function plgrsOnConfirmedOrderStoreDataPayment(  $retinashop_order_id, retinashopCart $cart, $prices) {
      return null;
      }
     */

    /**
     * Reimplementation of rsPaymentPlugin::plgrsOnConfirmedOrder()
     *
	...
	$dbValues['retinashop_order_id'] = $order['details']['BT']->retinashop_order_id;
	$dbValues['payment_method_id'] = $order['details']['BT']->retinashop_paymentmethod_id;
	...
	if ($this->error) {
	    ...
	    $this->_handlePaymentCancel($order['details']['BT']->retinashop_order_id, $html);
	...
	$modelOrder = rsModel::getModel('orders');
	$order['order_status'] = $new_status;
	$order['customer_notified'] = 1;
	$order['comments'] = '';
	$modelOrder->updateStatusForOneOrder($order['details']['BT']->retinashop_order_id, $order, true);
    ...
    function _handlePaymentCancel($retinashop_order_id, $html) {
	if (!class_exists('retinashopModelOrders'))
	    require( RPATH_rs_admin . DS . 'models' . DS . 'orders.php' );
	$modelOrder = rsModel::getModel('orders');
	$modelOrder->remove(array('retinashop_order_id' => $retinashop_order_id));
	// error while processing the payment
	$mainframe = JFactory::getApplication();
	$mainframe->enqueueMessage($html);
	$mainframe->redirect(JRoute::_('index.php?option=com_retinashop&view=cart&task=editpayment'), RText::_('COM_RETINASHOP_CART_ORDERDONE_DATA_NOT_VALID'));
    }

    function plgrsGetPaymentCurrency($retinashop_paymentmethod_id, &$paymentCurrencyId) {
	...
	$vendorId = 1; //retinashopModelVendor::getLoggedVendor();
	$db = JFactory::getDBO();

	$q = 'SELECT   `retinashop_currency_id` FROM `#__retinashop_currencies` WHERE `currency_code_3`= "USD"';
	$db->setQuery($q);
	...
	if ($this->_validate_creditcard_data(false)) {
	    ...
	    $creditCardInfos .= '<br /><span class="rspayment_cardinfo">' . RText::_('rsPAYMENT_AUTHORIZENET_CCTYPE') . $this->_cc_type . '<br />';
	    $creditCardInfos .=RText::_('rsPAYMENT_AUTHORIZENET_CCNUM') . $cc_number . '<br />';
	    $creditCardInfos .= RText::_('rsPAYMENT_AUTHORIZENET_CVV2') . '****' . '<br />';
	    $creditCardInfos .= RText::_('rsPAYMENT_AUTHORIZENET_EXDATE') . $this->_cc_expire_month . '/' . $this->_cc_expire_year;
	...
	//$options[] = JHTML::_('select.option', '', RText::_('rsPAYMENT_AUTHORIZENET_SELECT_CC_TYPE'), 'creditcard_type', $name);
	if (!is_array($creditCards)) {
	    $creditCards = (array) $creditCards;
	}
	foreach ($creditCards as $creditCard) {
	    $options[] = JHTML::_('select.option', $creditCard, RText::_('rsPAYMENT_AUTHORIZENET_' . strtoupper($creditCard)));
	}
	if ($multiple) {
	...
	if (!Creditcard::validate_credit_card_number($this->_cc_type, $this->_cc_number)) {
	    $this->_errormessage[] = 'rsPAYMENT_AUTHORIZENET_CARD_NUMBER_INVALID';
	    $this->_cc_valid = false;
	}

	if (!Creditcard::validate_credit_card_cvv($this->_cc_type, $this->_cc_cvv)) {
	    $this->_errormessage[] = 'rsPAYMENT_AUTHORIZENET_CARD_CVV_INVALID';
	    $this->_cc_valid = false;
	}
	...
	return array(
	    ...
	    'x_state' => isset($usrBT->retinashop_state_id) ? $this->_getField(ShopFunctions::getStateByID($usrBT->retinashop_state_id), 20) : '',
	    'x_country' => isset($usrBT->retinashop_country_id) ? $this->_getField(ShopFunctions::getCountryByID($usrBT->retinashop_country_id), 60) : '',
	...
	return array(
	    ...
	    'x_ship_to_state' => isset($usrST->retinashop_state_id) ? $this->_getField(ShopFunctions::getStateByID($usrST->retinashop_state_id), 20) : '',
	    'x_ship_to_country' => isset($usrST->retinashop_country_id) ? $this->_getField(ShopFunctions::getCountryByID($usrST->retinashop_country_id), 60) : '',
	);
    }
	...
	if ($response) {
		...
		$this->approved = false;
		$this->error = true;
		$error_message = RText::_('rsPAYMENT_AUTHORIZENET_UNKNOWN') . $response;
		// send email to vendor
		$this->sendEmailToVendorAndAdmins(RText::_('rsPAYMENT_AUTHORIZENET_ERROR_EMAIL_SUBJECT'), $error_message);
	    ...
	    $retinashop_order_id = retinashopModelOrders::getOrderIdByOrderNumber($authorizeNetResponse['invoice_number']);
	    if (!$retinashop_order_id) {
		$this->approved = false;
		$this->error = true;
		$this->logInfo(RText::sprintf('rsPAYMENT_AUTHORIZENET_NO_ORDER_NUMBER', $authorizeNetResponse['invoice_number']), 'ERROR');
		$this->sendEmailToVendorAndAdmins(RText::sprintf('rsPAYMENT_AUTHORIZENET_NO_ORDER_NUMBER', $authorizeNetResponse['invoice_number']), RText::sprintf('rsPAYMENT_AUTHORIZENET_ERROR_WHILE_PROCESSING_PAYMENT', $authorizeNetResponse['invoice_number']));
		$html = Jtext::sprintf('rsPAYMENT_AUTHORIZENET_ERROR', $authorizeNetResponse['response_reason_text'], $authorizeNetResponse['response_code']) . "<br />";
	    ...
	    $this->approved = false;
	    $this->error = true;
	    $this->logInfo(RText::_('rsPAYMENT_AUTHORIZENET_CONNECTING_ERROR'), 'ERROR');
	    $this->sendEmailToVendorAndAdmins(RText::_('rsPAYMENT_AUTHORIZENET_ERROR_EMAIL_SUBJECT'), RText::_('rsPAYMENT_AUTHORIZENET_CONNECTING_ERROR'));
	    return RText::_('rsPAYMENT_AUTHORIZENET_CONNECTING_ERROR');
	}
// Prep
// get all know columns of the table
	...
	$response_fields['retinashop_order_id'] = $retinashop_order_id;
	$response_fields['invoice_number'] = $authorizeNetResponse['invoice_number'];
	$response_fields['authorizeresponse_raw'] = $response;

	$this->storePSPluginInternalData($response_fields, 'retinashop_order_id', true);

	$currencyModel = rsModel::getModel('Currency');
	...
	$html .= '</table>' . "\n";
	$this->logInfo(RText::_('rsPAYMENT_AUTHORIZENET_ORDER_NUMBER') . " " . $authorizeNetResponse['invoice_number'] . ' payment approved', 'message');
	return $html;
    }
     ...
     * @author Valerie Isaksen
     */
    protected function plgrsOnShowOrderFEPayment($retinashop_order_id, $retinashop_paymentmethod_id, &$payment_name) {
