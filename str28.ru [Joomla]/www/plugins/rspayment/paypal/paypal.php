...
class plgrsPaymentPaypal extends rsPSPlugin {
	...
	$this->_tablepkey = 'id'; //retinashop_paypal_id';
	$this->_tableId = 'id'; //'retinashop_paypal_id';
	...
	$this->logInfo('plgrsConfirmedOrder order number: ' . $order['details']['BT']->order_number, 'message');
	...
	if (!class_exists('TableVendors'))
	    require(RPATH_rs_admin . DS . 'table' . DS . 'vendors.php');
	$vendorModel = rsModel::getModel('Vendor');
	$vendorModel->setId(1);
	$vendor = $vendorModel->getVendor();
	$vendorModel->addImages($vendor, 1);
	$this->getPaymentCurrency($method);
	$q = 'SELECT `currency_code_3` FROM `#__retinashop_currencies` WHERE `retinashop_currency_id`="' . $method->payment_currency . '" ';
	$db = JFactory::getDBO();
	$db->setQuery($q);
...
if ($totalInPaymentCurrency <= 0) {
     rsInfo(RText::_('rsPAYMENT_PAYPAL_PAYMENT_AMOUNT_INCORRECT'));
	    return false;
}
	$merchant_email = $this->_getMerchantEmail($method);
	if (empty($merchant_email)) {
	    rsInfo(RText::_('rsPAYMENT_PAYPAL_MERCHANT_EMAIL_NOT_SET'));
	    return false;
	}
	...
	$post_variables = Array(
	    ...
	    'element_name' => RText::_('rsPAYMENT_PAYPAL_ORDER_NUMBER') . ': ' . $order['details']['BT']->order_number,
	    ...
	    "zip" => $address->zip,
	    "city" => $address->city,
	    "state" => isset($address->retinashop_state_id) ? ShopFunctions::getStateByID($address->retinashop_state_id) : '',
	    "country" => ShopFunctions::getCountryByID($address->retinashop_country_id, 'country_2_code'),
	    "email" => $order['details']['BT']->email,
	    "night_phone_b" => $address->phone_1,
	    "return" => JROUTE::_(JURI::root() . 'index.php?option=com_retinashop&view=pluginresponse&task=pluginresponsereceived&on=' . $order['details']['BT']->order_number . '&pm=' . $order['details']['BT']->retinashop_paymentmethod_id),
	    //"return" => JROUTE::_(JURI::root() . 'index.php?option=com_retinashop&view=pluginresponse&task=pluginnotification&tmpl=component'),
	    "notify_url" => JROUTE::_(JURI::root() . 'index.php?option=com_retinashop&view=pluginresponse&task=pluginnotification&tmpl=component'),
	    "cancel_return" => JROUTE::_(JURI::root() . 'index.php?option=com_retinashop&view=pluginresponse&task=pluginUserPaymentCancel&on=' . $order['details']['BT']->order_number . '&pm=' . $order['details']['BT']->retinashop_paymentmethod_id),
	...
	/*
	  ...
	  $post_variables["element_name_" . $i] = substr(strip_tags($product->product_name), 0, 127);
	  $post_variables["element_number_" . $i] = $i;
	  ...
	  $post_variables["element_name_" . $i] = RText::_('rsPAYMENT_PAYPAL_SHIPMENT_PRICE');
	  $post_variables["element_number_" . $i] = $i;
	  ...
	  $post_variables["element_name_" . $i] = RText::_('rsPAYMENT_PAYPAL_PAYMENT_PRICE');
	  $post_variables["element_number_" . $i] = $i;
	...
	$html .= '<form action="' . "https://" . $url . '" method="post" name="rs_paypal_form" >';
	$html.= '<input type="submit"  value="' . RText::_('rsPAYMENT_PAYPAL_REDIRECT_MESSAGE') . '" />';
	foreach ($post_variables as $name => $value) {
	    $html.= '<input type="hidden" name="' . $name . '" value="' . htmlspecialchars($value) . '" />';
	}
	$html.= '</form></div>';
	$html.= ' <script type="text/javascript">';
	$html.= ' document.rs_paypal_form.submit();';
	...
	rsdebug('PAYPAL plgrsOnPaymentResponseReceived', $paypal_data);
	...
	$retinashop_paymentmethod_id = JRequest::getInt('pm', '');
	if (empty($order_number) or empty($retinashop_paymentmethod_id) or !$this->selectedThisByMethodId($retinashop_paymentmethod_id)) {
	    return null;
	}
	if (!($retinashop_order_id = retinashopModelOrders::getOrderIdByOrderNumber($order_number))) {
		return null;
	...
	rsInfo(Jtext::_('rsPAYMENT_PAYPAL_PAYMENT_CANCELLED'));
	$session = JFactory::getSession();
	$return_context = $session->getId();
    ...
    /*
     *   plgrsOnPaymentNotification() - This event is fired by Offline Payment. It can be used to validate the payment data as entered by the user.
     * Return:
     * Parameters:
     *  None
	...
	if (!($retinashop_order_id = retinashopModelOrders::getOrderIdByOrderNumber($paypal_data['invoice']))) {
	    return;
	}

	$vendorId = 0;
	if (!($payment = $this->getDataByOrderId($retinashop_order_id))) {
	    return;
	}

	$method = $this->getrsPluginMethod($payment->retinashop_paymentmethod_id);
	if (!$this->selectedThisElement($method->payment_element)) {
	    return false;
	}
	...
	$this->_storePaypalInternalData($method, $paypal_data, $retinashop_order_id);
	$modelOrder = rsModel::getModel('orders');
	$order = array();
	$error_msg = $this->_processIPN($paypal_data, $method, $retinashop_order_id);
	 ...
	 *
	 */
	if (empty($paypal_data['payment_status']) || ($paypal_data['payment_status'] != 'Completed' && $paypal_data['payment_status'] != 'В ожидании')) {
	    //return false;
	}
	...
	if (strcmp($paypal_data['payment_status'], 'Completed') == 0) {
	    ...
	    $order['comments'] = RText::sprintf('rsPAYMENT_PAYPAL_PAYMENT_STATUS_CONFIRMED', $order_number);
	} elseif (strcmp($paypal_data['payment_status'], 'В ожидании') == 0) {
	    $key = 'rsPAYMENT_PAYPAL_PENDING_REASON_FE_' . strtoupper($paypal_data['pending_reason']);
	    if (!$lang->hasKey($key)) {
		$key = 'rsPAYMENT_PAYPAL_PENDING_REASON_FE_DEFAULT';
	    }
	    $order['comments'] = RText::sprintf('rsPAYMENT_PAYPAL_PAYMENT_STATUS_PENDING', $order_number) . RText::_($key);
	    $order['order_status'] = $method->status_pending;
	} else {
	    $order['order_status'] = $method->status_canceled;
	}

	$this->logInfo('plgrsOnPaymentNotification return new_status:' . $order['order_status'], 'message');
    ...
    function _storePaypalInternalData($method, $paypal_data, $retinashop_order_id) {
	...
	//$response_fields[$this->_tablepkey] = $this->_getTablepkeyValue($retinashop_order_id);
     ...
     * @see components/com_retinashop/helpers/rsPSPlugin::plgrsOnShowOrderBEPayment()
     */
	...
	if (!($paymentTable = $this->_getPaypalInternalData($retinashop_order_id) )) {
	    // JError::raiseWarning(500, $db->getErrorMsg());
	    return '';
	}
	$this->getPaymentCurrency($paymentTable);
	$q = 'SELECT `currency_code_3` FROM `#__retinashop_currencies` WHERE `retinashop_currency_id`="' . $paymentTable->payment_currency . '" ';
	$db = JFactory::getDBO();
	$db->setQuery($q);
	$currency_code_3 = $db->loadResult();
    ...
    function _getPaypalInternalData($retinashop_order_id, $order_number = '') {
	$db = JFactory::getDBO();
	$q = 'SELECT * FROM `' . $this->_tablename . '` WHERE ';
	if ($order_number) {
	    $q .= " `order_number` = '" . $order_number . "'";
	} else {
	    $q .= ' `retinashop_order_id` = ' . $retinashop_order_id;
	}
	...
	// read the post from PayPal main and add 'cmd'
	...
	// post back to PayPal main to validate
	...
	if (!$fps) {
	    $this->sendEmailToVendorAndAdmins("error with paypal", RText::sprintf('rsPAYMENT_PAYPAL_ERROR_POSTING_IPN', $errstr, $errno));
	    return RText::sprintf('rsPAYMENT_PAYPAL_ERROR_POSTING_IPN', $errstr, $errno); // send email
	} else {
		...
		if (strcmp($res, 'VERIFIED') == 0) {
		    ...
		    $this->sendEmailToVendorAndAdmins("error with paypal IPN NOTIFICATION", RText::_('rsPAYMENT_PAYPAL_ERROR_IPN_VALIDATION') . $res);
		    return RText::_('rsPAYMENT_PAYPAL_ERROR_IPN_VALIDATION') . $res;
		}
	    }
	}
	...
	if (!($hostname == "www.sandbox.paypal.com" && $test_ipn == 1 )) {
	    ...
	    //rsMail($mosConfig_mailfrom, $mosConfig_fromname, $debug_email_address, $mailsubject, $mailbody );
	    $this->sendEmailToVendorAndAdmins($mailsubject, $mailbody);
	}
    }
