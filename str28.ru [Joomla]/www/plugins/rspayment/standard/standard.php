...
class plgrsPaymentStandard extends rsPSPlugin {
...
// 		rsdebug('Plugin stuff',$subject, $config);
	$this->_loggable = true;
...
// 		$params = new JParameter($payment->payment_params);
	...
	$filename = 'com_retinashop';
	$lang->load($filename, RPATH_admin);
	$vendorId = 0;
	...
	$this->_retinashop_paymentmethod_id = $order['details']['BT']->retinashop_paymentmethod_id;
	$dbValues['payment_name'] = $this->renderPluginName($method) . '<br />' . $method->payment_info;
	$dbValues['order_number'] = $order['details']['BT']->order_number;
	$dbValues['retinashop_paymentmethod_id'] = $this->_retinashop_paymentmethod_id;
	...
	$html = '<table class="rsorder-done">' . "\n";
	$html .= $this->getHtmlRow('STANDARD_PAYMENT_INFO', $dbValues['payment_name'], "rsorder-done-payinfo");
	if (!empty($payment_info)) {
	    $lang = JFactory::getLanguage();
	    if ($lang->hasKey($method->payment_info)) {
		$payment_info = RText::_($method->payment_info);
	    } else {
		$payment_info = $method->payment_info;
	    }
		$html .= $this->getHtmlRow('STANDARD_PAYMENTINFO', $payment_info, "rsorder-done-payinfo");
	}
	if (!class_exists('retinashopModelCurrency'))
	    require(RPATH_rs_admin . DS . 'models' . DS . 'currency.php');
	$currency = CurrencyDisplay::getInstance('', $order['details']['BT']->retinashop_vendor_id);
	$html .= $this->getHtmlRow('STANDARD_ORDER_NUMBER', $order['details']['BT']->order_number, "rsorder-done-nr");
	$html .= $this->getHtmlRow('STANDARD_AMOUNT', $currency->priceDisplay($order['details']['BT']->order_total), "rsorder-done-amount");
...
// 		$params = new JParameter($payment->payment_params);
     ...
     * @author Max Milbers
      ...
      public function plgrsOnCheckoutCheckDataPayment(  retinashopCart $cart) {
      return null;
      }
     */
     ...
     * @author Oscar van Eijk
     *
      public function plgrsOnEditOrderLineBEPayment(  $_orderId, $_lineId) {
      return null;
      }
     ...
     * @param int $retinashop_order_id : payment  order id
     ...
     *
     *
      public function plgrsOnPaymentNotification() {
      return null;
      }

      /**
     * plgrsOnPaymentResponseReceived
     ...
     *
     *
      function plgrsOnPaymentResponseReceived(, &$retinashop_order_id, &$html) {
      return null;
      }
     */
}
