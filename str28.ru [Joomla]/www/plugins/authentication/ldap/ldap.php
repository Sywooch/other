...
class plgAuthenticationLdap extends JPlugin
{
	...
	function onUserAuthenticate($credentials, $options, &$response)
	{
		...
		if (empty($credentials['password']))
		{
			...
			$response->error_message = RText::_('RGLOBAL_AUTH_PASS_BLANK');
			return false;
		}
		...
		jimport('retina.client.ldap');
		$ldap = new JLDAP($this->params);

		if (!$ldap->connect())
		{
			$response->status = JAuthentication::STATUS_FAILURE;
			$response->error_message = RText::_('RGLOBAL_AUTH_NO_CONNECT');
			return;
		}
		...
		switch($auth_method)
		{
			case 'search':
			{
				...
				if ($bindtest)
				{
					...
					if (isset($binddata[0]) && isset($binddata[0]['dn'])) {
						...
						$response->status = JAuthentication::STATUS_FAILURE;
						$response->error_message = RText::_('RGLOBAL_AUTH_USER_NOT_FOUND');
					}
				}
				else
				{
					$response->status = JAuthentication::STATUS_FAILURE;
					$response->error_message = RText::_('RGLOBAL_AUTH_NO_BIND');
				}
			}	break;
			...
			case 'bind':
			{
				...
				if ($success) {
					...
					$response->status = JAuthentication::STATUS_FAILURE;
					$response->error_message = RText::_('RGLOBAL_AUTH_BIND_FAILED');
				}
			}	break;
		}
		...
		{
			$response->status = JAuthentication::STATUS_FAILURE;
			if (!strlen($response->error_message)) $response->error_message = RText::_('RGLOBAL_AUTH_INCORRECT');
		}
		else
		{
