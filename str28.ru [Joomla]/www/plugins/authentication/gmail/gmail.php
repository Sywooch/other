...
class plgAuthenticationGMail extends JPlugin {
	...
	function onUserAuthenticate($credentials, $options, & $response) {
		...
		if (function_exists('curl_init')) {
			...
			if (strlen($credentials['username']) && strlen($credentials['password'])) {
				...
				$code = curl_getinfo ($curl, CURLINFO_HTTP_CODE);
					...
					switch ($code) {
					case 200:
						$message = RText::_('RGLOBAL_AUTH_ACCESS_GRANTED');
						$success = 1;
					break;
					case 401:
						$message = RText::_('RGLOBAL_AUTH_ACCESS_DENIED');
					break;
					default:
						$message = RText::_('RGLOBAL_AUTH_UNKNOWN_ACCESS_DENIED');
						break;
				}
				} else {
				...
				$message = RText::_('RGLOBAL_AUTH_USER_BLACKLISTED');
			}
		} else {
		...
		if ($success) {
			...
			$response->error_message	= RText::sprintf('RGLOBAL_AUTH_FAILED', $message);
		}
	}
}
