...
class plgButtonReadmore extends JPlugin
{
	...
	public function onDisplay($name)
	{
		...
		$present = RText::_('PLG_READMORE_ALREADY_EXISTS', true) ;
		$js = "
			function insertReadmore(editor) {
				var content = $getContent
				if (content.match(/<hr\s+id=(\"|')main-readmore(\"|')\s*\/*>/i)) {
					alert('$present');
					return false;
				} else {
					jInsertEditorText('<hr id=\"main-readmore\" />', editor);
				}
			}
			";
		...
		$button->set('text', RText::_('PLG_READMORE_BUTTON_READMORE'));
