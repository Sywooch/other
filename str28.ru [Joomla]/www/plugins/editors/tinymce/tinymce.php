...
class plgEditorTinymce extends JPlugin
{
	...
	function onInit()
	{
		...
		$design1_path = RPATH_SITE . '/design1';
		...
		if ( $content_css_custom ) {
			...
			// If it is not a URL, assume it is a file name in the current template folder
				...
				$content_css = 'content_css : "'. JURI::root() .'design1/'. $template . '/css/'. $content_css_custom .'",';

				// Issue warning notice if the file is not found (but pass name to $content_css anyway to avoid TinyMCE error
				if (!file_exists($design1_path . '/' . $template . '/css/' . $content_css_custom)) {
					$msg = sprintf (RText::_('PLG_TINY_ERR_CUSTOMCSSFILENOTPRESENT'), $content_css_custom);
			...
			if ($use_content_css) {
				// first check design1 folder for default template
				// if no editor.css file in design1 folder, check main template folder
				if (!file_exists($design1_path . '/' . $template . '/css/editor.css')) {
					$template = 'main';

					// if no editor.css file in main folder, show alert
					if (!file_exists($design1_path . '/main/css/editor.css')) {
						JError::raiseNotice('SOME_ERROR_CODE', RText::_('PLG_TINY_ERR_EDITORCSSFILENOTPRESENT'));
					}
					else {
						$content_css = 'content_css : "' . JURI::root() .'design1/main/css/editor.css",';
					}
				}
				else {
					$content_css = 'content_css : "' . JURI::root() .'design1/'. $template . '/css/editor.css",';
				}
			}
		}
		...
		switch ($mode)
		{
			...
			case 2: /* Extended mode*/
				...
				tinyMCE.init({
					...
					document_base_url : \"". JURI::root() ."\",
					//design1
					template_external_list_url :  \"". JURI::root() ."media/editors/tinymce/design1/template_list.js\",
					// Layout
					$content_css
					// Advanced theme
					theme_advanced_toolbar_location : \"$toolbar\",
	...
	function _toogleButton($name)
	{
		...
		$return .= "<div class=\"button2-left\"><div class=\"blank\"><a href=\"#\" onclick=\"tinyMCE.execCommand('mceToggleEditor', false, '$name');return false;\" title=\"".RText::_('PLG_TINY_BUTTON_TOGGLE_EDITOR')."\">".RText::_('PLG_TINY_BUTTON_TOGGLE_EDITOR')."</a></div></div>";
		$return .= "</div>\n";
