 ...
 * @subpackage	main.logout
 */
class plgmainLogout extends JPlugin
{
	/**
	 * Object Constructor.
	 *
	...
	function __construct(&$subject, $config)
	{
		...
		$hash = JApplication::getHash('plgmainLogout');
		if (JFactory::getApplication()->isSite() and JRequest::getString($hash, null , 'cookie'))
		{
			...
			JError::setErrorHandling(E_ALL, 'callback', array('plgmainLogout', 'handleError'));
		}
	}
	...
	static function handleError(&$error)
	{
		...
		if ($error->getCode() == 403 and $app->isSite()) {
			...
			$app->redirect('index.php', RText::_('PLG_main_LOGOUT_REDIRECT'), null, true, false);
		}
		else {
