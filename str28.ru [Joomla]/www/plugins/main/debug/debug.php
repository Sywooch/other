 ...
 * @subpackage	main.debug
 */
class plgmainDebug extends JPlugin
{
	...
	public function __destruct()
	{
		...
		// Load language file
		$this->loadLanguage('plg_main_debug');
		...
		$html .= '<div id="main-debug" class="profiler">';

		$html .= '<h1>'.RText::_('PLG_DEBUG_TITLE').'</h1>';
	 ...
	 *
	 * @param   string  $element    The element to display
	 * @param   array   $errors  Errors occured during execution
	 *
	 * @return string
	 */
	protected function display($element, array $errors = array())
	{
		$title = RText::_('PLG_DEBUG_'.strtoupper($element));
		...
		$fncName = 'display'.ucfirst(str_replace('_', '', $element));
		...
		$js = "toggleContainer('dbgContainer".$element."');";
		...
		$html .= '<div '.$style.' class="dbgContainer" id="dbgContainer'.$element.'">';
		$html .= $this->$fncName();
		$html .= '</div>';
	...
	protected function displayQueries()
	{
		...
		$html .= '<h4>'.RText::sprintf('PLG_DEBUG_QUERIES_LOGGED',  $db->getCount()).'</h4>';
		...
		$html .= '<h4>'.RText::sprintf('PLG_DEBUG_QUERY_TYPES_LOGGED', $totalQueryTypes) . '</h4>';

		if ($totalSelectQueryTypes)
		{
			$html .= '<h5>'.RText::sprintf('PLG_DEBUG_SELECT_QUERIES').'</h5>';
			...
			{
				$html .= '<li><code>'.RText::sprintf('PLG_DEBUG_QUERY_TYPE_AND_OCCURRENCES'
		...
		{
			$html .= '<h5>'.RText::sprintf('PLG_DEBUG_OTHER_QUERIES').'</h5>';
	...
	protected function displayLanguageFilesLoaded()
	{
		...
		foreach (JFactory::getLanguage()->getPaths() as $extension => $files)
		{
			foreach ($files as $file => $status)
			{
				...
				$html .= ($status)
				? RText::_('PLG_DEBUG_LANG_LOADED')
				: RText::_('PLG_DEBUG_LANG_NOT_LOADED');
	...
	protected function displayUntranslatedStrings()
	{
		...
		{
			$html .= "\n\n# ".($file ? $this->formatLink($file) : RText::_('PLG_DEBUG_UNKNOWN_FILE'))."\n\n";
			$html .= implode("\n", $keys);
		}
	...
	/**
	 * Replaces the Retina root with "JROOT" to improve readability.
	...
	{
		$link = str_replace(RPATH_ROOT, 'JROOT', $file);
		$link .=($line) ? ':'.$line : '';
