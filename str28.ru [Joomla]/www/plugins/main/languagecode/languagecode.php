 ...
 */
class plgmainLanguagecode extends JPlugin
{
	/**
	...
	public function onContentPrepareForm($form, $data)
	{
		...
		{
			$this->_subject->setError('RERROR_NOT_A_FORM');
			return false;
		}
		...
		if ($form->getName() != 'com_plugins.plugin'
			|| isset($data->name) && $data->name != 'plg_main_languagecode'
			|| empty($data) && !$app->getUserState('plg_main_language_code.edit')
		)
		{
			return true;
		}
		...
		$app->setUserState('plg_main_language_code.edit', $data->name == 'plg_main_languagecode');

		// Get site languages
		$languages = JLanguage::getKnownLanguages(RPATH_SITE);
...
<form>
	<fields name="params">
		<fieldset
			name="languagecode"
			label="PLG_main_LANGUAGECODE_FIELDSET_LABEL"
			description="PLG_main_LANGUAGECODE_FIELDSET_DESC"
		>
			<field
				name="'.strtolower($tag).'"
				type="text"
				description="' . htmlspecialchars(RText::sprintf('PLG_main_LANGUAGECODE_FIELD_DESC', $language['name']), ENT_COMPAT, 'UTF-8') . '"
