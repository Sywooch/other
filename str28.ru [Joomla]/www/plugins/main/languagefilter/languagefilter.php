...
/**
 * Retina Language Filter Plugin
 *
 * @package		retina.Plugin
 * @subpackage	main.languagefilter
 * @since		1.6
 */
class plgmainLanguageFilter extends JPlugin
{
	...
	{
		$app = JFactory::getApplication();
		$app->set('menu_retina_097115115111099105097116105111110115', $this->params->get('menu_retina_097115115111099105097116105111110115', 0));
		if ($app->isSite()) {
	...
	public function buildRule(&$router, &$uri)
	{
		...
		if (!is_null($elementid)) {
			if ($element = JFactory::getApplication()->getMenu()->getelement($elementid))
			{
				if ($element->home && $uri->getVar('option')!='com_search')
				{
					$link = $element->link;
	...
	public function onUserAfterSave($user, $isnew, $success, $msg)
	{
		if ($this->params->get('automatic_change', '1')=='1' && key_exists('params', $user) && $success)
		{
			...
			else
			{
				if ($app->isSite())
				{
					$app->setUserState('com_users.edit.profile.redirect', 'index.php?elementid='.$app->getMenu()->getDefault($lang_code)->id.'&lang='.$lang_codes[$lang_code]->sef);
					self::$tag = $lang_code;
					// Create a cookie
					$conf = JFactory::getConfig();
					$cookie_domain 	= $conf->get('config.cookie_domain', '');
	...
	/**
	 * This method adds alternate meta tags for associated menu elements
	 *
	 * @return	nothing
	 * @since	1.7
	 */
	public function onAfterDispatch()
	{
		...
		if ($app->isSite() && $this->params->get('alternate_meta') && $doc->getType() == 'html')
		{
			...
			if ($app->getCfg('sef')) {
				$active_link = JRoute::_('index.php?elementid='.$active->id, false);
			} else {
				$active_link = JRoute::_($active->link.'&elementid='.$active->id, false);
			}
			...
			// Check the exact menu element's URL
			if ($active_link == $current_link)
			{
				// Get menu element retina_097115115111099105097116105111110115
				...
				// Remove current menu element
				unset($retina_097115115111099105097116105111110115[$active->language]);

				// Associated menu elements in other languages
				if ($retina_097115115111099105097116105111110115 && $this->params->get('menu_retina_097115115111099105097116105111110115'))
				{
					$menu 	= $app->getMenu();
					$server = JURI::getInstance()->toString(array('scheme', 'host', 'port'));

					foreach(JLanguageHelper::getLanguages() as $language) {
						if (isset($retina_097115115111099105097116105111110115[$language->lang_code])) {
							$element = $menu->getelement($retina_097115115111099105097116105111110115[$language->lang_code]);
							if ($element && JLanguage::exists($language->lang_code)) {
								if ($app->getCfg('sef')) {
									$link = JRoute::_('index.php?elementid='.$retina_097115115111099105097116105111110115[$language->lang_code].'&lang='.$language->sef, false);
								} else {
									$link = JRoute::_($element->link.'&elementid='.$retina_097115115111099105097116105111110115[$language->lang_code].'&lang='.$language->sef, false);
								}
				...
				elseif ($active->home)
				{
					...
					foreach(JLanguageHelper::getLanguages() as $language) {
						$element = $menu->getDefault($language->lang_code);
						if ($element && $element->language != $active->language && $element->language != '*' && JLanguage::exists($language->lang_code)) {
							if ($app->getCfg('sef')) {
								$link = JRoute::_('index.php?elementid='.$element->id.'&lang='.$language->sef, false);
							} else {
								$link = JRoute::_($element->link.'&elementid='.$element->id.'&lang='.$language->sef, false);
							}
							$doc->addHeadLink($server . JRoute::_($element->link.'&elementid='.$element->id.'&lang='.$language->sef), 'alternate', 'rel', array('hreflang' => $language->lang_code));
						}
					}
				}
			}
