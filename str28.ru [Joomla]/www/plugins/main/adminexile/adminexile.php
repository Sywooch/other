...
*
* AdminExile version 1.2 for retina 1.6.x devloped by RicheyWeb
*
*/
// no direct access 2
defined( '_REXEC' ) or die( 'Restricted access' );

jimport( 'retina.plugin.plugin' );

/**
 * AdminExile main plugin
 */
class plgmainAdminExile extends JPlugin
{
	/**
	 * Constructor
	 *
	 ...
	 * @since	1.0
	 */
	function plgmainAdminExile( &$subject, $config )
	{
	...
	function onAfterInitialise()
	{
		...
		// this plugin is meant for admin
		if($app->isAdmin()) {
		...
		if($app->getUserState("plg_sys_adminexile.$key",false)) {
			...
			if(array_key_exists($key,JRequest::get('GET'))) {
				...
				header("Location: ".JURI::root()."admin");
				return true;
			} else {
