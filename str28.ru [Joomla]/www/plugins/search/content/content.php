...
require_once RPATH_SITE.'/components/com_content/router.php';
...
class plgSearchContent extends JPlugin
{
	...
	function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		require_once RPATH_SITE.'/admin/components/com_search/helpers/search.php';
		...
		if ($sContent && $limit > 0)
		{
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = ContentHelperRoute::getArticleRoute($element->slug, $element->catslug);
				}
			}
			$rows[] = $list;
		}
		...
		{
			$searchArchived = RText::_('JARCHIVED');
			...
			// find an elementid for archived to use if there isn't another one
			$element	= $app->getMenu()->getelements('link', 'index.php?option=com_content&view=archive', true);
			$elementid = isset($element) ? '&elementid='.$element->id : '';

			if (isset($list3))
			{
				foreach($list3 as $key => $element)
				{
					$date = JFactory::getDate($element->created);

					$created_month	= $date->format("n");
					$created_year	= $date->format("Y");

					$list3[$key]->href	= JRoute::_('index.php?option=com_content&view=archive&year='.$created_year.'&month='.$created_month.$elementid);
				}
			}
