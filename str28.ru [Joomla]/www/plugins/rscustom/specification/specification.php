...
class plgrsCustomSpecification extends rsCustomPlugin {
...
// 		self::$_this = $this;
	...
	function getTableSQLFields() {
	    ...
	    'retinashop_product_id' => 'int(11) UNSIGNED DEFAULT NULL',
	    'retinashop_custom_id' => 'int(11) UNSIGNED DEFAULT NULL',
	...
	*
	*/
	public function plgrsSelectSearchableCustom(&$selectList,&$searchCustomValues,$retinashop_custom_id)
	{
		$db =JFactory::getDBO();
		$db->setQuery('SELECT `retinashop_custom_id`, `custom_title` FROM `#__retinashop_customs` WHERE `custom_element` ="'.$this->_name.'"');
		if ($this->selectList = $db->loadAssocList() ) {
			//rsdebug('$this->selectedPlugin',$this->selectedPlugin);
			foreach ($this->selectList as $selected_custom_id) {
				if ($retinashop_custom_id == $selected_custom_id['retinashop_custom_id']) {
	...
	*/
	public function plgrsAddToSearch(&$where,&$PluginJoinTables,$custom_id)
	{
		if ($keyword = rsRequest::uword('custom_specification_name1', null, ' ')) {
			$db = JFactory::getDBO();
	...
	function plgrsOnProductEdit($field, $product_id, &$row,&$retValue) {
		...
		// $this->tableFields = array ( 'id', 'retinashop_custom_id', 'custom_specification_default1', 'custom_specification_default2' );
		$this->getCustomParams($field);
		$this->getPluginCustomData($field, $product_id);

		// 		$data = $this->getrsPluginMethod($field->retinashop_custom_id);
		// 		rsTable::bindParameterable($field,$this->_xParams,$this->_varsToPushParam);
		...
		$html .='<input type="hidden" value="'.$this->retinashop_custom_id.'" name="plugin_param['.$row.']['.$this->_name.'][retinashop_custom_id]">';
		$html .='</div>';
		// 		$field->display =
		$retValue .= $html  ;
		$row++;
	...
	function plgrsOnDisplayProductFE($product,&$idx,&$group) {
		...
		$this->getPluginCustomData($group, $product->retinashop_product_id);
	...
	function plgrsOnStoreProduct($data,$plugin_param){
		// $this->tableFields = array ( 'id', 'retinashop_product_id', 'retinashop_custom_id', 'custom_specification_default1', 'custom_specification_default2' );
	 ...
	 */
	protected function plgrsOnStoreInstallPluginTable($psType) {
