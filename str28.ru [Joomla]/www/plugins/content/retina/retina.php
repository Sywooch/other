 ...
 * @subpackage	Content.retina
 * @since		1.6
 */
class plgContentretina extends JPlugin
{
	/**
	...
	public function onContentAfterSave($context, &$article, $isNew)
	{
		...
		// Messaging for new elements
		JModel::addIncludePath(RPATH_admin.'/components/com_messages/models', 'MessagesModel');
		JTable::addIncludePath(RPATH_admin.'/components/com_messages/tables');

		$db = JFactory::getDbo();
		$db->setQuery('SELECT id FROM #__users WHERE sendEmail = 1');
		$users = (array) $db->loadColumn();

		$default_language = JComponentHelper::getParams('com_languages')->get('admin');
	...
	/**
	 * Don't allow categories to be deleted if they contain elements or subcategories with elements
	 *
	...
	public function onContentBeforeDelete($context, $data)
	{
		...
		if (isset($tableInfo[$extension]))
		{
			...
			// See if this category has any content elements
			$count = $this->_countelementsInCategory($table, $data->get('id'));
			...
			{
				// Show error if elements are found in the category
				if ($count > 0 ) {
					$msg = RText::sprintf('COM_CATEGORIES_DELETE_NOT_ALLOWED', $data->get('title')) .
					RText::plural('COM_CATEGORIES_N_elementS_ASSIGNED', $count);
					JError::raiseWarning(403, $msg);
					$result = false;
				}
				// Check for elements in any child categories (if it is a leaf, there are no child categories)
				if (!$data->isLeaf()) {
					$count = $this->_countelementsInChildren($table, $data->get('id'), $data);
					if ($count === false)
					{
						$result = false;
					}
					...
					{
						$msg = RText::sprintf('COM_CATEGORIES_DELETE_NOT_ALLOWED', $data->get('title')) .
						RText::plural('COM_CATEGORIES_HAS_SUBCATEGORY_elementS', $count);
						JError::raiseWarning(403, $msg);
						$result = false;
					}
				}
	...
	/**
	 * Get count of elements in a category
	 *
	 * @param	string	table name of component table (column is catid)
	 * @param	int		id of the category to check
	 * @return	mixed	count of elements found or false if db error
	 * @since	1.6
	 */
	private function _countelementsInCategory($table, $catid)
	{
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		// Count the elements in this category
	...
	/**
	 * Get count of elements in a category's child categories
	 *
	 * @param	string	table name of component table (column is catid)
	 * @param	int		id of the category to check
	 * @return	mixed	count of elements found or false if db error
	 * @since	1.6
	 */
	private function _countelementsInChildren($table, $catid, $data)
	{
		$db = JFactory::getDbo();
