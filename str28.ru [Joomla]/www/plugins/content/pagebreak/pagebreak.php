...
jimport('retina.html.pagination');
jimport('retina.html.html.sliders');
jimport('retina.html.html.tabs');

/**
 * Page break plugin
 *
 * <b>Usage:</b>
 * <code><hr class="main-pagebreak" /></code>
 * <code><hr class="main-pagebreak" title="The page title" /></code>
 * or
 * <code><hr class="main-pagebreak" alt="The first page" /></code>
 * or
 * <code><hr class="main-pagebreak" title="The page title" alt="The first page" /></code>
 * or
 * <code><hr class="main-pagebreak" alt="The first page" title="The page title" /></code>
 *
...
class plgContentPagebreak extends JPlugin
{
	...
	public function onContentPrepare($context, &$row, &$params, $page = 0)
	{
		...
		$regex = '#<hr(.*)class="main-pagebreak"(.*)\/>#iU';
		...
		if (JString::strpos($row->text, 'class="main-pagebreak') === false) {
			return true;
		}
		...
		if ($n > 1) {
			...
			if ($style == 'pages') {
				...
				// Page text.
				$text[$page] = str_replace('<hr id="main-readmore" />', '', $text[$page]);
				$row->text .= $text[$page];
				// $row->text .= '<br />';
				...
				foreach ($text as $key => $subtext) {
					...
					if ($key >= 1) {
						...
						if (isset($match['alt'])) {
							...
							$title	= RText::sprintf('PLG_CONTENT_PAGEBREAK_PAGE_NUM', $key);
						}
	...
	{
		$heading = isset($row->title) ? $row->title : RText::_('PLG_CONTENT_PAGEBREAK_NO_TITLE');
		...
		{
			$headingtext= RText::_('PLG_CONTENT_PAGEBREAK_ARTICLE_INDEX');
		...
		foreach ($matches as $bot) {
			...
			if (@$bot[0]) {
				...
				if (@$attrs2['alt']) {
					...
					$title	= RText::sprintf('PLG_CONTENT_PAGEBREAK_PAGE_NUM', $i);
				}
			} else {
				$title	= RText::sprintf('PLG_CONTENT_PAGEBREAK_PAGE_NUM', $i);
			}
		...
		if ($this->params->get('showall')) {
			...
			<li>
					...
					<a href="'. $link .'" class="'.$class.'">'
					. RText::_('PLG_CONTENT_PAGEBREAK_ALL_PAGES') .
					'</a>
	...
	{
		$pnSpace = '';
		if (RText::_('RGLOBAL_LT') || RText::_('RGLOBAL_LT')) {
			$pnSpace = ' ';
		}
		...
		if ($page < $n-1) {
			...
			// Next >>
			$next = '<a href="'. $link_next .'">' . RText::_('JNEXT') . $pnSpace . RText::_('RGLOBAL_GT') . RText::_('RGLOBAL_GT') .'</a>';
		} else {
			$next = RText::_('JNEXT');
		}
		...
		if ($page > 0) {
			...
			// << Prev
			$prev = '<a href="'. $link_prev .'">'. RText::_('RGLOBAL_LT') . RText::_('RGLOBAL_LT') . $pnSpace . RText::_('JPREV') .'</a>';
		} else {
			$prev = RText::_('JPREV');
		}
