...
class MailtoController extends JController
{
	...
	function send()
	{
		...
		if ($timeout == 0 || time() - $timeout < 20) {
			JError::raiseNotice(500, RText:: _ ('COM_MAILTO_EMAIL_NOT_SENT'));
			return $this->mailto();
		}
		...
		$subject_default	= RText::sprintf('COM_MAILTO_SENT_BY', $sender);
		...
		{
			$error	= RText::sprintf('COM_MAILTO_EMAIL_INVALID', $email);
		...
		{
			$error	= RText::sprintf('COM_MAILTO_EMAIL_INVALID', $from);
			JError::raiseWarning(0, $error);
		}
		...
		$msg	= RText :: _('COM_MAILTO_EMAIL_MSG');
