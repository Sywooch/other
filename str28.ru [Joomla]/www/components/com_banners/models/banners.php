...
/**
 * Banners model for the retina Banners component.
 *
 * @package		Retina.Site
 * @subpackage	com_banners
 * @since		1.6
...
class BannersModelBanners extends JModelList
{
	...
	public function getelements()
	{
		if (!isset($this->cache['elements'])) {
			$this->cache['elements'] = parent::getelements();

			foreach ($this->cache['elements'] as &$element)
			{
				$parameters = new JRegistry;
				$parameters->loadString($element->params);
	...
	public function impress()
	{
		...
		foreach ($elements as $element)
		{
			...
			// track impressions
			$trackImpressions = $element->track_impressions;
			if ($trackImpressions < 0 && $element->cid) {
				$trackImpressions = $element->client_track_impressions;
			}
