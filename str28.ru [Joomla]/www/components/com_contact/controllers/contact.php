...
class ContactControllerContact extends JControllerForm
{
	...
	public function submit()
	{
		...
		if($params->get('validate_session', 0)) {
			if(JFactory::getSession()->getState() != 'active'){
				JError::raiseWarning(403, RText::_('COM_CONTACT_SESSION_INVALID'));
		...
		if (!JError::isError($sent)) {
			$msg = RText::_('COM_CONTACT_EMAIL_THANKS');
		}
	...
	private function _sendEmail($data, $contact)
	{
			...
			$copytext 	= RText::sprintf('COM_CONTACT_COPYTEXT_OF', $contact->name, $sitename);
			...
			// Prepare email body
			$prefix = RText::sprintf('COM_CONTACT_ENQUIRY_TEXT', JURI::base());
			$body	= $prefix."\n".$name.' <'.$email.'>'."\r\n\r\n".stripslashes($body);
			...
			if ( array_key_exists('contact_email_copy', $data)  ) {
				$copytext		= RText::sprintf('COM_CONTACT_COPYTEXT_OF', $contact->name, $sitename);
				$copytext		.= "\r\n\r\n".$body;
				$copysubject	= RText::sprintf('COM_CONTACT_COPYSUBJECT_OF', $subject);
