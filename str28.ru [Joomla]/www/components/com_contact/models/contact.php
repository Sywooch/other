...
class ContactModelContact extends JModelForm
{
	 ...
	 * @since	1.6
	 */
	protected $view_element = 'contact';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_contact.contact', 'contact', array('control' => 'rinputform', 'load_data' => true));
		if (empty($form)) {
			return false;
		}
	...
	public function &getelement($pk = null)
	{
		...
		if (!isset($this->_element[$pk])) {
			try
			{
				...
				$query->select($this->getState('element.select', 'a.*') . ','.$case_when.','.$case_when1);
				...
				if (empty($data)) {
					throw new JException(RText::_('COM_CONTACT_ERROR_CONTACT_NOT_FOUND'), 404);
				}

				// Check for published state if filter set.
				if (((is_numeric($published)) || (is_numeric($archived))) && (($data->published != $published) && ($data->published != $archived)))
				{
					JError::raiseError(404, RText::_('COM_CONTACT_ERROR_CONTACT_NOT_FOUND'));
				}
				...
				$this->_element[$pk] = $data;
			}
			catch (JException $e)
			{
				$this->setError($e);
				$this->_element[$pk] = false;
			}
		...
		if ($this->_element[$pk])
		{
			if ($extendedData = $this->getContactQuery($pk)) {
				$this->_element[$pk]->articles = $extendedData->articles;
				$this->_element[$pk]->profile = $extendedData->profile;
			}
		}
  		return $this->_element[$pk];
	...
	protected function getContactQuery($pk = null)
	{
		...
		if ($pk) {
			...
			if ($result) {
				...
				require_once RPATH_admin.'/components/com_users/models/user.php';
				$userModel = JModel::getInstance('User', 'UsersModel', array('ignore_request' => true));
					$data = $userModel->getelement((int)$result->user_id);
