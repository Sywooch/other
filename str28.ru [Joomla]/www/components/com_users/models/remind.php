...
class UsersModelRemind extends JModelForm
{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_users.remind', 'remind', array('control' => 'rinputform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	public function processRemindRequest($data)
	{
		...
		$link	= 'index.php?option=com_users&view=login'.$elementid;
		...
		// Check for an error.
		if ($return !== true) {
			$this->setError(RText::_('COM_USERS_MAIL_FAILED'), 500);
			return false;
		}
