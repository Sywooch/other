...
/**
 * Êëàññ ìîäåëè ïğîôèëåé äëÿ ïîëüçîâàòåëÿ.
 *
 * @package		Retina.Site
...
class UsersModelProfile extends JModelForm
{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_users.profile', 'profile', array('control' => 'rinputform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	/**
	 * Ìåòîä äëÿ ñîõğàíåíèÿ äàííûõ ôîğìû.
	 *
	 * @param	array		Äàííûå ôîğìû.
	 * @return	mixed		Èäåíòèôèêàòîğ ïîëüçîâàòåëÿ â ñëó÷àå óñïåõà, false â ñëó÷àå âîçíèêíîâåíèÿ îøèáêè.
	 * @since	1.6
	 */
	public function save($data)
	{
		...
		// Bind the data.
		if (!$user->bind($data)) {
			$this->setError(RText::sprintf('USERS PROFILE BIND FAILED', $user->getError()));
			return false;
		}
