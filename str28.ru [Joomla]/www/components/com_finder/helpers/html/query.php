...
abstract class JHtmlQuery
{
	...
	public static function explained(FinderIndexerQuery $query)
	{
		...
		foreach ($query->included as $token)
		{
			...
			{
				$parts[] = '<span class="query-required">' . RText::sprintf('COM_FINDER_QUERY_TOKEN_REQUIRED', $token->term) . '</span>';
			}
		}
		...
		foreach ($query->included as $token)
		{
			if (!$token->required && (!isset($token->derived) || $token->derived == false))
			{
				$parts[] = '<span class="query-optional">' . RText::sprintf('COM_FINDER_QUERY_TOKEN_OPTIONAL', $token->term) . '</span>';
			}
		}
		...
		foreach ($query->excluded as $token)
		{
			...
			{
				$parts[] = '<span class="query-excluded">' . RText::sprintf('COM_FINDER_QUERY_TOKEN_EXCLUDED', $token->term) . '</span>';
			}
		}
		...
		if ($query->date1)
		{
			$date = JFactory::getDate($query->date1)->format(RText::_('DATE_FORMAT_LC'));
			$parts[] = '<span class="query-start-date">' . RText::sprintf('COM_FINDER_QUERY_START_DATE', $query->when1, $date) . '</span>';
		}

		// Process the end date.
		if ($query->date2)
		{
			$date = JFactory::getDate($query->date2)->format(RText::_('DATE_FORMAT_LC'));
			$parts[] = '<span class="query-end-date">' . RText::sprintf('COM_FINDER_QUERY_END_DATE', $query->when2, $date) . '</span>';
		}
		...
		if (!empty($query->filters))
		{
			...
			foreach ($query->filters as $branch => $nodes)
			{
				...
				foreach ($nodes as $title => $id)
				{
					...
					$parts[] = '<span class="query-taxonomy">' . RText::sprintf('COM_FINDER_QUERY_TAXONOMY_NODE', $title, $branch) . '</span>';
				}
			}
		}

		// Build the interpreted query.
		return count($parts) ? RText::sprintf('COM_FINDER_QUERY_TOKEN_INTERPRETED', implode(RText::_('COM_FINDER_QUERY_TOKEN_GLUE'), $parts)) : null;
	}
