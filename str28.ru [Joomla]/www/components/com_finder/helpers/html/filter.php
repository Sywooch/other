...
abstract class JHtmlFilter
{
	...
	public static function slider($options = array())
	{
		...
		$html .= RText::_('COM_FINDER_FILTER_SELECT_ALL_LABEL');
		$html .= '</label>';
		$html .= '</dt>';
		...
		foreach ($branches as $bk => $bv)
		{
			...
			$html .= RText::sprintf('COM_FINDER_FILTER_BRANCH_LABEL', RText::_(FinderHelperLanguage::branchSingular($bv->title)));
			$html .= '</label>';
			$html .= '</dd>';
		}
			...
			foreach ($branches as $bk => $bv)
			{
				...
				foreach($branches[$bk]->nodes as $node_id => $node) {
					...
					if ($language->hasKey($key)) {
						$branches[$bk]->nodes[$node_id]->title = RText::_($key);
					}
				}

				// Add the Search All option to the branch.
				array_unshift($branches[$bk]->nodes, array('id' => null, 'title' => RText::_('COM_FINDER_FILTER_SELECT_ALL_LABEL')));
			}
	...
	public static function dates($query, $options)
	{
		...
		if (!empty($showDates))
		{
			...
			$operators = array();
			$operators[] = JHtml::_('select.option', 'before', RText::_('COM_FINDER_FILTER_DATE_BEFORE'));
			$operators[] = JHtml::_('select.option', 'exact', RText::_('COM_FINDER_FILTER_DATE_EXACTLY'));
			$operators[] = JHtml::_('select.option', 'after', RText::_('COM_FINDER_FILTER_DATE_AFTER'));
