...
class FinderModelSearch extends JModelList
{
	...
	public function getResults()
	{
		...
		$query->where($db->quoteName('link_id') . ' IN (' . implode(',', array_keys($elements)) . ')');
	...
	protected function getResultsTotal()
	{
		...
		do
		{
			...
			// Iterate through the set to extract the unique elements.
			for ($i = 0, $c = count($results); $i < $c; $i++)
			{
			 ...
			 * enough elements for the page, we can stop here.
			 */
			if (empty($this->requiredTerms))
			{
				// If we need more elements and they're available, make another pass.
			 ...
			 * over the elements and remove any elements that do not match all of the
			...
			foreach ($this->requiredTerms as $token => $required)
			{
				...
				// Remove any elements that do not match the required term.
	...
	protected function getResultsData()
	{
		...
		do
		{
			 ...
			 * the unique elements to the set.
			 */
			else
			{
				// Iterate through the set to extract the unique elements.
			...
			if ($more && count($sorted) < ($this->getState('list.start') + $this->getState('list.limit')))
			{
				...
				// Merge the found elements.
				$elements = array_merge($elements, $sorted);
