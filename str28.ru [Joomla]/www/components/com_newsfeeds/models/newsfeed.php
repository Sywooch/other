 ...
 * @since		1.5
 */
class NewsfeedsModelNewsfeed extends JModelelement
{
	/**
	...
	public function &getelement($pk = null)
	{
		...
		if (!isset($this->_element[$pk])) {
			try
			{
				...
				$query->select($this->getState('element.select', 'a.*'));
				...
				if (empty($data)) {
					throw new JException(RText::_('COM_NEWSFEEDS_ERROR_FEED_NOT_FOUND'), 404);
				}

				// Check for published state if filter set.
				if (((is_numeric($published)) || (is_numeric($archived))) && (($data->published != $published) && ($data->published != $archived))) {
					JError::raiseError(404, RText::_('COM_NEWSFEEDS_ERROR_FEED_NOT_FOUND'));
				}
