...
class ContentModelArticles extends JModelList
{
	...
	function getListQuery()
	{
		...
		switch ($featured)
		{
			...
			default:
				...
				// between featured/unfeatured elements.
				break;
		}
	...
	public function getelements()
	{
		...
		foreach ($elements as &$element)
		{
			...
			$articleParams->loadString($element->attribs);

			// Unpack readmore and layout params
			$element->alternative_readmore = $articleParams->get('alternative_readmore');
			$element->layout = $articleParams->get('layout');

			$element->params = clone $this->getState('params');

			// For blogs, article params override menu element params only if menu param = 'use_article'
			// Otherwise, menu element params control the layout
			// If menu element is 'use_article' and there is no article param, use global
			if ((JRequest::getString('layout') == 'blog') || (JRequest::getString('view') == 'featured')
				...
				if (count($articleArray) > 0) {
					...
					$element->params->merge($articleParams);
				}
			}
			else {
			...
			// get display date
			switch ($element->params->get('list_show_date'))
			{
				case 'modified':
					$element->displayDate = $element->modified;
					break;

				case 'published':
					$element->displayDate = ($element->publish_up == 0) ? $element->created : $element->publish_up;
					break;

				default:
				case 'created':
					$element->displayDate = $element->created;
					break;
			}
			...
			if (!$guest) {
				$asset	= 'com_content.article.'.$element->id;

				// Check general edit permission first.
				if ($user->authorise('core.edit', $asset)) {
					$element->params->set('access-edit', true);
				}
				// Now check if edit.own is available.
				elseif (!empty($userId) && $user->authorise('core.edit.own', $asset)) {
					// Check for a valid user and that they are the owner.
					if ($userId == $element->created_by) {
						$element->params->set('access-edit', true);
					}
				}
			}
			...
			if ($access) {
				...
				$element->params->set('access-view', true);
			}
			else {
				// If no access filter is set, the layout takes some responsibility for display of limited information.
				if ($element->catid == 0 || $element->category_access === null) {
					$element->params->set('access-view', in_array($element->access, $groups));
				}
				else {
					$element->params->set('access-view', in_array($element->access, $groups) && in_array($element->category_access, $groups));
				}
			}
		}
