...
require_once RPATH_admin.'/components/com_content/models/article.php';
...
class ContentModelForm extends ContentModelArticle
{
	 ...
	 *
	 * @return	mixed	Content element data object on success, false on failure.
	 */
	public function getelement($elementId = null)
	{
		// Initialise variables.
		$elementId = (int) (!empty($elementId)) ? $elementId : $this->getState('article.id');

		// Get a row instance.
		$table = $this->getTable();

		// Attempt to load the row.
		$return = $table->load($elementId);
		...
		if (!empty($value->fulltext)) {
			$value->articletext .= '<hr id="main-readmore" />'.$value->fulltext;
		}
