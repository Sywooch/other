...
class ContentModelCategory extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Load the parameters. Merge Global and Menu element params into new object
		...
		// filter.order
		$elementid = JRequest::getInt('id', 0) . ':' . JRequest::getInt('elementid', 0);
		$orderCol = $app->getUserStateFromRequest('com_content.category.list.' . $elementid . '.filter_order', 'filter_order', '', 'string');
		if (!in_array($orderCol, $this->filter_fields)) {
			$orderCol = 'a.ordering';
		}
		$this->setState('list.ordering', $orderCol);

		$listOrder = $app->getUserStateFromRequest('com_content.category.list.' . $elementid . '.filter_order_Dir',
		...
		if ((JRequest::getCmd('layout') == 'blog') || $params->get('layout_type') == 'blog') {
			...
			$limit = $app->getUserStateFromRequest('com_content.category.list.' . $elementid . '.limit', 'limit', $params->get('display_num'));
		}
	...
	function getelements()
	{
		...
		if ($this->_articles === null && $category = $this->getCategory()) {
			...
			if ($limit >= 0) {
				$this->_articles = $model->getelements();
	...
	protected function _buildContentOrderBy()
	{
		...
		$elementid		= JRequest::getInt('id', 0) . ':' . JRequest::getInt('elementid', 0);
		$orderCol	= $app->getUserStateFromRequest('com_content.category.list.' . $elementid . '.filter_order', 'filter_order', '', 'string');
		$orderDirn	= $app->getUserStateFromRequest('com_content.category.list.' . $elementid . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
		$orderby	= ' ';
	...
	public function getCategory()
	{
		if (!is_object($this->_element)) {
			if( isset( $this->state->params ) ) {
				$params = $this->state->params;
				$options = array();
				$options['countelements'] = $params->get('show_cat_num_articles', 1) || !$params->get('show_empty_categories_cat', 0);
			}
			else {
				$options['countelements'] = 0;
			}
			...
			if (is_object($this->_element)) {
				$user	= JFactory::getUser();
				$userId	= $user->get('id');
				$asset	= 'com_content.category.'.$this->_element->id;

				// Check general create permission.
				if ($user->authorise('core.create', $asset)) {
					$this->_element->getParams()->set('access-create', true);
				}
