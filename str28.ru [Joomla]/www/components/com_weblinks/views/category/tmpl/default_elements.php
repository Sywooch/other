...
<?php if (empty($this->elements)) : ?>
	<p> <?php echo RText::_('COM_WEBLINKS_NO_WEBLINKS'); ?></p>
<?php else : ?>
...
<form action="<?php echo htmlspecialchars(JFactory::getURI()->toString()); ?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="category">
		...
		<thead><tr>
			...
			<th class="hits">
					<?php echo JHtml::_('grid.sort',  'RGLOBAL_HITS', 'hits', $listDirn, $listOrder); ?>
			</th>
			<?php endif; ?>
		</tr>
	</thead>
		...
		<?php endif; ?>
			...
			<p>
				<?php if ($this->params->get('icons') == 0) : ?>
					 <?php echo RText::_('COM_WEBLINKS_LINK'); ?>
				<?php elseif ($this->params->get('icons') == 1) : ?>
					<?php if (!$this->params->get('link_icons')) : ?>
						<?php echo JHtml::_('image', 'main/'.$this->params->get('link_icons', 'weblink.png'), RText::_('COM_WEBLINKS_LINK'), NULL, true); ?>
					<?php else: ?>
						<?php echo '<img src="'.$this->params->get('link_icons').'" alt="'.RText::_('COM_WEBLINKS_LINK').'" />'; ?>
					<?php endif; ?>
				<?php endif; ?>
				<?php
					...
					$height	= $element->params->get('height');
					if ($width == null || $height == null) {
						$width	= 600;
						$height	= 500;
					}

					switch ($element->params->get('target', $this->params->get('target')))
					{
						case 1:
							// open in a new window
							echo '<a href="'. $link .'" target="_blank" class="'. $menuclass .'" rel="nofollow">'.
								$this->escape($element->title) .'</a>';
							break;
				...
				<?php // Code to add the edit link for the weblink. ?>
						...
						<?php if ($canEdit) : ?>
							<ul class="actions">
								<li class="edit-icon">
									<?php echo JHtml::_('icon.edit', $element, $params); ?>
								</li>
							</ul>
						<?php endif; ?>
			</p>

			<?php if (($this->params->get('show_link_description')) and ($element->description !='')): ?>
				<?php echo $element->description; ?>
			<?php endif; ?>
		</td>
		<?php if ($this->params->get('show_link_hits')) : ?>
		<td class="hits">
			<?php echo $element->hits; ?>
		</td>
		<?php endif; ?>
	</tr>
	<?php endforeach; ?>
	...
	<?php /* if ($canCreate) : // TODO This is not working due to some problem in the router, I think. Ref issue #23685 ?>
		<?php echo JHtml::_('icon.create', $element, $element->params); ?>
 	<?php  endif; */ ?>
