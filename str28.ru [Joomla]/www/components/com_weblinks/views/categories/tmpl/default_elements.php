...
<?php foreach($this->elements[$this->parent->id] as $id => $element) : ?>
	...
	<li<?php echo $class; ?>>
	<?php $class = ''; ?>
		<span class="element-title"><a href="<?php echo JRoute::_(WeblinksHelperRoute::getCategoryRoute($element->id));?>">
			<?php echo $this->escape($element->title); ?></a>
		</span>
		<?php if ($this->params->get('show_subcat_desc_cat') == 1) :?>
		<?php if ($element->description) : ?>
			<div class="category-desc">
				<?php echo JHtml::_('content.prepare', $element->description, '', 'com_weblinks.categories'); ?>
			</div>
		<?php endif; ?>
        <?php endif; ?>
