...
jimport('retina.application.component.controllerform');
...
class WeblinksControllerWeblink extends JControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($recordId) {
			$categoryId = (int) $this->getModel()->getelement($recordId)->catid;
		}
	 ...
	 *
	 * @param	int		$recordId	The primary key id for the element.
	 * @param	string	$urlVar		The name of the URL variable for the id.
	 *
	 * @return	string	The arguments to append to the redirect URL.
	 * @since	1.6
	 */
	protected function getRedirectToelementAppend($recordId = null, $urlVar = null)
	{
		$append = parent::getRedirectToelementAppend($recordId, $urlVar);
	...
	public function go()
	{
		...
		// Get the model, requiring published elements
		$modelLink	= $this->getModel('Weblink', '', array('ignore_request' => true));
		$modelLink->setState('filter.published', 1);

		// Get the element
		$link	= $modelLink->getelement($id);

		// Make sure the element was found.
		if (empty($link)) {
			return JError::raiseWarning(404, RText::_('COM_WEBLINKS_ERROR_WEBLINK_NOT_FOUND'));
		}

		// Check whether element access level allows access.
		$user	= JFactory::getUser();
		...
		if ($link->url) {
			...
			return JError::raiseWarning(404, RText::_('COM_WEBLINKS_ERROR_WEBLINK_URL_INVALID'));
		}
	}
}
