...
<div class="ask-a-question-view">
	<h1><?php echo RText::_('COM_RETINASHOP_PRODUCT_RECOMMEND')  ?></h1>
	...
	<div class="form-field">
		...
		<form method="post" class="form-validate" action="<?php echo JRoute::_('index.php?option=com_retinashop&view=productdetails&retinashop_product_id='.$this->product->retinashop_product_id.'&retinashop_category_id='.$this->product->retinashop_category_id.'&tmpl=component') ; ?>" name="askform" id="askform" >
			<label><?php echo RText::_('COM_RETINASHOP_USER_FORM_EMAIL')  ?> : <input type="text" value="<?php echo $this->user->email ?>" name="email" id="email" size="30" class="validate[required,custom[email]]"/></label>
				<br />
			<label>
				<?php
				$ask_comment = RText::sprintf('COM_RETINASHOP_COMMENT', rsConfig::get('rs_asks_minimum_comment_length', 50), rsConfig::get('rs_asks_maximum_comment_length', 2000));
				echo $ask_comment;
				?>
				<br />
				...
				<div class="submit">
					<input class="highlight-button" type="submit" name="submit_ask" title="<?php echo RText::_('COM_RETINASHOP_RECOMMEND_SUBMIT')  ?>" value="<?php echo RText::_('COM_RETINASHOP_RECOMMEND_SUBMIT')  ?>" />
