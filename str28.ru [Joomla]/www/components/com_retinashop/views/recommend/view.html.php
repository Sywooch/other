...
* @author Max Milbers
*/
class retinashopViewrecommend extends rsView {
	...
	*/
	function display($tpl = null) {
		...
		/* Set the titles */
		$document->setTitle(RText::sprintf('COM_RETINASHOP_PRODUCT_DETAILS_TITLE',$product->product_name.' - '.RText::_('COM_RETINASHOP_PRODUCT_RECOMMEND')));
		...
		if ($retinashop_category_id == 0 && !empty($product)) {
			if (array_key_exists('0', $product->categories)) $retinashop_category_id = $product->categories[0];
		}
	...
	function renderMailLayout($vendor, $recipient) {
		...
		$this->comment = JRequest::getString('comment');
	 	$this->subject = RText::sprintf('COM_RETINASHOP_RECOMMEND_PRODUCT',$recipient, $this->product->product_name);
	 	parent::display();
	}
