...
echo strip_tags(RText::_('COM_RETINASHOP_MAIL_SHOPPER_YOUR_ORDER')) . "\n" . "\n";
echo strip_tags(RText::sprintf('COM_RETINASHOP_MAIL_SHOPPER_SUMMARY', $this->vendor->vendor_store_name)) . "\n" . "\n";

echo RText::sprintf('COM_RETINASHOP_MAIL_SHOPPER_CONTENT', $this->shopperName, $this->vendor->vendor_store_name, $this->orderDetails['details']['BT']->order_total, $this->orderDetails['details']['BT']->order_number, $this->orderDetails['details']['BT']->order_pass, $this->orderDetails['details']['BT']->created_on) . "\n" . "\n";

echo "\n" . strip_tags(RText::sprintf('COM_RETINASHOP_MAIL_ORDER_STATUS', $this->orderDetails['details']['BT']->order_status_name));
...
if ($this->orderDetails['history'][$nb - 1]->customer_notified && !(empty($this->orderDetails['history'][$nb - 1]->comments))) {
    ...
    echo "\n\n";
    echo RText::_('COM_RETINASHOP_MAIL_SHOPPER_YOUR_ORDER_LINK') . ' : ' . JURI::root() . 'index.php?option=com_retinashop&view=orders&layout=details&order_number=' . $this->orderDetails['details']['BT']->order_number . '&order_pass=' . $this->orderDetails['details']['BT']->order_pass . "\n";

    if (!empty($this->orderDetails['details']['BT']->customer_note)) {
	echo "\n" . strip_tags(RText::sprintf('COM_RETINASHOP_MAIL_SHOPPER_QUESTION', $this->orderDetails['details']['BT']->customer_note));
    }
    echo "\n\n";
