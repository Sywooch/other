...
<div class="productdetails-view">
		...
		<div class="width50 floatright">
			<div class="spacer-buy-area">
				...
				*/ ?>
				...
				<?php
				$rating = empty($this->rating)? RText::_('COM_RETINASHOP_UNRATED'):$this->rating->rating;
				echo RText::_('COM_RETINASHOP_RATING') . $rating;

				// Product Price
				if ($this->show_prices) { ?>
				<div style="background-color:yellow !important" 
				class="product-price" id="productPrice<?php echo $this->product->retinashop_product_id ?>">
				<?php
				...
				$url = JRoute::_(juri::root().'index.php?option=com_retinashop&view=productdetails&task=askquestion&retinashop_product_id='.$this->product->retinashop_product_id.'&retinashop_category_id='.$this->product->retinashop_category_id.'&tmpl=component'); ?>
				...
				<?php
					$link = JRoute::_(juri::root().'index.php?option=com_retinashop&view=manufacturer&retinashop_manufacturer_id='.$this->product->retinashop_manufacturer_id.'&tmpl=component');
					...
					if (strtolower(JRequest::getWord('output')) == "pdf"){
						...
						<span class="bold"><?php echo RText::_('COM_RETINASHOP_PRODUCT_DETAILS_MANUFACTURER_LBL') ?></span><a class="modal" href="<?php echo $link ?>"><?php echo $text ?></a>
				<?PHP } ?>
				</div>
				<?php } ?>
	...
	<div class="product-packaging">
		<span class="bold"><?php echo RText::_('COM_RETINASHOP_PRODUCT_PACKAGING2') ?></span>
		<br />
		<?php
		...
		if ($this->product->box) $product_packaging .= RText::_('COM_RETINASHOP_PRODUCT_PACKAGING2').$this->product->box;
...
/*	if ($this->product->related && !empty($this->product->related)) {
		...
		<div class="related-products-view">
			<h4><?php echo RText::_('COM_RETINASHOP_RELATED_PRODUCTS_HEADING') ?></h4>
					...
					<div class="product floatleft<?php echo $Relatedcellwidth . $show_vertical_separator ?>">
						<div class="spacer">
							<div>
								...
								echo JHTML::link($related->link, RText::_ ( 'COM_RETINASHOP_PRODUCT_DETAILS' ), array ('title' => $related->product_name, 'class' => 'product-details' ) ); ?>
								</div>
							</div>
						<div class="clear"></div>
						</div>
	...
	if($this->allowRating || $this->showReview) {
		...
		for ($num=0 ; $num <= $maxrating; $num++  ) {
			$title = (RText::_("rs_RATING_TITLE").' : '. $num . '/' . $maxrating) ;
			$stars[] = JHTML::image($starsPath.$num.'.gif', RText::_($num.'_STARS'), array("title" => $title) );
		} ?>
...
//					}
	...
	// else echo RText::_('COM_RETINASHOP_REVIEW_LOGIN'); // Login to write a review!
	?>
</div>