 ...
 * Handle the orders view
 */
class retinashopViewOrders extends rsView {
	...
	public function display($tpl = null)
	{
		...
		if($format=='pdf'){
			...
			$document->setTitle( RText::_('COM_RETINASHOP_ACC_ORDER_INFO') );
			$pathway->addelement(RText::_('COM_RETINASHOP_ACC_ORDER_INFO'));
		    } else {
			$document->setTitle( RText::_('COM_RETINASHOP_ORDERS_VIEW_DEFAULT_TITLE') );
			$pathway->addelement(RText::_('COM_RETINASHOP_ORDERS_VIEW_DEFAULT_TITLE'));
		    }
		}
...
// 			if(!empty($cuid)){
				$order_list_link = JRoute::_('index.php?option=com_retinashop&view=orders&layout=list');
// 			} else {
// 				$order_list_link = false;
// 				$order_list_link = JRoute::_('index.php?option=com_retinashop&view=orders');;
// 			}
			...
			if(empty($cuid)){
				...
				if ($orderPass = JRequest::getString('order_pass',false)){
					...
					if(empty($orderId)){
						echo RText::_('COM_RETINASHOP_RESTRICTED_ACCESS');
						return;
					}
			...
			$returnValues = $dispatcher->trigger('plgrsOnShowOrderFEShipment',array(  $orderDetails['details']['BT']->retinashop_order_id, $orderDetails['details']['BT']->retinashop_shipmentmethod_id, &$shipment_name));

			$payment_name='';
			if(!class_exists('rsPSPlugin')) require(RPATH_rs_PLUGINS.DS.'rspsplugin.php');
			JPluginHelper::importPlugin('rspayment');
			$dispatcher = JDispatcher::getInstance();
			$returnValues = $dispatcher->trigger('plgrsOnShowOrderFEPayment',array( $orderDetails['details']['BT']->retinashop_order_id, $orderDetails['details']['BT']->retinashop_paymentmethod_id,  &$payment_name));
			...
			// $pane = JPane::getInstance((__rs_ORDER_USE_SLIDERS?'Sliders':'Tabs'));
