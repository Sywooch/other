...
<div class="billto-shipto">
	<div class="width50 floatleft">
		...
		<span><span class="rsicon rs2-billto-icon"></span>
		<?php echo RText::_('COM_RETINASHOP_USER_FORM_BILLTO_LBL'); ?></span>
		<?php // Output Bill To Address ?>
		<div class="output-billto">
		<?php

		foreach($this->cart->BTaddress['fields'] as $element){
		...
		<a class="details" href="<?php echo JRoute::_('index.php?option=com_retinashop&view=user&task=editaddresscart&addrtype=BT',$this->useXHTML,$this->useSSL) ?>">
		<?php echo RText::_('COM_RETINASHOP_USER_FORM_EDIT_BILLTO_LBL'); ?>
		</a>
	...
	<div class="width50 floatleft">
		...
		<span><span class="rsicon rs2-shipto-icon"></span>
		<?php echo RText::_('COM_RETINASHOP_USER_FORM_SHIPTO_LBL'); ?></span>
		<?php // Output Bill To Address ?>
		<div class="output-shipto">
		<?php
		if(empty($this->cart->STaddress['fields'])){
			echo RText::sprintf('COM_RETINASHOP_USER_FORM_EDIT_BILLTO_EXPLAIN',RText::_('COM_RETINASHOP_USER_FORM_ADD_SHIPTO_LBL') );
		} else {
			if(!class_exists('rsHtml'))require(RPATH_rs_admin.DS.'helpers'.DS.'html.php');
			echo RText::_('COM_RETINASHOP_USER_FORM_ST_SAME_AS_BT'). rsHtml::checkbox('STsameAsBT',$this->cart->STsameAsBT).'<br />';
			foreach($this->cart->STaddress['fields'] as $element){
				if(!empty($element['value'])){ ?>
					<!-- <span class="titles"><?php echo $element['title'] ?></span> -->
					<?php
					if ($element['name'] == 'first_name' || $element['name'] == 'middle_name' || $element['name'] == 'zip') { ?>
						<span class="values<?php echo '-'.$element['name'] ?>" ><?php echo $this->escape($element['value']) ?></span>
					<?php } else { ?>
						<span class="values" ><?php echo $this->escape($element['value']) ?></span>
						<br class="clear" />
					<?php
					}
				}
		...
		<a class="details" href="<?php echo JRoute::_('index.php?option=com_retinashop&view=user&task=editaddresscart&addrtype=ST&cid[]='.$this->cart->lists['current_id'],$this->useXHTML,$this->useSSL) ?>">
		<?php echo RText::_('COM_RETINASHOP_USER_FORM_ADD_SHIPTO_LBL'); ?>
		</a>
...
<fieldset>
	<table
		...
		width="100%">
		<tr>
			<th align="left"><?php echo RText::_('COM_RETINASHOP_CART_NAME') ?></th>
			<th align="left"><?php echo RText::_('COM_RETINASHOP_CART_SKU') ?></th>
			<th
				align="center"
				width="60px"><?php echo RText::_('COM_RETINASHOP_CART_PRICE') ?></th>
			<th
				align="right"
				width="140px"><?php echo RText::_('COM_RETINASHOP_CART_QUANTITY') ?>
				/ <?php echo RText::_('COM_RETINASHOP_CART_ACTION') ?></th>


                                        <?php if ( rsConfig::get('show_tax')) { ?>
                                <th align="right" width="60px"><?php  echo "<span  class='priceColor2'>".RText::_('COM_RETINASHOP_CART_SUBTOTAL_TAX_AMOUNT') ?></th>
				<?php } ?>
                                <th align="right" width="60px"><?php echo "<span  class='priceColor2'>".RText::_('COM_RETINASHOP_CART_SUBTOTAL_DISCOUNT_AMOUNT') ?></th>
				<th align="right" width="70px"><?php echo RText::_('COM_RETINASHOP_CART_TOTAL') ?></th>
			</tr>
		...
		<?php
		$i=1;
// 		rsdebug('$this->cart->products',$this->cart->products);
		foreach( $this->cart->products as $pkey =>$prow ) { ?>
			<tr valign="top" class="sectiontableentry<?php echo $i ?>">
				<td align="left" >
					<?php if ( $prow->retinashop_media_id) {  ?>
				...
				<td align="center" >
				<?php
// 					rsdebug('$this->cart->pricesUnformatted[$pkey]',$this->cart->pricesUnformatted[$pkey]['priceBeforeTax']);
...
// 					echo $prow->salesPrice ;
				...
				<input type="text" title="<?php echo  RText::_('COM_RETINASHOP_CART_UPDATE') ?>" class="inputbox" size="3" maxlength="4" name="quantity" value="<?php echo $prow->quantity ?>" />
				<input type="hidden" name="view" value="cart" />
				<input type="hidden" name="task" value="update" />
				<input type="hidden" name="cart_retinashop_product_id" value="<?php echo $prow->cart_element_id  ?>" />
				<input type="submit" class="rsicon rs2-add_quantity_cart" name="update" title="<?php echo  RText::_('COM_RETINASHOP_CART_UPDATE') ?>" align="middle" value=" "/>
			  </form>
					<a class="rsicon rs2-remove_from_cart" title="<?php echo RText::_('COM_RETINASHOP_CART_DELETE') ?>" align="middle" href="<?php echo JRoute::_('index.php?option=com_retinashop&view=cart&task=delete&cart_retinashop_product_id='.$prow->cart_element_id  ) ?>"> </a>
				</td>
				...
				<?php
				if (rsConfig::get('checkout_show_origprice',1) && !empty($this->cart->pricesUnformatted[$pkey]['basePriceWithTax']) && $this->cart->pricesUnformatted[$pkey]['basePriceWithTax'] != $this->cart->pricesUnformatted[$pkey]['salesPrice'] ) {
		...
		<!--Begin of SubTotal, Tax, Shipment, Coupon Discount and Total listing -->
                  <?php if ( rsConfig::get('show_tax')) { $colspan=3; } else { $colspan=2; } ?>
		<tr>
			<td colspan="4">&nbsp;</td>
		  ...
		  <tr class="sectiontableentry1">
			<td colspan="4" align="right"><?php echo RText::_('COM_RETINASHOP_ORDER_PRINT_PRODUCT_PRICES_TOTAL'); ?></td>
			...
			<?php
		if (rsConfig::get('coupons_enable')) {
		?>
			<tr class="sectiontableentry2">
				<td colspan="4" align="left">
				    <?php if(!empty($this->layoutName) && $this->layoutName=='default') {
					   // echo JHTML::_('link', JRoute::_('index.php?view=cart&task=edit_coupon',$this->useXHTML,$this->useSSL), RText::_('COM_RETINASHOP_CART_EDIT_COUPON'));
		...
		foreach($this->cart->cartData['DBTaxRulesBill'] as $rule){ ?>
			<tr class="sectiontableentry<?php $i ?>">
				...
				<td align="right"> </td>
                                <?php } ?>
				<td align="right"><?php echo $this->currencyDisplay->createPriceDiv($rule['retinashop_calc_id'].'Diff','', $this->cart->pricesUnformatted[$rule['retinashop_calc_id'].'Diff'],false); ?></td>
				<td align="right"><?php echo $this->currencyDisplay->createPriceDiv($rule['retinashop_calc_id'].'Diff','', $this->cart->pricesUnformatted[$rule['retinashop_calc_id'].'Diff'],false); ?> </td>
			</tr>
			<?php
			if($i) $i=1; else $i=0;
	...
	<tr class="sectiontableentry1">
		...
		<?php	/*	<td colspan="2" align="right"><?php echo RText::_('COM_RETINASHOP_ORDER_PRINT_SHIPPING'); ?> </td> */?>
				<td colspan="4" align="left">
				...
				if(!empty($this->layoutName) && $this->layoutName=='default' && !$this->cart->automaticSelectedShipment  )
				    ...
				    RText::_('COM_RETINASHOP_CART_SHIPPING');
				}
				} else { ?>
                                <td colspan="4" align="left">
				<?php echo $this->cart->cartData['shipmentName']; ?>
		...
		<tr class="sectiontableentry1">
				...
				<?php echo $this->cart->cartData['paymentName']; ?>
				     <br />
				<?php if(!empty($this->layoutName) && $this->layoutName=='default') echo JHTML::_('link', JRoute::_('index.php?view=cart&task=editpayment',$this->useXHTML,$this->useSSL), $this->select_payment_text,'class=""'); else RText::_('COM_RETINASHOP_CART_PAYMENT'); ?> </td>
		  ...
		  <tr class="sectiontableentry2">
			<td colspan="4" align="right"><?php echo RText::_('COM_RETINASHOP_CART_TOTAL') ?>: </td>
			...
			?>
		       ...
		       <tr class="sectiontableentry2">
					    <td colspan="4" align="right"><?php echo RText::_('COM_RETINASHOP_CART_TOTAL_PAYMENT') ?>: </td>
