...
defined('_REXEC') or die('Restricted access for invoices');
 ...
 * @package		retinashop
 */
class retinashopControllerInvoice extends JController
{
...
// 		$view->headFooter = true;
	...
	function checkStoreInvoice($orderDetails = 0){
		...
		$path = rsConfig::get('forSale_path',0);
		if($path===0 ){
			rsError('No path set to store invoices');
			return false;
		} else {
			$path .= 'invoices'.DS;
			if(!file_exists($path)){
				rsError('Path wrong to store invoices, folder invoices does not exist '.$path);
				return false;
			} else if(!is_writable( $path )){
				rsError('Cannot store pdf, directory not writeable '.$path);
				return false;
			}
		}
		...
		$path .= 'rsinvoice_'.$invoiceNumber.'.pdf';
		...
		$this->addViewPath( RPATH_rs_SITE.DS.'views' );
		$format = 'html';
		$viewName= 'invoice';
		$view = $this->getView($viewName, $format);

		$view->addTemplatePath( RPATH_rs_SITE.DS.'views'.DS.'invoice'.DS.'tmpl' );
		...
		$pdf->SetCreator('Invoice by retinashop 2, used library tcpdf');
		$pdf->SetAuthor($view->vendor->vendor_name);

		$pdf->SetTitle(RText::_('COM_RETINASHOP_INVOICE_TITLE'));
		$pdf->SetSubject(RText::sprintf('COM_RETINASHOP_INVOICE_SUBJ',$view->vendor->vendor_store_name));
		$pdf->SetKeywords('Invoice by retinashop 2');
...
if(!file_exists(RPATH_rs_LIBRARIES.DS.'tcpdf'.DS.'tcpdf.php')){
	rsError('Controller invoice: For the pdf invoice, you must install the tcpdf library at '.RPATH_rs_LIBRARIES.DS.'tcpdf');
} else {
	if(!class_exists('TCPDF'))	require_once(RPATH_rs_LIBRARIES.DS.'tcpdf'.DS.'tcpdf.php');
