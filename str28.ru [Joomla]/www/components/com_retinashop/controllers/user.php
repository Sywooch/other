 ...
 * @package		retinashop
 */
class retinashopControllerUser extends JController
{
	...
	public function __construct()
	{
		...
		$this->useXHTML = true;
		rsdebug('retinashopControllerUser');
	 ...
	 * controll flow in the controller and not in the layout. The layout is everytime calling a standard retina task.
	 *
	 * @author Max Milbers
	 */
	...
	function saveCheckoutUser(){
		...
		$this->setRedirect( JRoute::_('index.php?option=com_retinashop&view=cart&task=checkout',$this->useXHTML,$this->useSSL), $msg );
	}
	...
	function saveCartUser(){
		...
		$msg = $this->saveData(true);
		$this->setRedirect(JRoute::_( 'index.php?option=com_retinashop&view=cart' ),$msg);
	}
	...
	function saveUser(){
		...
		$this->setRedirect( JRoute::_('index.php?option=com_retinashop&view=user'), $msg );
	}
 ...
 *
		JPluginHelper::importPlugin('rsuserfield');
		...
		$dispatcher->trigger('plgrsOnUserVerify',array(&$valid,$new));
		if( $valid == false ) {
// 			rsError('COM_RETINASHOP_CAPTCHA_CODE_WRONG','COM_RETINASHOP_CAPTCHA_CODE_WRONG');
			return false;
		}*/

// 		rsdebug('$currentUser',$currentUser);
