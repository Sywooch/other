...
*/
retina.combobox = {};
retina.combobox.transform = function(el, options)
{
	// Add the editable option to the select.
	var o = new Element('option', {'class':'custom'}).set('text', retina.RText._('ComboBoxInitString', 'type custom...'));
	o.inject(el, 'top');
	...
	el.addEvent('keypress', function(e){
		...
		if (this.options.selectedIndex == 0)
		{
			...
			{
				if (customString == retina.RText._('ComboBoxInitString', 'type custom...')) {
					customString = '';
				}
			}
			...
			if (e.key == 'backspace')
			{
				...
				if (customString == '') {
					customString = retina.RText._('ComboBoxInitString', 'type custom...');
				}
...
// Load the combobox behavior into the retina namespace when the document is ready.
window.addEvent('domready', function(){
	$$('select.combobox').each(function(el){
		retina.combobox.transform(el);
	});
});
