	...
	tinymce.create('tinymce.plugins.AutoSave', {
		...
		setupStorage : function(ed) {
			...
			tinymce.each([
				function() {
					...
					if (localStorage) {
						localStorage.setelement(testKey, testVal);

						if (localStorage.getelement(testKey) === testVal) {
							localStorage.removeelement(testKey);
				...
				function() {
					...
					if (sessionStorage) {
						sessionStorage.setelement(testKey, testVal);

						if (sessionStorage.getelement(testKey) === testVal) {
							sessionStorage.removeelement(testKey);
		...
		storeDraft : function() {
			...
			if (storage) {
				...
				if (!storage.getelement(self.key) && !editor.isDirty())
					return;
				...
				if (content.length > editor.settings.autosave_minlength) {
					...
					if (!self.storage.autoExpires)
						self.storage.setelement(self.key + "_expires", expires);

					self.storage.setelement(self.key, content);
		...
		restoreDraft : function() {
			...
			if (storage) {
				content = storage.getelement(self.key);
		...
		hasDraft : function() {
			...
			if (storage) {
				...
				exists = !!storage.getelement(self.key);
				if (exists) {
					// Storage needs autoexpire
					if (!self.storage.autoExpires) {
						expDate = new Date(storage.getelement(self.key + "_expires"));
		...
		removeDraft : function() {
			...
			if (storage) {
				...
				content = storage.getelement(key);
				storage.removeelement(key);
				storage.removeelement(key + "_expires");
