	...
	window.Media = {
		...
		moveStates : function(to_form, field) {
			...
			function parseQueryParams(str) {
				...
				if (str) {
					tinymce.each(str.split('&'), function(element) {
			...
			function setOptions(type, names) {
				var i, name, formelementName, value, list;

				if (type == data.type || type == 'global') {
					names = tinymce.explode(names);
					for (i = 0; i < names.length; i++) {
						name = names[i];
						formelementName = type == 'global' ? name : type + '_' + name;
						...
						if (list) {
							if (to_form) {
								setVal(formelementName, list[name], type == 'video' || type == 'audio' ? name : '');
							} else {
								delete list[name];

								value = getVal(formelementName);
								if ((type == 'video' || type == 'audio') && value === true)
									value = name;

								if (defaultStates[formelementName]) {
									if (value !== defaultStates[formelementName]) {
										value = "" + value;
										list[name] = value;
									}
								} else if (value) {
