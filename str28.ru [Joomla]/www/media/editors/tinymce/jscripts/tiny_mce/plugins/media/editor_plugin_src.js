 ...
 *
 * Copyright 2009, Moxiecode mains AB
...
(function() {
	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			var self = this, lookup = {}, i, y, element, name;

			function isMediaImg(node) {
				return node && node.nodeName === 'IMG' && ed.dom.hasClass(node, 'mceelementMedia');
			};
			...
			for (i = 0; i < mediaTypes.length; i++) {
				...
				for (y = 0; y < element.clsids.length; y++)
					lookup['clsid:' + element.clsids[y]] = element;

				for (y = 0; y < element.mimes.length; y++)
					lookup[element.mimes[y]] = element;

				lookup['mceelement' + name] = element;
				lookup[name.toLowerCase()] = element;
			...
			tinymce.each(ed.getParam("media_types",
				...
				extensions = tinymce.explode(element[1].toLowerCase());
				for (i = 0; i < extensions.length; i++) {
					type = lookup[element[0].toLowerCase()];
			...
			ed.onPreInit.add(function() {
				...
				ed.serializer.addNodeFilter('img', function(nodes, name, args) {
					...
					while (i--) {
						node = nodes[i];
						if ((node.attr('class') || '').indexOf('mceelementMedia') !== -1)
			...
			ed.onInit.add(function() {
				...
				if (ed.theme && ed.theme.onResolveName) {
					ed.theme.onResolveName.add(function(theme, path_object) {
						if (path_object.name === 'img' && ed.dom.hasClass(path_object.node, 'mceelementMedia'))
				...
				if (ed && ed.plugins.contextmenu) {
					ed.plugins.contextmenu.onContextMenu.add(function(plugin, menu, element) {
						if (element.nodeName === 'IMG' && element.className.indexOf('mceelementMedia') !== -1)
		...
		dataToImg : function(data, force_absolute) {
			...
			img = self.editor.dom.create('img', {
				...
				'class' : 'mceelementMedia mceelement' + self.getType(data.type).name,
		...
		/**
		 * Get type element by extension, class, clsid or mime type.
		 *
		 * @method getType
		 * @param {String} value Value to get type element by.
		 * @return {Object} Type element object or undefined.
		 */
		getType : function(value) {
			var i, values, typeelement;

			// Find type by checking the classes
			values = tinymce.explode(value, ' ');
			for (i = 0; i < values.length; i++) {
				typeelement = this.lookup[values[i]];
		...
		imgToObject : function(node, args) {
			var self = this, editor = self.editor, video, object, embed, iframe, name, value, data,
				source, sources, params, param, typeelement, i, element, mp4Source, replacement,
			...
			data = JSON.parse(data);
			typeelement = this.getType(node.attr('class'));
			...
			if (typeelement.name === 'Iframe') {
				...
				tinymce.each(rootAttributes, function(name) {
					...
					if (name == 'class' && value)
						value = value.replace(/mceelement.+ ?/g, '');
			...
			if (this.editor.settings.media_use_script) {
				...
				value.value = 'write' + typeelement.name + '(' + JSON.serialize(tinymce.extend(data.params, {
			...
			if (typeelement.name === 'Video' && data.video.sources[0]) {
				...
				if (mp4Source) {
					...
					typeelement = self.getType('flash');
				} else
					data.params.src = '';
			}

			// Add HTML5 audio element
			if (typeelement.name === 'Audio' && data.video.sources[0]) {
				// Create new object element
			...
			if (typeelement.name === 'EmbeddedAudio') {
			...
			if (data.params.src) {
				...
				for (name in data.params) {
					...
					if (name === 'src' && typeelement.name === 'WindowsMedia')
						name = 'url';
				...
				if (this.editor.getParam('media_strict', true)) {
					object.attr({
						...
						type: typeelement.mimes[0]
					});
				} else {
					object.attr({
						classid: "clsid:" + typeelement.clsids[0],
						codebase: typeelement.codebase
					});
		 ...
		 * <img class="mceelementMedia mceelementFlash" width="100" height="100" data-mce-json="{..}" />
		 *
		...
		objectToImg : function(node) {
			...
			img.attr({
				id : id,
				'class' : 'mceelementMedia mceelement' + (type || 'Flash'),
				style : style,
