	...
	tinymce.create('tinymce.plugins.SpellcheckerPlugin', {
		...
		createControl : function(n, cm) {
			...
			if (n == 'spellchecker') {
				...
				c.onRenderMenu.add(function(c, m) {
					m.add({title : 'spellchecker.langs', 'class' : 'mceMenuelementTitle'}).setDisabled(1);
					each(t.languages, function(v, k) {
						...
						o.onclick = function() {
							...
							mi.setSelected(1);
							t.selectedelement.setSelected(0);
							t.selectedelement = mi;
							t.selectedLang = v;
						};
		...
		_removeWords : function(w) {
			...
			each(dom.select('span').reverse(), function(n) {
				if (n && (dom.hasClass(n, 'mceelementHiddenSpellWord') || dom.hasClass(n, 'mceelementHidden'))) {
		...
		_markWords : function(wl) {
			...
			each(nl, function(n) {
				...
				if (rx.test(v)) {
					...
					elem = dom.create('span', {'class' : 'mceelementHidden'});
					...
					if (tinymce.isIE) {
						...
						while ((pos = v.indexOf('<mcespell>')) != -1) {
							...
							elem.appendChild(dom.create('span', {'class' : 'mceelementHiddenSpellWord'}, txt));
						}
						...
						elem.innerHTML = v.replace(rx, '$1<span class="mceelementHiddenSpellWord">$2</span>');
					}
		...
		_showMenu : function(ed, e) {
			...
			if (dom.hasClass(wordSpan, 'mceelementHiddenSpellWord')) {
				m.removeAll();
				m.add({title : 'spellchecker.wait', 'class' : 'mceMenuelementTitle'}).setDisabled(1);
				...
				t._sendRPC('getSuggestions', [t.selectedLang, dom.decode(wordSpan.innerHTML)], function(r) {
					...
					if (r.length > 0) {
						m.add({title : 'spellchecker.sug', 'class' : 'mceMenuelementTitle'}).setDisabled(1);
						each(r, function(v) {
						...
						m.add({title : 'spellchecker.no_sug', 'class' : 'mceMenuelementTitle'}).setDisabled(1);
		...
		_checkDone : function() {
			...
			each(dom.select('span'), function(n) {
				if (n && dom.hasClass(n, 'mceelementHiddenSpellWord')) {
					o = true;
					return false;
				}
			});
