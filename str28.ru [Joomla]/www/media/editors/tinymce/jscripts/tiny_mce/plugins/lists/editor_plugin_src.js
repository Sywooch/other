 ...
 *
 * Copyright 2011, Moxiecode mains AB
...
(function() {
	...
	function splitNestedLists(element, dom) {
		...
		if (nested) {
			wrapelement = dom.create('li', { style: 'list-style-type: none;'});
			dom.split(element, nested);
			dom.insertAfter(wrapelement, nested);
			wrapelement.appendChild(nested);
			wrapelement.appendChild(nested);
			element = wrapelement.previousSibling;
		}
		return element;
	}
	...
	function findelementToOperateOn(e, dom) {
		var element;
		if (!dom.is(e, 'li,ol,ul')) {
			element = dom.getParent(e, 'li');
			if (element) {
				e = element;
			}
		}
	...
	tinymce.create('tinymce.plugins.Lists', {
		init: function(ed) {
            ...
            // If we are at the end of a paragraph in a list element, pressing enter should create a new list element instead of a new paragraph.
			...
			function isOnLastListelement() {
				var li = getLi();
				var grandParent = li.parentNode.parentNode;
				var isLastelement = li.parentNode.lastChild === li;
				return isLastelement && !isNestedList(grandParent) && isEmptyListelement(li);
			}
			...
			function isInEmptyListelement() {
				return ed.selection.isCollapsed() && isEmptyListelement(getLi());
			}
			...
			function isEmptyListelement(li) {
			...
			function isEmptyIE9Li(li) {
				// only consider this to be last element if there is no list element content or that content is nbsp or space since IE9 creates these
			...
			function getListKeyState(e) {
				if (isTabInList(e)) {
					return LIST_TABBING;
				} else if (isEnterWithoutShift(e) && isOnLastListelement()) {
					return LIST_ESCAPE;
				} else if (isEnterWithoutShift(e) && isInEmptyListelement()) {
					return LIST_EMPTY_element;
			...
			function cancelDefaultEvents(ed, e) {
				...
				if (state == LIST_TABBING || state == LIST_EMPTY_element || tinymce.isGecko && state == LIST_ESCAPE) {
					Event.cancel(e);
				}
			}
			
            // Creates a new list element after the current selection's list element parent
			...
			function imageJoiningListelement(ed, e) {
				var prevSibling;
				...
				// move the caret to the end of the list element
				...
				// save a bookmark at the end of the list element
				var bookmark = ed.selection.getBookmark();

				// copy the image an its text to the list element
			...
			ed.onKeyUp.add(function(ed, e) {
				...
				} else if (state == LIST_EMPTY_element) {
					var li = getLi();
			...
			function fixListelement(parent, reference) {
				// a zero-sized non-breaking space is placed in the empty list element so that the nested list is
			...
			function fixIndentedListelementForGecko(ed, e) {
				if (isEnter(e)) {
					var li = getLi();
					if (li) {
						...
						if (grandParent && grandParent.nodeName == 'LI' && grandParent.firstChild == parent && li == parent.firstChild) {
							fixListelement(grandParent, parent);
						}
					}
				}
			}

			function fixIndentedListelementForIE8(ed, e) {
				if (isEnter(e)) {
					var li = getLi();
					if (ed.dom.select('ul li', li).length === 1) {
						var list = li.firstChild;
						fixListelement(li, list);
					}
				}
			}
			...
			ed.onKeyDown.add(imageJoiningListelement);
            ed.onKeyDown.add(createNewLi);

			if (tinymce.isGecko) {
				ed.onKeyUp.add(fixIndentedListelementForGecko);
			}
			if (tinymce.isIE8) {
				ed.onKeyUp.add(fixIndentedListelementForIE8);
			}
		...
		applyList: function(targetListType, oppositeListType) {
			...
			function convertListelementToParagraph(element) {
			...
			each(selectedBlocks, function(e) {
				e = findelementToOperateOn(e, dom);
			...
			if (hasNonList &&!hasSameType || hasOppositeType || selectedBlocks.length === 0) {
				...
				actions = {
					defaultAction: convertListelementToParagraph,
					elements: this.selectedBlocks()
				};
			}
		...
		indent: function() {
			...
			function createWrapelement(element) {
				var wrapelement = dom.create('li', { style: 'list-style-type: none;'});
				dom.insertAfter(wrapelement, element);
				return wrapelement;
			}

			function createWrapList(element) {
				var wrapelement = createWrapelement(element),
				...
				wrapelement.appendChild(wrapList);
				return wrapList;
			}
		...
		process: function(actions) {
			...
			function processElement(element) {
				...
				element = findelementToOperateOn(element, dom);
