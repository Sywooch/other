...
(function() {
	function findParentLayer(node) {
		do {
			if (node.className && node.className.indexOf('mceelementLayer') != -1) {
				return node;
			}
	...
	tinymce.create('tinymce.plugins.Layer', {
		...
		_visualAid : function(ed, e, s) {
			...
			tinymce.each(dom.select('div,p', e), function(e) {
				if (/^(absolute|relative|fixed)$/i.test(e.style.position)) {
					if (s)
						dom.addClass(e, 'mceelementVisualAid');
					else
						dom.removeClass(e, 'mceelementVisualAid');

					dom.addClass(e, 'mceelementLayer');
				}
			});
		},
		...
		_insertLayer : function() {
			...
			ed.dom.add(body, 'div', {
				...
				'class' : 'mceelementVisualAid mceelementLayer'
		...
		_toggleAbsolute : function() {
			...
			if (le) {
				if (le.style.position.toLowerCase() == "absolute") {
					...
					ed.dom.removeClass(le, 'mceelementVisualAid');
					ed.dom.removeClass(le, 'mceelementLayer');
				} else {
					if (le.style.left == "")
						le.style.left = 20 + 'px';
