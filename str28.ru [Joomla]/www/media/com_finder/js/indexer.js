...
var FinderIndexer = new Class({
	...
	handleResponse: function (json, resp) {
		try {
			...
			if (json.start) this.totalelements = json.totalelements;
			...
			try {
				...
				if (error == '') {
					error = retina.RText._('COM_FINDER_NO_ERROR_RETURNED');
				}
				document.id('finder-progress-header').set('text', retina.RText._('COM_FINDER_AN_ERROR_HAS_OCCURRED')).addClass('finder-error');
	...
	handleFailure: function (xhr) {
		...
		var header = json ? json.header : retina.RText._('COM_FINDER_AN_ERROR_HAS_OCCURRED');
		var message = json ? json.message : retina.RText._('COM_FINDER_MESSAGE_RETURNED') + ' <br />' + json
	...
	updateProgress: function (header, message) {
		this.progress = (this.offset / this.totalelements) * 100;
