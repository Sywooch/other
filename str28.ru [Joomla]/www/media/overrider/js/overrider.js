 ...
 * @since		2.5
 */
retina.overrider.refreshCache = function()
{
	var req = new Request.JSON({
		...
		onSuccess: function(r)
		{
			...
			if (r.messages)
			{
				retina.renderMessages(r.messages);
			}
		...
		{
			alert(retina.RText._('COM_LANGUAGES_VIEW_OVERRIDE_REQUEST_ERROR'));
			document.id('refresh-status').dissolve();
		}.bind(this),
 ...
 * @since		2.5
 */
retina.overrider.searchStrings = function(more)
{
	...
	if (!more)
	{
		this.states.searchstring 	= document.id('rinputform_searchstring').value;
		this.states.searchtype		= 'value';
		if (document.id('rinputform_searchtype0').checked)
		{
	...
	{
		document.id('rinputform_searchstring').addClass('invalid');
 ...
 * @since		2.5
 */
retina.overrider.insertResults = function(results)
{
	...
	Array.each(results, function (element, index) {
		var div = new Element('div', {
			'class':	'result row' + index%2,
			onclick:	'retina.overrider.selectString(' + this.states.counter + index + ');',
		});
	...
	if(!results.length)
	{
		var noresult = new Element('div', {
			html: retina.RText._('COM_LANGUAGES_VIEW_OVERRIDE_NO_RESULTS')
		});
 ...
 * @since		2.5
 */
retina.overrider.selectString = function(id)
{
	document.id('rinputform_key').value = document.id('override_key' + id).get('html');
	document.id('rinputform_override').value = document.id('override_string' + id).get('html');
