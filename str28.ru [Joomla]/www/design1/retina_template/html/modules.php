...
<h<?php echo $headerLevel; ?> class="js_heading"><span class="backh"> <span
	...
	title="<?php echo RText::_('TPL_BEEZ2_CLICK'); ?>"
	onclick="auf('module_<?php echo $module->id; ?>'); return false"
	class="opencloselink" id="link_<?php echo $module->id?>"> <span
	class="no"><img src="design1/retina_template/images/plus.png"
	alt="<?php if ($state == 1) { echo RText::_('TPL_BEEZ2_ALTOPEN');} else {echo RText::_('TPL_BEEZ2_ALTCLOSE');} ?>" />
...
function modChrome_beezTabs($module, $params, $attribs)
{
	...
	if ($modulecount == 1) {
		...
		foreach($modules as $rendermodule) {
			...
			{
			echo '<a href="#" class="unseen" onclick="nexttab(\'module_'. $rendermodule->id.'\');return false;" id="next_'.$rendermodule->id.'">'.RText::_('TPL_BEEZ2_NEXTTAB').'</a>';
			}
			echo '</div>';
		}
		$modulecount--;
