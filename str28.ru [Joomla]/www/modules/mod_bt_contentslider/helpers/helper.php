...
require_once RPATH_SITE.DS.'components'.DS.'com_content'.DS.'helpers'.DS.'route.php';
if( !defined('ThumbLoaded') )
{
	require_once RPATH_SITE.DS.'modules'.DS.'mod_bt_contentslider'.DS.'classes'.DS.'images.php';
...
class modBtContentSliderHelper {
	...
	public static function getList( &$params, $module ){
		// create thumbnail folder
	 	$thumbPath = RPATH_SITE.DS.'modules'.DS.$module->module.DS.'images'.DS;
		...
		$path = RPATH_SITE.DS.'modules'.DS.'mod_bt_contentslider'.DS.'classes'.DS.$source.".php";
		//echo $path;
		//die();
		...
		//2.Render thumb
		//3.Get List
	 	$elements = $object->setThumbPathInfo($thumbPath,$thumbUrl)
	...
	function fetchHead($params){
		...
		if(file_exists(RPATH_BASE.DS.'design1'.DS.$template.DS.'html'.DS.'mod_bt_contentslider'.DS.'css'.DS.'btcontentslider.css'))
		{
			$document->addStyleSheet(  JURI::root().'design1/'.$template.'/html/mod_bt_contentslider/css/btcontentslider.css');
		}
		else{
