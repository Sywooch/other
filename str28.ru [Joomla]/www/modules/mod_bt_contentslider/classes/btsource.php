...
abstract class BTSource {
	...
	public function renderThumb($path, $width = 280, $height = 150, $isThumb = true) {
		if ($isThumb) {
			...
			$imagSource = RPATH_SITE . DS . str_replace('/', DS, $path);
	...
	//create thumb and save link to element
	public function generateImages($element, $isThumb = true) {
		//
		$element = $this->parseImages($element);

		foreach ($this->_imagesRendered as $key => $value) {

			if ($element->{$key} && $image = $this->renderThumb($element->{$key}, $value[0], $value[1], $isThumb)) {
				$element->{$key} = $image;
