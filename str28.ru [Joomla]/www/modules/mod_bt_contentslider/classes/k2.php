...
class BtK2DataSource extends BTSource {
	 ...
	 */
	public function lookingForK2Image($element, $size = 'XL') {
		//Image
		$element->imageK2Image = '';
		if (JFile::exists(RPATH_SITE . DS . 'media' . DS . 'k2' . DS . 'elements' . DS . 'cache' . DS . md5("Image" . $element->id) . '_' . $size . '.jpg'))
			$element->imageK2Image = JURI::base() . 'media/k2/elements/cache/' . md5("Image" . $element->id) . '_' . $size . '.jpg';
		return $element;
	}
	...
	public function parseImages(&$row) {
		//Get first
		//Get k2 element image form media/k2/
	...
	/**
	 * get the list of k2 elements
	 *
	 * @param JParameter $params;
	 * @return Array
	 */
	public function getList() {
		if (!is_file(RPATH_SITE . DS . "components" . DS . "com_k2" . DS . "k2.php")) {
			return array();
		}
		...
		require_once RPATH_SITE . DS . 'modules' . DS . 'mod_bt_contentslider' . DS . 'helpers' . DS . 'route_k2.php';
		require_once(JPath::clean(RPATH_SITE . '/components/com_k2/helpers/utilities.php'));

		$query = "SELECT  a.*, c.name as category_title,
						c.id as categoryid, c.alias as categoryalias, c.params as categoryparams" . " FROM #__k2_elements as a" . " LEFT JOIN #__k2_categories c ON c.id = a.catid";
		...
		foreach ($data as $key => &$element) {
			...
			if (in_array($element->access, $user->authorisedLevels())) {
				//get link k2
				$element->link = JRoute::_(BTContentSliderK2Route::getelementRoute($element->id . ':' . $element->alias, $element->catid . ':' . $element->categoryalias, $elementid));
			}
			else {
			...
			if ($limitDescriptionBy == 'word') {
				...
				$element->description = self::substrword($element->introtext, $maxDesciption, $stringtags);

			}
			else {

				$element->description = self::substring($element->introtext, $maxDesciption, $stringtags);

			}
			//var_dump($element); die();

			$element->categoryLink = urldecode(JRoute::_(BTContentSliderK2Route::getCategoryRoute($element->catid . ':' . urlencode($element->categoryalias)), $elementid));

			//Get name author
			//If set get, else get username by userid
			if ($show_author) {
				if (!empty($element->created_by_alias)) {
					$element->author = $element->created_by_alias;
				}
				else {
					$author = &JFactory::getUser($element->created_by);
					$element->author = $author->name;
				}
				$element->authorLink = JRoute::_(BTContentSliderK2Route::getUserRoute($element->created_by));
			}
