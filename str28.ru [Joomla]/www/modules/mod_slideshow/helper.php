...
class modVtNiceSlideshowHelper{
	...
	public static function &validParams($params){
		...
		if( $param == 'false' ) $params->set('element_description', '');
	...
	private static function _makeImages($params){
		...
		$src	= JPath::clean(RPATH_BASE.'/media/mod_slideshow/design1/common');
		...
		$src	= JPath::clean(RPATH_BASE.'/media/mod_slideshow/design1/backgnd/'.$params->get('theme'));
	...
	private static function _makeCSS($params){
		...
		$path	= JPath::clean( RPATH_BASE.'/media/mod_slideshow/design1/common' );
		...
		$path	= JPath::clean( RPATH_BASE.'/media/mod_slideshow/design1/backgnd'.DS.$params->get('theme') );
		...
		$css	= str_replace( '#wowslider-container', '#slideshow58', $css );
	...
	private static function _makeScript($params){
		...
		$path	= JPath::clean( RPATH_BASE.'/media/mod_slideshow/design1/effects/'.$effect );
		switch( $effect ){
			case 'squares':
	...
	public static function addjQuery($source='local', $version='latest'){
		...
		switch($source){
			case 'local':
				JHtml::script("media/mod_slideshow/js/jquery/$version/jquery.min.js");
				break;
			case 'google':
	...
	public static function getParam($param, $position=1, $separator='|'){
		...
		// Found the separator in string
			...
			$param = ($separator = "\n") ? str_replace(array("\r\n","\r"), "\n", $param) : $param;
			$elements = explode($separator, $param);
			if ( ($position > 0) && ($position < count($elements)+1) ) return $elements[$position-1];
		}
	...
	public static function getSlider($params, $separator = "\n"){
		...
		$element_dir		= $params->get('element_dir');
		
		$links	= $params->get('element_url');
		$links	= str_replace("|", "\r\n", $links);

		$target = $params->get('element_target');
		
		$titles	= $params->get('element_title');
		$titles	= str_replace("|", "\r\n", $titles);

		$descriptions = $params->get('element_description');
		...
		foreach($elements as $i=>$path){
			...
			$element	= "<img src=\"$path\" alt=\"$title\" title=\"$title\" id=\"wows6_$i\" />";
			$element 	= (!empty($link)) ? "<a href=\"$link\" target=\"$target\">" . $element . "</a>" : $element; 
			$element 	= "<li>$element$desc</li>";

			$slider['images']	.= $element;
			
			$element	= "<a href=\"#\" title=\"$title\">";
			if (!empty($thumb)){
				$element .= "<img src=\"$thumb\" alt=\"$title\"/>";
			}
			
			$element	.= $i."</a>";
			
			$slider['bullets']	.= $element;
		}
	...
	/*
	 * Get the Paths of elements
	 */
	public static function getelements($params){

		$param	= $params->get('element_path');
		...
		if (empty($param)){
			...
			$param	= JFolder::files(RPATH_BASE.DS.'images'.DS.$param, $filter, true, true, $exclude, $excludefilter);
			foreach($param as $key=>$value){
				$value = substr($value, strlen(RPATH_BASE.DS) - strlen($value));
				$param[$key] = self::validPath($value);
			}
		}
	...
	public static function validPath($path){
		...
		$path = ltrim($path, DS);
		if (!is_file(RPATH_BASE.DS.$path)) return '';
