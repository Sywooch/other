...
require dirname(__FILE__).'/rsloader.php';

JFactory::getLanguage()->load('com_retinashop');

if(!class_exists('sj_rs_extrasliderres_helper')){
	class sj_rs_extrasliderres_helper  {
		...
		protected function _getList( $params, $module ){
			if(class_exists('rsModel')) {
				...
				for ($i = 0; $i < count($categories); $i++){
					is_object($categories[$i]) && array_push($category_ids, $categories[$i]->retinashop_category_id);
				}
				...
				$show_price   = $params->get('element_price_display', 1);
				...
				if(!empty($products)){
					foreach($products as $i => $product){
						if(array_key_exists($products[$i]->retinashop_product_id, $custom)){
								$products[$i]->link = trim($custom[$products[$i]->retinashop_product_id]->url);
							}
							$product->images =(!empty($product->images[0]->file_url_thumb) &&  file_exists($product->images[0]->file_url))?$product->images[0]->file_url:'modules/'.YTools::getModule()->module.'/retina_097115115101116115/images/nophoto.png';
						}
		...
		public function getProducts( $pids = null ){
			...
			if ( count($product_ids) ){
				...
				$productModel = new retinashopModelProductExtend();
				$productLoaded = $productModel->getProducts($product_ids);
				foreach ($productLoaded as $i => $product){
					if ( !isset($this->products[$product->retinashop_product_id]) ){
						$this->products[$product->retinashop_product_id] = &$productLoaded[$i];
					}
				}
			}
		...
		public function getProductsListing($group, $order, $limit, $show_price, $category_ids){
			...
			if(class_exists('retinashopModelProductExtend')){
				$productModel = new retinashopModelProductExtend();
				...
				if ( count($loadProducts) ){
					foreach ($loadProducts as $i => $product){
						if ( !isset($this->products[$product->retinashop_product_id]) ){
							$this->products[$product->retinashop_product_id] = &$loadProducts[$i];
						}
						$products[] = $this->products[$product->retinashop_product_id];
					}
				}
				return $products;
			}	
			...
			if (is_null($this->categories)){
				...
				if ( count($categories) ){
					...
					foreach ($categories as $i => $category){
						if (!isset($category->retinashop_media_id)){
							$table = &JTable::getInstance('category_medias', 'Table', array());
							$category->retinashop_media_id = $table->load((int)$category->retinashop_category_id);
						}
...
if(!class_exists('retinashopModelProductExtend') && class_exists('retinashopModelProduct')){
	class retinashopModelProductExtend extends retinashopModelProduct{
		
		/**
		 * Override retinashop function
		 *
		 * @see retinashopModelProduct::sortSearchListQuery()
		 */
		
		function sortSearchListQuery($onlyPublished = true, $retinashop_category_id = false, $group = false, $nbrReturnProducts = false){

			$app = JFactory::getApplication() ;

			$groupBy = 'group by p.`retinashop_product_id`';
			...
			if ($this->searchplugin !== 0){
				...
				$ret = $dispatcher->trigger('plgrsAddToSearch', array(&$where, &$PluginJoinTables, $this->searchplugin));
				foreach($ret as $r){
					if(!$r) $useCore = false;
				}
			}
			...
			if($useCore){
				...
				// rsdebug('my $this->searchcustoms ',$this->searchcustoms);
				if ( !empty($this->searchcustoms) ){
					$joinCustom = true ;
					foreach ($this->searchcustoms as $key => $searchcustom) {
						$custom_search[] = '(pf.`retinashop_custom_id`="'.(int)$key.'" and pf.`custom_value` like "%' . $this->_db->getEscaped( $searchcustom, true ) . '%")';
					}
				...
				if($app->isSite() && !rsConfig::get('use_as_catalog',0) && rsConfig::get('stockhandle', 'none')=='disableit' ){
					$where[] = ' p.`product_in_stock`>"0" ';
				}

				if ( $retinashop_category_id !== false ){
					$joinCategory = true ;
					
					if ( is_string($retinashop_category_id) && preg_match('/[\s|,]+/', $retinashop_category_id)){
						$retinashop_category_id = preg_split('/[\s|,]+/', $retinashop_category_id);
					}
					if ( !is_array($retinashop_category_id) ){
						settype($retinashop_category_id, 'array');
					}
					
					$where[] = ' `#__retinashop_product_categories`.`retinashop_category_id` IN ('.implode(',', $retinashop_category_id).')';
				}
				...
				if ( $app->isSite() ) {
					if(!class_exists('retinashopModelUser')) require(RPATH_rs_admin.DS.'models'.DS.'user.php');
					$usermodel = rsModel::getModel('user');
					$currentrsuser = $usermodel->getUser();
					$retinashop_shoppergroup_ids =  (array)$currentrsuser->shopper_groups;

					if(is_array($retinashop_shoppergroup_ids)){
						foreach ($retinashop_shoppergroup_ids as $key => $retinashop_shoppergroup_id){
							$where[] .= '(s.`retinashop_shoppergroup_id`= "' . (int) $retinashop_shoppergroup_id . '" OR' . ' (s.`retinashop_shoppergroup_id`) IS NULL )';
						}
						$joinShopper = true;
					}
				}

				if ($this->retinashop_manufacturer_id) {
					$joinMf = true ;
					$where[] = ' `#__retinashop_product_manufacturers`.`retinashop_manufacturer_id` = '.$this->retinashop_manufacturer_id;
				}
				...
				switch ($this->source_order) {
					...
					case 'mf_name':
						...
						break;
					case 'ordering':
						$orderBy = ' ORDER BY `#__retinashop_product_categories`.`ordering` ';
						$joinCategory = true ;
						break;
					case 'product_price':
						//$filters[] = 'p.`retinashop_product_id` = p.`retinashop_product_id`';
			...
			// $selectFindRows = 'SELECT SQL_CALC_FOUND_ROWS * FROM `#__retinashop_products` ';
			// $selectFindRows = 'SELECT COUNT(*) FROM `#__retinashop_products` ';
			if( $joinLang ){
				$select = ' * FROM `#__retinashop_products_'.rsLANG.'` as l';
				$joinedTables = ' JOIN `#__retinashop_products` AS p using (`retinashop_product_id`)';
			} else {
				$select = ' * FROM `#__retinashop_products` as p';
				$joinedTables = '';
			}

			if ($joinCategory == true) {
				$joinedTables .= ' LEFT JOIN `#__retinashop_product_categories` ON p.`retinashop_product_id` = `#__retinashop_product_categories`.`retinashop_product_id`
						LEFT JOIN `#__retinashop_categories_'.rsLANG.'` as c ON c.`retinashop_category_id` = `#__retinashop_product_categories`.`retinashop_category_id`';
			}
			
			if ($joinMf == true) {
				$joinedTables .= ' LEFT JOIN `#__retinashop_product_manufacturers` ON p.`retinashop_product_id` = `#__retinashop_product_manufacturers`.`retinashop_product_id`
						LEFT JOIN `#__retinashop_manufacturers_'.rsLANG.'` as m ON m.`retinashop_manufacturer_id` = `#__retinashop_product_manufacturers`.`retinashop_manufacturer_id` ';
			}

			if ($joinPrice == true) {
				$joinedTables .= ' LEFT JOIN `#__retinashop_product_prices` as pp ON p.`retinashop_product_id` = pp.`retinashop_product_id` ';
			}
			
			if ($this->searchcustoms) {
				$joinedTables .= ' LEFT JOIN `#__retinashop_product_customfields` as pf ON p.`retinashop_product_id` = pf.`retinashop_product_id` ';
			}
			
			if ($this->searchplugin!==0) {
				if (!empty( $PluginJoinTables) ) {
					$plgName = $PluginJoinTables[0] ;
					$joinedTables .= ' LEFT JOIN `#__retinashop_product_custom_plg_'.$plgName.'` as '.$plgName.' ON '.$plgName.'.`retinashop_product_id` = p.`retinashop_product_id` ' ;
				}
			}
			
			if ($joinShopper == true) {
				$joinedTables .= ' LEFT JOIN `#__retinashop_product_shoppergroups` ON p.`retinashop_product_id` = `#__retinashop_product_shoppergroups`.`retinashop_product_id`
						LEFT  OUTER JOIN `#__retinashop_shoppergroups` as s ON s.`retinashop_shoppergroup_id` = `#__retinashop_product_shoppergroups`.`retinashop_shoppergroup_id`';
			}
