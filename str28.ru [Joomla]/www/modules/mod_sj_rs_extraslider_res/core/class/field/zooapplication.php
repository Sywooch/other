	...
	class _Core_Field_ZooApplication extends JFormField {
		...
		protected function getInput(){
			if($this->com_zoo_installed()){
				...
				if (!$applications){
					if (file_exists(RPATH_admin .DS. 'components' .DS. 'com_zoo' .DS. 'config.php')){
						return RText::_('Loading zoo application error.');
					} else {
						return RText::_('Zoo component is not installed?');
					}
				} else {
