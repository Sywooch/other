...
if (!class_exists('YTools')){
	abstract class YTools {
		...
		public static function resize($image, $width, $height=null, $config=array()){
			...
			$replaces = array('/');
			if (RPATH_SITE!='/'){
				array_unshift($searchs,  RPATH_SITE);
		...
		public static function getModuleCache(){
			$cache = RPATH_CACHE . DS . self::getModule()->module;
		...
		private static function _getRelativePaths($filename, $detect_browser=true, $folder){
			if (self::isUrl($filename)){
				...
				if ($folder==='tmpl'){
					$includes_PATH = array(
							'design1' . DS . $template . DS . 'html' . DS . self::getModule()->module . DS,
							'modules' . DS . self::getModule()->module . DS . $folder . DS
					);
				} else {
					$includes_PATH = array(
							'design1' . DS . $template . DS . 'html' . DS . self::getModule()->module . DS . 'retina_097115115101116115' . DS . $folder . DS,
							'design1' . DS . $template . DS . 'html' . DS . self::getModule()->module . DS . 'retina_097115115101116115' . DS,
							'design1' . DS . $template . DS . 'html' . DS . self::getModule()->module . DS . $folder  . DS,
							'design1' . DS . $template . DS . 'html' . DS . self::getModule()->module . DS,

							'modules' . DS . self::getModule()->module . DS . 'retina_097115115101116115' . DS . $folder . DS,
							'modules' . DS . self::getModule()->module . DS . 'retina_097115115101116115' . DS,
							'modules' . DS . self::getModule()->module . DS . $folder  . DS,
		...
		public static function getRemoteFile($url){
			if (self::isUrl($url)){
				...
				$filename = RPATH_CACHE . DS . md5($url) . ".$ext";
