...
require_once RPATH_SITE . '/components/com_weblinks/helpers/route.php';
JModel::addIncludePath(RPATH_SITE . '/components/com_weblinks/models', 'WeblinksModel');
...
class modWeblinksHelper
{
	static function getList($params)
	{
		...
		for ($i =0, $count = count($elements); $i < $count; $i++) {
			$element = &$elements[$i];
			if ($element->params->get('count_clicks', $params->get('count_clicks')) == 1) {
				$element->link	= JRoute::_('index.php?option=com_weblinks&task=weblink.go&catid='.$element->catslug.'&id='. $element->slug);
			} else {
