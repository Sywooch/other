...
JHtml::addIncludePath(RPATH_SITE . '/components/com_finder/helpers/html');

// Load the smart search component language file.
$lang = JFactory::getLanguage();
$lang->load('com_finder', RPATH_SITE);
...
{
	$label = '<label for="mod-finder-searchword" class="finder' . $suffix . '">' . $params->get('alt_label', RText::_('RSEARCH_FILTER_SUBMIT')) . '</label>';
...
{
	$button = '<button class="button' . $suffix . ' finder' . $suffix . '" type="submit">' . RText::_('MOD_FINDER_SEARCH_BUTTON') . '</button>';
...
//<![CDATA[
	window.addEvent('domready', function() {
		...
		if (!document.id('mod-finder-searchword').getProperty('value')) {
			document.id('mod-finder-searchword').setProperty('value', '<?php echo RText::_('MOD_FINDER_SEARCH_VALUE', true); ?>');
		}
		...
		document.id('mod-finder-searchword').addEvent('focus', function() {
			if (this.getProperty('value') == '<?php echo RText::_('MOD_FINDER_SEARCH_VALUE', true); ?>') {
	...
	<div class="finder<?php echo $suffix; ?>">
		...
		<?php if ($params->get('show_advanced', 1)): ?>
			<?php if ($params->get('show_advanced', 1) == 2): ?>
				<br />
				<a href="<?php echo JRoute::_($route); ?>"><?php echo RText::_('COM_FINDER_ADVANCED_SEARCH'); ?></a>
