...
<?php $panes = ceil($total / $nr_events); ?>
<?php $inactive = $nr_events - $total % $nr_events; ?>
<?php $dates = modRseventsProSlider::getTimeline($events,$nr_events); ?>

<script type="text/javascript">
if (typeof RSESlider != 'undefined' && RSESlider.settings){
	RSESlider.settings['rs_events_slider_timeline<?php echo $module->id; ?>'] = {
		'offset': {'x': 0},
		'events': <?php echo $nr_events; ?>,
		'transition': <?php echo $teffect; ?>.<?php echo $tmethod; ?>,
		'duration': <?php echo $tduration; ?>000,
...
<div class="rs_eventsslider_timeline<?php echo $suffix; ?>" id="rs_eventsslider_timeline<?php echo $module->id; ?>">
	...
	<div class="rs_events_slider_timeline_container" id="rs_events_slider_timeline_container<?php echo $module->id; ?>">
		<div class="rs_events_slider_timeline" id="rs_events_slider_timeline<?php echo $module->id; ?>">
				<ul id="rs_events_slider_timeline_events" class="rs_events_slider_timeline_events">
					<?php foreach ($events as $event) { ?>
					<li>
						<div class="rs_events_slider_timeline_element">
							<?php if ($date) { ?>
							<span id="rse_date">
								<?php echo rseventsproHelper::date($event->start,'Y M j'); ?>
							</span>
							<?php } ?>
							<?php if ($title) { ?>
								<a id="rse_name" <?php echo $open; ?> href="<?php echo rseventsproHelper::route('index.php?option=com_rseventspro&layout=show&id='.rseventsproHelper::sef($event->id,$event->name),true,$itemid); ?>"><?php echo $event->name; ?></a>
							<?php } ?>
							<span id="rse_descr"><?php echo substr(strip_tags($event->description),0,$length); ?></span>
						</div>
					</li>
					<?php } ?>
		...
		<?php if ($panes > 1) { ?>
		<div class="resprobar_container" id="resprobar_container">
