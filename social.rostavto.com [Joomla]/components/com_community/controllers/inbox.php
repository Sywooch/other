...
class CommunityInboxController extends CommunityBaseController
{
    ...
    public function ajaxRemoveThumbnail($msgId){
        ...
        $objResponse = new JAXResponse();
        $objResponse->addScriptCall('cAppendReply', $html, $msgId);
        return $objResponse->sendResponse();
    }
	...
	public function ajaxAddReply($msgId, $reply, $photoId = 0)
	{
		...
		if(in_array($messageRecepient[0],$bannedList)){
			$objResponse->addScriptCall( 'alert' , JText::_( 'COM_COMMUNITY_USER_BANNED' ) );
			$objResponse->sendResponse ();
			return;
		}
		...
		$body = new JRegistry();
		$body->set('content',CLinkGeneratorHelper::replaceAliasURL($obj->body));
