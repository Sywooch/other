...
class CommunityPhotosController extends CommunityBaseController {
    ...
    public function ajaxSwitchPhotoTrigger($photoId) {
        ...
        $response->addScriptCall('setTimeout(function(){ joms.jQuery("#community-photo-walls").find("[data-type=wall-newcomment] [data-action=save]").trigger("start"); }, 2000);');
    ...
    public function ajaxAlbumSaveWall($message, $uniqueId, $appId = null, $photoId = 0) {
        ...
        if (empty($message) && $photoId == 0) {
            ...
            $wall = CWallLibrary::saveWall($uniqueId, $message, 'albums', $my, ($my->id == $album->creator), 'photos,album', 'wall.content', 0, $photoId);
            ...
            CUserHelper::parseTaggedUserNotification($message, $my, null, array('type' => 'album-comment','album_id' => $album->id,'album_creator_id' => $album->creator ));
    ...
    public function ajaxSaveWall($message, $uniqueId, $appId = null, $photoId = 0) {
        ...
        if (empty($message) && $photoId == 0) {
            ...
            $params->set('message', CLinkGeneratorHelper::replaceAliasURL($message));
            $params->set('photo', JText::_('COM_COMMUNITY_SINGULAR_PHOTO'));
            $params->set('photo_url', $url);
            // @rule: Send notification to the photo owner.
    ...
    private function _getAppPremission($app, $album) {
        ...
        if (isset($permission))
            return $permission;
        else return COMMUNITY_PRIVACY_PRIVATE;
    }
...
//			$url = $handler->getEditedAlbumURL( $albumid );
    ...
    public function checkPhotoAccess($albumid = null) {
        ...
        // verify album-level privacy setting
        if($albumid) {
            if(!CPrivacy::isAccessAllowed($my->id, $actor->id, 'album', 'album', $albumid)) $allowed = false;
        } else {
            // default privacy levels
            if(!CPrivacy::isAccessAllowed($my->id, $actor->id, 'privacyPhotoView', 'privacyPhotoView')) $allowed = false;
        }
    ...
    public function ajaxPreviewComment(){
        ...
        $originalPath = $uploadFolder. 'original_' . md5($my->id . '_comment' . time()) . CImageHelper::getExtension($file['type']);
        $fullImagePath = $uploadFolder. md5($my->id . '_comment' . time()) . CImageHelper::getExtension($file['type']);
        $thumbPath = $uploadFolder. 'thumb_' . md5($my->id . '_comment' . time()) . CImageHelper::getExtension($file['type']);
...
// 			            $image->rotateImage($public, 90);
    ...
    public function ajaxUploadAvatar($type, $id, $custom = '') {
        ...
        $content .= $customHTML;
        $content .= '<div id="avatar-upload"> <div style="color:#612119;"><p>Минимальный размер аватара 160х160 пикселей.</p></div>';
        $action = 'joms.jQuery(\'#jsform-uploadavatar\').attr(\'action\', joms.jQuery(\'#jsform-uploadavatar\').attr(\'action\')' . $customArg . ')';
        $content .= '<p style="color:#612119;">' . JText::_('COM_COMMUNIT_SELECT_IMAGE_INSTR') . '</p>';
        ...
        if (!empty($cTable->avatar)) {
            $content .= '<div id="avatar-cropper" style="color:#612119;">';
