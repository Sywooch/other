...
<!-- Admin options -->
	<?php if ($this->admin || $event->owner == $this->user || $event->sid == $this->user) { ?>
	<ul id="rsep_admin" style="display:none;">
...
<?php if ($event->allday) { ?>
	...
	<p class="rsep_date">
		...
		<a href="<?php echo rseventsproHelper::route('index.php?option=com_rseventspro&task=rseventspro.export&id='.rseventsproHelper::sef($event->id,$event->name)); ?>">
			<span class="rsep_export" style="display:none;"><?php echo JText::_('COM_RSEVENTSPRO_EXPORT_EVENT'); ?></span>
		</a> 
		<?php } ?>
	</p>
	<?php } ?>
...
<p class="rsep_posted" style="background-color:transparent;">
	<?php echo JText::_('COM_RSEVENTSPRO_EVENT_POSTED_BY');  ?> 
	<?php if (!empty($event->ownerprofile)) { ?><a href="<?php echo $event->ownerprofile; ?>"><?php } ?>
	<?php  echo'<span style="background-color:transparent;">';   echo $event->ownername; echo"</span>"; ?>
...
<p class="rsep_categories" style="display:none;">
