...
class CommunityRegisterController extends CommunityBaseController {
    ...
    public function ajaxCheckPassLength($lenght,$fieldName){
        ...
        {
            $this->ajaxSetMessage($fieldName, '', JText::sprintf('COM_COMMUNITY_PASSWORD_TOO_SHORT',$params->get('minimum_length',6)));
        }

        $objResponse->addScriptCall("joms.jQuery('#err".$fieldName."msg').hide");
        $objResponse->addScriptCall("joms.jQuery('#err".$fieldName."msg').html","&nbsp");
        return $objResponse->sendResponse();
    }
