...
function insert_title2(n)
{
var tmp1=document.getElementById('name').value;

var pos = tmp1.indexOf(':');
...
document.getElementById('name').value = tmp1;
}
</script>
...
<span style="float:left;">Выберите событие:</span>
</div>

<div style="width:51%; height:30px; float:left;">
<span style="float:left;">Выберите Вашу машину, к которой будет относиться событие:</span>
</div>		

</div>

		
<select style="float:left;" onchange="insert_title(this.value)">
...
$db = JFactory::getDbo();
$query_events_e = $db->getQuery(true);

$query_events_e->select(array('fieldid', 'name', 'title'));
$query_events_e->from('#__adsmanager_fields');

$conditions = array(
  'name LIKE \'ad_diagnostika%\' ');

$query_events_e->where($conditions);
//$query_events_e->where('name LIKE \'ad_z%\'');




$db->setQuery($query_events_e);

$results = $db->loadObjectList();

foreach($results as $element_e) {

$title_1=str_replace("Как часто проводится","",$element_e->title);
$title_1=trim($title_1);

$title_2=str_replace("Диагностика:","",$title_1);
$title_2=str_replace("Замена расходных материалов:","",$title_2);
$title_2=str_replace("Замена комплектующих:","",$title_2);
$title_2=trim($title_2);


    echo "<option value=\"".$title_2."\">".$title_1."</option>"; 
...
$query_events_e->where($conditions);
//$query_events_e->where('name LIKE \'ad_zamena%\'');
...
<!----========================================================--->
<select style="margin-left:10px;" onchange="insert_title2(this.value)">
<option value=""></option>
<?php
...
$query_events_1->select(array('id', 'value'));
$query_events_1->from('#__community_fields_values');
$query_events_1->where('user_id LIKE \''.$user->id.'\'');
$query_events_1->where('field_id LIKE \'17\'');
...
$MAS_option=explode(",",$element_1->value);

echo $MAS_option[1];

for ($i = 0; $i < (count($MAS_option)-1); $i++) 
  { 
    echo "<option value=\"".$MAS_option[$i]."\">".$MAS_option[$i]."</option>"; 
  } 
...
<!----========================================================--->
				...
				<div class="rs_period" style="display:none;">
					<label for="published"><?php echo JText::_('COM_RSEVENTSPRO_PUBLISH_EVENT'); ?></label>
					<div class="rs_calendar" >
						<select name="jform[published]" class="rs_sel" style="background-color:red;">
...
<option value="0" >Не опубликовано</option>						
	...
	<?php// echo JHtml::_('select.options', JHtml::_('jgrid.publishedOptions', $this->states), 'value', 'text', $this->row->published, true); ?>
				...
				<?php if (!empty($this->permissions['can_repeat_events'])) { ?>
				<?php  // || $this->admin  ?>
                ...
                <div class="rs_period" style="position: relative; z-index: 99;">
                	<label for="location" style="display:none;"><?php echo JText::_('COM_RSEVENTSPRO_EVENT_LOCATION'); ?></label>
                    <input type="text" value="default<?php// echo $this->escape($this->row->locationname); ?>" class="rs_inp" name="rs_location" id="rs_location" tabindex="3"
					style="background-color:transparent; display:none;"/>
					<input type="hidden" name="jform[location]" id="location" value="default<?php //echo $this->row->location; ?>"/>
					...
					<?php// } ?>
								...
								<div style="width:100%; height:30px; background-color:transparent; ">
				<span>Введите описание события:</span>
				...
				<p style="position: relative; z-index: 97; display:none;">
		...
		<div class="rs_right rs_display_none" id="rs_right_2" >
			<fieldset>
				...
				<!--	<div style="width:300px; height:40px; float:left;">
					<span>Выберите Вашу машину, для которой Вы хотите создать событие: </span>
					</div>-->
					
					<select class="inputbox rschosen" name="categories[]" id="categories"  
					style="background-color:transparent; ">
...
?>
	<?php echo JHtml::_('select.options', JHtml::_('category.options','com_rseventspro', array('filter.published' => array(1))),'value','text',$this->eventClass->getCategories()); ?>
					...
					<a style="display:none;" class="rs_add_category" id="rs_add" onclick="sm('rs_category_add',400,250)"><?php echo JText::_('COM_RSEVENTSPRO_EVENT_ADD_CATEGORY'); ?></a>
					<?php //} ?>
				</div>            
			</fieldset>
			<fieldset>
		...
		<div id="rs_left">
			<div id="rs_event_photo" style="display:none;">
			...
			<ul id="rs_event_menu" style="display:none;">
