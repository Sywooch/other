        ...
        public function coreFilesCheck($path, $level = 0) {
            ...
            foreach ($di as $child) {
                if (!$child->isDot()) {
                    ...
                    if ($child->isDir()) {
                        $this->coreFilesCheck($child->getPathname(), $level++);
                    } elseif ($child->isFile()) {
                        $ext = strtolower($child->getExtension());
                        /* We do not check for common files to speed up */
                        if ($ext != 'html' || $ext != 'jpg' || $ext != 'png') {
                            ...
                            if (strpos($fileName, 'defines.community.php') !== false) {
                            ...
                            } elseif (strpos($fileName, 'community_version.php') !== false) {
                                /* Another exception */
                                $content = file_get_contents($child->getPathname());
                                ...
                                $checkSum = md5_file($child->getPathname());
                            }
                            if (!isset($this->hashList[$filePath])) {
                                
                            } else if ((isset($this->hashList[$filePath]) && $this->hashList[$filePath] != $checkSum)) {
                                echo '<li>' . $filePath . '<small> ' . '<span class="label label-important">' . JText::_('COM_COMMUNITY_TROUBLESHOOTS_MODIFIED') . '</span></small></li>';
                            } else {
