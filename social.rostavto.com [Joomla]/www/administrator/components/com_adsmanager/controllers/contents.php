...
class AdsmanagerControllerContents extends JController
{
		...
		if ($conf->auto_publish == 0)
		{
			if ($conf->send_email_on_validation_to_user == 1) {	
				...
				foreach($cid as $id) {
					...
					$model->sendMailToUser($conf->validation_subject,$conf->validation_text,$user,$c,$conf);
	...
	function save($src=null, $orderingFilter = '', $ignore = '')
	{
		...
		if (($contentid != 0)&&($conf->auto_publish == 0)) {
			if (($previouspublished == 0)&&(JRequest::getInt('published', 0))) {
				$usermodel = $this->getModel("user");
				$user = $usermodel->getUser($content->userid);
				$cmodel = $this->getModel("content");
				$cmodel->sendMailToUser($conf->validation_subject,$conf->validation_text,$user,$content,$conf);
		...
		if ($category != 0)
			$app->redirect( 'index.php?option=com_adsmanager&c=contents&catid='.$category, JText::_('ADSMANAGER_CONTENT_SAVED') );	
		else
