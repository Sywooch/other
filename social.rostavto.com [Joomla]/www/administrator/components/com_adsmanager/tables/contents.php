...
class AdsmanagerTableContents extends JTable
{
	...
	function bindContent($post,$files,$conf,$model,$plugins)
    {
		...
		foreach($this->data['images'] as $i => $img) {
			if (in_array($img->index,$deleted_images)) {
				JFile::delete(JPATH_SITE."/images/com_adsmanager/ads/tmp/".$img->image);
				JFile::delete(JPATH_SITE."/images/com_adsmanager/ads/tmp/".$img->thumbnail);
	...
	function save()
    {
    	...
    	foreach($this->data['delimages'] as $image) {
			JFile::delete(JPATH_SITE."/images/com_adsmanager/ads/".$image->image);
			JFile::delete(JPATH_SITE."/images/com_adsmanager/ads/".$image->thumbnail);
    ...
    {		
		...
		*/
		...
		{	
			$pict = JPATH_SITE."/images/com_adsmanager/ads/".$img->image;
			if ( file_exists( $pict)) {
				JFile::delete($pict);
			}
			$pic = JPATH_SITE."/images/com_adsmanager/ads/".$img->thumbnail;
			if ( file_exists( $pic)) {
				JFile::delete($pic);
			}
		}
