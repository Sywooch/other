FD40.plugin("lodash",function($){function basicIndexOf(array,value,fromIndex){for(var index=(fromIndex||0)-1,length=array.length;++index<length;)if(array[index]===value)return index;return-1}function cacheIndexOf(cache,value){var type=typeof value;if(cache=cache.cache,"boolean"==type||null==value)return cache[value];"number"!=type&&"string"!=type&&(type="object");var key="number"==type?value:keyPrefix+value;return cache=cache[type]||(cache[type]={}),"object"==type?cache[key]&&basicIndexOf(cache[key],value)>-1?0:-1:cache[key]?0:-1
...
!freeGlobal||freeGlobal.global!==freeGlobal&&freeGlobal.window!==freeGlobal||(window=freeGlobal);var _=$._=runInContext();$.each(_,function(prop,func){Object.hasOwnProperty.call(_,prop)&&!/VERSION/.test(name)&&($[prop]||($[prop]=func))})});