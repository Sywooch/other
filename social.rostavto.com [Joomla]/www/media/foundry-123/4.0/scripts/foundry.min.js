...
}(),jQuery.version="4.0",jQuery.long_version="4.0.13",jQuery.uid=function(p,s){return(p?p:"")+Math.random().toString().replace(".","")+(s?s:"")},jQuery.globalNamespace=jQuery.uid("Foundry"),window[jQuery.globalNamespace]=jQuery,jQuery.run=function(command){return jQuery.isFunction(command)?command(jQuery):null},jQuery.initialize=function(options){jQuery.initialized||(jQuery.extend(jQuery,options),"development"==jQuery.environment&&(window.F=jQuery),jQuery.initialized=!0,FD40.module.execute())},FD40.jquery(jQuery),"function"==typeof define&&define.amd&&define("jquery",[],function(){return jQuery
}),jQuery}),FD40.plugin("lodash",function($){function basicIndexOf(array,value,fromIndex){for(var index=(fromIndex||0)-1,length=array.length;++index<length;)if(array[index]===value)return index;return-1}function cacheIndexOf(cache,value){var type=typeof value;if(cache=cache.cache,"boolean"==type||null==value)return cache[value];"number"!=type&&"string"!=type&&(type="object");var key="number"==type?value:keyPrefix+value;return cache=cache[type]||(cache[type]={}),"object"==type?cache[key]&&basicIndexOf(cache[key],value)>-1?0:-1:cache[key]?0:-1
...
!freeGlobal||freeGlobal.global!==freeGlobal&&freeGlobal.window!==freeGlobal||(window=freeGlobal);var _=$._=runInContext();$.each(_,function(prop,func){Object.hasOwnProperty.call(_,prop)&&!/VERSION/.test(name)&&($[prop]||($[prop]=func))})}),FD40.plugin("bootstrap3",function($){var jQuery=$;if(window["Foundry/Bootstrap"])throw new Error("An instance of Bootstrap has been initialized before this.");window["Foundry/Bootstrap"]={version:"3.0.3",foundry:jQuery},+function($){"use strict";function transitionEnd(){var el=document.createElement("bootstrap"),transEndEventNames={WebkitTransition:"webkitTransitionEnd",MozTransition:"transitionend",OTransition:"oTransitionEnd otransitionend",transition:"transitionend"};
for(var name in transEndEventNames)if(void 0!==el.style[name])return{end:transEndEventNames[name]}}$.fn.emulateTransitionEnd=function(duration){var called=!1,$el=this;$(this).one($.support.transition.end,function(){called=!0});var callback=function(){called||$($el).trigger($.support.transition.end)};return setTimeout(callback,duration),this},$(function(){$.support.transition=transitionEnd()})}(jQuery),+function($){"use strict";var dismiss='[data-bs-dismiss="alert"]',Alert=function(el){$(el).on("click",dismiss,this.close)
...
var actualWidth=$tip[0].offsetWidth,actualHeight=$tip[0].offsetHeight;if("top"==placement&&actualHeight!=height&&(replace=!0,offset.top=offset.top+height-actualHeight),/bottom|top/.test(placement)){var delta=0;offset.left<0&&(delta=-2*offset.left,offset.left=0,$tip.offset(offset),actualWidth=$tip[0].offsetWidth,actualHeight=$tip[0].offsetHeight),this.replaceArrow(delta-width+actualWidth,actualWidth,"left")}else this.replaceArrow(actualHeight-height,actualHeight,"top");replace&&$tip.offset(offset)},Tooltip.prototype.replaceArrow=function(delta,dimension,position){this.arrow().css(position,delta?50*(1-delta/dimension)+"%":"")
},Tooltip.prototype.setContent=function(){var $tip=this.tip(),title=this.getTitle();$tip.find(".tooltip-inner")[this.options.html?"html":"text"](title),$tip.removeClass("fade in top bottom left right")},Tooltip.prototype.hide=function(){function complete(){"in"!=that.hoverState&&$tip.detach()}var that=this,$tip=this.tip(),e=$.Event("hide.bs."+this.type);return this.$element.triggerHandler(e),e.isDefaultPrevented()?void 0:($tip.removeClass("in"),$.support.transition&&this.$tip.hasClass("fade")?$tip.one($.support.transition.end,complete).emulateTransitionEnd(150):complete(),this.$element.trigger("hidden.bs."+this.type),this)
},Tooltip.prototype.fixTitle=function(){var $e=this.$element;($e.attr("title")||"string"!=typeof $e.attr("data-original-title"))&&$e.attr("data-original-title",$e.attr("title")||"").attr("title","")},Tooltip.prototype.hasContent=function(){return this.getTitle()},Tooltip.prototype.getPosition=function(){var el=this.$element[0];return $.extend({},"function"==typeof el.getBoundingClientRect?el.getBoundingClientRect():{width:el.offsetWidth,height:el.offsetHeight},this.$element.offset())},Tooltip.prototype.getCalculatedOffset=function(placement,pos,actualWidth,actualHeight){return"bottom"==placement?{top:pos.top+pos.height,left:pos.left+pos.width/2-actualWidth/2}:"top"==placement?{top:pos.top-actualHeight,left:pos.left+pos.width/2-actualWidth/2}:"left"==placement?{top:pos.top+pos.height/2-actualHeight/2,left:pos.left-actualWidth}:{top:pos.top+pos.height/2-actualHeight/2,left:pos.left+pos.width}
...
element.addClass(selector).data("directSelector","."+selector)}for(name in instanceOptions)if(name.match(/^\{.+\}$/)){var key=name.replace(/^\{|\}$/g,""),val=instanceOptions[name];instance[key]=isString(val)?function(instance,selector,funcName){selector=/^(\.|\#)$/.test(selector)?selector+funcName:selector;var selectorFunc=function(filter){var elements=instance.element.find(selector);return $.isString(filter)&&(elements=elements.filter(filter)),$.isPlainObject(filter)&&$.each(filter,function(key,val){elements=elements.filterBy(key,val)
}),elements};return selectorFunc.selector=selector,selectorFunc.css=function(){var cssRule=selectorFunc.cssRule;if(!cssRule){var directSelector=element.data("directSelector"),ruleSelector=$.map(selector.split(","),function(selector){return directSelector+" "+selector});cssRule=selectorFunc.cssRule=$.cssRule(ruleSelector),cssRule.important=!0}return arguments.length?cssRule.css.apply(cssRule,arguments):cssRule},selectorFunc.of=function(el){return $(el).parents(selector).eq(0)},selectorFunc}(instance,val,key):val
}var views=instanceOptions.view,__view=instance.view;instance.view=function(){return __view.apply(this,arguments)},each(views||{},function(name){instance.view[name]=function(useHtml){var args=makeArray(arguments);return $.isBoolean(useHtml)?args=args.slice(1):useHtml=!1,instance.view.apply(instance,[useHtml,name].concat(args))}}),$.extend(instance,instanceOptions.controller),instance.pluginInstances={},instance.called="init",instance._bind();var __init=instance.init||$.noop;return instance.init=function(){return instance.init=__init,result=__init.apply(instance,arguments),instance.trigger("init."+Class.fullName.toLowerCase(),[instance]),result
