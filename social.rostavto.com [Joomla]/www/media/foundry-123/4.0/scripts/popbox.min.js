...
}popbox.position={classname:position,my:x1+" "+y1,at:x2+" "+y2,using:function(coords,feedback){var tooltip=$(this),classname=popbox.position.classname,top=coords.top,left=coords.left,offset=popbox.offset;switch(pos[0]){case"top":case"bottom":var vertical=feedback.vertical;vertical==pos[0]&&(classname=classname.replace(/top|bottom/gi,"top"==vertical?"bottom":"top")),top="top"==vertical?top+offset:top-offset;break;case"left":case"right":var horizontal=feedback.horizontal;feedback.horizontal==pos[0]&&(classname=classname.replace(/left|right/gi,"left"==feedback.horizontal?"right":"left")),left="left"==horizontal?left+offset:left-offset
...
popbox&&popbox.hide()}),"click"==popbox.toggle){var doc=$(document),hideOnClick=Popbox.toggleEvent+".popbox."+popbox.uid;doc.off(hideOnClick).on(hideOnClick,function(event){var targets=$(event.target).parents().andSelf();targets.filter(popbox.button).length>0||targets.filter(popbox.tooltip).length>0||(doc.off(hideOnClick),popbox.hide())})}var win=$(window),repositionOnResize="resize.popbox"+popbox.uid;if(win.off(repositionOnResize).on(repositionOnResize,function(){popbox.tooltip.length>0&&popbox.tooltip.position(popbox.position)
}),popbox.tooltip.length>0)return popbox.tooltip.appendTo("body").position(popbox.position),popbox.trigger("popboxActivate",[popbox]),void 0;if($.isFunction(popbox.content)){var options=popbox.content(popbox);if(popbox.update(options),popbox.wait)return}$.isDeferred(popbox.content)&&("pending"==popbox.content.state()&&(popbox.loader.appendTo("body").position(popbox.position),popbox.trigger("popboxLoading",[popbox])),popbox.content.always(function(){popbox.wait=!1}).done(function(html){if(!(popbox.tooltip.length>0)&&popbox.enabled){popbox.loader.detach();
