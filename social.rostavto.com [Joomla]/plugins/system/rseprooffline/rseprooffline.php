...
class plgSystemRseproOffline extends JPlugin
{
	...
	*/
	...
	public function rsepro_showForm($vars) {
		...
		if (isset($vars['method']) && $vars['method'] == $this->rsprooption) {
			...
			$enable = JPluginHelper::isEnabled('system', 'rseprooffline');
			if (!$enable) return;
			...
			$html .= '<tr><td height="40"></td><td><img alt="'.JText::_('COM_RSEVENTSPRO_PLG_PLUGIN_OFFLINE_CARDS').'" src="'.JURI::root().'components/com_rseventspro/assets/images/cc_logo.gif"></td></tr>'."\n";
			$html .= '<tr>'."\n";
			...
			$html .= '<td>'.JText::_('COM_RSEVENTSPRO_PLG_PLUGIN_OFFLINE_CARD_EXP_MONTH').' '.$exp_month.'  '.JText::_('COM_RSEVENTSPRO_PLG_PLUGIN_OFFLINE_CARD_EXP_YEAR').' '.$exp_year.'</td>'."\n";
			$html .= '</tr>'."\n";
			$html .= '<tr>'."\n";
			...
			$html .= '</tr>'."\n";
			$html .= '<tr>'."\n";
			$html .= '<td colspan="2"><button type="submit" class="rs_button">'.JText::_('COM_RSEVENTSPRO_PLG_PLUGIN_OFFLINE_PAY').'</button></td>'."\n";
			$html .= '</tr>'."\n";
			$html .= '</table>'."\n";
			$html .= '</fieldset>'."\n";
			$html .= '</div>'."\n";
	...
	*/
	...
	public function rsepro_processForm($vars) {
		...
		if (empty($ids)) $app->redirect(rseventsproHelper::route('index.php?option=com_rseventspro&layout=default',false),JText::_('COM_RSEVENTSPRO_PLG_PLUGIN_OFFLINE_INVALID_SUBSCRIPTION'));
		...
		$app->redirect(rseventsproHelper::route('index.php?option=com_rseventspro&layout=show&id='.rseventsproHelper::sef($event->id,$event->name),false),JText::_('COM_RSEVENTSPRO_PLG_PLUGIN_OFFLINE_PAYMENT_RECEIVED'));
	}
	...
	public function rsepro_info($vars) {
		...
		if (isset($vars['method']) && $vars['method'] == $this->rsprooption) {
			...
			if (!empty($data)) {
				...
				echo '<legend>'.JText::_('COM_RSEVENTSPRO_TRANSACTION_PAYMENT_DETAILS').'</legend>';
