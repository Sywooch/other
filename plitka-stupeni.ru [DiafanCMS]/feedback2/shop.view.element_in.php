...
echo '<div class="text">';
	...
	echo '<div class="rdescr tmp2">Материал: <span class="base">'.$result['ids_param'][3]['value'].'</span></div>';
		...
		echo '<div class="clear4"></div>';
		echo '<div class="rdescr tmp2">Представлен в магазине: <span class="name">'.(($result['shit']==1)?"ДА":"").'</span></div>';
		echo '<div class="clear4"></div>';
		echo '</div>';
	...
	echo '<div class="text_zag" style="font-size:20px;">Все элементы коллекции <a href="'.BASE_PATH_HREF.$collection['coll_url'].'/">'.$collection['collection'].'</a></div>';
			...
			$query = DB::query('SELECT s.id, s.[name],s.shit, r.rewrite FROM {shop_rel} sr
...
if($element['shit']==1){echo '<div class="elem-size">Представлен в магазине : Да</div>';}
