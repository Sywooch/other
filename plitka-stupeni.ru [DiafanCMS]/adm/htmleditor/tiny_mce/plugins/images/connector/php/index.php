...
class TinyImageManager {
	...
	function TinyImageManager() {
		...
		switch($action) {
			...
			case 'multiuploadfile':
				echo $this->MultiUploadFile($_GET['path'], $_GET['pathtype']);
				exit();
			break;
			...
			while (false !== ($file = readdir($handle))) {
				if ($file != "." && $file != "..") {
					...
					if (is_file($dir.'/'.$file) ) {
						...
						elseif ($type == 'files' && !in_array(strtolower($file_info['extension']),$this->ALLOWED_FILES)) {
							continue;
						}
						$path = str_replace(DIR_ROOT, '', $dir);
						$link = str_replace(DIR_ROOT, '', $path).'/'.$file;
	...
	function UploadFile($dir, $type) {
		...
		foreach ($ufiles['name'] as $k=>$v) {
			...
			$md5 = $this->format_name($filename, $extension, $dir);
			$file = $md5.'.'.$extension;
			...
			{
				$files[$file]['imageinfo'] = array(0 => '', 1 => '');
			}
	...
	function multiUploadFile($dir, $type) {
		...
		if (isset($_GET['qqfile'])) {
			...
			echo '{error:\'Незвозможно загрузить файл\'}';
			exit;
		}
		
		if (strpos($file->getName(), '.') !== false) {
			$extension = end(explode('.', $file->getName()));
			$filename = substr($file->getName(), 0, strlen($file->getName()) - strlen($extension) - 1);
		} else {
			echo '{error:\'Незвозможно загрузить файл\'}';
			exit;
		}
		...
		//$md5 = md5($file->getName().rand(0,9999999));
		$md5 = $this->format_name($filename, $extension, $dir);
		$new = $md5.'.'.$extension;

		if (! $file->save($dir.'/'.$new)) {
			echo '{error:\'Незвозможно загрузить файл\'}';
			exit;
		}
		chmod($dir.'/'.$new, 0777);
		//Проверка на изображение
		if ($pathtype == 'images') {
			$files[$new]['imageinfo'] = getimagesize($dir.'/'.$new);
			
			if (empty($files[$new]['imageinfo'])) {
				unlink($dir.'/'.$file);
				echo '{error:\'Незвозможно загрузить файл\'}';
				exit;
			}
		...
		//$path = $path['dirname'];
		$path = str_replace(DIR_ROOT, '', $dir);
		$link = str_replace(DIR_ROOT, '', $path).'/'.$new;
		if ($extension=='jpg' || $extension=='jpeg') {
			
			$er = new phpExifReader($dir.'/'.$new);
			
			$files[$new]['exifinfo'] = $er->getImageInfo();
			
			$files[$new]['general'] = array(
				'filename' => $new,
				'name'	=> $filename,
				'ext'	=> $extension,
				'path'	=> $path,
				'link'	=> $link,
				'size'	=> filesize($dir.'/'.$new),
				'date'	=> filemtime($dir.'/'.$new),
				'width'	=> $files[$new]['imageinfo'][0],
				'height'=> $files[$new]['imageinfo'][1],
				'md5'	=> $md5
			);
	...
	function ShowDir($dir, $type) {
		...
		foreach ($dir as $v) {
			...
			$ret .= '
   <table class="imageBlock0" cellpadding="0" cellspacing="0" filename="'.$v['general']['filename'].'" fname="'.$v['general']['name'].'" type="'.$type.'" ext="'.strtoupper($v['general']['ext']).'" path="'.$v['general']['path'].'" linkto="'.DIR_REAL.$v['general']['link'].'" fsize="'.$v['general']['size'].'" fsizetext="'.$this->bytes_to_str($v['general']['size']).'" date="'.date('d.m.Y H:i',$v['general']['date']).'" fwidth="'.$v['general']['width'].'" fheight="'.$v['general']['height'].'" md5="'.$v['general']['md5'].'" '.$middle_thumb_attr.'><tr><td valign="bottom" align="center">
    <div class="imageBlock1">
     <div class="imageImage" '.$div_params.'>
      <img src="'.DIR_REAL.$thumb.'" '.$img_params.' alt="'.$v['general']['name'].'" />
     </div>
	...
	function GetThumb($dir, $md5, $filename, $mode, $width=100, $height=100) {
		...
		{
			chmod($path.'/.thumbs/'.$md5.'_'.$width.'_'.$height.'_'.$mode.'.jpg', 0777);
		...
		if ($isfile)
		{
			$url = '/adm/htmleditor/tiny_mce/plugins/images/img/fileicons/';

			$ext = strtolower(end(explode('.', $filename)));
			
			if (!empty($ext) && file_exists(ABSOLUTE_PATH.$url.$ext.'.png'))
			{
				return '/..'.$url.$ext.'.png';
			}
			else
		...
		if (is_dir($path.'/.thumbs')) {
			if ($pathtype == 'images') {
				...
				if ($handle) {
					while (false !== ($file = readdir($handle))) {
						if ($file != "." && $file != "..") {
							if ($file == $md5.'_100_100_2.'.end(explode('.', $file))) {
	...
	private function format_name($name, $extension, $dir)
	{
		$rus = array('а', 'б', 'в', 'г', 'д', 'е', 'ё', 'ж', 'з', 'и', 'й', 'к', 'л', 'м', 'н', 'о', 'п', 'р', 'с', 'т', 'у', 'ф', 'х', 'ц', 'ч', 'ш', 'щ', 'ы', 'э', 'ю', 'я', 'А', 'Б', 'В', 'Г', 'Д', 'Е', 'Ё', 'Ж', 'З', 'И', 'Й', 'К', 'Л', 'М', 'Н', 'О', 'П', 'Р', 'С', 'Т', 'У', 'Ф', 'Х', 'Ц', 'Ч', 'Ш', 'Щ', 'Ы', 'Э', 'Ю', 'Я', ' ');
		$lat = array('a', 'b', 'v', 'g', 'd', 'e', 'yo', 'zh', 'z', 'i', 'y', 'k', 'l', 'm', 'n', 'o', 'p', 'r', 's', 't', 'u', 'f', 'kh', 'ts', 'ch', 'sh', 'sch', 'y', 'e', 'yu', 'ya', 'A', 'B', 'V', 'G', 'D', 'E', 'YO', 'ZH', 'Z', 'I', 'Y', 'K', 'L', 'M', 'N', 'O', 'P', 'R', 'S', 'T', 'U', 'F', 'KH', 'TS', 'CH', 'SH', 'SCH', 'Y', 'E', 'YU', 'YA', '_');
		$name = strtolower(preg_replace('/[^A-Za-z0-9_\.]+/', '', str_replace($rus, $lat, $name)));
