...
$(document).ready(function() { 
	...
	$('#menuUploadFiles').click(function(){
		...
		var uploader = new qq.FileUploader({
			...
			maxConnections: 30,
			params: {action : "multiuploadfile", path : path.path, pathtype : path.type}
		});
	...
	$('.imageBlock0').live('dblclick', function(){
		...
		if (e.attr('type') == 'files')
		{
			...
			win.document.getElementById(tinyMCEPopup.getWindowArg("input")).value = e.attr('linkto');
		/*
		...
		else
		{
			...
			if (typeof(win.ImageDialog) != "undefined") {
				...
				if (win.ImageDialog.showPreviewImage) win.ImageDialog.showPreviewImage(e.attr('linkto'));
				win.document.getElementById("alt").value = e.attr('fname');
				win.document.getElementById("title").value = e.attr('fname');
			}
		/*
			if (e.attr('fmiddle')) {
