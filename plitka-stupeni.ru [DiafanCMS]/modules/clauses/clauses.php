...
class Clauses extends Controller
{
	...
	{
		$this->diafan->_tpl->get($this->view, 'clauses', $this->result);
	}
	...
	/**
	 * Шаблонная функция: блок связанных статей
	 * 
	 * @param array $attributes атрибуты шаблонного тега
	 * @return void
	 */
	public function show_block_rel($attributes)
	{
		...
		if (! $attributes["template"] || ! $this->diafan->_tpl->get('show_block_rel_'.$attributes["template"], 'clauses', $result))
		{
			$this->diafan->_tpl->get('show_block_rel', 'clauses', $result);
		}
	}
}