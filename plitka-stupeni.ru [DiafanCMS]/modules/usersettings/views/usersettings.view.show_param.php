 ...
 *
 * Шаблон вывода дополнительных полей в настройках аккаунта
 * 
 * @package    Diafan.CMS
 * @author     diafan.ru
 * @version    5.2
	...
	foreach ($result[$name] as $row)
	{
		...
		switch ($row["type"])
		{
			...
			case "images":
				...
				{
					echo $this->get('images', 'usersettings', array("rows" => $result[$prefix.'images'][$row["id"]], "prefix" => $prefix, "param_id" => $row["id"]));
				}
