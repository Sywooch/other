...
if (! DB::query_result("SELECT id FROM {modules} WHERE module_name='ads' LIMIT 1"))
{
	include ABSOLUTE_PATH."includes/404.php";
}
$site_ids = $this->diafan->_route->id_module('ads');
if(empty($site_ids))
{
	include ABSOLUTE_PATH."includes/404.php";
}
...
$result = DB::query("SELECT e.id, e.created, e.[name], e.[anons], e.cat_id, e.site_id FROM {ads} AS e"
...
while ($row = DB::fetch_array($result))
{
	...
	<item>
		...
		.($this->diafan->configmodules("comments", "ads", $row["site_id"]) ? "
