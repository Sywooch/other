<?php
/**
 * Дополнительные характеристики объявлений
 *
...
{
	echo '<div class="ads_param'.($param["type"] == 'title' ? '_title' : '').'">'.
	($param["type"] == "images"?'':$param["name"]); //не выводим заголовок для фотографии
	if ($param["value"])
	{
		echo ' <span class="ads_param_value">';
		if($param["type"] == "attachments")
		{
			foreach($param["value"] as $a)
			{
				if ($a["is_image"])
				{
					...
					{
						echo ' <a href="'.$a["link"].'" rel="prettyPhoto[gallery'.$result["id"].'ads]"><img src="'.$a["link_preview"].'"></a> <a href="'.$a["link"].'" rel="prettyPhoto[gallery'.$result["id"].'ads_link]">'.$a["name"].'</a>';
					}
					else
					{
