...
class News_admin extends Frame_admin
{
	...
	public $variables = array (
		'main' => array (
			...
			'act' => array(
				...
				'help' => 'Если не отмечена, новость не будет выводиться на сайте',
				'default' => true,
				'multilang' => true,
			),
			'hr1' => 'hr',
			...
			'anons' => array(
				'type' => 'editor',
				'name' => 'Анонс',
				'help' => 'Краткая начальная часть новости, выводится в списках',
				'multilang' => true,
				'height' => 200,
			),
			'text' => array(
	...
	{
		if(! $this->diafan->configmodules("cat", "news", $this->diafan->site))
		{
	...
	public function show()
	{
		...
		{
			echo '<div class="error">'.sprintf($this->diafan->_('В %sнастройках%s модуля подключены категории, чтобы начать добавлять новость создайте хотя бы одну %sкатегорию%s.'), '<a href="'.BASE_PATH_HREF.'news/config/">', '</a>','<a href="'.BASE_PATH_HREF.'news/category/'.($this->diafan->site ? 'site'.$this->diafan->site.'/' : '').'">', '</a>').'</div>';
		}
		else
		{
	...
	{
		$this->diafan->del_or_trash_where("news_rel", "element_id=".$del_id." OR rel_element_id=".$del_id, $trash_id);
		$this->diafan->del_or_trash_where("news_counter", "element_id=".$del_id, $trash_id);
	}
}