		...
		switch($l["type"])
		{
			case "first":
				if(!check_page($result,'1'))
					$text .= '<a href="'.BASE_PATH_HREF.$l["link"].'" rel="nofollow">Первая</a> ';
				break;

			case "current":
				if($all)
				{
					$text .= '<a href="'.BASE_PATH_HREF.$l["link"].'" rel="nofollow">'.$l["name"].'</a> ';
				}
				else
				{
			...
			case "last":
				if(!check_page($result,$l['nen']))
					$text .= '<a href="'.BASE_PATH_HREF.$l["link"].'" rel="nofollow">Последняя('.$l['nen'].')</a> ';
			break;
...
if($prev)
	$text .= '<a href="'.$prev.'" rel="nofollow">< предыдущая</a>';
if($next)
	$text .= '<a href="'.$next.'" rel="nofollow">следующая ></a>';
$text .= '</div></div>';
