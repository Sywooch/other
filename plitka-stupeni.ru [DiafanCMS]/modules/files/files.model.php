...
class Files_model extends Model
{
	...
	public function list_()
	{
		...
		if ($this->result["rows"])
		{
			foreach ($this->result["rows"] as $i => $row)
			{
				...
				$this->result["rows"][$i]["name"] = $this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'files', _LANG);

				$this->result["rows"][$i]["anons"] = $this->diafan->_useradmin->get($row["anons"], 'anons', $row["id"], 'files',  _LANG);
			}
		}
		$this->theme_view();
	...
	private function list_query_count($time)
	{
		$count = DB::query_result(
			...
			.($this->diafan->_user->id ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='files'" : "")
	...
	private function list_query($time)
	{
		$result = DB::query_range(
			"SELECT e.id, e.[name], e.[anons], e.timeedit, e.cat_id, e.site_id, e.link FROM {files} AS e"
			.($this->diafan->_user->id ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='files'" : "")
			." WHERE e.[act]='1' AND e.trash='0' AND e.site_id=%d"
			." AND e.date_start<=%d AND (e.date_finish=0 OR e.date_finish>=%d)"
			." AND (e.access='0'"
	...
	public function first_page()
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, $this->diafan->module))
		{
			...
			while ($row = DB::fetch_array($result))
			{
				...
				$cat["link_all"] = $this->diafan->_route->link($row["site_id"], 'files', $row["id"]);
		...
		if (! empty($this->result["categories"]))
		{
			foreach ($this->result["categories"] as $cat => $rows)
			{
				...
				$this->result["categories"][$cat]["name"] = $this->diafan->_useradmin->get($rows["name"], 'name', $rows["id"], 'files_category', _LANG);

				$this->result["categories"][$cat]["anons"] = $this->diafan->_useradmin->get($rows["anons"], 'anons', $rows["id"], 'files_category', _LANG);
				...
				foreach ($rows["children"] as $k => $row)
				{
					$this->result["categories"][$cat]["children"][$k]["name"] =
						$this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'files_category', _LANG);

					$this->result["categories"][$cat]["children"][$k]["anons"] =
						$this->diafan->_useradmin->get($row["anons"], 'anons', $row["id"], 'files_category', _LANG);
				}
				...
				foreach ($rows["rows"] as $k => $row)
				{
					...
					$this->result["categories"][$cat]["rows"][$k]["name"] = $this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'files', _LANG);

					$this->result["categories"][$cat]["rows"][$k]["anons"] =
						$this->diafan->_useradmin->get($row["anons"], 'anons', $row["id"], 'files',  _LANG);
				}
			}
		}
	...
	private function first_page_cats_query()
	{
		...
		.($this->diafan->_user->id ? " LEFT JOIN {access} AS a ON a.cat_id=c.id AND a.module_name='files'" : "")
	...
	{
		$result = DB::query_range(
		"SELECT e.id, e.[name], e.cat_id, e.timeedit, e.[anons], e.site_id, e.link FROM {files} AS e"
	...
	public function list_category()
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, $this->diafan->module))
		{
			...
			$this->result["level"] = count($this->diafan->get_parents($this->diafan->cat, 'files_category'));
			$this->result["path"] = $this->get_path('files');
			...
			if ($this->diafan->configmodules("children_elements"))
			{
				$cat_ids = $this->diafan->get_children($this->diafan->cat, "files_category");
		...
		$this->result["text"] = $this->diafan->_useradmin->get($this->result["text"], 'text', $this->diafan->cat, 'files_category', _LANG);
		...
		if ($this->result["rows"])
		{
			...
			{
				$this->result["children"][$id]["name"] = $this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'files_category', _LANG);
				$this->result["children"][$id]["anons"] = $this->diafan->_useradmin->get($row["anons"], 'anons', $row["id"], 'files_category', _LANG);
			}
			foreach ($this->result["path"] as $k => $path)
			{
				if ($k == 0)
					continue;
				$this->result["path"][$k]["name"] = $this->diafan->_useradmin->get($path["name"], 'name', $path["id"], 'files_category', _LANG);
			}
		}
		$this->result["paginator"] = $this->diafan->_tpl->get('get', 'paginator', $this->result["paginator"]);
		return $this->result;
	...
	{
		$result = DB::query("SELECT id, [name], [text], timeedit, [descr], [keywords], sort, parent_id, [title_meta], access, theme, view FROM {files_category}"
	...
	private function list_category_previous_next($sort, $parent_id)
	{
		...
		if ($previous)
		{
			...
			$this->result["previous"]["link"] = $this->diafan->_route->link($this->diafan->cid, "files", $previous["id"]);
		}
		...
		if ($next)
		{
			...
			$this->result["next"]["link"] = $this->diafan->_route->link($this->diafan->cid, "files", $next["id"]);
		}
	}
	...
	public function id()
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, $this->diafan->module))
		{
			...
			if ($row["link"])
			{
				$this->result["link"] = $row["link"];
			}
			else
			{
				$result = DB::query("SELECT id, name, size FROM {attachments} WHERE module_name='files' AND element_id='%d' AND trash='0'", $row["id"]);
				if ($row_file = DB::fetch_array($result))
				{
					$this->result["link"] = BASE_PATH.'attachments/get/'.$row_file["id"]."/".$row_file["name"];
					$this->result["size"] = $this->diafan->convert($row_file["size"]);
				}
			}
		...
		$this->result["text"]  = $this->diafan->_useradmin->get($this->result["text"], 'text', $this->result["id"], 'files', _LANG);

		$this->result["anons"] = $this->diafan->_useradmin->get($this->result["anons"], 'anons', $this->result["id"], 'files', _LANG);
	...
	{
		$row = DB::fetch_array(DB::query("SELECT id, [name], [anons], [text], timeedit, cat_id, [keywords], [descr], sort, [title_meta], link, access, theme, view FROM {files}"
		." WHERE [act]='1' AND id = %d AND trash='0' AND site_id=%d"
	...
	{
		$previous = DB::fetch_array(DB::query(
		"SELECT e.[name], e.id, e.cat_id FROM {files} AS e"
		. ($this->diafan->_user->id ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='files'" : "")
		. " WHERE e.[act]='1' AND e.trash='0' AND e.site_id=%d"
		...
		if ($previous)
		{
			...
			$this->result["previous"]["id"] = $previous["id"];
			$this->result["previous"]["link"] = $this->diafan->_route->link($this->diafan->cid, "files", $previous["cat_id"], $previous["id"]);
		}
		$next = DB::fetch_array(DB::query(
		"SELECT e.[name], e.id, e.cat_id FROM {files} AS e"
		. ($this->diafan->_user->id ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='files'" : "")
		...
		if ($next)
		{
			...
			$this->result["next"]["id"] = $next["id"];
			$this->result["next"]["link"] = $this->diafan->_route->link($this->diafan->cid, "files", $next["cat_id"], $next["id"]);
		}
	}
	...
	public function show_block($count, $cat_ids, $site_ids, $images, $images_variation, $sort)
	{
		...
		if ($sort == "rand" || ! $this->result = $this->diafan->_cache->get($cache_meta, "files"))
		{
			if(! $this->validate_attribute_site_cat('files', $site_ids, $cat_ids))
			{
				return false;
			}
			...
			if ($sort == "rand")
			{
				...
				"SELECT COUNT(DISTINCT e.id) FROM {files} as e"
				.($cat_ids
				  ? " INNER JOIN {files_category_rel} as c ON c.element_id=e.id"
			...
			{
				$result = DB::query_range(
				"SELECT e.id, e.[name],e.[anons], e.cat_id, e.timeedit, e.site_id, e.link FROM {files} AS e"
				.($cat_ids
				? " INNER JOIN {files_category_rel} as r ON r.element_id=e.id"
			...
			{
				$cat = DB::fetch_array(DB::query("SELECT [name], site_id, id FROM {files_category} WHERE id=%d LIMIT 1", $cat_ids[0]));
				
				$this->result["name"] = $cat["name"];
				$this->result["link_all"] = $this->diafan->_route->link($cat["site_id"], 'files', $cat["id"]);
			...
			if ($sort != "rand")
			{
				$this->diafan->_cache->save($this->result, $cache_meta, "files");
			}
		}
	...
	public function show_block_rel($count, $images, $images_variation)
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, "files"))
		{
			$this->result["rows"] = array();
	...
	public function get_elements($result, $images_config = '')
	{
		...
		while ($row = DB::fetch_array($result))
		{
			if (! $this->diafan->configmodules("cat", "files", $row["site_id"]))
			{
				$row["cat_id"] = 0;
			}
			...
			if ($row["link"])
			{
				$row["link_file"] = $row["link"];
			}
			else
			{
			    $result_file = DB::query("SELECT id, name, size FROM {attachments} WHERE module_name='files' AND element_id='%d' AND trash='0'", $row["id"]);
			    if ($row_file = DB::fetch_array($result_file))
			    {
				$row["link_file"] = BASE_PATH.'attachments/get/'.$row_file["id"]."/".$row_file["name"];
				$row["size"] = $this->diafan->convert($row_file["size"]);
			    }
			}

			$row["link"] = $this->diafan->_route->link($row["site_id"], "files", $row["cat_id"], $row["id"]);
			unset($row["cat_id"]);

			if ($this->diafan->configmodules("images", "files", $row["site_id"]))
			{
				if (is_array($images_config) && $images_config["count"] > 0)
				{
					$row["img"]  = $this->diafan->_images->get(
							$images_config["variation"], $row["id"], 'files',
							$row["site_id"], $row["name"], 0, false,
							$images_config["count"], $row["link"]
						);
				}
				elseif($this->diafan->configmodules("list_img", "files", $row["site_id"]))
				{
					$row["img"]  = $this->diafan->_images->get(
							'medium', $row["id"], 'files', $row["site_id"], $row["name"], 0, false,
					        $this->diafan->configmodules("list_img", "files", $row["site_id"]) == 1 ? 1 : 0,
					        $row["link"]
						);
				}
			}		
	...
	private function get_children_category($parent_id, $time)
	{
		...
		{
			$child["link"] = $this->diafan->_route->link($child["site_id"], 'files', $child["id"]);
			if ($this->diafan->configmodules("images_cat") && $this->diafan->configmodules("list_img_cat"))
			{
