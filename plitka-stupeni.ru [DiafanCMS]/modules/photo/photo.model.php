...
class Photo_model extends Model
{
	...
	public function list_()
	{
		...
		if ($this->result["rows"])
		{
			foreach ($this->result["rows"] as $i => $row)
			{
				...
				$this->result["rows"][$i]["name"] = $this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'photo', _LANG);

				$this->result["rows"][$i]["anons"] = $this->diafan->_useradmin->get($row["anons"], 'anons', $row["id"], 'photo',  _LANG);
			}
		}
		$this->theme_view();
	...
	private function list_query_count($time)
	{
		$count = DB::query_result(
			...
			.($this->diafan->_user->id ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='photo'" : "")
	...
	private function list_query($time)
	{
		$result = DB::query_range(
			"SELECT e.id, e.[name], e.[anons], e.timeedit, e.cat_id, e.site_id FROM {photo} AS e"
			.($this->diafan->_user->id ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='photo'" : "")
			." WHERE e.[act]='1' AND e.trash='0' AND e.site_id=%d"
			." AND e.date_start<=%d AND (e.date_finish=0 OR e.date_finish>=%d)"
			." AND (e.access='0'"
	...
	public function first_page()
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, $this->diafan->module))
		{
			...
			while ($row = DB::fetch_array($result))
			{
				...
				{
					$cat_ids = $this->diafan->get_children($row["id"], "photo_category");
					$cat_ids[] = $row["id"];
				}
				else
				{
				...
				$cat["link_all"] = $this->diafan->_route->link($row["site_id"], 'photo', $row["id"]);
		...
		if (! empty($this->result["categories"]))
		{
			foreach ($this->result["categories"] as $cat => $rows)
			{
				...
				$this->result["categories"][$cat]["name"] = $this->diafan->_useradmin->get($rows["name"], 'name', $rows["id"], 'photo_category', _LANG);
				$this->result["categories"][$cat]["anons"] = $this->diafan->_useradmin->get($rows["anons"], 'anons', $rows["id"], 'photo_category', _LANG);
				...
				foreach ($rows["children"] as $k => $row)
				{
					$this->result["categories"][$cat]["children"][$k]["name"] =
						$this->diafan->_useradmin->get($row["name"], 'name', $k, 'photo_category', _LANG);

					$this->result["categories"][$cat]["children"][$k]["anons"] =
						$this->diafan->_useradmin->get($row["anons"], 'anons', $k, 'photo_category', _LANG);
				}
				...
				foreach ($rows["rows"] as $k => $row)
				{
					...
					$this->result["categories"][$cat]["rows"][$k]["name"] =
						$this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'photo', _LANG);

					$this->result["categories"][$cat]["rows"][$k]["anons"] =
						$this->diafan->_useradmin->get($row["anons"], 'anons', $row["id"], 'photo', _LANG);
				}
			}
		}
	...
	private function first_page_cats_query()
	{
		...
		.($this->diafan->_user->id ? " LEFT JOIN {access} AS a ON a.cat_id=c.id AND a.module_name='photo'" : "")
	...
	{
		$result = DB::query_range(
		"SELECT e.id, e.[name], e.cat_id, e.timeedit, e.[anons], e.site_id FROM {photo} AS e"
	...
	public function list_category()
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, $this->diafan->module))
		{
			...
			$this->result["level"] = count($this->diafan->get_parents($this->diafan->cat, 'photo_category'));
			$this->result["path"] = $this->get_path('photo');
			...
			if ($this->diafan->configmodules("children_elements"))
			{
				$cat_ids = $this->diafan->get_children($this->diafan->cat, "photo_category");
		...
		$this->result["text"] = $this->diafan->_useradmin->get($this->result["text"], 'text', $this->diafan->cat, 'photo_category', _LANG);
		...
		if ($this->result["rows"])
		{
			...
			foreach ($this->result["children"] as $i => $row)
			{
				$this->result["children"][$i]["name"] = $this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'photo_category', _LANG);
				$this->result["children"][$i]["anons"] = $this->diafan->_useradmin->get($row["anons"], 'anons', $row["id"], 'photo_category', _LANG);
			}
			foreach ($this->result["path"] as $k => $path)
			{
				if ($k == 0)
					continue;
				$this->result["path"][$k]["name"] = $this->diafan->_useradmin->get($path["name"], 'name', $path["id"], 'photo_category', _LANG);
			}
		}
		$this->result["paginator"] = $this->diafan->_tpl->get('get', 'paginator', $this->result["paginator"]);
		return $this->result;
	...
	{
		$result = DB::query("SELECT id, [name], [text], timeedit, [descr], [keywords], sort, parent_id, [title_meta], access, theme, view FROM {photo_category}"
	...
	private function list_category_previous_next($sort, $parent_id)
	{
		...
		if ($previous)
		{
			...
			$this->result["previous"]["link"] = $this->diafan->_route->link($this->diafan->cid, "photo", $previous["id"]);
		}
		...
		if ($next)
		{
			...
			$this->result["next"]["link"] = $this->diafan->_route->link($this->diafan->cid, "photo", $next["id"]);
		}
	}
	...
	public function id()
	{
		...
		$this->result["text"]  = $this->diafan->_useradmin->get($this->result["text"], 'text', $this->diafan->show, 'photo', _LANG);
		...
		if (! empty($_SERVER["HTTP_X_REQUESTED_WITH"]) && ! empty($_POST["module_ajax"]) && $_POST["module_ajax"] == "photo")
		{
			header('Content-Type: text/html; charset=utf-8');
			$this->result["ajax"] = true;
			$this->diafan->_tpl->get('id', 'photo', $this->result);
			exit;
		}
	...
	{
		$row = DB::fetch_array(DB::query("SELECT id, [name], [anons], [text], timeedit, cat_id, [keywords], [descr], sort,"
		." [title_meta], access, theme, view FROM {photo}"
		." WHERE [act]='1' AND id = %d AND trash='0' AND site_id=%d"
	...
	{
		$previous = DB::fetch_array(DB::query(
		"SELECT e.[name], e.id, e.cat_id FROM {photo} AS e"
		. ($this->diafan->_user->id ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='photo'" : "")
		. " WHERE e.[act]='1' AND e.trash='0' AND e.site_id=%d"
		...
		if ($previous)
		{
			...
			$this->result["previous"]["id"] = $previous["id"];
			$this->result["previous"]["link"] = $this->diafan->_route->link($this->diafan->cid, "photo", $previous["cat_id"], $previous["id"]);
		}
		$next = DB::fetch_array(DB::query(
		"SELECT e.[name], e.id, e.cat_id FROM {photo} AS e"
		. ($this->diafan->_user->id ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='photo'" : "")
		...
		if ($next)
		{
			...
			$this->result["next"]["id"] = $next["id"];
			$this->result["next"]["link"] = $this->diafan->_route->link($this->diafan->cid, "photo", $next["cat_id"], $next["id"]);
		}
	}
	 ...
	 * @return array
	 */
	public function show_block($count, $cat_ids, $site_ids, $sort, $images_variation)
	{
		$time = mktime(23, 59, 0, date("m"), date("d"), date("Y"));
		...
		if ($sort == "rand" || ! $this->result = $this->diafan->_cache->get($cache_meta, "photo"))
		{
			if(! $this->validate_attribute_site_cat('photo', $site_ids, $cat_ids))
			{
				return false;
			}
			...
			if ($sort == "rand")
			{
				...
				"SELECT COUNT(DISTINCT e.id) FROM {photo} as e"
				.($cat_ids
				  ? " INNER JOIN {photo_category_rel} as c ON c.element_id=e.id"
			...
			{
				$result = DB::query_range(
				"SELECT e.id, e.[name],e.[anons], e.cat_id, e.timeedit, e.site_id FROM {photo} AS e"
				.($cat_ids
				? " INNER JOIN {photo_category_rel} as r ON r.element_id=e.id"
				...
				$this->result["rows"] = array_merge($this->result["rows"], $this->get_elements($result, $images_variation));
			}

			// если категория только одна, задаем ссылку на нее
			if (!empty($this->result["rows"]) && count($cat_ids) == 1)
			{
				$cat = DB::fetch_array(DB::query("SELECT [name], site_id, id FROM {photo_category} WHERE id=%d LIMIT 1", $cat_ids[0]));
				
				$this->result["name"] = $cat["name"];
				$this->result["link_all"] = $this->diafan->_route->link($cat["site_id"], 'photo', $cat["id"]);
			...
			if ($sort != "rand")
			{
				$this->diafan->_cache->save($this->result, $cache_meta, "photo");
			}
		}
	...
	public function show_block_rel($count, $images_variation)
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, "photo"))
		{
			$this->result["rows"] = array();
			...
			$this->result["rows"] = $this->get_elements($result, $images_variation);
			$this->diafan->_cache->save($this->result, $cache_meta, "photo");
		}
	 ...
	 * @return array
	 */
	public function get_elements($result, $images_variation = 'medium')
	{
		...
		while ($row = DB::fetch_array($result))
		{
			...
			{
				$row["link"] = $this->diafan->_route->link($row["site_id"], "photo", $row["cat_id"], $row["id"]);
			}
			else
			{
				$row["link"] = 'large';
	...
	private function get_children_category($parent_id, $time)
	{
		...
		{
			$child["link"] = $this->diafan->_route->link($child["site_id"], 'photo', $child["id"]);
			if ($this->diafan->configmodules("images_cat") && $this->diafan->configmodules("list_img_cat"))
			{
			...
			if($this->diafan->configmodules("count_child_list"))
			{
				if ($this->diafan->configmodules("children_elements"))
				{
					$cat_ids = $this->diafan->get_children($child["id"], "photo_category");
					$cat_ids[] = $child["id"];
				}
				else
				{
