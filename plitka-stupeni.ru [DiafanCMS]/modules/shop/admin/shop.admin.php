...
class Shop_admin extends Frame_admin
{
	...
	public function prepare_config()
	{
		...
		if(isset($isNowCat[2]))
		{
			...
			if($nowType == 'p' && $nowID > 0)
			{
				$res = DB::query('SELECT rel_element_id FROM {shop_rel} WHERE element_id = "'.$nowID.'" AND trash = "0"');
				$inArr = array();
				while($s = DB::fetch_array($res))
				{
					$inArr[] = $s['rel_element_id'];
				}
				if(!empty($inArr)) $this->where = ' AND e.id IN ('.implode(',', $inArr).')';
			}

			if($nowType == 'c' && $nowID > 0)
			{
				$res = DB::query('SELECT r.rel_element_id FROM {shop} s
										LEFT JOIN {shop_rel} r ON r.element_id = s.id AND r.trash = "0"
									WHERE s.cat_id = "'.$nowID.'" AND s.trash = "0"');
				$inArr = array();
				while($s = DB::fetch_array($res))
				{
					$inArr[] = $s['rel_element_id'];
