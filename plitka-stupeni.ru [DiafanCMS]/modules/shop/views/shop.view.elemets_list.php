...
<div class="text">
	...
	<?
			...
			if(!empty($result['anons']) && empty($this->diafan->page) && !isset($_GET['view']))
			{
				$this->htmleditor('<index>'.$result['anons'].'</index>');
			}
			else if(!empty($result['anons'])&& !isset($_GET['view']))
			{
				$this->htmleditor('<noindex>'.$result['anons'].'</noindex>');
			}
			echo '</div>';
	?>
...
if (!empty($result["rows"]))
{
	...
	$get_param_string = '?cat_ids='.$result["cat_ids"].'&sort_type='.$sort_type.'&page='.$page;
		...
		echo '<div class="slider2">';
			...
			*/
			...
			echo '<div class="slider1_t">';
				...
				# print_r($categorys);
				echo '<a href="' . BASE_PATH_HREF . $row["link"] . $get_param_string . '">' . $row["name"] . '</a>';
				echo '<span class="slider1_span1"><a href="'.BASE_PATH_HREF.$categorys['factory_url'].'/">'.$categorys['factory'].'</a></span>';
...
if(!empty($result['text']) && empty($this->diafan->page) && !isset($_GET['view']))
{
	echo '<div class="text_t">';
