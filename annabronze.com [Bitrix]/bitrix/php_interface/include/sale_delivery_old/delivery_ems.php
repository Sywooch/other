...
class CDeliveryEMS
{
	...
	function __GetLocation($location)
	{
		...
		{
			static $arEMSRussiaList;

			if (!is_array($arEMSRussiaList))
			{
				if (file_exists(dirname(__FILE__).'/ems/russia.php'))
					require_once(dirname(__FILE__).'/ems/russia.php');
			}
			...
			//$arLocation['REGION_NAME_ORIG'] = ToUpper($arLocation['CITY_NAME_ORIG']);
			//$arLocation['REGION_SHORT_NAME'] = ToUpper($arLocation['CITY_SHORT_NAME']);
			//$arLocation['REGION_NAME_LANG'] = ToUpper($arLocation['CITY_NAME_LANG']);
			//$arLocation['REGION_NAME'] = ToUpper($arLocation['CITY_NAME']);
			

			if (is_array($arEMSRussiaList))
			{
				//if (array_key_exists($arLocation['CITY_NAME_ORIG'], $arEMSRussiaList))
				//	$arLocation['EMS_ID'] = $arEMSRussiaList[$arLocation["CITY_NAME_ORIG"]];
				//elseif (array_key_exists($arLocation['CITY_SHORT_NAME'], $arEMSRussiaList))
				//	$arLocation['EMS_ID'] = $arEMSRussiaList[$arLocation["CITY_SHORT_NAME"]];
				//elseif (array_key_exists($arLocation['CITY_NAME_LANG'], $arEMSRussiaList))
				//	$arLocation['EMS_ID'] = $arEMSRussiaList[$arLocation["CITY_NAME_LANG"]];
				//elseif (array_key_exists($arLocation['CITY_NAME'], $arEMSRussiaList))
				//	$arLocation['EMS_ID'] = $arEMSRussiaList[$arLocation["CITY_NAME"]];
				...
				{
					if (array_key_exists($arrNames[$i], $arEMSRussiaList))
					{
						$arLocation['EMS_ID'] = $arEMSRussiaList[$arrNames[$i]];
						break;
					}
				}
				
				//// Если города нет в списке EMS ищем по региону
				//if (empty($arLocation['EMS_ID']))
				//{
				//	$arrRegionNames = array(ToUpper($arLocation['REGION_NAME_ORIG']),
				//							ToUpper($arLocation['REGION_SHORT_NAME']),
				//							ToUpper($arLocation['REGION_NAME_LANG']),
				//							ToUpper($arLocation['REGION_NAME'])
				//							);
				//	$intNamesCou = count($arrRegionNames);
				//	for ($i=0; $i<$intNamesCou; $i++)
				//	{
				//		foreach ($arEMSRussiaList as $strName => $strID)
				//		{
				//			if (mb_strpos($strName, $arrRegionNames[$i]) !== false)
				//			{
				//				$arLocation['EMS_ID'] = $strID;
				//				break;
				//			}
				//		}
				//	}
			...
			{
				$arLocation['EMS_RUSSIA_NOT_LOADED'] = true;
			}
		}
		else
		{
	...
	function Calculate($profile, $arConfig, $arOrder, $STEP, $TEMP = false)
	{
		...
		if (isset($arLocationTo['EMS_RUSSIA_NOT_LOADED']))
		{
			// get cities and proceed to next step

			$data = CDeliveryEMS::__EMSQuery('ems.get.locations', array('type' => 'russia', 'plain' => 'true'));
			//pre($data, true);
			...
			if ($fp = fopen(dirname(__FILE__)."/ems/russia.php", "w"))
			{
				fwrite($fp, '<'."?\r\n");
				fwrite($fp, '$'."arEMSRussiaList = array();\r\n");
				foreach ($arCitiesList as $key => $value)
				{
					fwrite($fp, '$'."arEMSRussiaList['".addslashes($key)."'] = '".htmlspecialcharsbx(trim($value))."';\r\n");
				}
				fwrite($fp, '?'.'>');
				fclose($fp);
			}
