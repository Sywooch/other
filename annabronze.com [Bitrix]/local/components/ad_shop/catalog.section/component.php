...
//echo "FILTER_NAME ".$arParams["FILTER_NAME"]."<br>";
...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
	...
	$time = $time_1 - $time_start;
	//echo "".$time." seconds<br>";
	//#########


	unset($arPriceFilter);
	$intIBlockID = $arResult["IBLOCK_ID"];
	...
	while($arItem = $rsElements->GetNext()) {
		...
		if (CCatalogSKU::IsExistOffers($arItem["ID"], $intIBlockID)) {
			...
			//'ACTIVE' => "Y", '!PRICE' => "",
			$rsOffers = CIBlockElement::GetList(array("PRICE" => "ASC"), array('ACTIVE' => "Y", 'IBLOCK_ID' => $mxResult['IBLOCK_ID'], 'PROPERTY_' . $mxResult['SKU_PROPERTY_ID'] => $arItem["ID"]));
			$log=0;
			...
			while ($arOffer = $rsOffers->GetNext()) {
				...
				if(!empty($ar_price["PRICE"])){ $log=1; break; }

			}
			if($log != 0){
				$arPriceFilter[]=$arItem["ID"];
			}
		}
	...
	if($arParams["COLOR_FILTER_COLOR_NUMBER"]!=""){

		unset($arColorFilter);
		//сформировть массив идентификаторов товаров, которые подходят под выбранный цвет
		...
		while($arItem = $rsElements->GetNext()) {
			...
			{
				//'ACTIVE' => "Y", '!PRICE' => "",
				$rsOffers_color = CIBlockElement::GetList(array("PRICE"=>"ASC"),array('ACTIVE' => "Y", 'IBLOCK_ID' => $mxResult['IBLOCK_ID'], 'PROPERTY_'.$mxResult['SKU_PROPERTY_ID'] => $arItem["ID"]));
				...
				while ($arOffer_color = $rsOffers_color->GetNext()){
					...
					$ar_price = GetCatalogProductPrice($arOffer_color["ID"], 1);
					if(empty($ar_price["PRICE"])) continue;


					$db_props = CIBlockElement::GetProperty($arOffer_color["IBLOCK_ID"], $arOffer_color["ID"], array("sort" => "asc"), Array("CODE"=>$colorProperty));
		...
		// получить пересечение $arPriceFilter и $arColorFilter
		$arColorFilter = array_intersect($arPriceFilter, $arColorFilter);




		if(empty($arColorFilter)){
			$arFilter["ID"]=array("0");
		}else{
			$arFilter["ID"]=$arColorFilter;
		}



	}else{
		$arFilter["ID"]=$arPriceFilter;
	}
	...
	$time = $time_2 - $time_1;
	//echo $time_2." - ".$time_1."<br>";
	//echo "".$time." seconds <br>";
	//#########


	if($arParams["ELEMENT_SORT_FIELD"]=="PROPERTY_MINIMUM_PRICE" || $arParams["ELEMENT_SORT_FIELD"]=="PROPERTY_CML2_ARTICLE2"){
		...
		//список товаров текущего инфоблока и раздела
		$rsElements = CIBlockElement::GetList($arSort, array_merge($arrFilter, $arFilter), false, false, $arSelect);
		$arActiveColorTmp = Array();
		while($arItem = $rsElements->GetNext()) {
			...
			if(CCatalogSKU::IsExistOffers($arItem["ID"], $arItem["IBLOCK_ID"]))
			{
				...
				$db_props = CIBlockElement::GetProperty($arItem["IBLOCK_ID"], $arItem["ID"], array("sort" => "asc"), Array("CODE"=>"MINIMUM_PRICE"));
				if($ar_props = $db_props->Fetch()){

					if($ar_props["VALUE"]==""){
						//минимальная цена не задана

						//достать торговое предложение с минимальной ценой
						//'ACTIVE' => "Y", '!PRICE' => "",
						$rsOffers = CIBlockElement::GetList(array("CATALOG_PRICE_1"=>"ASC"),array('ACTIVE' => "Y", 'IBLOCK_ID' => $mxResult['IBLOCK_ID'], 'PROPERTY_'.$mxResult['SKU_PROPERTY_ID'] => $arItem["ID"]));
						while ($arOffer = $rsOffers->GetNext()) {

							$ar_price = GetCatalogProductPrice($arOffer["ID"], 1);

							CIBlockElement::SetPropertyValueCode($arItem["ID"], "MINIMUM_PRICE", $ar_price["PRICE"]);
				...
				if($arParams["COLOR_FILTER_COLOR_NUMBER"]!="") {
					// id элемента  -  цена активного цвета

					//'ACTIVE' => "Y", '!PRICE' => "",
					$rsOffers_colorFilter = CIBlockElement::GetList(array("CATALOG_PRICE_1"=>"ASC"),array('ACTIVE' => "Y", 'IBLOCK_ID' => $mxResult['IBLOCK_ID'], 'PROPERTY_'.$mxResult['SKU_PROPERTY_ID'] => $arItem["ID"]));
					while ($arOffer_colorFilter = $rsOffers_colorFilter->GetNext()) {



						$db_props_colorFilter = CIBlockElement::GetProperty($arOffer_colorFilter["IBLOCK_ID"], $arOffer_colorFilter["ID"], array("sort" => "asc"),
							Array("CODE"=>$colorProperty));
						if($ar_props_colorFilter = $db_props_colorFilter->Fetch()){
							$COLOR_colorFilter = $ar_props_colorFilter["VALUE"];

						}

						$COLOR_INDEX_colorFilter=$CATALOG_COLORS[$COLOR_colorFilter];

						if($COLOR_INDEX_colorFilter == $arParams["COLOR_FILTER_COLOR_NUMBER"]){
							$ar_price_colorFilter = GetCatalogProductPrice($arOffer_colorFilter["ID"], 1);
							$arActiveColorTmp[$arItem["ID"]] = $ar_price_colorFilter["PRICE"];
							break;
						}
		...
		if($arParams["COLOR_FILTER_COLOR_NUMBER"]!="") {
			...
			if($arParams["ELEMENT_SORT_FIELD"] == "PROPERTY_MINIMUM_PRICE"){
				...
				if($arParams["ELEMENT_SORT_ORDER"] == "desc"){
					arsort($arActiveColorTmp);
				}else{
					asort($arActiveColorTmp);
				}
	...
	if($arParams["COLOR_FILTER_COLOR_NUMBER"]!="") {
		...
		foreach($arActiveColorTmp as $k => $v) {

			//'ACTIVE' => "Y", '!PRICE' => "",
			$rsElements = CIBlockElement::GetList($arSort, Array('ACTIVE' => "Y", "ID" => $k), false, false, $arSelect);
		...
		$countOnPage = $arNavParams["nPageSize"];
		...
		$arResult["ITEMS"] = array_slice($elements, (($page - 1) * $countOnPage), $countOnPage);
...
//===============здесь узнаем количество остатков на складах в городе для всех товаров на странице=============================
$this->AbortResultCache();
	...
	foreach($arResult["ITEMS"] as &$arItems)
	{
		if($arItems["ID"] == $arStore["PRODUCT_ID"])
		{
			if(!empty($arResult["AMOUNTS"][$arItems["ID"]])) {
				$arResult["AMOUNTS"][$arItems["ID"]]["AMOUNT"] = $arResult["AMOUNTS"][$arItems["ID"]]["AMOUNT"] + $arStore["AMOUNT"];
			}
...
//получить все категории одного уровня с текущей
$sectionId=$arResult["ID"];
unset($arSectionsThisLevel);


if($arResult["IBLOCK_SECTION_ID"]==""){
	//категория лежит в корне

	$arFilter = Array('IBLOCK_ID'=>$arResult["IBLOCK_ID"], 'GLOBAL_ACTIVE'=>'Y', 'DEPTH_LEVEL'=>"1");
	$db_list = CIBlockSection::GetList(Array('NAME' => 'asc'), $arFilter, true);


	while($ar_result = $db_list->GetNext())
	{
		$arSectionsThisLevel[]=$ar_result;
	}
	...
	$blockList = $Block->GetList($arORDER, $arFILTER, false);
...
$arResult["SectionsThisLevel"]=$arSectionsThisLevel;
...
//перенесено из result_modifier (START)
...
if(LANGUAGE_ID==='en'){
	$IBLOCK_ID = CATALOG_IBLOCK_OFFERS_ID_EN;
	$arColors = unserialize(BX_IBLOCK_CATALOG_COLORS_ARRAY_EN);
}else{
	$IBLOCK_ID = CATALOG_IBLOCK_OFFERS_ID_RU;
	$arColors = unserialize(BX_IBLOCK_CATALOG_COLORS_ARRAY_RU);
}
...
foreach($arResult["ITEMS"] as $cell=>$arElement)
{
	...
	if (is_array($mxResult)) {
		//'ACTIVE' => "Y", '!PRICE' => "",
		$rsOffers = CIBlockElement::GetList(array("PRICE" => "ASC"), array('ACTIVE' => "Y", 'IBLOCK_ID' => $mxResult['IBLOCK_ID'], 'PROPERTY_' . $mxResult['SKU_PROPERTY_ID'] => $arElement["ID"]));
	}
	...
	if(CCatalogSKU::IsExistOffers($arElement["ID"], $intIBlockID))
	{
		...
		while ($arOffer = $rsOffers->GetNext()){
			...
			//$offerQuantity = $offerParameters["QUANTITY"];
			...
			//}else{
			//	$obStoreOffer = CCatalogStoreProduct::GetList(array(), array('PRODUCT_ID' =>$arOffer['ID']), false,false,array());
			//}
			...
			//$arOffer["QUANTITY"] = $offerQuantity;//доступное количество

			//зануляем доступное количество, если оно <0
			if($arOffer["QUANTITY"] < 0){ $arOffer["QUANTITY"] = 0; }

			$db_props = CIBlockElement::GetProperty($arOffer["IBLOCK_ID"], $arOffer["ID"], array("sort" => "asc"), Array("CODE"=>$colorProperty));
			if($ar_props = $db_props->Fetch()){
				$COLOR = $ar_props["VALUE"];
			...
			while($res_arr = $res->Fetch()) {
				...
				if ($res_arr['CODE'] == $colorProperty) {//"ID" => $arOffer["ID"],
					$property_enums = CIBlockPropertyEnum::GetList(Array("DEF" => "DESC", "SORT" => "ASC"), Array("IBLOCK_ID" => $IBLOCK_ID, "CODE" => $colorProperty));
		...
		if(count($arResult["ITEMS"][$cell]["OFFERS"]) == 0){
			unset($arResult["ITEMS"][$cell]);
			continue;
		}


		$arrOffersTmp = array_msort($arResult["ITEMS"][$cell]["OFFERS"], array('COLOR_INDEX'=>SORT_ASC));//'COLOR'=>SORT_ASC
		$arResult["ITEMS"][$cell]["OFFERS"] = $arrOffersTmp;
		...
		while ($arOffer = $rsOffers->GetNext())
		{
			...
			$arResult["ITEMS"][$cell]["MIN_PRODUCT_OFFER_PRICE"] = $ar_price["PRICE"];
			$arResult["ITEMS"][$cell]["MIN_PRODUCT_OFFER_PRICE_PRINT"] = $ar_price["PRICE"]." ".$ar_price["CURRENCY"];
			$arResult["ITEMS"][$cell]["MIN_PRODUCT_PRICE"] = $ar_price["PRICE"];
			$arResult["ITEMS"][$cell]["MIN_PRODUCT_PRICE_PRINT"] = $ar_price["PRICE"]." ".$ar_price["CURRENCY"];
		...
		//выбираем случайное торговое предложение из самых дешёвых
		//исключить одинаковых соседей
		//выбираем из бронзы и латуни


		$arOffersPriceTmp = Array();
		foreach($arResult["ITEMS"][$cell]["OFFERS"] as $k => $v){
			$arOffersPriceTmp[$k] = $v["PRICE"];
		}


		asort($arOffersPriceTmp);

		$minimalOfferPrice = current($arOffersPriceTmp);

		$arOffersNumbersTmp = Array();

		foreach($arOffersPriceTmp as $k => $v){
			if($v == $minimalOfferPrice){
				$arOffersNumbersTmp[] = $k;
			}
		}
		$countOffers=count($arOffersNumbersTmp);
		...
		while(1){
			...
			$randomOffer=$arOffersNumbersTmp[$randomOffer];
			

			if($arResult["ITEMS"][$cell]["OFFERS"][$randomOffer]["COLOR_INDEX"] != $lastRandomOffer
				&& in_array($arResult["ITEMS"][$cell]["OFFERS"][$randomOffer]["COLOR"], $arColors)){
		...
		//срабатывание фильтра цветов
		//--------------------------------------------------------------
		//if(isset($_GET["color"]) && $_GET["color"]!=""){
		if($arParams["COLOR_FILTER_COLOR_NUMBER"]!=""){
			foreach($arResult["ITEMS"][$cell]["OFFERS"] as $k => $v){
				if($v["COLOR_INDEX"]==$arParams["COLOR_FILTER_COLOR_NUMBER"]){
					$randomOffer=$k;
					break;
				}
			}
...
//перенесено из result_modifier (END)
