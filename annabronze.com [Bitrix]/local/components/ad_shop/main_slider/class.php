...
class MainSliderComponent extends CBitrixComponent
{
	...
	protected function checkModules()
	{
		if (!Main\Loader::includeModule('iblock'))
			throw new Main\LoaderException(Loc::getMessage('MAIN_SLIDER_CLASS_IBLOCK_MODULE_NOT_INSTALLED'));
	}
	...
	protected function getResult()
	{
		...
		while ($element = $iterator->GetNext())
		{
			...
			if(!empty($PREVIEW_PICTURE)) {
				$renderImage = CFile::ResizeImageGet($PREVIEW_PICTURE, Array("width" => 1230));
				$PREVIEW_PICTURE = $renderImage['src'];
			}

			$db_props = CIBlockElement::GetProperty($this->arParams['IBLOCK_ID'], $element['ID'], array("sort" => "asc"),
				Array("CODE"=>"LINK"));
