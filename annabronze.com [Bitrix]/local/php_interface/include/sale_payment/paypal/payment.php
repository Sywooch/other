	...
	<table border="0" width="100%" cellpadding="2" cellspacing="2">
		<form action="https://www.<?=$domain?>paypal.com/cgi-bin/webscr" method="post">
			<td align="center">
				...
				if(!empty(CSalePaySystemAction::GetParamValue("SHOULD_PAY"))){
				...
				if(strlen(CSalePaySystemAction::GetParamValue("NOTIFY_URL"))>0)
				{
					...
					$url = CSalePaySystemAction::GetParamValue("NOTIFY_URL");

					$arUrl = parse_url($url);

					$newUrl = $arUrl["scheme"]."://".$_SERVER["SERVER_NAME"].$arUrl["path"];
				...
				if(strlen(CSalePaySystemAction::GetParamValue("RETURN"))>0)
				{
					...
					$url = CSalePaySystemAction::GetParamValue("RETURN");
