...
<div class="b-popup" id="gallery_popup" >
    ...
    <div class="b-popup-gallery js-popup-gallery">
        ...
        <? if(!empty($arIBlockElement["PROPERTIES"]["MORE_PHOTO"]["VALUE"])){  ?>
        ...
        <?
            ...
            if(!empty($arIBlockElement["PREVIEW_PICTURE"])){
                $img = CFile::GetFileArray($arIBlockElement["PREVIEW_PICTURE"]);
                $img = CFile::ResizeImageGet( $img, array( "width" => 68, "height" => 68 ), BX_RESIZE_IMAGE_EXACT, true, array() );
                $imgBigDefault = $img["src"];
            }else{
                $imgBigDefault = BX_DEFAULT_NO_PHOTO_IMAGE;
                $noDefaultPhoto = 1;
            }
