					...
					<li class="compare_item_ws">
						<div class="image">
							<a href="<?=$arItem["DETAIL_PAGE_URL"]?>">
								...
								<?}else{?>
									<img src="<?=SITE_TEMPLATE_PATH?>/images/noimage90.gif" alt="<?=$arItem["NAME"]?>" title="<?=$arItem["NAME"]?>" />
								<?}?>
							</a>
						</div>
						...
						<!--noindex-->
							<a rel="nofollow" class="delete" href="<?=$arItem["DELETE_URL"]?>" item="<?=$arItem["ID"]?>"></a>
						<!--/noindex-->
					</li>
				<?}?>
			</ul>
...
<script>
	$(document).ready(function() {
		...
		$(".compare_list .delete").click(function()
		{
			var itemID = $(this).attr("item");
			var item = $(".inner_left a[element_id=#"+itemID+"]")
			item.removeClass("active");
			item.attr("onclick", "return addToCompare(this, \'detail\', \'"+item.attr("href")+"\');");
		});
		
		if ($(".compare_list .compare_item_ws").length<=0)
		{
			$(".compare_frame.popup").hide(); 
			$(".jqmOverlay").hide();
		}
		...
		$.get( '<?=SITE_DIR?>catalog/compare.php?ajax_compare=1', $.proxy
		(
			function(data) { jsAjaxUtil.InsertDataToNode('<?=SITE_DIR?>ajax/show_compare.php', 'compare', true); } 
		));
