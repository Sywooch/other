<?if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();?>
<a name="tb"></a>
<a href="<?=$arResult["URL_TO_LIST"]?>"><?=GetMessage("SALE_RECORDS_LIST")?></a>
<br /><br />

<?if( strlen($arResult["ERROR_MESSAGE"]) <= 0 ){?>
	<form class="ishop cart" method="post" action="<?=POST_FORM_ACTION_URI?>">
		<?=bitrix_sessid_post()?>
		<input type="hidden" name="ID" value="<?=$arResult["ID"]?>">
		<?=GetMessage("SALE_CANCEL_ORDER1") ?>
		<a href="<?=$arResult["URL_TO_DETAIL"]?>"><?=GetMessage("SALE_CANCEL_ORDER2")?> #<?=$arResult["ACCOUNT_NUMBER"]?></a>?
		<b><?= GetMessage("SALE_CANCEL_ORDER3") ?></b><br /><br />
		<?= GetMessage("SALE_CANCEL_ORDER4") ?>:<br /><br />
		<textarea name="REASON_CANCELED" cols="60" rows="3" class="b-form__textarea"></textarea><br />

		<input type="hidden" name="CANCEL" value="Y">
		<button class="button2 btn _full" type="submit" name="action"
				value="<?= GetMessage("SALE_CANCEL_ORDER_BTN") ?>">
			<span><?= GetMessage("SALE_CANCEL_ORDER_BTN") ?></span>
		</button>
	</form>
<?}else{
	echo ShowError($arResult["ERROR_MESSAGE"]);
}?>