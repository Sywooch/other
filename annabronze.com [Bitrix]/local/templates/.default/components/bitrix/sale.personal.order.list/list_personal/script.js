	...
	BX.Sale.PersonalOrderComponent.PersonalOrderList = {
		init : function(params)
		{
			...
			Array.prototype.forEach.call(rowWrapper, function(wrapper)
			{
				...
				BX.bindDelegate(wrapper, 'click', { 'class': 'js-sale-order-list-payment-table' }, BX.proxy(function(event)
				{
					event.preventDefault();
					var block = wrapper.getElementsByClassName('spoiler')[0];
					if(block.style.display == 'block'){
						block.style.display = "none";
						event.target.innerHTML = $(event.target).attr("data-hide-text");
					}else{
						block.style.display = "block";
						event.target.innerHTML = $(event.target).attr("data-show-text");
					};
