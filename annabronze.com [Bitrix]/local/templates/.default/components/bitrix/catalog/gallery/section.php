...
if( $count_sections > 0 ){?>
	<!--<div class="shadow-item_info cat_list"><img border="0" alt="" src="<?=SITE_TEMPLATE_PATH?>/images/shadow-item_info.png"></div>
	...
	<!--<div class="sideRight shop">
		...
		<?/*
			...
			if (CModule::IncludeModule("iblock"))
			{
				...
				if($obCache->InitCache(36000, serialize($arFilter), "/iblock/catalog")) { $arCurSection = $obCache->GetVars(); }
				else
				{
					$arCurSection = array();
					...
					if(defined("BX_COMP_MANAGED_CACHE"))
					{
						...
						if ($arCurSection = $dbRes->GetNext()) { $CACHE_MANAGER->RegisterTag("iblock_id_".$arParams["IBLOCK_ID"]); }
						$CACHE_MANAGER->EndTagCache();
					}
					else { if(!$arCurSection = $dbRes->GetNext()) $arCurSection = array(); }
