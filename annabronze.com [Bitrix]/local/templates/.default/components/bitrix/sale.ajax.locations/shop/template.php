...
if ($arParams["AJAX_CALL"] != "Y" && count($arParams["LOC_DEFAULT"]) > 0 && $arParams["PUBLIC"] != "N"):
	...
	foreach ($arParams["LOC_DEFAULT"] as $val):
		...
		if ((($val["ID"] == IntVal($_REQUEST["NEW_LOCATION_".$arParams["ORDER_PROPS_ID"]])) ||
				($val["ID"] == $arParams["CITY"])) && (!isset($_REQUEST["CHANGE_ZIP"]) || $_REQUEST["CHANGE_ZIP"] != "Y"))
		{
			$checked = "checked";
			$isChecked = "Y";
			$disabled = true;
		...
		<div><input onChange="<?=$arParams["ONCITYCHANGE"]?>;" <?=$checked?> type="radio"
					name="NEW_LOCATION_<?=$arParams["ORDER_PROPS_ID"]?>" value="<?=$val["ID"]?>" id="loc_<?=$val["ID"]?>" />
			<label for="loc_<?=$val["ID"]?>"><?=$val["LOC_DEFAULT_NAME"]?></label>
		</div>
	<?endforeach;?>

	<input <? if($isChecked!="Y") echo 'checked';?> type="radio" onclick="newlocation(<?=$arParams["ORDER_PROPS_ID"]?>);"
													name="NEW_LOCATION_<?=$arParams["ORDER_PROPS_ID"]?>" value="0" id="loc_0" />
...
<!--<div class="js-form__preload-container b-form__preload-container " style="position: relative;"><!-- is-loading -->
...
<?if (count($arResult["COUNTRY_LIST"]) > 0):?>
	<div class="b-form__row" style="margin-bottom:0px;">
	<?
	...
	?>
	<select <?if($disabled) echo "disabled";?> id="<?=$arParams["COUNTRY_INPUT_NAME"].$arParams["CITY_INPUT_NAME"]?>"
											   
											   name="<?=$arParams["COUNTRY_INPUT_NAME"].$arParams["CITY_INPUT_NAME"]?>" onChange="<?=$change?>" type="location">
...
<?if (count($arResult["REGION_LIST"]) > 0):?>
	...
	<select <?=$id?> <?if($disabled) echo "disabled";?>
		...
		name="<?=$arParams["REGION_INPUT_NAME"].$arParams["CITY_INPUT_NAME"]?>" onChange="<?=$change?>" type="location">
...
<?if (count($arResult["CITY_LIST"]) > 0):?>
	...
	<select <?=$id?> <?if($disabled) echo "disabled";?>
		...
		name="<?=$arParams["CITY_INPUT_NAME"]?>"<?if (strlen($arParams["ONCITYCHANGE"]) > 0):?> onchange="<?=$arParams["ONCITYCHANGE"]?>"<?endif;?> type="location">
