...
<div class="item_info">
	...
	<div class="right_info">
		...
		<div class="information item_ws">
			...
			<?if ($arResult["CAN_BUY"] || (!empty( $arResult["OFFERS"]) && $arResult["MIN_PRODUCT_OFFER_PRICE_PRINT"]) || !empty( $arResult["PRICES"])){?>
				<div class="middle_info">
					<table cellspacing="0" cellpadding="0" border="0" width="100%"><tr><td>
					
						<?if( !empty( $arResult["OFFERS"] ) ){?>
							<?if ($arResult["MIN_PRODUCT_OFFER_PRICE_PRINT"]):?><div class="price_block bottom10"><div class="price"><span><?=GetMessage("CATALOG_FROM");?> <?=$arResult["MIN_PRODUCT_OFFER_PRICE_PRINT"]?></span></div></div><?endif;?>
			...
			<?}?>
                                ...
                                <!--<span>Остальные склады: </br></span>
                                <?//foreach($arResult["OTHER_STOREHOUSE"] as $other_store):?>
                                    <?//if(count($other_store["ADDR"]) > 0):?>
                                        <?//=$other_store["ADDR"];?> - в количестве: <?//=$other_store["COUNT"];?> штук </br>
                                    <?//endif;?>
                                <?//endforeach;?>-->

			<div class="top_info<?if( !empty( $arResult["OFFERS"]) && !$arResult["MIN_PRODUCT_OFFER_PRICE_PRINT"] ):?> cut<?endif;?>">
                <?if($arResult["ENABLE_PRODUCTS"] == "Y"):?>
                    <div class="available_block">
                        <span><?=GetMessage('CT_IS_AVAILABLE')?>: </br></span>
                        <?foreach($arResult["DEFAULT_STOREHOUSE"] as $def_store):?>
                            <?if(count($def_store["ADDR"]) > 0):?>
                                <span class="address"><?=$def_store["ADDR"];?> </span> <span> (остаток  <?=$def_store["COUNT"];?> шт) </br></span>
                            <?endif;?>
                        <?endforeach;?>
                    </div>
                <?else:?>
                    <div class="noavailable_block">
                        <?=GetMessage('CATALOG_NOT_AVAILABLE')?></br>
                    </div>
                <?endif;?>
