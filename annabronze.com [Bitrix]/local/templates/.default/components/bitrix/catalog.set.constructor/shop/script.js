...
{
	var wrapObj = document.getElementById("set_container");
	$(element.parentNode).css({"width": "0px"});
	...
	BX.ajax.post(
		...
		function(result)
		{
			...
			else
			{
				...
				BX.removeClass(BX.findChild(wrapObj, {className:"bx_item_set_current_price"}, true, false), "new");
				//BX.removeClass(BX.findChild(wrapObj, {className:"price_block"}, true, false), "top"); //I don't know why it doesn't work
				$(".bx_item_set_result_block .price_block").removeClass("top");
			}
	...
	BX.ajax.post(
		...
		function(result)
		{
			...
			if (json.formatOldSum)
			{
				...
				BX("bx_catalog_set_construct_sum_old_price_"+element_id).style.display = "block";
				BX.addClass(BX("bx_catalog_set_construct_sum_price_"+element_id), "new");
			}
			else
			{
				BX("bx_catalog_set_construct_sum_old_price_"+element_id).style.display = "none";
				BX.removeClass(BX("bx_catalog_set_construct_sum_price_"+element_id), "new");
			}
			if (json.formatDiscDiffSum)
			{
...
catalogSetConstructPopup.prototype.catalogSetAdd = function(element, emptyObj)
{
	...
	if (emptyObj)
	{
		...
		var objMarks = BX.findChild(element, {className:"marks"}, true, false);
		var objInfo = BX.findChild(element, {className:"info"}, true, false);
		...
		var newSetItem = BX.create('DIV', {
			props: {className: "bx_kit_item_children"},
			children: [objImg, objMarks, objInfo, objDeleteIcon]
		});

		emptyObj.appendChild(newSetItem);
		BX.removeClass(emptyObj, "bx_kit_item_empty");
...
catalogSetConstructPopup.prototype.equalizeHeights = function()
{
...
catalogSetConstructPopup.prototype.catalogSetDelete = function(element)
{
	...
	var newSetIteminner = BX.create('DIV', {
		props: {className: "table_item_inner"},
		children: [objImg, objMarks, objInfo, objAddIcon]
	});
	
	var newSetItem = BX.create('DIV', {
		props: {className: "bx_kit_item table_item"+discountClass},
		children: [newSetIteminner]
	});
	...
	BX.addClass(empty_obj, "bx_kit_item_empty");
...
function showCatalogSetAdd2BasketPopup(setMainPictureUrl, element_id)
{
	...
	$('.add_item_frame').jqm({trigger: '.add_item_popup', onLoad: function(hash){ onLoadjqm('add_item', hash); }, ajax: BX.ajaxSiteDir+'ajax/add_item.php?SET=Y&ELEMENT_ID='+element_id});
	$('.add_item_popup').click();

	jsAjaxUtil.InsertDataToNode(BX.ajaxSiteDir+'ajax/show_cart.php', 'basket', false);
	jsAjaxUtil.InsertDataToNode(BX.ajaxSiteDir+'ajax/show_small_cart.php', 'basket_small', false);
