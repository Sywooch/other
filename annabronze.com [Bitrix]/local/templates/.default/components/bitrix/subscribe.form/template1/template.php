...
?>
	<form action="<?=$arResult["FORM_ACTION"]?>" class="ishop">

	<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
		<label for="sf_RUB_ID_<?=$itemValue["ID"]?>">
			<input type="checkbox" style="display:inline" name="sf_RUB_ID[]" id="sf_RUB_ID_<?=$itemValue["ID"]?>" value="<?=$itemValue["ID"]?>"<?if($itemValue["CHECKED"]) echo " checked"?> /> <?=$itemValue["NAME"]?>
		</label><br />
	<?endforeach;?>
		<br />
		<table border="0" cellspacing="0" cellpadding="2">
			...
			<tr>
				<td>
<button class="button" type="submit" name="OK" value="Отправить"><span><?=GetMessage("subscr_form_button")?></span></button></td>
			</tr>
		</table>
	</form>
<?
