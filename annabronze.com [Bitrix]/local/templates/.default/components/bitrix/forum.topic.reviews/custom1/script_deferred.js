...
function PostFormAjaxResponse(response, postform, preorder)
{
	...
	if (result.reload){
		//BX.reload(curpage);
		$(".js-comments__item-ok-message-moderate").show();
	}
	...
	if (result.status)
	{
		...
		if ((!!result.messageID) && (message = BX('message'+result.messageID))){
			...
			$(".js-comments__item-ok-message").show();
		}
...
alert(result.messageID);
	message = BX('message'+result.messageID);
	BX.scrollToNode(message);
*/
	...
	if (result.statusMessage){
		...
		$(".js-comments__item-error-message").show();
		$(".js-comments__item-error-message .js-comments__item-text").html($(result.statusMessage).text());
...
function ValidateForm(form, ajax_type, ajax_post, preorder)
{
	...
	$(".review-text").removeClass("error-input");
	...
	if (MessageLength < 2){
		...
		$(".review-text").addClass("error-input");
	...
	if (AuthorLength == 0){
		...
		$("#REVIEW_AUTHOR").addClass("error-input");
	}

	var MailLength = form.REVIEW_EMAIL.value.length;
	if (MailLength == 0){
		errors += oErrors['no_mail'] + "<br>";
		$("#REVIEW_EMAIL").addClass("error-input");
	}else{
		var pattern = /^([a-z0-9_\.-])+@[a-z0-9-]+\.([a-z]{2,4}\.)?[a-z]{2,4}$/i;
		if(!pattern.test(form.REVIEW_EMAIL.value)){
