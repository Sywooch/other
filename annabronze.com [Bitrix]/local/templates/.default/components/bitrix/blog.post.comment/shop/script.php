...
function waitResult(id)
{
	...
	if(ob.innerHTML.length > 0)
	{
		...
		else
		{
			...
			{
				if(id == 0){//пишем именно комментарий, а не ответ
					$('#new_comment_cont_00').append(ob.innerHTML);
					$('html, body').animate({ scrollTop: ($("#new_comment_cont_00").offset().top - 115) }, 500);

				}else{//пишем ответ
					BX('new_comment_cont_'+id).innerHTML += ob.innerHTML;
				}
				$(".b-comments__form-notice").html("<?=CUtil::addslashes(GetMessage("OK_MESSAGE"))?>");
...
function submitComment()
{
	...
	var errors = "";
	var MessageLength = $("#comment").val().length;



	if (MessageLength < 2){
		errors += oErrors['no_message'] + "<br>";
		$(".lha-textarea").addClass("error-input");
	}
	...
	var AuthorLength = $("#user_name").val().length;
	if (AuthorLength == 0){
		errors += oErrors['no_author'] + "<br>";
		$("#user_name").addClass("error-input");
	}

	var MailLength = $("#user_email").val().length;
	if (MailLength == 0){
