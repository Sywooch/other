	...
	else
	{
		...
		<div class="blog-post-fields blog-edit-fields">
			<div class="blog-post-field blog-post-field-title blog-edit-field blog-edit-field-title">
				<input maxlength="255" size="70" tabindex="1" type="text"
					   name="POST_TITLE" id="POST_TITLE"
					   value="<?=$arResult["PostToShow"]["TITLE"]?>"
					   placeholder="<?=GetMessage("BLOG_SHOW_TITLE");?>"
					   class="b-form__input">
			</div>
			...
			<?if($arParams["ALLOW_POST_CODE"]):?>
				...
				<div class="blog-post-field blog-post-field-code blog-edit-field blog-edit-field-code">
					<label for="CODE" class="blog-edit-field-caption"><?=GetMessage("BLOG_P_CODE")?>: </label>
					<?=$arResult["PATH_TO_POST1"]?><a href="javascript:changeCode()" title="<?=GetMessage("BLOG_CHANGE_CODE")?>"
													  id="post-code-text">
						<?=(strlen($arResult["PostToShow"]["CODE"]) > 0) ? $arResult["PostToShow"]["CODE"] : GetMessage("BLOG_P_CODE");?>
					</a>
					<span id="post-code-input">
						<input maxlength="255" size="70" tabindex="2" type="text" name="CODE" id="CODE"
							   value="<?=$arResult["PostToShow"]["CODE"]?>" class="b-form__input">
						<image id="code_link" title="<?echo GetMessage("BLOG_LINK_TIP")?>" class="linked"
							   src="/bitrix/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>"
							   onclick="set_linked()" />
					</span><?=$arResult["PATH_TO_POST2"]?>
					
				</div>
				<div class="blog-clear-float"></div>
			<?endif;?>
			<div class="blog-post-field blog-post-field-date blog-edit-field blog-edit-field-post-date">
				<span><input type="hidden" id="DATE_PUBLISH_DEF" name="DATE_PUBLISH_DEF"
							 value="<?=$arResult["PostToShow"]["DATE_PUBLISH"];?>"
							 class="b-form__input">
		...
		<div class="blog-post-message blog-edit-editor-area blog-edit-field-text">
			...
			<div class="b-form__row ">
				...
				<label for="TAGS" class="b-form__row-label"><?=GetMessage("BLOG_CATEGORY")?>:</label>
				...
				<div class="b-form__row-input"><?
					if(IsModuleInstalled("search"))
					{
						...
						$APPLICATION->IncludeComponent("bitrix:search.tags.input", "shop", $arSParams);
					}
					else
					{
						?><input type="text" class="b-form__input" id="TAGS" tabindex="4"
								 name="TAGS" size="30" value="<?=$arResult["PostToShow"]["CategoryText"]?>">
						<?
					}?>
				</div>
			</div>
			...
			<div class="b-form__row ">
				...
				<input name="ENABLE_COMMENTS" id="ENABLE_COMMENTS" type="checkbox"
								 value="N"<?if($arResult["PostToShow"]["ENABLE_COMMENTS"] == "N") echo " checked"?>>

				<label for="ENABLE_COMMENTS"><?=GetMessage("BLOG_ENABLE_COMMENTS")?></label>

			</div>


			<div class="b-form__row ">
				<input name="FAVORITE_SORT" id="FAVORITE_SORT" type="checkbox"
					   value="100"<?if(IntVal($arResult["PostToShow"]["FAVORITE_SORT"]) > 0) echo " checked"?>>
				<label for="FAVORITE_SORT"><?=GetMessage("BLOG_FAVORITE_SORT")?></label>
			</div>
		...
		<div class="blog-post-params">
			...
			<div class="blog-post-field blog-post-field-access blog-edit-field">
				...
				<div class="b-form__row ">
					<label class="b-form__row-label"><?=GetMessage("BLOG_ACCESS")?>:</label>
					<div class="b-form__row-input">


						<input name="blog_perms" value="0" onClick="show_special()" id="blog_perms_0"

							   type="radio"<?=$arResult["PostToShow"]["ExtendedPerms"]=="Y" ? "" : " checked"?>>
						<label for="blog_perms_0"><?=GetMessage("BLOG_DEFAULT_PERMS")?></label>
						<br />
						<input name="blog_perms" value="1" onClick="show_special()" id="blog_perms_1"

							   type="radio"<?=$arResult["PostToShow"]["ExtendedPerms"]=="Y" ? " checked" : ""?>>
						<label for="blog_perms_1"><?=GetMessage("BLOG_SPECIAL_PERMS")?></label>
		...
		<?if($arParams["SEO_USE"] == "Y"):
			...
			<div class="blog-post-field blog-post-field-seo blog-edit-field">
				...
				<div id="blog-show-seo-fields"<?if(!$bShowSEO):?> style="display:none;"<?endif;?>>
					...
					<div class="b-form__row ">
						<label class="b-form__row-label"><?=GetMessage("BLOG_SHOW_SEO_TITLE");?>:</label>
						<div class="b-form__row-input">
					...
					<div class="b-form__row ">
						<label class="b-form__row-label"><?=GetMessage("BLOG_SHOW_SEO_TAGS");?>:</label>
						<div class="b-form__row-input">
					...
					<div class="b-form__row ">
						<label class="b-form__row-label"><?=GetMessage("BLOG_SHOW_SEO_DESCRIPTION");?>:</label>
						<div class="b-form__row-input">
							<textarea rows="6" name="SEO_DESCRIPTION" class="b-form__textarea"><?=$arResult["PostToShow"]["SEO_DESCRIPTION"]?></textarea>
		...
		<div class="blog-post-buttons blog-edit-buttons">
			...
			<input tabindex="5" type="submit" name="save"
				   class="btn _full _little-font _inline"
				   value="<?=GetMessage("BLOG_PUBLISH")?>">
			<input type="submit" name="apply"
				   class="btn _full _little-font _inline"
				   value="<?=GetMessage("BLOG_APPLY")?>">
			...
			<?if($arResult["perms"] >= BLOG_PERMS_WRITE):?>
				<input type="submit" name="draft"
					   class="btn _full _little-font _inline"
					   value="<?=GetMessage("BLOG_TO_DRAFT")?>">
			<?endif;?>
		</div>
		</div>
		</form>
