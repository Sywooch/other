...
if (\Bitrix\Main\Loader::includeModule("iblock"))
{
    ...
    if (!empty($arResult['DISPLAY_PROPERTIES']['CLIENT_USER_ID']['VALUE']))
    {
        ...
        $arResult['CLIENT_INFO'] = array(
            'NAME' => $arResult['DISPLAY_PROPERTIES']['CONTACTS_NAME']['VALUE'],
    ...
    while ($arBet = $dbBets->GetNext())
    {
        ...
        if (!array_key_exists($arBet['PROPERTY_USER_ID_VALUE'], $arResult['BETS_INFO']['CARRIERS']))
        {
            ...
            $arResult['BETS_INFO']['CARRIERS'][$arBet['PROPERTY_USER_ID_VALUE']]['AVATAR'] = GetUserFirstAutoPicture($arUser['ID']);
        }
    }
...
if (is_object($cp))
{
    $cp->SetResultCacheKeys(array('DATE_ACTIVE_TO'));
}