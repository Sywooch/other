<?php
if(isset($_GET['edit_ads']) === true && empty($row__00['TITLE']) === false)
{
...
<div class="content category-shipping">
	<div class="container_center wrapper">
		...
		{
			if(isset($_GET['submit_myform']) === true)
			{
				?>
				<h2 style="text-align:center;margin-top:50px;">Сохранено</h2>
				<?php
			}
			?>
			<h1>Редактировать товар - <?=$row__01['NAME']?></h1>
			<form name="category-shipping" method="post">
				<?=bitrix_sessid_post()?>
				...
				<div class="padding_top">
					...
					<input type="text" name="TITLE" value="<?=$row__01['NAME']?>" class="title" placeholder="Что нужно перевезти"/>
					<br/>
					
					<textarea rows="6" cols="104" name="DESCRIPTION" class="description" placeholder="Этажи спуска/подъема, наличие лифтов и прочие детали перевозки"><?=$row__00['BODY']?></textarea>
					<br/>                                
