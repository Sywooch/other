...
<form action="<?=POST_FORM_ACTION_URI?>" name="category-shipping" method="post" enctype="multipart/form-data">
    ...
    <div class="dostavka-block"><a class="container" href="#">Контейнер</a></div>
    ...
    <input type="text" name="PROPERTY[NAME][0]" value="<?=$arResult['ELEMENT']['NAME']?>" class="title" placeholder="Размеры контейнера, что нужно перевезти"/>
    <br/>
    
    <textarea rows="6" cols="104" name="PROPERTY[DETAIL_TEXT][0]" class="description" placeholder="Тип перевозимого груза, способ оплаты и прочие детали перевозки"><?=$arResult['ELEMENT']['DETAIL_TEXT']?></textarea>
    <br/>                                
