...
$arSections = array(
    ...
    '17' => array('css' => 'animals', 'name' => 'Животные'),
    '18' => array('css' => '', 'name' => ''),
    '19' => array('css' => 'rubish', 'name' => 'Вывоз мусора'),
    '20' => array('css' => 'passajir', 'name' => 'Пассажироперевозки'),
    '21' => array('css' => 'mebel', 'name' => 'Мебель'),
    '22' => array('css' => 'pereezd', 'name' => 'Переезд'),
    '23' => array('css' => 'car', 'name' => 'Авто/мото'),
...
<form action="<?=POST_FORM_ACTION_URI?>" name="category-shipping" method="post" enctype="multipart/form-data">
    ...
    <div class="dostavka-block"><a class="<?=$arSections[$SECTION_ID]['css']?>" href="#"><?=$arSections[$SECTION_ID]['name']?></a></div>
    <input type="hidden" name="PROPERTY[IBLOCK_SECTION][]" value="<?=$SECTION_ID?>">
    ...
    <textarea rows="6" cols="104" name="PROPERTY[PREVIEW_TEXT][0]" class="description" placeholder="Этажи спуска/подъема, наличие лифтов и прочие детали перевозки"><?=$arResult['ELEMENT']['PREVIEW_TEXT']?></textarea>
    <br/>                                
    ...
    <select name="PROPERTY[82][0]" class="porter-count">
        <option>Можно ли перевезти?</option>
        <?foreach ($arResult['PROPERTY_LIST_FULL'][82]['ENUM'] as $enumID => $arEnum):?>
        <option value="<?=$enumID?>"><?=$arEnum['VALUE']?></option>
        <?endforeach?>
    </select>
    ...
    <h2>Контакты</h2>
    <?/*<label class="label_check" for="already-posted"><input name="already-posted" id="already-posted" value="1" type="checkbox" /> Я уже размещал запрос на muravey.by</label><br/><br/>*/?>
    
    <input type="text" name="PROPERTY[51][0]" value="<?=$arResult['ELEMENT_PROPERTIES']['51']['0']['VALUE']?>" class="name" placeholder="Имя *"/>
    <a href="#" class="contact-help">Зачем мне нужно указать контакты?</a>
    <br/>
    
    <input type="text" name="PROPERTY[52][0]" value="<?=$arResult['ELEMENT_PROPERTIES']['52']['0']['VALUE']?>" class="email" placeholder="E-mail адрес *"/>
    <br/>
    
    <input type="text" name="PROPERTY[53][0]" value="<?=$arResult['ELEMENT_PROPERTIES']['53']['0']['VALUE']?>" class="phone" placeholder="Мобильный телефон"/>
    <a href="#" class="contact-info">Контактные данные будут доступны только выбранному Вами перевозчику</a>
    <br/>
    
    <input type="hidden" name="PROPERTY[56][0]" value="<?=$USER->GetID()?>" />
