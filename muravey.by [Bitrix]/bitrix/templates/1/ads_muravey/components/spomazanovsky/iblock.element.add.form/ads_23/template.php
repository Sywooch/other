...
<form action="<?=POST_FORM_ACTION_URI?>" name="category-shipping" method="post" enctype="multipart/form-data">
    ...
    <div class="dostavka-block"><a class="car" href="#">Авто/мото</a></div>
    ...
    <input type="text" name="PROPERTY[NAME][0]" value="<?=$arResult['ELEMENT']['NAME']?>" class="title" placeholder="Что нужно перевезти, марка и модель (сколько штук)"/>
    <br/>
    
    <textarea rows="6" cols="104" name="PROPERTY[DETAIL_TEXT][0]" class="description" placeholder="Прочие детали перевозки"><?=$arResult['ELEMENT']['DETAIL_TEXT']?></textarea>
    <br/>                                
    ...
    <textarea rows="6" cols="104" name="PROPERTY[DETAIL_TEXT][0]" placeholder="Способ перевозки (фургон/эвакуатор/автовоз), состояние (на ходу или нет), наличие документов"><?=$arResult['ELEMENT']['DETAIL_TEXT']?></textarea>
    <br/><br/>
