...
#        2 - год написания статьи или  номер страницы навигации
#        3 - месяц
#        4 - число месяца
#        5 - alias статьи
...
/*
 доделать:
 функцию вывода статей, в ней 3 ключевых момента:
 1 вывод статей в соответствии с запросом (учитывающим год, дату, категорию) (запросы готовы, вырабатываются в init_vars и generate_actions)
 2 вывод в соответствии со страницей (например page2)
 3 вывод навигации статей
 +
...
class main_articles extends AbstractClass{
	...
	var $items_count; // количество всех выводимых статей (согласно фильтру, во всех страницах)

	function ArticlesClass(        $sub_alias /*запрашиваемая страница разложенная в массив*/        )
	{
		...
		$sub_alias,   	// путь разложенный в массив
                                                           'articles',    		// название таблицы с которой будем работать
                                                           'articles'        	// название модуля (то как модуль называется в таблице modules)
		);
		...
		$h1,							// главная надпись
		$body,							// тело статьи
		$last_articles,						// список последних статей
		$articles,
		$arrows_articles,					// нумерация
		$keywords,
		$description,
		$tpl,
		...
		if ($this->tpl == 'main')
		{
			...
			$title   = $row['title'] ? $row['title'] : 'Статьи';
			$h1      = $row['h1'] ? $row['h1'] : 'Статьи';
			$body = $row['body'];
		}
		...
		if (count($this->std->alias) == 0)
		{
			...
			$articles_last_onmain = $this->getLastArticles($this->std->settings['articles_on_non_articles_page'], 'last');  // вывод списка статей на неновостную страницу
		}
...
';
  ...
  Calendar.setup(
	...
	/**
	 * генерируем список статей
	 *
	 *
	 * @param string $tpl        - темплейт
	 * @return unknown           - возвращает готовую строку, список статей
	 */
	 ...
	 */
	{
		...
		$_articles,
		$category,
		$_articles_categories_current,                   // шаблон списка статей
		$date_format,
		$_articles_time;
		$body = '';
...
//		$tempstr = str_replace("{NEWS_IMAGE}", $img, $tempstr);
	...
	/**
	 * формируем список статей для  новостной страницы
	 *
	 *
	 * @param unknown_type $articles_limit        - количество выводимых статей на одну страницу
	 * @return unknown                        - возвращает готовую строку, список статей
	 */
	...
	function getLastArticles($articles_limit, $tpl)
	{
		global
		$template,
		$_articles,                   // шаблон списка статей
		$_articles_delimiter,
		$_articles_time;
		$body = '';
		...
		// если не запрашивается конкретная статья
		if (!isset($this->current_url_array[4]) || 1)
		{
			...
			// если просматриваются статьи без указания страницы, то по умолчанию ставим максимально возможную
			...
			# формирование кусочка sql запроса, ограничевающего выводимые статьи
			# необходимо для навигации: много статей выводить на нескольких страницах
			$limit_sql = " LIMIT ".$this->start_page.", ".$articles_limit;

			// запрос определённого количества статей начиная с определённого номера
			$sql = "SELECT * FROM ".$this->db_table." WHERE is_active=1 ORDER BY `timestamp` DESC $limit_sql";

			if ($this->current_url_array[0] == 'articles')
			{

					#2008 если в алиасе присутствует год - запрос к БД статей этого года
					...
					{
						# -----------------------------------------------------------------------
						# вывод данных статьи из результата запроса к БД
						# -----------------------------------------------------------------------
			...
			if(!$body)
			{
				$this->ModulError( "getLastArticles-> таблица статей пуста sql: $sql");
			}
		}
	...
	/**
	 * нумерация статей в обратном порядке
	 *
	 *
	 * @param unknown_type $articles_limit                - количество выводимых статей на одну страницу
	 ...
	 */
	function getArticlesArrows($articles_limit /*количество статей выводимых на страницу*/, $arrows_limit /*количество номерков в строке нумерации*/)
	{
		global
		$host,
		$_articles_page_active,        // указатель на страницу статей, активный
		$_articles_page_unactive,      // указатель на страницу статей, не активный
		$template;                 // глобальна переменная темплейта, для вывода ошибочной страницы

		$arrows_articles        = '';
		$sql         = "SELECT count(*) as count FROM ".$this->db_table." WHERE is_active = 1";
		#2008 если введен год в алиасе - считаем число статей определенного года
		if ( is_numeric($this->current_url_array[1]) )
		{
		...
		if ($articles_count == 0)
		{
			$this->ModulError( "getArticlesArrows-> таблица статей пуста sql: $sql");
		}
		...
		#2008 если после articles стоит цифра (год) - делаем ссылки с годом вида /articles/год/page
		$year_base_url = '';
		if ( is_numeric($this->current_url_array[1]) )
		{
	...
	function getCurentArticles()
	{
		...
		$_articles_time,                          // временной формат
		$_articles_img,
		$img,
		$_articles_cat,
		...
		if ($this->std->db->query($sql, $rows) > 0)
		{
			foreach ($rows as $row)
			{
				...
				if ($row['img'] != "")
				{
					...
					// замена
					$img = str_replace($search, $replace, $_articles_img);			// основная
				}
				else
				{		// картинки нет, пусто
					$img	= "";
		...
		else
		{
			...
			$h1 = $title = 'Ошибка 404';
			$this->ModulError( "getCurentArticles-> Нет статьи с таким аллиасом sql: $sql");
		}
