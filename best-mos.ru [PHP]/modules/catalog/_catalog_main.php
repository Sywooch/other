...
class main_catalog extends AbstractClass {
	...
	function createCatalogPath()
	{
		...
		{
			$alias .= $this->fsm_id[$this->current_url_array[$i]]['id']."/";
			
			$res .= $_catalog_path_item_active;
			$res = str_replace("{TITLE}", $this->fsm_id[$this->current_url_array[$i]]['title'], $res);
			$res = str_replace("{ALIAS}", $alias, $res);			
		}
		$i = count($this->current_url_array)-1;
		if (isset($this->fsm_id[$this->current_url_array[$i]]))
		{
			$res .= $_catalog_path_item_active;
			$alias .= $this->fsm_id[$this->current_url_array[$i]]['id']."/";
			$res = str_replace("{TITLE}", $this->fsm_id[$this->current_url_array[$i]]['title'], $res);
...
*/
	...
	{
		global $uri;
		global $_catalog_child_menu, $_catalog_child_menu_with_childs, $_catalog_child_menu_without_childs, $_catalog_category_child_menu;
