...
class mod_news
{
        ...
        function process_module()
        {
                ...
                $this->output        .= "<td><a href='?action=news&id=-1'>Список</a> </td>";
                $this->output        .= "<td>| <a href='?action=news_add&id=".$id."'>Новая</a> </td>";
                # если тематикаи включены - создание ссылки на редактирование тематик
                if ($this->std->settings["news_gen_cat"])
				{
					 $this->output        .= "<td>| <a href='?action=news_cat'>Редактирование тематик</a> </td>";
				}
               

                if ($id != '-1')
                {
                        $sql = "select pid from `se_news` where id='".$id."'";
                        if ($this->std->db->query($sql, $rows) > 0)
                        {
                                $this->output        .= "<td>| <a href='?action=news&id=".$rows[0]['pid']."'>На уровень выше</a></td>";
                        }
                }
                ...
                switch(@$this->std->input['action'])
                {
                        ...
                        case 'news_del':
                              ...
                              $this->news_del($itemid, 0 /* удалять */);
                              $this->news_content( );
                              break;
                        case 'news_active':
                              $this->news_active();
...
***************************************************************************************/
        function news_content( )
        {
                ...
                ****************************************************************************/
                $this->output        .= "<center>";
                // запрашиваем пункты
                $sql         = "select * from `se_news` order by timestamp desc";
                if ($this->std->db->query($sql, $rows) > 0){
                        ...
                        foreach ($rows as $row)
                        {
                                ...
                                if ($item_active != ''){
                                 ...
                                 $this->output        .= '<td align=center><input name="'.$row['id'].'_item_active" type="checkbox" '.$item_active.' title="'.$title.'"
                                                                        OnClick="document.location.href =\'/admin/?action=news_active&id='.$id.'&itemid='.$row['id'].'\'"></td>';
                                ...
                                $this->output   .=         '<td width=30 align=center><a href=
                                                                 "javascript:doConfirm(\'Удалить новость?\',\'/admin/?action=news_del&id='.$id.'&itemid='.$row['id'].'\')"
                ...
                else
                {
                        ...
                        if( $title )
                        {
                                ...
                                $this->std->db->do_query( "SELECT id FROM se_mitems WHERE alias='/news/' AND pid IN(".implode(',', $father_ids).")" );
                ...
                $this->output .= '<center><form method=post enctype=multipart/form-data>
                        ...
                        <tr>
                        <td align=right>Активировать страницу вывода новостей:</td>
                        <td>
		...
		##################################
        # редактирование категорий новости
		##################################
        ...
        function news_cat()
        {
				...
				if ($_GET['change']=='on')
				{
						$this->std->db->do_query("UPDATE `se_news_cats` SET `is_active`=1 WHERE id='".$_GET['id']."' ");
				}
				# деактивируем при нажатие на чекбокс
				elseif ($_GET['change']=='off')
				{

						$this->std->db->do_query("UPDATE `se_news_cats` SET `is_active`=0 WHERE id='".$_GET['id']."' ");
				}
					...
					$this->std->table_order('news_cats');
						...
						if ($this->std->db->query($sql, $rows) > 0)
						{
								...
								foreach ($rows as $row)
								{
										...
										$n_title = 'cat_'.$row['id'];
										$this->std->db->do_query("UPDATE `se_news_cats` SET `title`='".$_POST["$n_title"]."' WHERE id='".$row['id']."' ");
                ...
                if($_POST['add_cat'] and $_POST['new_cat'])
                {
						...
						$this->std->db->do_query("SELECT MAX(item_order) as max FROM  se_news_cats");
						$max = $this->std->db->fetch_row();
						$news_id = $max['max'] + 1;

                		$this->std->db->do_query("INSERT INTO `se_news_cats` (`title`, `item_order`) VALUES ('".$_POST['new_cat']."', '".$news_id."')");
                ...
                if ($_GET['del'])
                {
                		$sql = "DELETE FROM `se_news_relations_cats` WHERE id_cats='".$_GET['del']."'";
						$this->std->db->do_query($sql);

                		$sql = "DELETE FROM `se_news_cats` WHERE id='".$_GET['del']."'";
						$this->std->db->do_query($sql);



	                # при удалении уменьшаем на 1 порядок всех тематик, которые имеют item_order больше удаляемой
	                $sql         = "SELECT * FROM se_news_cats ORDER BY item_order ASC";
					$this->std->db->query($sql, $rows);
					$id = 1;
					foreach ($rows as $row)
					{
							$this->std->db->do_query("UPDATE `se_news_cats` SET `item_order`='".$id."' WHERE id='".$row['id']."' ");
							$id++;
                ...
                if ($row_count > 0)
                {
                        ...
                        $this->output        .= "<form method='post' action='/admin/?action=news_cat'><input type='hidden' name='change' value='1'>";
                        $this->output        .= '<table border="1">';
                        $this->output        .= '<tr>
                        ...
                        while ($row = $this->std->db->fetch_row())
                        {
                               ...
                               $this->output        .= '<td align=center><input name="'.$row['id'].'_item_active" type="checkbox" '.$item_active.'
                                                                        OnClick="document.location.href =\'/admin/?action=news_cat&change='.$change.'&id='.$row['id'].'\'"></td>';
                                ...
                                //$this->output .= $this->std->order_button($row_count, $i, $id, $row, 'news_cat');
                                $io = $row['item_order']-1;
                                $this->output .= $this->std->order_button($row_count, $io, '-1', $row, 'news_cat');
                                
                                if( $row['alias'] != '/' and
                                    (!@in_array($row['alias'], $modulename) or $id!=-1) )
                                {
                                        $this->output        .= "<td width=30 align=center><a href=";
                                        $this->output        .= "\"javascript:doConfirm('Удалить категорию?','/admin/?action=news_cat&del=".$row['id']."')\"";
                        ...
                        $this->output        .= 'Нет категорий'."<form method='post' action='/admin/?action=news_cat'><input type='hidden' name='change' value='1'>";
                }
                $this->output        .= '';
        ...
        function news_add( )
        {
                ...
                if ($this->std->input['request_method'] == 'post')
                {
                        ...
                        {       // если поля заполнены, значит можно сохранять
                                ...
                                if(empty($this->std->input['itemid']))
                                {
                                        ...
                                        $sql = "SELECT id FROM se_news WHERE timestamp>={$timenow} and timestamp<={$timethen} and alias='{$this->std->input['alias']}' and pid='-1'";
                                ...
                                {
                                        $sql = "select alias from se_news where id=".$this->std->input['itemid'];
                                        $this->std->db->do_query($sql);
                                        $old_data = $this->std->db->fetch_row();

                                        // проверяем на существование алиаса
                                        $sql = "SELECT id FROM se_news WHERE timestamp>{$timenow} and timestamp<{$timethen} and alias='{$this->std->input['alias']}' and id!='{$this->std->input['itemid']}'";
                                        $this->std->db->do_query($sql);
                                ...
                                if ($err_msg == ''){
                                            ...
                                            # если новость создается, его id в $_GET['itemid'] нет, езнаем id последней вставленной новости
                                            if( empty($_GET['itemid']) )
                                            {
                                   				$this->std->db->do_query("SELECT MAX(id) as max FROM  se_news");
                								$max = $this->std->db->fetch_row();
                								$news_id = $max['max'] + 1;
                                            }
                                            else
							                ...
							                # берем из базы тематики этой новости
                                            # если новости нет в селекте - удаляем тематику из новости

							                /*
							                $this->std->db->do_query( "SELECT * FROM `se_news_relations_cats` WHERE id_news='".$news_id."'" );
									        $id  = 1;
									        while($row = $this->std->db->fetch_row())
									        {
										            $id_cats[$id] = $row['id_cats'];
	                        		                $id_news[$i] = $row['id_news'];
	                        		                ...
	                        		                {
	                        		                		$this->std->db->do_query("DELETE FROM `se_news_relations_cats` WHERE id_news='".$news_id."' AND id_cats='".$row['id_cats']."'");

	                        		                        //mysql_query("DELETE FROM `se_news_relations_cats` WHERE id_news='".$news_id."' AND id_cats='".$row['id_cats']."'");
	                        		                        //print "$id_cats[$id]".'<br>'."DELETE FROM `se_news_relations_cats` WHERE id_news='".$news_id."' AND id_cats='".$row['id_cats']."'";
	                        		                }
										        	$id++;
									        }
									       ...
									       */
											...
											if ($this->std->settings['news_gen_cat'])
											{
		                                            ...
		                                            $sql = "SELECT * FROM `se_news_relations_cats` WHERE id_news='".$news_id."'";
		                                            $id  = 1;
									                if ($this->std->db->query($sql, $rows) > 0)
									                {
                                            ...
                                            # берем из селектлиста тематики к новости
                                            # если тематики нет в новостях  - добавляем тематику к новости
											$i = 0;
											while ($_POST['subjects'][$i])
											{
						                        # если тематики нет в новостях  - добавляем тематику к новости
						                        if ($this->std->db->query(("SELECT `id_cats` FROM `se_news_relations_cats` WHERE id_news='".$news_id."' AND id_cats='".$_POST['subjects'][$i]."'"), $rows) == 0)
						                        //!mysql_num_rows(mysql_query("SELECT `id_cats` FROM `se_news_relations_cats` WHERE id_news='".$news_id."' AND id_cats='".$_POST['subjects'][$i]."'")))
						                        {
                        		                	$this->std->db->do_query("INSERT INTO `se_news_relations_cats` (`id_news`, `id_cats`) VALUES('".$news_id."', '".$_POST['subjects'][$i]."' )");
						                        }
											...
											while ($new_cat[$i])
											{
												$sql = "SELECT id FROM se_news_cats WHERE `title`='".$new_cat[$i]."'";
												$this->std->db->do_query($sql);

												# если нет такой тематики - записываем
												if ( !$rows = $this->std->db->fetch_row() )
												{
													$sql = "INSERT INTO `se_news_cats` (`title`, `item_order`) VALUES ('".$new_cat[$i]."', '".(mysql_result(mysql_query("SELECT MAX(item_order) FROM  se_news_cats"),0)+1+$i)."')";
													$this->std->db->do_query($sql);
												}

												# узнаём id категории
												$id_cats = mysql_result(mysql_query("SELECT `id` FROM `se_news_cats` WHERE title='".$new_cat[$i]."'"),0);
												# если тематика не прикреплена к новости - прикрепляем
						                        if (!mysql_num_rows(mysql_query("SELECT `id_cats` FROM `se_news_relations_cats` WHERE id_news='".$news_id."' AND id_cats='".$id_cats."'")))
						                        {
                        		                	mysql_query("INSERT INTO `se_news_relations_cats` (`id_news`, `id_cats`) VALUES('".$news_id."', '".$id_cats."' )");
						                        }
                                        ...
                                        if (empty($this->std->input['itemid'])) {
                                                ...
                                                $sql        = "select max(item_order) as num_order from se_news where pid=".$this->std->input['id'];
                                                $this->std->db->do_query($sql);
                                                $max_order = -1;
                                                if ($this->std->db->getNumRows())
                                                {
                                                ...
                                                $sql = "SELECT id FROM se_news WHERE timestamp>={$timenow} and timestamp<={$timethen} and alias='{$this->std->input['alias']}' ";
                                                $this->std->db->do_query($sql);
												...
												{
                                                $this->std->db->do_insert( 'news', array( 'pid'       => -1,
                                        ...
                                        else
                                        {
                                                ...
                                                $this->std->db->do_update( 'news', array( 'timestamp' => $this->std->getTimestamp($this->std->input['timestamp']),
                                                                                         'alias'     => $this->std->input['alias'],
                                                                                         'title'     => $this->std->input['title'],
                                                                                         'sbody'     => $this->std->input['sbody'],
                                                                                         'body'      => $_POST['body'],
                                        ...
                                        {       // есть картинка
		                                        $this->std->uploadFile( $_FILES["img"], $itemid."_big", "news", &$err_msg);
                                                $img = strstr($_FILES["img"]["name"],".");
                                                if ($err_msg == "")
		                                        {		// в таблице фиксируем тип загруженной картинки
		                                        		$this->std->db->do_query( "UPDATE se_news SET img = '".strstr($_FILES["img"]["name"],".")."' WHERE id=".$itemid);

		                                        		// библиотека
				                                        require_once( INCLUDE_PATH."/lib/class_image.php" );
				                                        $class_image = new ClassImage();
				                                        $class_image->std = $this->std;
				                                        $class_image->source		= FILES_PATH."/news/".$itemid."_big".$img;
				                                        $class_image->ResizeType	= $this->std->settings["news_th_resize"];
				                                        $class_image->ResizeMethod	= $this->std->settings["news_th_method"];


				                                        // ресайз 1 - картинка для страницы товара
				                                        $class_image->result		= FILES_PATH."/news/".$itemid.$img;
				                                        $class_image->resize( $this->std->settings["news_th_width"], $this->std->settings["news_th_height"] );


				                                        // ресайз 2 - аватар картинки
				                                        $class_image->ResizeType	= $this->std->settings["news_av_resize"];
				                                        $class_image->ResizeMethod	= $this->std->settings["news_av_method"];
				                                        $class_image->result		= FILES_PATH."/news/".$itemid."_av".$img;
				                                        $class_image->resize( $this->std->settings["news_av_width"], $this->std->settings["news_av_height"]);

				                                        // ресайз 3 - большая картинка картинки
				                                        $class_image->ResizeType	= $this->std->settings["news_big_resize"];
				                                        $class_image->ResizeMethod	= $this->std->settings["news_big_method"];
				                                        $class_image->result		= FILES_PATH."/news/".$itemid."_big".$img;
				                                        $class_image->resize( $this->std->settings["news_big_width"], $this->std->settings["news_big_height"] );
				                                }
                                        ...
                                        {
										extract($this->std->input);
                                                header('Location: /admin/?action=news&id='.$id);
                                        }
                                        else
                                        {
                                                extract($this->std->input);
                ...
                else
                {
                        ...
                        if (isset($this->std->input['itemid']))
                        {
                                ...
                                $itemid = $this->std->input['itemid'];
                                $sql = "select * from `se_news` where id='".$itemid."'";
                                if ($this->std->db->query($sql, $rows) > 0)
                                {
                                        $row              = $rows[0];
...
<script type="text/javascript">
    Calendar.setup({
        ...
        singleClick    :    true
                ...
                {
                		$this->output .= '<tr><td align=right>Картинка:</td><td><input type=hidden name=img value="'.$img.'"><img src="/'.FILES_PATH.'/news/'.$itemid."_av".$img.'"><a href="/admin/?action=news_add&id='.$id.'&itemid='.$itemid.'&imgdel=yes">Удалить</a></td>';
                }
				...
				if ($this->std->settings['news_gen_cat'])
				{
		                ...
		                $this->std->db->do_query( "SELECT * FROM `se_news_cats`  ORDER BY item_order ASC" );
				        ...
				        while($row = $this->std->db->fetch_row())
				        {
				                $news_title[$id] = $row['title'];
		                        //$news_id[$i] = $row['id'];
		
		                        # если тематика выбрана для данной новости - отмечаем в селекте
		                        if (@mysql_result(mysql_query("SELECT `id_cats` FROM `se_news_relations_cats` WHERE id_news='".$_GET['itemid']."'  AND id_cats='".$row['id']."' "),0))
		                        {
...
';
      if ($this->std->settings['news_gen_cat']) $this->output .= '
 						<tr>
                            <td align=right>Тематика новости</td>
...
';
         ...
         */
        function news_recurs_del($id, $delnode = 0)
        {
                $sql        = "select * from se_news where pid=".$id;
                $result      = $this->std->db->do_query($sql);
                if ($this->std->db->getNumRows( ) )
                {
                        while($row = $this->std->db->fetch_row( ))
                        {
                                $this->news_recurs_del($row['id']);  // очередной реккурсивный вызов
                        }
                }
                ...
                if ($delnode != 1)
                {
                        $sql = "delete from se_news where id=".$id;
                        $this->std->db->do_query($sql);
                }
        }
...
*************************************************************************************************************/
        function news_del($id, $delnode = 0  /* признак, удалять саму вершину или удалить только её подчинённых */)
        {
                $this->news_recurs_del($id, $delnode);
        }
...
************************************************************************/
        function news_active( )
        {
                ...
                $sql        = "select is_active from `se_news` where id=".$itemid;
                if ($this->std->db->query($sql, $rows) > 0)
                {
                        // меняем данные обрабатываемого каталога, делаем его активным либо неактивным
                        $this->std->db->do_update( 'news', array( 'is_active' => $rows[0]['is_active'] ? 0 : 1), "id=".$itemid );
                }
   ...
   /*     function news_order( )
        {
                ...
                $sql = "select id, item_order from `se_news` where pid='".$id."' order by item_order";
                ...
                if ( $row = $this->std->db->fetch_row())
                {
                        if ($act == 'up')
                        {
                                        ...
                                        {
                                                $this->std->db->do_update( 'news', array( 'item_order' => ($order-1) ), "id=".$itemid );
                                                $this->std->db->do_update( 'news', array( 'item_order' => $order ), "id=".$row['id'] );
                                                break;
                                        }
                                }else{
                                        if ($row['item_order'] == $order+1){
...
*/
        ...
        function news_menu($alias_arr)
        {

                $cheched = '';
                if (in_array("/news/",$alias_arr))
                {
                        $cheched = 'checked';
                }
                $form = @$nbsp."<table><tr><td width='0'></td><td align=left>
                                                                                        - <input type='checkbox' name='-1_checkbox' $cheched>&nbsp;Модуль вывода новостей
                                                                                          <input type='hidden' name='-1_url' value='/news/'>
                                                                                          <input type='hidden' name='-1_name' value='Новости'>
                                                                                </td></tr></table>";
        ...
        function delImg($itemid)
        {
	            ...
	            {       // удаляем картинку
	                    $sql = "SELECT img FROM se_news where id={$itemid}";
	                    if ($this->std->db->query($sql, $rows) > 0)
	                    {

                                $this->std->db->do_update("news", array("img" => ""), "id={$itemid}");

                                @unlink(FILES_PATH."/news/".$itemid.$rows[0]["img"]);	// удаление оригинала
                                @unlink(FILES_PATH."/news/".$itemid."_big".$rows[0]["img"]);	// удаление основной
     							@unlink(FILES_PATH."/news/".$itemid."_av".$rows[0]["img"]);	// удаление аватара
	                    }
	            }
        }
