...
class mod_static
{
        ...
        function process_module()
        {
                ...
                $this->output        .= "<td><a href='?action=static&id={$id}'>Список</a> </td>";
                $this->output        .= "<td>| <a href='?action=static_add&id=".$id."'>Новая страница</a> </td>";
                ...
                if ($id != '-1')
                {
                        ...
                        if ( $rows = $this->std->db->fetch_row() )
                        {
                                ...
                                $this->output        .= "<td>| <a href='?action=static&id=".$rows['pid']."'>На уровень выше</a></td>";
                        }
                }
        ...
        function static_content( )
        {
                ...
                ****************************************************************************/
                ...
                if ($row_count > 0)
                {
                        ...
                        $this->output        .= '<tr><td colspan=2 align=center>&nbsp;</td><td align=center>*</td><td align=left>Название</td><td align=left>Алиас</td><td align=center colspan=2>Порядок</td><td>&nbsp;</td></tr>';
                        ...
                        while ($row = $this->std->db->fetch_row())
                        {
                                ...
                                if ($item_active != ''){
                                 ...
                                 $this->output   .=         '<td width=30 align=center><a href="/admin/?action=static_add&id='.$id."&itemid=".$row['id'].'"  title="Редактировать"><img src="/'.ADMIN_FOLDER.'/image/img_edit.png"></a></td>';

                                 if ($row['childs'] > 0){
                                         $this->output   .= '<td width=30 align=center><a href="/admin/?action=static&id='.$row['id'].'"  title="Просмотреть подразделы"><img src="/'.ADMIN_FOLDER.'/image/catalog.gif"></a></td>';
                                 }else{
                                 ...
                                 if( $row['alias'] == 'index'  )
                                 {
                                         $this->output        .= '<td align=center><input name="'.$row['id'].'_item_active" type="checkbox" checked disabled></td>';
                                 }
                                 ...
                                 else
                                 {
                                         $this->output        .= '<td align=center><input name="'.$row['id'].'_item_active" type="checkbox" '.$item_active.' title="'.$title.'"
                                                                        OnClick="document.location.href =\'/admin/?action=static_active&id='.$id.'&itemid='.$row['id'].'\'"></td>';
                                 }
                        ...
                        $this->output         .= '<tr><td colspan=8 align=right><input type="submit" value="Обновить позиции" /></tr>
                                                  <tr><td colspan=8 align=center>* - Активность раздела</td></tr></table></form>';
                }else{
        ...
        function static_add( )
        {
                                ...
                                if ($err_msg == ''){//Если всё ништяк - пишем в базу, да.
                                        ...
                                        if (!isset($itemid)) {
                                                ...
                                                $this->std->db->do_insert( 'static', array( 'pid'       => $parent,
                                                                                     ...
                                                                                     'title'     => $title,
                                                                                     'menu'      => $menu,
                                                                                     'sbody'     => str_replace("'", "&#146", $_POST["sbody"]),
                ...
                else
                {
                        ...
                        {        // если переходит именно к редактированию страницы, то возможно нужно будет перекинуть на определённый модуль, для редактирования контента уже там
                                ...
                                // если карта сайта и модуль установлен то перекидываем на карту сайта
                                if( $alias == 'sitemap' and $pid==-1)
                                {
                                        $sql = "select * from `se_modules` where modulename='sitemap' AND is_active=1";
                                        $this->std->db->do_query($sql);
                                ...
                                // если запрашивается новости?
                                elseif( $alias == 'news' and $pid==-1)
                                {
                                        $sql = "select * from `se_modules` where modulename='news' AND is_active=1";
                                        $this->std->db->do_query($sql);
                                ...
                                elseif( $alias == 'catalog' and $pid==-1)
                                {
                                        $sql = "select * from `se_modules` where modulename='catalog' AND is_active=1";
                                        $this->std->db->do_query($sql);

                                        if ($this->std->db->getNumRows())
                                        {
                                                @header("Location: /admin/?action=catalog");
                                                exit;
                                        }
                                }
                                // если запрашивается горячий каталог?
                                elseif( $alias == 'hotcatalog' and $pid==-1)
                                {
                                        $sql = "select * from `se_modules` where modulename='hotcatalog' AND is_active=1";
                                        $this->std->db->do_query($sql);

                                        if ($this->std->db->getNumRows())
                                        {
                                                @header("Location: /admin/?action=hotcatalog");
                                                exit;
                                        }
                                }
                                // если запрашивается дефолтный модуль?
                                elseif( $alias == 'standalone' and $pid==-1)
                                {
                                        $sql = "select * from `se_modules` where modulename='standalone' AND is_active=1";
                                        $this->std->db->do_query($sql);

                                        if ($this->std->db->getNumRows())
                                        {
                                                @header("Location: /admin/?action=standalone");
                                                exit;
                                        }
                                }
                                // если запрашивается горячий каталог?
                                elseif( $alias == 'gallery' and $pid==-1)
                                {
                                        $sql = "select * from `se_modules` where modulename='gallery' AND is_active=1";
                                        $this->std->db->do_query($sql);
                                ...
                                // если запрашивается FAQ?
                                elseif( $alias == 'faq' and $pid==-1)
                                {
                                        $sql = "select * from `se_modules` where modulename='faq' AND is_active=1";
                                        $this->std->db->do_query($sql);

                                        if ($this->std->db->getNumRows())
                                        {
                                                @header("Location: /admin/?action=faq");
                                                exit;
                                        }
                                }

                                // если запрашивается Actions?
                                elseif( $alias == 'actions' and $pid==-1)
                                {
                                        $sql = "select * from `se_modules` where modulename='actions' AND is_active=1";
                                        $this->std->db->do_query($sql);

                                        if ($this->std->db->getNumRows())
                                        {
                                                @header("Location: /admin/?action=actions");
                                                exit;
                                        }
                                }

                                // если запрашивается Voting?
                                elseif( $alias == 'voting' and $pid==-1)
                                {
                                        $sql = "select * from `se_modules` where modulename='voting' AND is_active=1";
                                        $this->std->db->do_query($sql);

                                        if ($this->std->db->getNumRows())
                                        {
                                                @header("Location: /admin/?action=voting");
                                                exit;
                                        }
                                }

                                // если запрашивается Blog?
                                elseif( $alias == 'blog' and $pid==-1)
                                {
                                        $sql = "select * from `se_modules` where modulename='blog' AND is_active=1";
                                        $this->std->db->do_query($sql);

                                        if ($this->std->db->getNumRows())
                                        {
                                                @header("Location: /admin/?action=blog");
                                                exit;
                                        }
                                }
                ...
                $this->output .= '<center><form method=post enctype=multipart/form-data>
                        ...
                        <td>
                        <input type=text '.$disabled.' name=alias value="'.$alias.'">
                        </td>
                        </tr>
...
<script type="text/javascript">
    Calendar.setup({
        ...
        ***************************************************************************************************/
        ...
        {
                $sql        = "select * from se_static where is_active = 1 and pid = ".$pid." ORDER BY pid, item_order";  // странички с определённым родителем
                $this->std->db->do_query($sql);
                        ...
                        if ($this->std->db->query($sql, $rows) > 0)
                        {
                                        ...
                                        if ($this->std->db->query($sql, $rows) > 0)
                                        {
                                                foreach ($rows as $row)
                                                {
                                                                ...
                                                                if( file_exists(MODULES_PATH."/{$father}/{$father}_admin.php") )
                                                            	{
		                                                                $counstructor = 'mod_'.$father;
		                                                                $method       = $father.'_renum';

		                                                                if( file_exists(MODULES_PATH."/{$father}/{$father}_config.php") )
		                                                                {
		                                                                        require_once MODULES_PATH."/{$father}/{$father}_config.php";
		                                                                }

		                                                                require_once MODULES_PATH."/{$father}/{$father}_admin.php";
