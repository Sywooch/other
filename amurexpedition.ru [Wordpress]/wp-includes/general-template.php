...
<li class="pager-item"><a title="На страницу номер 2" href="/ru/amur/novosti?page=1">2</a></li>
<li class="pager-item"><a title="На страницу номер 3" href="/ru/amur/novosti?page=2">3</a></li>
<li class="pager-next"><a href="/ru/amur/novosti?page=1">›</a></li>
<li class="pager-last last"><a href="/ru/amur/novosti?page=2">»</a></li>
</ul></div>
...
function paginate_links( $args = '' ) {
	...
	if ( $args['prev_next'] && $current && 1 < $current ) :
		 ...
		 */
		$page_links[] = '<a class="prev page-numbers tmp4" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . $args['prev_text'] . '</a>';
	endif;
	for ( $n = 1; $n <= $total; $n++ ) :
		if ( $n == $current ) :
			$page_links[] = "<span class='page-numbers tmp5 current'>" . $args['before_page_number'] . number_format_i18n( $n ) . $args['after_page_number'] . "</span>";
			$dots = true;
		else :
			if ( $args['show_all'] || ( $n <= $end_size || ( $current && $n >= $current - $mid_size && $n <= $current + $mid_size ) || $n > $total - $end_size ) ) :
				...
				/** This filter is documented in wp-includes/general-template.php */
				$page_links[] = "<a class='page-numbers tmp6' href='" . esc_url( apply_filters( 'paginate_links', $link ) ) . "'>" . $args['before_page_number'] . number_format_i18n( $n ) . $args['after_page_number'] . "</a>";
				$dots = true;
	...
	if ( $args['prev_next'] && $current && $current < $total ) :
		...
		/** This filter is documented in wp-includes/general-template.php */
		$page_links[] = '<a class="next page-numbers tmp1" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . $args['next_text'] . '</a>';
	endif;
	switch ( $args['type'] ) {
		case 'array' :
			return $page_links;
		...
		case 'list' :
			$r .= "<div class='item-list'><ul class='page-numbers pager'>\n\t<li>";
			$r .= join("</li>\n\t<li>", $page_links);
			$r .= "</li>\n</ul></div>\n";
			break;
