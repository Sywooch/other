...
window.addEvent('domready',function() {
    ...
    //update_form(false);
    if(document.id('virtuemart_country_id') && document.id('virtuemart_country_id').value=='') {
    ...
    if(document.id('shipto_virtuemart_country_id') && document.id('shipto_virtuemart_country_id').value=='') {
...
function submit_order() {	
	...
	if(document.id('STsameAsBT').checked==true) {
		...
		if(document.id('shipto_virtuemart_country_id'))
	...
	<div class="cart-view tmp1">
	<div>
		<div class="width50 floatleft">
			<h3><span><span><?php echo JText::_('COM_VIRTUEMART_CART_TITLE'); ?></span></span></h3>
		</div>
	...
	$sum1=$this->currencyDisplay->createPriceDiv('salesPrice','', $this->cart->pricesUnformatted,false);
	
	$sum1=str_replace("руб","",$sum1);
	$sum1=trim($sum1);
	...
	if($sum1<=$sum2){
		...
		echo "<h1>Чтобы оформить заказ, нужно совершить попкупок более чем на 20000 руб.</h1>";
	...
	<div class="cart-view tmp2" style=" <?php  if($sum1<=$sum2) { echo"display:none;";  } ?>">
...
?>
			...
			if($userFieldsModel->getIfRequired('agreed')){
				...
				echo VmHtml::checkbox('tosAccepted',$this->cart->tosAccepted,1,0,'class="terms-of-service" checked type=hidden');
