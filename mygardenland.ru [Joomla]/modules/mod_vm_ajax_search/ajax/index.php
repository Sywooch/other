...
//if (!is_numeric($prods)) $mainframe->close(); 
		...
		else
		{
		 // VM2
	      ...
	      ." union (select p.virtuemart_product_id, l.product_name from #__virtuemart_products AS p, #__virtuemart_products_".VMLANG." as l where (l.product_name LIKE '%".$db->getEscaped($keyword)."%' ".$or.") and p.published = '1' and p.virtuemart_product_id = l.virtuemart_product_id LIMIT 0,".$prods.")"."\n"
	    ...
	    foreach ($ps as $row)
	    {
	      ...
	      echo '<div id="vm_ajax_search_results2'.$myid.'_'.$n.'" class="vm_ajax_search_row_'.$x.'" onmouseover="javascript:op_hoverme(this);"  onclick="javascript: aj_redirect('."'prod".$row['product_id'].'_'.$myid."'".');">';
	      $width = 50; $height = 50;
	      $h2 = $height;
		  ...
		  $value = $href; 
	      echo '<div class="floatleft" style="float: left;  min-height: 55px;">';
	      ...
	      Resize_Image::showImage($row['product_thumb_image'], 50, 50); 
	      echo '</div>';
	      echo '</div>';
//	      echo '<br style="clear: both;" />';


	      echo '<div class="floatlright" style=" display: block; overflow: none; padding: 0; margin: 0;  min-height: 55px;"/>';
	      $id = ' id="prod'.$row['product_id'].'_'.$myid.'" ';
	      echo '<a class="product_lnk_ajax_text" style="text-align: left; margin-top: -3px; " href="'.$href.'" '.$id.'>'.$pname.'</a>';
	      echo '</div>';
		  
		  echo '<input type="hidden" name="op_ajax_results" id="vm_ajax_search_results2_'.$myid.'_value_'.$n.'" value="'.$value.'" />';
