...
$day[6] = "СБ"; 
			...
			echo '<a href="/afisha.html?day='.date("Y-m-d H:i:s").'"><div class="date '.$class.'" id="date_0" onclick="date(0, \''.date("Y-m-d H:i:s").'\');" style="width:100px !important;"><span class="date_num">'.$time.'</span>
			...
			for($i=1;$i<90;$i++){
				...
				echo '<a href="/afisha.html?day='.date("Y-m-d H:i:s", time()+(86400*$i)).'"><div class="date '.$class.'" id="date_'.$i.'" onclick="date('.$i.', \''.date("Y-m-d H:i:s", time()+(86400*$i)).'\');"><span class="date_num">'.$time.'</span>
...
-->        
		...
		echo'<span class="cat_name tmp9"><a href="/afisha.html?subcategory='.$it->id.'">'.$it->name.'</a></span>';
...
<div id="k2Container" class="tmp1 itemView<?php echo ($this->item->featured) ? ' itemIsFeatured' : ''; ?><?php if($this->item->params->get('pageclass_sfx')) echo ' '.$this->item->params->get('pageclass_sfx'); ?>">
...
<div class="itemHeader">
  ...
  $database2 = JFactory::getDbo();
		   $database2->setQuery("SELECT * FROM #__k2_items WHERE catid=18 AND title=".($this->item->id)."");
		    $list2 = $database2->loadObjectList();
		    $count=0;
		     foreach($list2 as $it) {
		      $count=1;
...
//echo $this->item->id;
	    ...
	    $cat_id=$it->id;
			...
			$database->setQuery("SELECT * FROM #__banners WHERE catid='".($cat_id)."'");
		$list = $database->loadObjectList();
 ...
 /*
    ...
    var way = "default"; // направление
        ...
        $j(window).scroll(function()
        {
            ...
            if($j(".banners_rotator").is(":hover")){
  alert("1123");
 // $j(".banners_rotator").scrollTo('img:eq(5)', {duration:3000});
...
/*
  ...
  */
  ...
  /*
  document.getElementById("rotator").addEventListener("wheel", myFunction);
var tempScrollTop,currentScrollTop = 0;
function myFunction() {
...
<a href="/component/k2/item/<?php echo $it2->id;  ?>-<?php echo $it2->alias; ?>" style="width:100%; height:100%; display:block;"></a>
