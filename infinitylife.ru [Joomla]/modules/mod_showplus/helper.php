...
class ShowPlusSlideshow {
	...
	*/
	private function addHeadDeclarations($id, $data = null) {
		...
		if ('linear' != $this->params->transition_easing) {
			$args[] = 'transition:Fx.Transitions.'.ucfirst($this->params->transition_easing).'.ease'.str_replace(' ', '', ucwords(strtr($this->params->transition_timing, ':', ' ')));
		}
