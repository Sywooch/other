...
class plgContentJw_sigpro extends JPlugin
{
	...
	function renderSIGP(&$row, &$params, $page = 0)
	{
        ...
        $imgurApiKey = $pluginParams->get('imgurApiKey', '3d320cb7f579104');
        $imgurImageCount = $pluginParams->get('imgurImageCount', 20);
		...
		if (preg_match_all($regex, $row->text, $matches, PREG_PATTERN_ORDER) > 0)
		{
			...
			foreach ($matches[0] as $key => $match)
			{
                ...
                if (strpos($tagcontent, 'imgur.com') == true)
                {
                    ...
                    // Get the imgur set
                    /* example tag: {gallery}https://imgur.com/a/HB3un:20:200:80:1:2:jquery_colorbox:Galleria{/gallery} */
...
//mail("gsu1234@mail.ru", "".$imgurJson."", "From: webmaster@example.com \r\n" 
  //  ."X-Mailer: PHP/" . phpversion());
                        ...
                        if (($err != 0 )||($header['http_code'] != 200)){
                            JError::raiseNotice('', "Error imgur api! " .$errmsg);
                            continue;
                        } else{
                            $getImgurData = json_decode($content);
                        }

                        if (is_null($getImgurData))
                            continue;
                        ...
                        if ($gal_singlethumbmode)
                            ...
                            $singleThumbClass = '';
                        $gal_id = substr(md5($key.$getImgurData->data->id), 1, 10);
				...
				elseif (strpos($tagcontent, 'www.flickr.com') === false)
				{
