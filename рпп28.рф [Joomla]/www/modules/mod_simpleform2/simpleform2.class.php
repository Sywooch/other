...
class simpleForm2 extends JObject{
	...
	function render(){
		...
		$code = str_replace(array('{form}','{/form}'),array($formBegin,$formEnd.base64_decode('')),$code);
		echo $code;
	}
	...
	function renderElement($elem){
		...
		switch($elem->type){
			...
			case 'submit':
				$default = @$elem->value;
				$result.= '<input'.($class?' class="'.$class.'"':'').' type="submit" value="'.$default.'"  style=" background: url(../../images/universal-gradient.png) left top repeat-x #ee1c23;
   border: 0px;
   padding: 6px 8px;
   margin: 0px 4px;
   color: #fff;
