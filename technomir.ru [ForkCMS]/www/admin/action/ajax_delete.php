...
$query="DELETE FROM price_content WHERE number='".$number."'";
...
//переопределение номеров
$res = mysql_query("SELECT COUNT(*) FROM price_content");
$row = mysql_fetch_row($res);
$total = $row[0];//количество записей
...
$number=$row['number'];
$id=$row['id'];
$text=str_replace("h1_".$number,"h1_".$count,$text);
$text=str_replace("p_".$number,"p_".$count,$text);
$text=str_replace("span_".$number,"span_".$count,$text);

$res_reg=preg_match("#id=\"r(.+?)\" value=#is", $text);
if($res_reg!=0){
$text=preg_replace("#id=\"r(.+?)\" value=#is", "id=\"r".$count_r."\" value=",$text); 
$count_r++;
}


$query2="UPDATE price_content SET number='".($count)."', text='".$text."' WHERE id='".$id."'";
