...
if (version_compare(JVERSION, '3.0', '<'))
		...
		<script type="text/javascript">
			jQuery(function(panels) {
				...
				if(i==1){
					jQuery('a[href=#options-icons]').parent('li').css('display','list-item');
					jQuery('a[href=#options-images]').parent('li').css('display','none');
				} else {
					jQuery('a[href=#options-icons]').parent('li').css('display','none');
					jQuery('a[href=#options-images]').parent('li').css('display','list-item');
				}
				});
			});
			jQuery(document).ready(function() {
