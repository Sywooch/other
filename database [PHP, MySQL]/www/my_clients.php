...
//каждый пользователь может видеть только записи, которые добавил он.
if(!isset($_GET['sort'])){
...
$query="SELECT * FROM qdfmain WHERE User='".$_SESSION['user']."' ORDER BY Client DESC";
}
else if($_GET['sort']==3){
$query="SELECT * FROM qdfmain WHERE User='".$_SESSION['user']."' ORDER BY Loyalty";
}
else if($_GET['sort']==4){
$query="SELECT * FROM qdfmain WHERE User='".$_SESSION['user']."' ORDER BY Loyalty DESC";
}
...
$ind2=0;
 ...
 <td style="background-color:#3637ea; width:200px; padding:2px"><a href="action/delete.php?id='.$row['ID'].'" style="color:white">Удалить запись</a></br></br>
...
<td style="width:60px; padding:2px; color:white"><a href="input2.php?id='.$row['ID'].'" alt="" style="color:white">Добавить событие</a></br></br>
<a href="input_contact_form.php?id='.$row['ID'].'" alt="" style="color:white">Добавить контакт</a></br></br>
<a href="move_client_form.php?id='.$row['ID'].'" alt="" style="color:white">Передать клиента другому менеджеру</a></br></br>
';
...
<a href="move_client_form.php?id='.$row['ID'].'" alt="" style="color:white">Передать клиента другому менеджеру</a></br></br></td></br>';
if($ind2==1){echo'<a href="contacts2.php?id='.$row['ID'].'" alt="" style="color:lime">Посмотреть контакты</a>
</td>';};
echo'</tr>';
}
...
<option value="my_clients.php?sort=1"'; if($_GET['sort']==1){echo' selected';} echo'>"Клиенты" - возрастание</option>
<option value="my_clients.php?sort=2"'; if($_GET['sort']==2){echo' selected';} echo'>"Клиенты" - убывание</option>
<option value="my_clients.php?sort=3"'; if($_GET['sort']==3){echo' selected';} echo'>"Лояльность" - возрастание</option>
<option value="my_clients.php?sort=4"'; if($_GET['sort']==4){echo' selected';} echo'>"Лояльность" - убывание</option>
...
<div id="f2" name="f2" style="width:520px; height:30px; margin-top:10px">
			...
			border-color:white;padding:10px; margin-top:10px; display:none">
			 <form id="responsesForm" action="action/search_my_clients.php" enctype="multipart/form-data" method="post" accept-charset="utf-8">
