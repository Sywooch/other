...
if((!isset($_GET['id']))||($_GET['id']==NULL)||($_GET['id']=="")||(!isset($_GET['name']))||($_GET['name']==NULL)||($_GET['name']=="")){
echo'Ошибка. Некорректный параметр.';
exit;
}
...
<span>Управление клиентами - Передача клиента другому менеджеру</span>
</div>
...
<?php
echo'
<span>От кого передаётся: '.$name.'</span>';
?>
</div>
...
$query4="SELECT * FROM users WHERE name<>'".$_SESSION['user']."' and name<>'".$name."' ";
...
<form id="responsesForm" action="action/move_client_2.php?id='.$id.'&user2='.$name.'" enctype="multipart/form-data" method="post" accept-charset="utf-8">
