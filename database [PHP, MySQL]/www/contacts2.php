...
if(!isset($_GET['sort'])){
$query="SELECT * FROM tblcontacts WHERE ClientTD='".$_GET['id']."' ORDER BY Contact";
}
else if($_GET['sort']==1){
$query="SELECT * FROM tblcontacts WHERE ClientTD='".$_GET['id']."' ORDER BY Contact";
}
else if($_GET['sort']==2){
$query="SELECT * FROM tblcontacts WHERE ClientTD='".$_GET['id']."' ORDER BY Contact DESC";
}
else if($_GET['sort']==3){
$query="SELECT * FROM tblcontacts WHERE ClientTD='".$_GET['id']."' ORDER BY City";
}
else if($_GET['sort']==4){
$query="SELECT * FROM tblcontacts WHERE ClientTD='".$_GET['id']."' ORDER BY City DESC";
}
else if($_GET['sort']==5){
$query="SELECT * FROM tblcontacts WHERE ClientTD='".$_GET['id']."' ORDER BY AddTime";
}
else if($_GET['sort']==6){
$query="SELECT * FROM tblcontacts WHERE ClientTD='".$_GET['id']."' ORDER BY AddTime DESC";
}
...
<option value="contacts.php?sort=1&id='.$id.'"'; if($_GET['sort']==1){echo' selected';} echo'>"Контакты" - возрастание</option>
<option value="contacts.php?sort=2&id='.$id.'"'; if($_GET['sort']==2){echo' selected';} echo'>"Контакты" - убывание</option>
<option value="contacts.php?sort=3&id='.$id.'"'; if($_GET['sort']==3){echo' selected';} echo'>"Город" - возрастание</option>
<option value="contacts.php?sort=4&id='.$id.'"'; if($_GET['sort']==4){echo' selected';} echo'>"Город" - убывание</option>
<option value="contacts.php?sort=5&id='.$id.'"'; if($_GET['sort']==5){echo' selected';} echo'>"Дата добавления" - возрастание</option>
<option value="contacts.php?sort=6&id='.$id.'"'; if($_GET['sort']==6){echo' selected';} echo'>"Дата добавления" - убывание</option>
</select></td></tr>
...
<div id="f2" name="f2" style="width:520px; height:30px; margin-top:10px">
			...
			border-color:white;padding:10px; margin-top:10px; display:none">
			 <form id="responsesForm" action="action/search_contacts.php" enctype="multipart/form-data" method="post" accept-charset="utf-8">
                        ...
                        <table border="0">
                            ...
                            <tr><!--текст-->
                                ...
                                <td style="height:85px; width:360px; border-style:solid; border-width:1px; border-color:white">
									...
									<span>Кто добавил запись<br></span>
...
<!--поиск-->
<a href="clients.php"><input type="button" value="Назад" style="height:40px; width:100px; cursor:pointer; margin-top:20px" /></a>
