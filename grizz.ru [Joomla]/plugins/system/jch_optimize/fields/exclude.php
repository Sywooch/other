...
abstract class JFormFieldExclude extends JchTextarea
{
        ...
        protected function getParams()
        {
                ...
                {
                        $plugin       = JchOptimizePluginHelper::getPlugin();
                        ...
                        $GLOBALS['oJchParams'] = $pluginParams;
                }
        ...
        protected function getParser()
        {
                if (!isset($GLOBALS['oJchParser']))
                {
                        ...
                        if ($sHtml)
                        {
                                $GLOBALS['oJchParser'] = new JchOptimizeParser($oParams, $sHtml);
                        }
                        else
                        {
                                throw new Exception;
        ...
        protected function getMenuUrl()
        {
                ...
                $oUri->setQuery(str_replace('index.php?', '', $oMenuItem->link) . '&Itemid=' . $oMenuItem->id . '&jchbackend=2');
        ...
        protected function setOptions()
        {
                ...
                if (!$oFileRetriever->isUrlFOpenAllowed() && !$GLOBALS['bTextArea'])
                {
                        $GLOBALS['bTextArea'] = TRUE;
        ...
        protected function getInput()
        {
                ...
                $sField = JHTML::_('select.genericlist', $this->aOptions, $this->name . '[]', 'class="inputbox" multiple="multiple" size="5"', 'id',
                                   'name', $this->value, $this->id) .
                        '
                                <button type="button" onclick="showTextBox(\'' . $this->jch_params . '\', this)">Add option</button>
                                <div style="margin-top:3px;display:none;" id="jch_div_' . $this->jch_params . '">
                                        <input id="jch_textbox_' . $this->jch_params . '" type="text">
                                        <button type="button" onclick="addJchOption(\'' . $this->jch_params . '\')">Add option</button>
                                </div>
';
        ...
        protected function getImages($sAction = 'exclude')
        {
                ...
                $aLinks = $this->getLinks($oParser);

                $sId       = md5('getImages' . serialize($oParser->params->get('jchmenu')));
                $aArgs     = array($aLinks['css'], $oParser);
        ...
        protected function getLinks()
        {
                if (!isset($GLOBALS['aJchLinks']))
                {
                        ...
                        $oLinkBuilder = new JchOptimizeLinkBuilder($oParser);
                        $sId          = md5('getLinks' . serialize($oParser->params->get('jchmenu') . $oParser->params->get('pro_searchBody')));
                        $aFunction    = array($this, 'processLinks');

                        $GLOBALS['aJchLinks'] = $oLinkBuilder->loadCache($aFunction, FALSE, $sId);
                }
