<?php 
	...
	{
		$last_check_filename = dirname(__FILE__).'/last_check_time';
		if (file_exists($last_check_filename))
		{
			$time_since_last = time() - filemtime($last_check_filename);
			if ($time_since_last > 600)	// recheck database every 10 minutes
			{
				$wrong_url = 'http';
				$correct_url = '/';
				htracer_admin_convert_wrong_urls($wrong_url, $correct_url);
				touch($last_check_filename);
			}
		}
