...
class main_static extends AbstractClass{
	...
	function getAlias(){
		...
		{ # если не главная страница
			...
			if ((in_array($this->current_url_array[0], $this->modules) && (count($this->current_url_array) > 1)))
			{
				# выход, должен отработать другой модуль				
			}
			else
			{
	...
	function getStaticVars()
	{
		...
		if (count($this->ContentCurPage) > 0)
		{
				...
				{	
					...
					$this->timelastmodifed         = $this->std->getNormalTime($row['lastmodified'], 'd.m.Y');
					...
					$author                        = $this->author;
					$h1                            = $this->h1;
					$body_date						= $this->timelastmodifed;
