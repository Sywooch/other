...
class class_tree_frontend extends class_tree
{
	...
	public function getIdPage()
	{
		...
		if ((count($this->std->alias) == 1) || !$this->initTreeWithoutSheet('id, pid, alias, is_redirect, menu'))
		{
			return -1;
		}
		...
		for ($i = 1; $i < $len; $i ++)
		{
				...
				else
				{
					if ($i == $len-1)
					{
						...
						$sql = "SELECT id, alias, menu FROM se_catalog WHERE pid = {$id} AND alias = '{$alias}'";
...
//		}
	...
	{		
		...
		if (is_array($this->nosheet_pid[$pid]))
		{
			...
			$this_url=request_url();
			$this_url=str_replace("http://www.naukaplus.com","",$this_url);
			
			foreach ($this->nosheet_pid[$pid] as $item)
			{
				$tmp = $this->skin['menuexpanded']['inactive'][$level];
				if (!in_array($item['id'], $this->id_for_alias))
				{
				...
				else
				{
					...
					if($this_url==($this->getAliasByPid($item))){
						$tmp = preg_replace('~<a\b[^>]*+>|</a\b[^>]*+>~', '', $tmp);
					
					}					
					
					
					
					//$tmp = str_replace("<a ", '<a id="'.$this_url.'" ', $tmp);
