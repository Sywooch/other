...
if (isset($_REQUEST['jaction'])){
			...
			if (count($imageList)){
				...
				foreach ($imgArr as $k=>$img){
					if (JFile::exists(JPATH_ROOT.'/'.$folder.'/'.$img['image'])){
						...
						$key = 
						$html .= '<div class="j-img brick small">
							<div>
							...
							<br>
							<div class="j-img-btn">
								<a href="javascript:void(0)" onclick="jModal('.$i.')" title="Edit"><i class="icon-edit"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" title="Delete" onclick="jDelete(\''.$img['image'].'\',this)"><i class="icon-delete" ></i></a>
							</div>
							</div>';
	...
	if ($task){
		...
		$jAction = new JImageFolderAction();	
		$jAction->$task();
	}
...
class JFormFieldJImageFolder extends JFormField {
	/**
	 * The form field type.
	 *
	 * @var    string
	 */
	public $type = 'JImageFolder';
	...
	private function init(){
		...
		var J_IMAGE_FOLDER_ACTION = "'.$uri.'/jimagefolder.php?jaction=folders";
		var J_IMAGE_ID = "'.JFactory::getApplication()->input->getInt('id').'";
		var J_IMAGE_FIELDNAME = "'.$this->fieldname.'";
		');
		...
		function jListImages(){
			var folder = jQuery("#'. $this->id.'").val();
			if(folder == ""){
				alert("Folder path required");
				return;
			}
			jQuery("#jListImage #jSort").html("<img src=\"'.str_replace("\\","/", str_replace(JPATH_SITE, JURI::root(true), dirname(dirname(__FILE__)) )).'/assets/images/loading.gif\" width=\"30\" height=\"30\" />");
			jQuery.post(J_IMAGE_FOLDER_ACTION,
				{
					task:"getListImage"
					,folder:folder
					,mid:J_IMAGE_ID
					,fieldname:J_IMAGE_FIELDNAME
					
				},function(res){
					if(res.success){
						jQuery("#jSort").html(res.imageHtml);
						return jQuery( "#jSort" ).gridly({selector:".j-img"});
					}else{
						jQuery("#jListImage #jSort").html("<strong style=\'color: red\'>Image not found</strong>");
						return ;	
					}
				},"json");
		};
		function jUpdateImages(){
			var images = jQuery("#jListImage").find("img");
			var config = {};
			images.each (function(index,element){
				var $this = jQuery(this),
					name = $this.data("name"),
					position = $this.closest(".j-img").data("position"),
					item = {};
		...
		function jDelete(image,element){
		...
		function jModal(id){
			...
			jQuery("#jTitle").val(jQuery(image).data("title"));
			jQuery("#jLink").val(jQuery(image).data("link"));
			jQuery("#jDescription").val(jQuery(image).data("description"));
			jQuery("#jModal").data("imageId",id);
			jQuery("#jModal").modal("show");
		}
		function jUpdateImgData(){
			var imageId = jQuery("#jModal").data("imageId");
			var images = jQuery("#jListImage").find("img");
			var title = jQuery("#jTitle").val(),
				link = jQuery("#jLink").val(),
				description = jQuery("#jDescription").val();
			var image = images.get(imageId);
			
			jQuery(image).data("link",link).data("title",title).data("description",description);
			
			jQuery("#jModal").find("input,textarea").each(function(){
				jQuery(this).val("");
			});
			jQuery("#jModal").modal("hide");
		}
		');
	}
	/**
	...
	protected function getInput()
	{
		...
		$html[]	= '<div id="jListImage" style="margin-top: 18px;"><div id="jSort" class="gridly"></div></div>';
	...
	private function createModal(){
		$html = '
			<div id="jModal" class="modal hide fade">
				<div class="modal-body">
					<div class="control-group">
						<label class="control-label" for="jTitle">Title</label>
						<div class="controls">
							<input type="text" id="jTitle">
						</div>
					</div>
					<div class="control-group">
						<label class="control-label" for="jLink">Link</label>
						<div class="controls">
							<input type="text" id="jLink">
						</div>
					</div>
					<div class="control-group">
						<label class="control-label" for="jDescription">Description</label>
						<div class="controls">
							<textarea id="jDescription" cols="80" rows="6"></textarea>
						</div>
					</div>
				</div>
				<div class="modal-footer">
					<button class="btn" data-dismiss="modal" type="button">Close</button>
					<button class="btn btn-primary" onclick="jUpdateImgData()" type="button" >Ok</button>
				</div>
			</div>
		';
		return $html;
	...
	public static function listFolderTree($path, $filter, $maxLevel = 100, $level = 1, $parent = 1)
	{
		...
		if ($level == 1)
		{
			...
			$GLOBALS['_j_folder_tree_index'] = 1;
		}
		if ($level < $maxLevel)
		{
			...
			{
				$id = ++$GLOBALS['_j_folder_tree_index'];
