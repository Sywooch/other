	...
	public static function getFN_List(&$params)
	{
		...
		switch ($params->get( 'ordering' ))
		{
			...
			case 'order':
				...
				break;
			case 'c_asc':
				$ordering		= ' ORDER BY a.created ASC';
				break;
			case 'c_dsc':
			default:
        ...
        if ( $option == 'com_content' && $view == 'article' && $id ) {
            ...
            if ( $params->get( 'tag', 0 ) == 1 ) {
                ...
                {
                	$joins .= ' LEFT JOIN #__contentitem_tag_map AS m ON m.content_item_id = a.id';
                	$joins .= ' INNER JOIN #__tags AS t ON m.tag_id = t.id';   
                    $where .= " and ".$db->quoteName('m.tag_id') . ' IN (' . $tagsToMatch . ')';
                }
             }
        }
		if ($catid) {
		...
		foreach ( $rows as &$row ) {
	        ...
	        if ( $layout ) :
				...
				if ( $catlink ) {
					 ...
					 $fn_category = '<a href="'.$cat_link.'">'.$fn_category.'</a>'; 
				}
