...
if ( ! function_exists( 'alx_google_fonts' ) ) {
	...
	function alx_google_fonts () {
		if ( ot_get_option('dynamic-styles') ) {
			...
			if ( ot_get_option( 'font' ) == 'ubuntu-cyr' ) { echo '<link href="http://fonts.googleapis.com/css?family=Ubuntu:300,400,700,300italic,400italic,700italic&subset=latin,cyrillic" rel="stylesheet" type="text/css">'. "\n"; }
...
if ( ! function_exists( 'alx_dynamic_css' ) ) {
	...
	function alx_dynamic_css() {
		if ( ot_get_option('dynamic-styles') ) {
			...
			if ( ( ot_get_option( 'font' ) == 'ubuntu' ) || ( ot_get_option( 'font' ) == 'ubuntu-cyr' ) ) { $styles .= 'body, input, textarea, button, select, label { font-family: "Ubuntu", Arial, sans-serif; }'."\n"; }
