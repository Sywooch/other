	...
	<? if ($event->latitude && $event->longitude) : ?>
		<div id="event_main_map_wrapper">
			...
			<div id="event_main_map">
				<img itemprop="maps" style="width: <?=$width?>px; height: <?=$height?>px; border: 1px solid #CCCCCC; border-radius: 3px 3px 3px 3px; margin-bottom:5px" src="http://maps.google.com/maps/api/staticmap?zoom=15&size=<?=$width?>x<?=$height?><? if (substr($_SERVER['HTTP_HOST'], 0, 9) != 'localhost' && substr($_SERVER['HTTP_HOST'], 0, 9) != '127.0.0.1' ) echo '&markers=icon:http://'.$_SERVER['HTTP_HOST'].str_replace('/administrator', '', KRequest::base()).'/media/com_ohanah/images/ohapp_mapmarker.png'; else echo '&markers=color:blue'; ?>|<?=$event->latitude?>,<?=$event->longitude?>&sensor=false" />
			</div>
		</div>
