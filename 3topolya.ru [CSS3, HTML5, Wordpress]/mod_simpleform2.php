...
$script.= 'var $j = jQuery.noConflict();
   $j(function(){    $j("form#'.$id.'").simpleform({url:"'.JURI::root().'modules/mod_simpleform2/engine.php",loaderImg:"'.JURI::root().'modules/mod_simpleform2/images/loading.gif",'.($userFunc!=''?'checkCallBack:'.$userFunc.',':'').($userResultFunc!=''?'resultCallBack:'.$userResultFunc.',':'').'});});';
...
if($task=='sendForm'&&count($post)>0&&$moduleID==(int)$module->id){
	...
	if($result!==false){
		...
		if($ok){
			...
			//echo $params->get('okText','<span class="text_ok">Данные успешно отправлены'));
