...
function wp_pagenavi( $args = array() ) {
	...
	list( $posts_per_page, $_GET['part'], $total_pages ) = $instance->get_pagination_args();
	...
	$start_page = $_GET['part'] - $half_page_start;

	if ( $start_page <= 0 )
		$start_page = 1;

//	$end_page = $paged + $half_page_end;
	$end_page = $_GET['part'] + $half_page_end;
		...
		case 1:
			...
			if ( !empty( $options['pages_text'] ) ) {
				$pages_text = str_replace(
					...
					array( $_GET['part'], number_format_i18n( $total_pages ) ),
					
					__( $options['pages_text'], 'wp-pagenavi' ) );
				$out .= "<span class='tmp1 {$class_names['pages']}'>$pages_text</span>";
			}
			...
			if ( $_GET['part'] > 1 && !empty( $options['prev_text'] ) ) {
				//$out .= $instance->get_single( $paged - 1, $options['prev_text'], array(
				$out .= $instance->get_single( $_GET['part'] - 1, $options['prev_text'], array(
			...
			if ( $larger_page_start )
				$out .= "<span class='tmp2 {$class_names['extend']}'>{$options['dotleft_text']}</span>";

			// Page numbers
			$timeline = 'smaller';
			foreach ( range( $start_page, $end_page ) as $i ) {
				//if ( $i == $paged && !empty( $options['current_text'] ) ) {
				if ( $i == $_GET['part'] && !empty( $options['current_text'] ) ) {
					
					$current_page_text = str_replace( '%PAGE_NUMBER%', number_format_i18n( $i ), $options['current_text'] );
					$out .= "<span class='tmp3 {$class_names['current']}'>$current_page_text</span>";
					$timeline = 'larger';
				} else {
			...
			if ( $_GET['part'] < $total_pages && !empty( $options['next_text'] ) ) {
				$out .= $instance->get_single( $_GET['part'] + 1, $options['next_text'], array(
...
class PageNavi_Call {
	...
	function get_pagination_args() {
		...
		return array( $posts_per_page, $_GET['part'], $total_pages );
	}
	...
	//	return ( 'multipart' == $this->type ) ? get_multipage_link( $page )."" : get_pagenum_link( $page )."";
		return ( 'multipart' == $this->type ) ? get_multipage_link( $page )."" : $m[0]."?part=".$page."";
